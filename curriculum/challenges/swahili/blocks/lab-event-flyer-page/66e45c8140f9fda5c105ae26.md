...
title: Napravite stranicu sa letkom za događaj
challengeType: 25
...
1. Trebalo bi da imate element `header` unutar tela (body).
1. Vaš element `header` treba da ima sliku za vaš događaj, i `h1` sa naslovom događaja, u tom redosledu. Možete koristiti ovu sliku ako želite: `https://cdn.freecodecamp.org/curriculum/labs/event.jpg`
1. Trebalo bi da imate element `main` unutar tela (body).
1. Vaš element `main` treba da ima najmanje dva elementa `section` koji prikazuju funkcije događaja.
1. Svaki vaš element `section` treba da sadrži `h2`.
1. Vaše telo (body) bi trebalo da ima gornji i donji padding od `50px`.
1. Vaše telo (body) bi trebalo da ima gornju i donju marginu od `0`, i levu i desnu marginu koja se centriraju.
1. Vaše telo (body) bi trebalo da ima širinu postavljenu u odnosu na širinu prozora (viewport).
1. Vaše telo (body) bi trebalo da koristi funkciju `calc` kako bi podesilo svoj atribut `min-height` na 100% visine prozora, minus sav padding primenjen na vrh i dno tela.
1. Trebalo bi da imate najmanje jedan `hr` unutar vašeg letaka (flyer).
1. Treba da postavite `width` svih elemenata `hr` i `section` na procenat vrednost u odnosu na njihov roditeljski element.

**Napomena:** Obavezno povežite svoj stilski list (stylesheet) u vašem HTML-u i primenite svoj CSS.
