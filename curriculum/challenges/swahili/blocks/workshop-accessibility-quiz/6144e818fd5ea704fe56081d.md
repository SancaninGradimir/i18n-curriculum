...
Dajte svakom `fieldset` adekvatan atribut `name`. Zatim, dajte obe neuređene liste `class` od `answers-list`.

Na kraju, koristite `legend` da dodelite naslov sadržaju `fieldset` postavljanjem pitanja istina/laž kao tekstualnog sadržaja.
