...
Neka ``header`` zauzme punu širinu svog roditeljskog kontejnera, postavi mu ``height`` na ``50px``, i postavi ``background-color`` na ``#1b1b32``. Zatim, postavi da ``display`` koristi _Flexbox_.
