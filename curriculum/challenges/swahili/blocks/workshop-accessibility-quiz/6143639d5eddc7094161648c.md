...
Važno je povezati svaki `input` sa odgovarajućim elementom `label`. Ovo pruža korisnicima asistivne tehnologije vizuelnu referencu za unos.

To se radi dodeljivanjem atributa `for` elementu `label`, koji sadrži `id` od `input`.

Ovaj deo će tražiti ime studenta, e-mail adresu i datum rođenja. Dodelite elementima `label` odgovarajuće atribute `for`, kao i tekstualni sadržaj. Zatim, povežite elemente `input` sa odgovarajućim elementima `label`.
