...
Popunjavajući sadržaj kviza, ispod `#student-info`, dodajte tri `div` elementa sa `class` od `info`.

Zatim, unutar svakog `div` ugnježđujte jedan `label` element i jedan `input` element.
