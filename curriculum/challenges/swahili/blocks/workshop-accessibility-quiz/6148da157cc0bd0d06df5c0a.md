...
Da biste poravnali polja za unos međusobno, kreirajte novi set pravila koji cilja sve `input` i `label` elemente unutar `.info` elementa i postavite svojstvo `display` na `inline-block`.

Takođe, poravnajte tekst elementa `label` desno.
