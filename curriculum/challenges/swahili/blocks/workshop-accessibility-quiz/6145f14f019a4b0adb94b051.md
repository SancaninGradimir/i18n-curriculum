...
Unutar prvog `div.answer` elementa, ugnježdi jedan obavezan `select` element sa tri `option` elementa.

Prvom `option` elementu dodeli `value` sa `""`, i tekst `Select an option`. Drugom `option` elementu dodeli `value` sa `yes`, i tekst `Yes`. Trećem `option` elementu dodeli `value` sa `no`, i tekst `No`.
