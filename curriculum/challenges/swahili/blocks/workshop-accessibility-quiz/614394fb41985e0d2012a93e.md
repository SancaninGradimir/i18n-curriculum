...
Brojevi pitanja nisu dovoljno opisni. Ovo je posebno istinito za osobe sa smanjenim vidom. Jedan način da se zaobiđe takav problem, bez dodavanja vidljivog teksta na element, jeste dodavanje teksta koji može pročitati samo čitač ekrana.

Ugnjezdite element `span` sa `class` od `sr-only` pre broja u svakom od elemenata `h3`.
