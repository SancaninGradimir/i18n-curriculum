...
Unutar `header`, pružite kontekst o stranici ugnježđivanjem jednog `img`, `h1` i `nav` elementa.

`img` bi trebalo da ukazuje na `https://cdn.freecodecamp.org/platform/universal/fcc_primary.svg`, ima `id` od `logo`, i ima tekst `alt` od `freeCodeCamp`.

`h1` bi trebalo da sadrži tekst `HTML/CSS Quiz`.
