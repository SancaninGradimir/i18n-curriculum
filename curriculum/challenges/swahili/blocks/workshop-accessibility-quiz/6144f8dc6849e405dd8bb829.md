...
Da bismo pružili funkcionalnost pitanja istina/lažno, potreban nam je skup ulaznih podataka koji ne dozvoljava odabir oba u isto vreme.

Unutar svakog elementa liste, ugnježđuje se jedan `label` element, a unutar svakog `label` elementa, ugnježđuje se jedan `input` element sa odgovarajućim `type`.
