...
Konačno, određeni tipovi animacija zasnovanih na pokretu mogu izazvati nelagodu kod nekih korisnika. Posebno, osobe sa <dfn>vestibularnim</dfn> poremećajima imaju osetljivost na određene pokretne okidače.

At-pravilo `@media` ima medijski atribut nazvan `prefers-reduced-motion` za postavljanje CSS-a na osnovu preferencija animacije korisnika. Može uzeti jednu od sledećih vrednosti:
...
Vi ćete saznati više o ovoj funkciji u budućoj lekciji. Trenutno, obavijte stil pravilo koje postavlja `scroll-behavior: smooth` unutar `@media` at-rule sa medijskim atributom `prefers-reduced-motion`, koji ima `no-preference` podešen kao vrednost.

Dobro urađeno. Završili ste vežbeni projekat kviza o pristupačnosti.
