...
Da bi se povećala pristupačnost stranice, atribut `role` može se koristiti za ukazivanje na svrhu elementa na stranici asistivnim tehnologijama. Atribut `role` je deo _Web Accessibility Initiative_ (WAI) i prihvata unapred postavljene vrednosti.

Dajte svakom elementu `section` ulogu `region`.
