...
Svaka `region` uloga zahteva etiketu, što pomaže korisnicima čitača ekrana da razumeju svrhu tog regiona. Jedna metoda za dodavanje etikete je dodavanje elementa zaglavlja unutar regiona, a zatim ga referenciranje atributom `aria-labelledby`.

Dodajte sledeće atribute `aria-labelledby` elementima `section`:

- `student-info`
- `html-questions`
- `css-questions`

Zatim, unutar svakog `section` elementa, ugnjezdite jedan `h2` element sa atributom `id` koji odgovara odgovarajućem atributu `aria-labelledby`. Dajte svakom `h2` prikladan tekstualni sadržaj.
