...
Dajte elementima `label` tekst takav da `input` dolazi pre teksta. Zatim, dajte elementima `input` `value` koji odgovara tekstu.

Tekst bi trebalo da bude ili `True` ili `False`.
