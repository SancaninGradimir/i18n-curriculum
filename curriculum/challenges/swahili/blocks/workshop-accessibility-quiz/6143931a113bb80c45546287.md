...
Unutar drugog `section` elementa, dodajte dva `div` elementa sa klasom `question-block`.

Zatim, unutar svakog `div.question-block` elementa, dodajte jedan `h3` element sa tekstom rastućih brojeva, počevši od `1`, i jedan `fieldset` element sa klasom `question`.
