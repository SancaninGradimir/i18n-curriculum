...
Iako nije potrebno za `label` elemente sa ugnježdenim `input`, i dalje je najbolja praksa eksplicitno povezati `label` sa njegovim odgovarajućim elementom `input`.

Sada, dodajte atribut `for` svakom od vaših četiri `label` koja povezuje `label` sa njegovim odgovarajućim radio `input`.
