...
Podrazumevane postavke za CSS Grid će kreirati dodatne redove kada je potrebno, za razliku od Flexbox-a. Postavite svojstvo `.social-icons` za selektor `grid-template-columns` sa vrednošću `repeat(5, 1fr)` kako biste rasporedili elemente u jednom redu.

# --hints--

Vaš selektor za `.social-icons` mora imati svojstvo `grid-template-columns` sa vrednošću `repeat(5, 1fr)`.
