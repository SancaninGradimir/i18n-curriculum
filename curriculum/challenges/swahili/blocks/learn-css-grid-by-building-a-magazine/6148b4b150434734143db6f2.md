...
Zapamtite da atribut `grid-column` određuje u kojoj koloni će element početi i završiti. Ponekad je slučaj da ne znate koliko kolona će vaša mreža imati, ali želite da se element završi u poslednjoj koloni. Da biste to uradili, možete koristiti `-1` za poslednju kolonu.

Kreiraj selektor za `.hero` i atribut vrednosti `grid-column` postavljene na `1 / -1`. Ovo će objasniti karakteristiku proširenja na celu širinu mreže.

# --hints--

Trebalo bi da imaš birač za `.hero`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Tvoj selektor za `.hero` mora imati svojstvo od `grid-column` postavljeno na `1 / -1`.
