...
Biće vam potrebno jedan red teksta i jedan red slike. Postavite svojstvo `.text-with-images` za selektor `grid-template-columns` da bude `1fr 2fr`. Takođe postavite svojstvo `column-gap` na `3rem` kako biste stvorili više prostora između tih redova.

# --hints--

Tvoj selektor za `.text-with-images` mora imati svojstvo `grid-template-columns` postavljeno na `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Vaš selektor za `.text-with-images` mora imati svojstvo od `column-gap` postavljeno na `3rem`.
