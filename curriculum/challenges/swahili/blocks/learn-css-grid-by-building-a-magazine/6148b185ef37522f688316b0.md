...
Sada možete stilizovati sadržaj elementa `.heading` koristeći CSS Grid.

Funkcija CSS `repeat()` se koristi za ponavljanje vrednosti, umesto ručnog unosa, i pomaže pri podešavanjima mreže. Na primer, postavljanje svojstva `grid-template-columns` na `repeat(20, 200px)` kreiraće 20 redova svaki sa širinom od `200px`.

Postavi svojstvo `.heading` sa vrednošću `grid-template-columns` za svoj element `repeat(2, 1fr)` ili kreiraj dva reda jednake širine.

# --hints--

Tvoj selektor za `.heading` mora imati svojstvo `grid-template-columns` sa vrednošću `repeat(2, 1fr)`.
