...
Unutar vašeg elementa sa `.image-wrapper`, postavite drugi element sa `img` svojstvo `src` sa vrednošću `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, svojstvo `alt` sa vrednošću `image of a calculator project`, svojstvo `loading` postavljeno na `lazy`, svojstvo `class` postavljeno na `image-2`, svojstvo `width` postavljeno na `400`, i svojstvo `height` postavljeno na `400`.

# --hints--

Tvoj drugi atribut od `img` treba da ima svojstvo `src` postavljeno na `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`.
...
Vaš drugi element od `img` treba da ima svojstvo `alt` postavljeno na `image of a calculator project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('alt') === 'image of a calculator project');
```

Tvoj drugi element od `img` mora imati svojstvo `loading` postavljeno na `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('loading') === 'lazy');
```

Tvoj drugi atribut od `img` mora imati svojstvo `class` postavljeno na `image-2`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.classList?.contains('image-2'));
```

Tvoj drugi element od `img` mora imati svojstvo `width` postavljeno na `400`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('width') === '400');
```

Tvoj drugi element od `img` mora imati svojstvo `height` postavljeno na `400`.
