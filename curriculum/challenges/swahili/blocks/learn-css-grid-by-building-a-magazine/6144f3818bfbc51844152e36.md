...
Koristite funkciju `minmax` da vaše redove prilagode bilo kojem uređaju. Funkcija `minmax` prima dva argumenta, prvi je minimalna vrednost, a drugi maksimalna vrednost. Ove vrednosti mogu biti dužina, procenat, `fr`, ili čak tekst kao `max-content`.

Prosledite svaku od vrednosti koje ste već naveli za svojstvo `grid-template-columns` unutar funkcije `minmax`, koristeći svaku vrednost kao drugi argument. Prvi argument treba da bude `2rem`, `min-content` i `2rem` po redoslu.

# --hints--

Vaš selektor za `main` mora imati svojstvo `grid-template-columns` postavljeno na `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
