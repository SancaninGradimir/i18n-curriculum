...
Unutar vašeg elementa sa `.image-wrapper`, za vaš treći element sa `img` postavite svojstvo `src` sa vrednošću `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`, svojstvo `alt` sa vrednošću `four people working on code`, svojstvo `loading` sa vrednošću `lazy`, svojstvo `class` postavljeno na `image-3`, svojstvo `width` postavljeno na `600`, i svojstvo `height` postavljeno na `400`.

# --hints--

Treće polje vašeg `img` mora imati atribut `src` postavljen na `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`.
...
Treći element vašeg `img` mora imati svojstvo `alt` postavljeno na `four people working on code`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('alt') === 'four people working on code');
```

Treći atribut vašeg `img` treba da ima svojstvo `loading` postavljeno na `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('loading') === 'lazy');
```

Treći vaš element od `img` mora imati svojstvo `class` postavljeno na `image-3`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.classList?.contains('image-3'));
```

Treći vaš element od `img` mora imati atribut od `width` postavljen na `600`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('width') === '600');
```

Treći vaš element od `img` mora imati atribut `height` postavljen na `400`.
