...
Kreiraj selektor za `body`. Postavi svojstvo `font-family` na `Baskervville`, sa alternativnim putem od `serif`. Postavi svojstvo `color` na `linen` i svojstvo `background-color` na `rgb(20, 30, 40)`.
...
Твој селектор за `body` мора имати svojstvo od `font-family` postavljeno na `Baskervville`, са алтернативом за `serif`.
...
Tvoj selektor za `body` mora imati svojstvo `color` postavljeno na `linen`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

Tvoj selektor za `body` mora imati svojstvo `background-color` postavljeno na `rgb(20, 30, 40)`.
