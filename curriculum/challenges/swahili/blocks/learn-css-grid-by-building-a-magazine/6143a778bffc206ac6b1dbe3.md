...
Kreiraj poslednji element od `p` na kraju tvog elementa `.text` i dodaj mu sledeći tekst:
...
Treba da dodaš šesti element `p` u element `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 7)
```

Šesta komponenta vašeg `p` mora imati navedeni tekst.
