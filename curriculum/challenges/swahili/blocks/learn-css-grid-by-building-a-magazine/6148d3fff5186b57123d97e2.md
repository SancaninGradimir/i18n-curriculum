...
Ovo nije važeći citat bez ispravnih navodnika. Možete ih dodati koristeći lažne selektore za CSS.

Kreiraj selektor za `.quote::before` i postavi svojstvo `content` da bude `"` sa sledećim mestom.

Takođe, kreiraj selektor za `.quote::after` i postavi svojstvo `content` da bude `"` ako je dostupno prethodno mesto.

# --hints--

Trebalo bi da imaš birač za `.quote::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Selektor vašeg `.quote::before` mora imati svojstvo `content` postavljeno na `'" '`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content?.match(/\"\\"\s\"/));
```

Trebalo bi da imate birač za `.quote::after`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

Vaš selektor za `.quote::after` mora imati svojstvo `content` postavljeno na `' "'`.
