...
Sada ste spremni da počnete sa uređivanjem mrežnog rasporeda. CSS Mreža pruža dvodimenzionalni mrežni raspored, omogućavajući vam da postavljate stvari kako horizontalno tako i vertikalno, dok zadržavate kontrolu nad radnjama kao što je postavljanje stvari preko drugih.

Počnite kreiranjem selektora za `main` i dodeljivanjem svojstva `display` koje je postavljeno na `grid`.

# --hints--

Trebalo bi da imaš birač za `main`.

```js
assert(new __helpers.CSSHelp(document).getStyle('main'));
```

Tvoj selektor za `main` mora imati svojstvo `display` postavljeno na `grid`.
