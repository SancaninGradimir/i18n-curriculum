...
Neki tekstovi su premešteni sa svog mesta. Premestite ih tako što ćete dodeliti elementu sa klasom `.first-paragraph::first-letter` atribut `float` koji je postavljen na `left` i atribut `margin-right` koji je postavljen na `1rem`.

# --hints--

Vaš selektor za `.first-paragraph::first-letter` mora imati svojstvo `float` postavljeno na `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

Tvoj selektor za `.first-paragraph::first-letter` mora imati svojstvo `margin-right` postavljeno na `1rem`.
