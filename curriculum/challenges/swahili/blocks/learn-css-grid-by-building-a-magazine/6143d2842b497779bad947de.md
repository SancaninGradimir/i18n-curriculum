...
Kada pokrenete vaš `CSS`, krši pravila `CSS` ciljajući sve elemente koji imaju ``*``, uključujući lažne selektore za ``::before`` i ``::after``.

Postavi atribute za `padding` i `margin` da budu `0`, i postavi atribut za `box-sizing` da bude `border-box`.
...
Vaš selektor za `*, ::before, ::after` mora imati svojstvo `padding` postavljeno na `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding === '0px');
```

Vaš selektor za `*, ::before, ::after` mora imati svojstvo `margin` postavljeno na `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin === '0px');
```

Tvoj selektor za `*, ::before, ::after` mora imati svojstvo `box-sizing` postavljeno na `border-box`.
