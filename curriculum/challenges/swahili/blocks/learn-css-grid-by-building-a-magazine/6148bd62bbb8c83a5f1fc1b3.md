...
Kreiraj selektor za `.hero-title` i svojstvo od `text-align` postavljeno na `center`, svojstvo `color` postavljeno na `orangered` i svojstvo `font-size` postavljeno na `8rem`.

# --hints--

Trebalo bi da imaš birač za `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

Vaš selektor za `.hero-title` mora imati svojstvo `text-align` postavljeno na `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

Vaš selektor za `.hero-title` mora imati svojstvo od `color` postavljeno na `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

Tvoj selektor za `.hero-title` mora imati svojstvo `font-size` postavljeno na `8rem`.
