...
Sada će algoritam automatskog rasporeda početi da radi kada dodate novi ikonni element. Međutim, ovaj algoritam postavlja širinu novog reda na `auto`, što neće odgovarati vašim trenutnim redovima.

Možete ovo otkazati koristeći svojstvo ``grid-auto-columns``. Postavite selektor za ``.social-icons`` svojstvo ``grid-auto-columns`` postavljeno na ``1fr``.

# --hints--

Selektor vašeg `.social-icons` mora imati svojstvo `grid-auto-columns` postavljeno na `1fr`.
