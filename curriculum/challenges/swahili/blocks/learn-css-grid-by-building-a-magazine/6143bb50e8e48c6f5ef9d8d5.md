...
Unutar vašeg elementa sa `aside`, kreiraj dva elementa od `img`, jedan element od `blockquote` i treći element od `img`. Postavi svojstvo elementa `blockquote` na `class`, postavljeno kao `image-quote`.

# --hints--

Trebalo bi da kreiraš tri elementa `img` unutar svog elementa `aside`.

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

Treba ti da napraviš komponentu `blockquote` unutar svoje komponente `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Vaš atribut od `blockquote` treba da ima svojstvo `class`, postavljeno na `image-quote`.

```js
assert(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

Tvoje nove karakteristike moraju biti u pravilnom redosledu.
