...
Ispod vašeg elementa `.heading`, kreirajte novi element `section` sa `class` podešenim na `text`. Unutar njega, kreirajte element `p` sa `class` podešenim na `first-paragraph` i sledećim tekstom:
...
Vaš novi element za `section` treba da dođe nakon vašeg elementa `.heading`.

```js
assert(document.querySelectorAll('section')?.[1]?.previousElementSibling?.className === 'heading');
```

Tvoja nova karakteristika `section` mora imati `class` postavljeno na `text`.

```js
assert(document.querySelectorAll('section')?.[1]?.className === 'text');
```

Treba da napraviš novu komponentu `p` unutar svoje komponente `.text`.

```js
assert(document.querySelector('.text')?.querySelectorAll('p')?.length === 1);
```

Tvoja nova karakteristika `p` bi trebalo da ima `class` postavljeno na `first-paragraph`.

```js
assert(document.querySelector('.text p')?.className === 'first-paragraph');
```

Tvoja nova funkcija za `p` treba da ima dostavljeni tekst.
