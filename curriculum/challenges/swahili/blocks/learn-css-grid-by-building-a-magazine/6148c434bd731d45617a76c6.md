...
Ako želite dodati više društvenih oznaka, ali ih zadržati u jednom redu, morate ispraviti `grid-template-columns` kako biste kreirali dodatne redove. Kao alternativa, možete koristiti svojstvo `grid-auto-flow`.

Ovo svojstvo uzima `row` ili `column` kao prvu vrednost, i opcionalnu drugu vrednost od `dense`. `grid-auto-flow` koristi direktan algoritam raspoređivanja za prilagođavanje izgleda mreže (grid). Postavljanje njegove vrednosti na `column` govori algoritmu da kreira novi red za sadržaj kada je potrebno. Vrednost `dense` omogućava algoritmu da se vrati i popuni praznine u mreži malim elementima, što može uzrokovati da elementi izgledaju van redosleda.

Za vašeg selektora `.social-icons`, postavite svojstvo `grid-auto-flow` na `column`.

# --hints--

Tvoj selektor za `.social-icons` mora imati svojstvo `grid-auto-flow` postavljeno na `column`.
