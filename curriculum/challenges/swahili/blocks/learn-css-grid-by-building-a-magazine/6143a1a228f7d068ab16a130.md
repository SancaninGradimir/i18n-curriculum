...
Ispod vašeg elementa sa `blockquote`, dodajte još jedan element sa `p` sa sledećim tekstom:
...
Trebalo bi da dodaš četvrti element `p` u svoj element `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 5);
```

Tvoj novi deo za `p` mora da dođe nakon tvog dela `blockquote`.
