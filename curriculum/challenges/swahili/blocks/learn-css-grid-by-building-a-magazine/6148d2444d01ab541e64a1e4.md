...
Generiši selektor za `.quote`. Postavi svojstvo `color` postavljeno na `#00beef`, svojstvo `font-size` postavljeno na `2.4rem`, i svojstvo `text-align` postavljeno na `center`.

# --hints--

Trebalo bi da imaš birač za `.quote`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Selektor tvog `.quote` mora imati svojstvo `color` postavljeno na `#00beef`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.color === 'rgb(0, 190, 239)');
```

Vaš selektor za `.quote` mora imati svojstvo `font-size` postavljeno na `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize === '2.4rem');
```

Tvoj selektor za `.quote` mora imati atribut od `text-align` postavljen na `center`.
