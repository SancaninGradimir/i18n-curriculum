...
Kreiraj selektor za `.author` i svojstvo od `font-size` postavljeno na `2rem` i svojstvo od `font-family` postavljeno na `Raleway` sa alternativnom metodom za `sans-serif`.

# --hints--

Trebalo bi da imaš birač za `.author`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author'));
```

Vaš selektor za `.author` mora imati svojstvo `font-size` postavljeno na `2rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

Tvoj selektor za `.author` mora imati svojstvo `font-family` postavljeno na `Raleway` ako postoji alternativa za `sans-serif`.
