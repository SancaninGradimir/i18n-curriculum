...
Kreiraj selektor za `.list-title, .list-subtitle` i postavi svojstvo `color` na `#00beef`.

# --hints--

Trebalo bi da imaš birač za `.list-title, .list-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

Tvoj selektor za `.list-title, .list-subtitle` mora imati svojstvo `color` postavljeno na `#00beef`.
