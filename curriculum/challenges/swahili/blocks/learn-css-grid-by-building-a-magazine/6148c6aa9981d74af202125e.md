...
Izaberite selektor za `.text` svojstvo `font-size` postavljeno na `1.8rem` i svojstvo `letter-spacing` postavljeno na `0.6px`.

# --hints--

Vaš selektor za `.text` mora imati svojstvo `font-size` postavljeno na `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

Vaš selektor za `.text` mora imati karakteristiku `letter-spacing` postavljenu na `0.6px`.
