...
Sada, nakon što je raspored časopisa završen, morate ga prilagoditi uređaju.

Počnite sa pretraga upitom za `@media` za `only screen` koji ima `max-width` od `720px`. Unutar toga, kreirajte selektor za `.image-wrapper` i svojstvo/atribut od `grid-template-columns` sa vrednošću `1fr`.

Ovo će smestiti/prikazati tri slike u jedan red na malom ekranu.

# --hints--

Trebalo bi da postoji novi zakon za `@media` za `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

Tvoj novi zakon `@media` bi trebalo da ima selektor `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

Tvoj novi uređaj `.image-wrapper` treba da ima karakteristiku `grid-template-columns` sa vrednošću `1fr`.
