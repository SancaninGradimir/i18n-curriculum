...
Kao Flexbox, korišćenjem CSS Grid možete rasporediti sadržaj mrežnih elemenata sa `align-items` i `justify-items`. `align-items` će rasporediti manje elemente duž redne ose, a `justify-items` će rasporediti manje elemente duž kolumnarne ose.

Selektor za `.social-icons` atribut/svojstvo `align-items` koje je postavljeno na `center`.

# --hints--

Vaš selektor za `.social-icons` mora imati svojstvo `align-items` postavljeno na `center`.
