...
Dodajte treći element za `p` na kraj vašeg elementa `.text`, i sa opremite ga sledećim tekstom:
...
Trebalo bi da kreirate treći element za `p` unutar vašeg elementa `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 3);
```

Vaše treće polje za `p` mora imati navedeni tekst.
