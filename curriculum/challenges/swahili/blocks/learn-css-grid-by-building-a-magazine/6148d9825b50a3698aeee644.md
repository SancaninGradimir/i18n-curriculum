...
Kreiraj selektor za `.lists` i postavi svojstvo `list-style-type` na `none`. Ovo će pronaći tačke sa kuglama na stavkama liste.

# --hints--

Trebalo bi da imate birač za `.lists`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Vaš selektor za `.lists` mora imati svojstvo od `list-style-type` postavljeno na `none`.
