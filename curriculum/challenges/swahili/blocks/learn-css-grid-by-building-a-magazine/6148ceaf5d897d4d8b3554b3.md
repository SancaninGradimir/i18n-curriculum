...
Magazini često koriste dvostrani tekst u svom štampanom sadržaju kako bi organizovali raspored i kontrolisali tok svog sadržaja. Iako ovo funkcioniše kod štampanog teksta, dvostrani tekst na veb stranici može predstavljati problem pristupačnosti, na primer stvarajući izazove za osobe sa disleksijom.

Da bi vaš projekat izgledao kao objavljeni časopis, dodelite selektoru `.text` svojstvo `text-align` postavljeno na `justify`.

# --hints--

Vaš selektor za `.text` mora imati svojstvo `text-align` postavljeno na `justify`.
