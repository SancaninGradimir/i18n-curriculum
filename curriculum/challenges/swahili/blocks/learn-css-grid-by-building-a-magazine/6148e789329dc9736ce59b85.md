...
Kreiraj pravilo za `.image-1, .image-3` i dodeli mu svojstvo od `grid-column` koje je postavljeno na `1 / -1`. Ovo će omogućiti prvom i trećem slici da pokriju celu širinu mreže.

# --hints--

Trebalo bi da imaš birač za `.image-1, .image-3`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Tvoj selektor za `.image-1, .image-3` mora imati svojstvo `grid-column` postavljeno na `1 / -1`.
