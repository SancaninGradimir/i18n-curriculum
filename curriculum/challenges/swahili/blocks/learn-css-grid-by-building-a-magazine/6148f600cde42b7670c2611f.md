...
Kreiraj novi upit za `@media` za `only screen` koji ima `max-width` od `600px`. Unutar toga, kreiraj pravilo za `.text-with-images` i dodeli atribut/svojstvo `grid-template-columns` sa vrednošću `1fr`.

Ovo će smanjiti vaše donje područje za unos teksta na jedan red na malom ekranu.

# --hints--

Trebalo bi da kreirate novi pretraga upit za `@media` za `only screen and (max-width: 600px)`. Ovo treba da bude pod vašim prethodnim pretraga upitom `@media`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

Tvoja nova traka za pretragu `@media` treba da ima filter `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

Tvoj novi kandidat za `.text-with-images` mora imati karakteristiku `grid-template-columns` sa vrednošću `1fr`.
