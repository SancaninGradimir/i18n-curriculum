...
Vaš element sa `.text` nije CSS Grid, ali možete kreirati red unutar elementa bez korišćenja Grida koristeći svojstvo `column-width`.

Selektor za tvoju `.text` osobinu od `column-width`, postavljeno na `25rem`.

# --hints--

Vaš selektor za `.text` mora imati svojstvo `column-width`, postavljeno na `25rem`.
