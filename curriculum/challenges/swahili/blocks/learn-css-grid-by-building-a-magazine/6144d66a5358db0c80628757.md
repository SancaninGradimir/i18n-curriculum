...
Kreiraj selektor za `html` i dodeli mu svojstvo `font-size` koje je postavljeno na `62.5%`. Ovo će postaviti podrazumevanu veličinu fonta za vašu veb stranicu na 10px (podrazumevana vrednost pretraživača je 16px).

Ovo će vam omogućiti da lako radite sa dimenzijama `rem` kasnije, jer će `2rem` biti 20px.

# --hints--

Treba da napraviš selektor za `html`.

```js
assert(new __helpers.CSSHelp(document).getStyle('html'));
```

Vaš selektor za `html` mora imati svojstvo `font-size` postavljeno na `62.5%`.
