...
Kreiraj selektor za `a`, ako je svojstvo `text-decoration` neka bude `none` sa svojstvom `color`, pa neka bude `linen`.

# --hints--

Trebalo bi da imaš birač za `a`.

```js
assert(new __helpers.CSSHelp(document).getStyle('a'));
```

Tvoj selektor za `a` mora imati svojstvo od `text-decoration` i da bude `none`.

```js
assert(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration === 'none');
```

Tvoj selektor za `a` mora imati svojstvo `color` da bude `linen`.
