...
Slike bi trebalo da budu u rasporedu od dva reda i tri kolone.

Izabirač za `.image-wrapper` svojstvo od `grid-template-columns` postavljeno na `2fr 1fr` i svojstvo od `grid-template-rows` postavljeno na `repeat(3, min-content)`. Ovo će pružiti naše mrežne linije koje menjaju visinu u zavisnosti od sadržaja, ali će redovi zadržati fiksnu širinu u odnosu na kontejner.

# --hints--

Vaš selektor za `.image-wrapper` mora imati svojstvo od `grid-template-columns` postavljeno na `2fr 1fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

Vaš selektor za `.image-wrapper` mora imati svojstvo `grid-template-rows` postavljeno na `repeat(3, min-content)`.
