...
Generišite treći pretraga upit za `@media` za `only screen` koji ima `max-width` od `550px`. Unutar njega, generišite selektor za `.hero-title` sa `font-size` postavljenim na `6rem`, selektor za `.hero-subtitle, .author, .quote, .list-title` sa `font-size` postavljenim na `1.8rem`, selektor za `.social-icons` sa `font-size` postavljenim na `2rem`, i selektor za `.text` sa `font-size` postavljenim na `1.6rem`.

# --hints--

Treba da imaš novi pretraga upit za `@media` za `only screen` koji ima `max-width` od `550px`. Ovo bi trebalo da dođe nakon tvoje dve prethodne.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

Tvoja nova Kanuni za `@media` treba da imaju opciju `.hero-title`, opciju `.hero-subtitle, .author, .quote, .list-title`, opciju `.social-icons` i opciju `.text`. Ove opcije treba da budu u ovom redosledu.
...
Vaš selektor za `.hero-title` mora imati `font-size` postavljen na `6rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize === '6rem');
```

Vaš selektor za `.hero-subtitle, .author, .quote, .list-title` mora imati `font-size` postavljen na `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize === '1.8rem');
```

Vaš selektor za `.social-icons` mora imati `font-size` postavljen na `2rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize === '2rem');
```

Vaš selektor za `.text` mora imati `font-size` podešen na `1.6rem`.
