...
Svojstvo `gap` je kratak način za postavljanje vrednosti `column-gap` i `row-gap` istovremeno. Kada mu se dodeli jedna vrednost, postavlja i `column-gap` i `row-gap` na tu vrednost. Kada mu se dodeli dve vrednosti, postavlja `row-gap` na prvu vrednost i `column-gap` na drugu vrednost.

Pružite selektor za `.image-wrapper` svojstvo od `gap` postavljeno na `2rem`.

# --hints--

Vaš element `.image-wrapper` mora imati atribut `gap` postavljen na `2rem`.
