...
Kreiraj pravilo za `.lists li` da cilja na stavkama liste unutar vašeg elementa `.lists`. Postavi svojstvo `margin-bottom` sa vrednošću od `1.5rem`.

# --hints--

Trebalo bi da imaš birač za `.lists li`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Vaš selektor za `.lists li` mora imati svojstvo `margin-bottom` sa vrednošću `1.5rem`.
