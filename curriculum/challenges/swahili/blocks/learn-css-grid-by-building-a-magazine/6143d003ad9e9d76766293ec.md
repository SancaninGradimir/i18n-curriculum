...
Unutar vašeg elementa sa `.image-quote`, rasporedite unutar element sa `hr`, element sa `p` i drugi element sa `hr`. Postavite element sa `p` atribut/svojstvo `class` na vrednost `quote` i tekst od `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Trebalo bi da dodaš dva elementa sa `hr` unutar svog elementa `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

Trebalo bi da dodate element `p` unutar vašeg elementa `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

Vaši elementi/podaci za `.image-quote` treba da budu u pravilnom redosledu.
...
Tvoja nova karakteristika od `p` treba da ima svojstvo `class` ako je jednako `quote`.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Vaša nova funkcija za `p` trebalo bi da ima tekst od `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
