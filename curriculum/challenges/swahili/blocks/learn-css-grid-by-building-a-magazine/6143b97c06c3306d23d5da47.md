...
Unutar tvog elementa `article`, kreiraj element `h3` sa `class` postavljenim na `list-title` i tekstom `A Brief History`. Ispod toga, kreiraj element `p` sa tekstom `Of the Curriculum`. Zatim kreiraj element `ul` sa klasom objekta `lists`.

# --hints--

Trebalo bi da kreiraš komponentu `h3` unutar svoje komponente `article`.

```js
assert.exists(document.querySelector('article h3'));
```

Treba da napraviš komponentu `p` unutar svoje komponente `article`.

```js
assert.exists(document.querySelector('article p'));
```

Trebaće ti da napraviš komponentu `ul` unutar tvoje komponente `article`.

```js
assert.exists(document.querySelector('article ul'));
```

Elementi unutar komponente `article` bi trebalo da budu u ispravnom redosledu.
...
Vaša nova funkcija za `h3` mora imati `class` postavljeno na `list-title`.

```js
assert(document.querySelector('article h3')?.className === 'list-title');
```

Tvoj novi element `h3` treba da ima tekst `A Brief History`.

```js
assert(document.querySelector('article h3')?.innerText === 'A Brief History');
```

Tvoja nova sekcija `p` mora imati tekst `Of the Curriculum`.

```js
assert(document.querySelector('article p')?.innerText === 'Of the Curriculum');
```

Vaša nova funkcija za `ul` treba da ima `class` postavljeno na `lists`.
