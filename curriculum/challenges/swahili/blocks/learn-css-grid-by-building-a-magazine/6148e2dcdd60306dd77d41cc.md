...
Vreme je da se ukrasi poslednji deo časopisa - slika.

Slike su obuhvaćene elementom `aside` koji koristi klasu objekata `image-wrapper`, stoga kreiraj selektor za `.image-wrapper`. Postavi atribut `display` na `grid`.

# --hints--

Trebalo bi da imate birač za `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

Vaš selektor za `.image-wrapper` mora imati svojstvo `display` postavljeno na `grid`.
