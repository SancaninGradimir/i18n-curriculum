...
Sada je vreme za stilizovanje `section` tvoje treće. Zapamti da ima vrednosti `text` i `text-with-images` za svojstvo elementa `class`, što znači da već nasleđuje stilove iz tvog pravila `.text`.

Napravi čitač za `.text-with-images` na poklopac sa karakteristike `display` da postane `grid`.

# --hints--

Trebalo bi da imaš birač za `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Vaš selektor za `.text-with-images` mora imati svojstvo `display` postavljeno na `grid`.
