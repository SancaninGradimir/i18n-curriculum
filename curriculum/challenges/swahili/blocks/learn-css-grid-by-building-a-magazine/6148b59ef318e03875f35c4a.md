...
Napravi/Kreiraj čarobnjak (ili: generator) za `img` na atributu sa vrednošću `width` da bude `100%`, na atributu sa vrednošću `object-fit` da bude `cover`.

Svojstvo `object-fit` objašnjava pretraživaču kako da postavi element unutar svog kontejnera. U ovom slučaju, `cover` će učiniti da slika popuni kontejner, iseckanjem ako je potrebno kako bi se izbeglo menjanje aspektnog odnosa.
...
Vaš element sa `img` mora imati svojstvo `width` da bi bio/bila `100%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('img')?.width === '100%');
```

Vaš selektor za `img` treba da ima svojstvo `object-fit` i bude `cover`.
