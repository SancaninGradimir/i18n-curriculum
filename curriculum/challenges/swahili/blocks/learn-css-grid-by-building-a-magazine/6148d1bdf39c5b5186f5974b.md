...
Kreiraj selektor za `hr`, sa atributom od `margin` postavljenim na `1.5rem 0`.

# --hints--

Trebalo bi da imaš birač za `hr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('hr'));
```

Tvoj selektor za `hr` mora imati svojstvo od `margin` postavljeno na `1.5rem 0`.
