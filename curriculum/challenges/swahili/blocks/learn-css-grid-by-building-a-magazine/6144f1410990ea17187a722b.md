...
Sada možete stilizovati vaš mrežni raspored. CSS Grid je sličan Flexbox-u jer ima posebna svojstva za roditeljske i detinjstva elemente.

U ovom slučaju, vaš roditeljski element je element `main`. Postavite sadržaj da ima raspored od tri reda dodavanjem atributa `grid-template-columns` sa vrednošću `1fr 94rem 1fr`. Ovo će stvoriti tri reda gde je srednji red širok `94rem`, a i prvi i poslednji red su deo 1 preostalog prostora u kontejneru mreže (grid container).

# --hints--

Vaš deo od `main` mora imati atribut `grid-template-columns` postavljen na `1fr 94rem 1fr`.
