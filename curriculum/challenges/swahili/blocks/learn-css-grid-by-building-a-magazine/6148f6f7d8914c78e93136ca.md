...
Kreiraj poslednji pretraga upit za `@media` za `only screen` sa `max-width` od `420px`. Unutar toga, kreiraj filter za `.hero-title` sa atributom koji je postavljen na `font-size` i definisan kao `4.5rem`.

Čestitam! Vaš dokument je sada završen.

# --hints--

Trebalo bi da imaš novi argument pretrage za `@media` za `only screen and (max-width: 420px)`. Ovo treba da bude poslednji argument u listi argumenata pretrage za `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Tvoja nova traka za pretragu `@media` treba da ima padajući meni `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

Tvoj selektor za `.hero-title` mora imati svojstvo `font-size` postavljeno na `4.5rem`.
