...
Kreiraj selektor za `.publish-date` i dodeli mu svojstvo `color` sa vrednošću `rgba(255, 255, 255, 0.5)`.

# --hints--

Trebalo bi da imate birač za `.publish-date`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

Tvoj selektor za `.publish-date` mora imati svojstvo `color` postavljeno na `rgba(255, 255, 255, 0.5)`.
