...
Mali naslov takođe zahteva formatiranje. Kreiraj selektor za `.hero-subtitle` i postavi svojstvo `font-size` postavljeno na `2.4rem`, svojstvo `color` postavljeno na `orangered`, i svojstvo `text-align` postavljeno na `center`.

# --hints--

Trebalo bi da imate birač za `.hero-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

Vaš selektor za `.hero-subtitle` mora imati svojstvo `font-size` postavljeno na `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

Vaš izbor za `.hero-subtitle` mora imati svojstvo `color` postavljeno na `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

Vaš selektor za `.hero-subtitle` mora imati atribut `text-align` postavljen na `center`.
