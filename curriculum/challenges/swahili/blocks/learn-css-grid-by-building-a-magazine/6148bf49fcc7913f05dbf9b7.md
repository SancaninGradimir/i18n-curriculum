...
Napravi ključ za `.author-name a:hover` na kipa sa atributom `background-color` ako je `#306203`.

Ovo će stvoriti efekat uspona za element `a` unutar `.author-name`, prikazujući prvobitnu zelenu boju freeCodeCamp u pozadini.

# --hints--

Trebalo bi da imaš birač za `.author-name a:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Tvoj selektor za `.author-name a:hover` mora imati svojstvo `background-color` ako je `#306203`.
