...
Svojstvo `place-items` može se koristiti za postavljanje vrednosti za `align-items` i `justify-items` istovremeno. Svojstvo `place-items` prima jednu ili dve vrednosti. Ako je navedena jedna vrednost, primenjuje se na svojstva `align-items` i `justify-items`. Ako su navedene dve vrednosti, prva vrednost primenjuje se na svojstvo `align-items`, a druga vrednost primenjuje se na svojstvo `justify-items`.

Toa kichaguzi cha `.image-wrapper` sifa ya `place-items` iliyowekwa kuwa `center`.

# --hints--

Tvoj selektor za `.image-wrapper` mora imati svojstvo `place-items` postavljeno na `center`.
