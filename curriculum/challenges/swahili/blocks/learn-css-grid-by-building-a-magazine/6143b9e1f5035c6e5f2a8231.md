...
Unutar vašeg elementa od `ul`, kreirajte šest elemenata od `li`. Dodajte element `h4` sa `class` postavljenim na `list-subtitle` i element od `p` za svaki vaš element od `li`.

Zatim izvučite sledeći tekst za elemente `h4` i `p`, po redu, gde će svaki `h4` koristiti ono što je sa leve strane vertikalnog separatora (pipe), a svaki `p` će koristiti ono što je sa desne strane:
...
Vaša komponenta od `ul` treba da ima šest komponenti od `li`.

```js
assert(document.querySelectorAll('.lists li')?.length === 6);
```

Svaki novi element `li` mora imati element `h4` i `p`.
...
`h4` yako ya kwanza inapaswa kuwa na maandishi ya `V1 - 2014`.

```js
assert(document.querySelectorAll('.lists li h4')?.[0]?.innerText === 'V1 - 2014');
```

`p` yako ya kwanza inapaswa kuwa na maandishi ya `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
`h4` yako ya pili inapaswa kuwa na maandishi ya `V2 - 2015`.

```js
assert(document.querySelectorAll('.lists li h4')?.[1]?.innerText === 'V2 - 2015');
```

`p` Tvoj drugi deo treba da ima tekst od `We added interactive algorithm challenges.`

```js
assert(document.querySelectorAll('.lists li p')?.[1]?.innerText === 'We added interactive algorithm challenges.');
```

`h4` tvoj treći treba da ima tekst od `V3 - 2015`.

```js
assert(document.querySelectorAll('.lists li h4')?.[2]?.innerText === 'V3 - 2015');
```

`p` tvoj treći treba da ima tekst od `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
`h4` Tvoja peta treba da ima tekst od `V5 - 2017`.

```js
assert(document.querySelectorAll('.lists li h4')?.[4]?.innerText === 'V5 - 2017');
```

`p` Vaš peti treba da ima tekst od `We added the back-end and data visualization challenges.`

```js
assert(document.querySelectorAll('.lists li p')?.[4]?.innerText === 'We added the back-end and data visualization challenges.');
```

`h4` Vaš šesti treba da ima tekst sa `V6 - 2018`.

```js
assert(document.querySelectorAll('.lists li h4')?.[5]?.innerText === 'V6 - 2018');
```

`p` yako ya sita inapaswa kuwa na maandishi ya `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.`
...
Šest vaših elemenata sa `h4` treba da se odvuče iz klase objekata `list-subtitle`.
