...
Kreirajte još jedan element `p` ispod vašeg elementa `.first-paragraph`, sa sledećim tekstom:
...
Treba da kreirate drugi element od `p` unutar vašeg elementa `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 2)
```

Vaše drugo polje za `p` treba da sadrži dostavljeni tekst.
