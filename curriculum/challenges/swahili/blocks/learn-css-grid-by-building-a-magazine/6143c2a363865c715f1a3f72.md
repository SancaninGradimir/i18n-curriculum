...
Unutar elementa `.image-wrapper`, postavi njegov prvi element `img` sa svojstvom elementa `src` sa vrednošću `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, svojstvom elementa `alt` sa vrednošću `image of the quote machine project`, svojstvom elementa `class` sa vrednošću `image-1`, svojstvom elementa `loading` postavljeno na `lazy`, svojstvom elementa `width` sa vrednošću `600`, i svojstvom elementa `height` sa vrednošću `400`.

# --hints--

Vaš prvi element od `img` mora imati svojstvo polja `src` postavljeno na `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`.
...
Prvi atribut vašeg `img` mora imati svojstvo atributa `alt` postavljeno na `image of the quote machine project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt') === 'image of the quote machine project');
```

Tvoj prvi element od `img` mora imati svojstvo elementa `class` postavljeno na `image-1`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Prvi atribut vašeg `img` treba da ima svojstvo atributa `loading` postavljeno na `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading') === 'lazy');
```

Prvi atribut vašeg `img` mora imati svojstvo atributa `width` postavljeno na `600`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width') === '600');
```

Prvi atribut vašeg `img` mora imati svojstvo atributa `height` postavljeno na `400`.
