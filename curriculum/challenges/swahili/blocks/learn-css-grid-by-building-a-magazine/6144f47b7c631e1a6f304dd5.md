...
Vaš časopis će imati tri glavne sekcije. Već ste postavili opšti raspored u pravilima `main`, ali možete podesiti poziciju u pravilima za decu.

Jedna opcija je svojstvo `grid-column`, koje je skraćenica za `grid-column-start` i `grid-column-end`. Svojstvo `grid-column` opisuje segment mreže, počevši od koje linije mreže i završavajući se na koju liniju.

Kreiraj pravilo za `.heading` i postavi svojstvo `grid-column` na `2 / 3`. Ovo će definisati element `.heading` koji počinje sa redom 2 i završava se u redu 3.

# --hints--

Trebalo bi da imaš birač za `.heading`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.heading'));
```

Vaš selektor za `.heading` mora imati svojstvo `grid-column` postavljeno na `2 / 3`.
