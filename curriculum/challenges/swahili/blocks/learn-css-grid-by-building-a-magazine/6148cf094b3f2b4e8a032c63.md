...
Veštački selektor `::first-letter` omogućava vam da ciljate na prvi karakter u tekstualnom sadržaju elementa.

Kreiraj selektor za `.first-paragraph::first-letter` i postavi svojstvo `font-size` na `6rem`. Takođe postavi svojstvo `color` da bude `orangered` ili ako je moguće.

# --hints--

Treba ti da imaš birač za `.first-paragraph::first-letter`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Vaš selektor za `.first-paragraph::first-letter` mora imati svojstvo `font-size` ako je `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

Vaš selektor za `.first-paragraph::first-letter` mora imati svojstvo `color` ako je `orangered`.
