...
Kreiraj widget za `.social-icons`. Postavi svojstvo `display` na `grid`, i svojstvo `font-size` na `3rem`.

# --hints--

Trebalo bi da imaš birač za `.social-icons`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

Vaš selektor za `.social-icons` mora imati svojstvo `display` postavljeno na `grid`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

Tvoj selektor za `.social-icons` treba da ima svojstvo `font-size` postavljeno na `3rem`.
