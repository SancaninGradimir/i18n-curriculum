...
Sada je vreme da dodate logiku u vašu petlju koja odgovarajuće inkrementira `count`.

Unutar petlje, koristite uslovnu izjavu za inkrementiranje `count` sa `1` ako varijabla `word` jednaka varijabli `match`. U suprotnom, ostavite vrednost `count` nepromenjenu.
