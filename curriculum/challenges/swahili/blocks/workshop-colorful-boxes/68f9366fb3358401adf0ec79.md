...
Sada ćete postaviti širinu i visinu elementa `.flex-container`. Da biste to uradili, dodajte svojstvo `width` sa vrednošću `70%` i svojstvo `height` sa vrednošću `600px`.

Definisano svojstvima `width` i `height` će postaviti koliko širine i visine uzima `div` iz svog roditeljskog elementa, u ovom slučaju element `body`.
