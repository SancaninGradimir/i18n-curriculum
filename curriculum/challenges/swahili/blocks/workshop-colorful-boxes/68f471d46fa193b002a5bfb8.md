...
Kreirajte selektor `.box6` i dodajte mu svojstvo `background` sa vrednošću `#3f51b5`.

Zatim dodajte svojstvo `order` sa vrednošću `5` vašem novom selektoru. Videćete da se element `.box6` pomera iza elementa `.box5`.

Takođe, dodajte svojstvo `flex-grow` sa vrednošću `1`. Ovo će učiniti da kutija 6 raste kako bi popunila bilo koji preostali prostor u kontejneru.

Time je radionica kolornih kutija gotova!
