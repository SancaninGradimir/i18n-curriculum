...
Da biste završili stilizovanje klase `flex-container`, dodajte svojstvo `padding` sa vrednošću `10px` i svojstvo `margin` sa vrednošću `20px auto`.

Zapamtite: `padding` dodaje razmak unutar elementa (između ivice i sadržaja), dok `margin` dodaje razmak izvan elementa (između ivice i drugih elemenata).
