...
Popuni sadržaj kratkog testa, ispod `#student-info`, dodaj tri elementa od `div` koji imaju `class` iz `info`.

Zatim, unutar svakog `div`, postavite u jedan element od `label` i jedan element od `input`.

# --hints--

Trebalo bi da razgradiš tri elementa `div` ispod elementa `h2#student-info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

Trebali biste dati `div` prvi `class` od `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Moraš mu dati `div` od stuba `class` od `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Treba da mu daš treći `div` od `class` na `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.className, 'info');
```

Trebalo bi da ga smestite unutar jedne sekcije `label` unutar prvog `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

Treba da ga smestite unutar jednog elementa od `input`, u prvom `div`, nakon `label`.
...
Trebalo bi da ga smestiš unutar jednog elementa od `label` unutar drugog `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

Trebalo bi da postavite unutar jednog elementa od `input` u drugom `div`, nakon `label`.
...
Trebalo bi da ga smestiš unutar jednog elementa `label` unutar trećeg `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

Treba ga postaviti unutar jednog elementa od `input` unutar `div` trećeg, nakon `label`.
