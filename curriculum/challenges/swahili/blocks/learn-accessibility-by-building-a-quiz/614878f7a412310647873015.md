...
Organizujte lepo `header`, koristeći _Flexbox_ da napravite razmak između elemenata i centrirate ih vertikalno.

# --hints--

Trebalo bi mu dati `header` `justify-content` od `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

Trebalo bi mu dati `header` `align-items` od `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

Trebalo bi mu dati `header` `top` od `0`.
