...
Povećaj unutrašnji razmak (padding) gore i levo za elemente ``.info``, i postavi druge vrednosti na ``0``.

# --hints--

Trebalo bi da koristiš alat `.info`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.info'));
```

Unapaswa kumpa `.info` nafasi ya ndani ya `padding-top` angalau `1px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

Trebao bi da mu pružiš `.info` unutrašnji opseg od `padding-right` za `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

Treba da mu daš unutrašnji prostor od `.info` za `padding-bottom` na `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

Treba da mu/jajemo prostor unutar `.info` na najmanje `1px` u `padding-left`.
