...
Stavite sav tekst unutar elementa `address` vertikalno poravnati po sredini i dodajte padding.

# --hints--

Trebalo bi da koristiš ekstrakter atributa `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

Trebalo bi mu dati `address` `text-align` od `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Trebalo bi da mu dostaviš `address` `padding-top` od najmanje `1px`.
...
Trebalo bi mu dati `address` `padding-right` od najmanje `1px`.
...
Unapaswa kumpa `address` `padding-bottom` ya angalau `1px`.
...
Treba mu dati `address` `padding-left` od najmanje `1px`.
