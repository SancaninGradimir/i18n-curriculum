...
Svaki `fieldset` će imati pitanje istina/laž.

Unutar svakog `fieldset`, postavite u jedan element sa `legend`, i jedan element sa `ul` koji ima dve opcije.

# --hints--

Trebalo bi da ga postavite u jedan element od `legend` unutar prvog elementa od `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > legend')?.length, 1);
```

Morate ga postaviti unutar jednog elementa od `ul`, unutar prvog elementa od `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > ul')?.length, 1);
```

Trebalo bi da postavite unutar dva elementa od `li`, unutar prvog elementa od `ul`.

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[0]?.querySelectorAll('li')?.length, 2);
```

Treba da ga smestite unutar prvog elementa od `legend`, a unutar drugog elementa od `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > legend')?.length, 1);
```

Morate ga smestiti unutar prvog elementa od `ul`, u drugom elementu od `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > ul')?.length, 1);
```

Morate smestiti dva elementa `li` unutar drugog elementa `ul`.
