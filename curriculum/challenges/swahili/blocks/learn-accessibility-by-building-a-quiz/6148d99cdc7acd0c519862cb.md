...
Ciljajte na sve elemente od `label` unutar elemenata od `.info`, i postavite im `width` da budu `10%`, te osigurajte da ne uzimaju manje od `55px`.

# --hints--

Trebalo bi da koristiš alat za `.info label` ili `.info > label`.
...
Trebalo bi da im date elemente `label` `width` od `10%`.
...
Trebalo bi da im date elemente `label` `min-width` od `55px`.
