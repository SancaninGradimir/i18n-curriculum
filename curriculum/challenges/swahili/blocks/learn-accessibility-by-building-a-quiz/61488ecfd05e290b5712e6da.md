...
Da bi prvi deo izgledao bolje unutar linije, ciljajte samo elemente od `input` unutar elemenata od `.info`, i postavite njihove `width` na `50%`, i poravnajte njihov tekst levo.

# --hints--

Trebalo bi da koristiš alat `.info input` ili `.info > input`.
...
Trebalo bi da im date elemente `input` `width` od `50%`.
...
Trebalo bi da im date elemente `input` `text-align` od `left`.
