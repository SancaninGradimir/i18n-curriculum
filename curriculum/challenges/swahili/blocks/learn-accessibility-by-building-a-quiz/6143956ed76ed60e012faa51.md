...
Unutar prvog elementa od `span`, dodaj tekst `Question`.

U drugom elementu od `span`, dodaj tekst za `Question`.

# --hints--

Trebalo bi mu dati prvi deo teksta `span` za `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent, 'Question');
```

Trebalo bi mu dati drugi deo teksta `span` od `Question`.
