...
Dodaj gornju ivicu za elemente sa `.formrow`, i unutrašnji razmak sa leve i desne strane. Ostale vrednosti unutrašnjeg razmaka bi trebalo da budu `0`.

Zatim, povećajte veličinu fonta za sve elemente od `input`.

# --hints--

Treba da koristiš alat `.formrow` za ciljanje elemenata `.formrow`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

Treba da dostavite `.formrow` ivica od `margin-top` od najmanje `1px`.
...
Morate pružiti `.formrow` interni prostor od `padding-top` za `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

Treba da ostaviš unutrašnji prostor od najmanje `1px` za `padding-right` u `.formrow`.
...
Treba da izvučeš unutrašnji prostor od `.formrow` za `padding-bottom` na `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

Trebalo bi da pružite `.formrow` unutrašnji prostor od `padding-left` od najmanje `1px`.
...
Trebalo bi da koristiš selektor `input` za ciljanje elemenata `input`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input'));
```

Unapaswa kutoa `input` fonti ya `font-size` kubwa kuliko `13px`.
