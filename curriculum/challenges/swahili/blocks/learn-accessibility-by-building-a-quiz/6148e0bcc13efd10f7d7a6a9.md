...
Promeni pozadinsku boju od `footer` u `#2a2a40` i koristi _Flexbox_ da centrirate tekst horizontalno.

# --hints--

Trebalo bi da koristiš ekstrakter atributa `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Trebalo bi mu dati `footer` `background-color` od `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Trebalo bi da mu/jaj daje `footer` `display` od `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Treba da mu daš `footer` `justify-content` od `center`.
