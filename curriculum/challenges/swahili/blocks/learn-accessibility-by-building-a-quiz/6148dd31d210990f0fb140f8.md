...
Važno je videti podrazumevanu granicu oko elemenata za `fieldset` tokom razvoja. Međutim, možda to nije stil koji želite.

Uklonite granicu i unutrašnji razmak na elementima `.question`.

# --hints--

Trebalo bi da koristiš alat `.question`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

Unapaswa kumpa `.question` nafasi ya ndani ya `border` yenye thamani ya `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Unapaswa kumpa `.question` nafasi ya ndani ya `padding-bottom` yenye thamani ya `0`.
