...
Kao što je sa drugim komponentama od `input` i `label`, povežite `textarea` sa njegovom odgovarajućom komponentom od `label` i dodelite mu atribut komponente `name`.

# --hints--

Trebalo bi mu dati atribut elementu `textarea` sa svojstvom elementa `id`.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

Ne možete uključiti prostor za element `id` za funkciju `textarea`.

```js
assert.match(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id, /^\S+$/);
```

Morate odabrati drugačije `id` za element `textarea` jer drugi element koristi `id`.
...
Treba mu dodeliti elementu ``label`` svojstvo elementa ``for`` koje odgovara ``id`` od elementa ``textarea``.
...
Trebalo bi mu dati atribut elementu `textarea` sa atributom elementa `name`.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.name);
```

Trebalo bi koristiti najmanje jedan ne-razmak karakter u atributu elementa za `name` za `textarea`.
