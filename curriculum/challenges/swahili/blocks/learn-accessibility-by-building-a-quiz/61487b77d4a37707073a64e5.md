...
Kada je širina ekrana mala, `h1` ne zatvara svoj tekstualni sadržaj kako treba. Poravnajte tekst elementa `h1` po sredini.

Zatim, uklonite unutrašnji razmak (padding) za `main` kako bi zaglavlje sekcije za `Student Info` bilo u potpunosti vidljivo.

# --hints--

Trebalo bi mu dati `h1` `text-align` od `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Trebalo bi da dodate selektor za `main` ili ciljate na element `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Trebalo bi mu dati `main` `padding-top` od najmanje `25px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

Morate da promenite samo vrednost `padding-top`.
