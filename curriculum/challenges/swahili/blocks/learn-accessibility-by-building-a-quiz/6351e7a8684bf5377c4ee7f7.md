...
Iako nije neophodno za elemente `label` koji imaju i `input` postavljene unutar, ipak je najbolje rešenje eksplicitno povezati `label` sa njegovim odgovarajućim elementom `input`.

Sada, dodaj svojstvo elementa `for` za svaku od vaših četiri `label` koja povezuju `label` sa njegovim odgovarajućim radio elementom `input`.

# --hints--

Treba da mu daš prvu vrednost atributa od `label` za element `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Morate mu dati `label` prvi atribut elementa `for` koji odgovara `id` atributu njegovog elementa `input`.
...
Treba da mu/judiš `label`, drugi atribut elementa od `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Trebalo bi dati `label`, drugo svojstvo elementa `for` koje odgovara `id` njegovog elementa `input`.
...
Trebalo bi mu dati treću karakteristiku elementa `label` za `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Treba da mu/daješ treći atribut elemenata `label` koji odgovara `for`, koji odgovara `id` njegovog elementa `input`.
...
Unapaswa kumpa `label` ya nne sifa ya kipengele ya `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Trebalo bi dati `label`, četvrti atribut elementa od `for` koji odgovara `id` njegovog elementa od `input`.
