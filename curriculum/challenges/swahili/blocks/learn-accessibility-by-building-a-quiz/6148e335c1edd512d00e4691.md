...
Poboljšanje navigacionih linkova bi trebalo da uzrokuje da se vidljivi deo ekrana direktno prebaci na odgovarajući odeljak. Međutim, ovaj skok može zbuniti neke korisnike.

Izaberi sve elemente i stavi `scroll-behavior` u `smooth`.
...
Trebalo bi mu/joj dati `*` `scroll-behavior` od `smooth`.
