...
Da bi navigacione dugme bile izgledala kao obična dugmad, uklonite donstručicu sa anchor tagova.

Zatim, kreiraj novi selektor koji cilja elemente navigacionog menija tako da kada pređeš mišem preko njih, promena boja pozadine i boje teksta, a kursor miša bude pokazivač.

# --hints--

Trebalo bi da koristiš postojeći alat `li > a` za postavljanje `text-decoration` na `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

Treba da koristite ili selektor `nav > ul > li:hover` ili `nav li:hover` za stilizovanje elemenata kada ih namigne pokazivač miša.
...
Trebalo bi da im date elemente `li` koje prebacuje miša `background-color` iz `#dfdfe2`.
...
Trebalo bi da im date elemente `li` koje prolazi mišem `color` sa `#1b1b32`.
...
Treba da im date elemente `li` koje prolazi mišem `cursor` na `pointer`.
