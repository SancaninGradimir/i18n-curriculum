...
Postavi širinu elemenata od `section` na `80%` njihovog roditeljskog kontejnera. Zatim, koristi granicu da centrirate elemente `section`, dodajući `10px` na donju ivicu.

Pia, osiguraj da elementi za `section` ne mogu biti veći od `600px` po širini.

# --hints--

Trebalo bi da koristiš čitač `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('section'));
```

Trebalo bi dati `section` `width` od `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.width, '80%');
```

Trebalo bi mu dati `section` `margin-top` od `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginTop, '0px');
```

Trebalo bi mu dati `section` `margin-right` od `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginRight, 'auto');
```

Treba da mu daš `section` `margin-bottom` od `10px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginBottom, '10px');
```

Trebalo bi mu dati `section` `margin-left` od `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginLeft, 'auto');
```

Trebalo bi mu dati `section` `max-width` od `600px`.
