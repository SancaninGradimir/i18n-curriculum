...
Element ``address`` ne zahteva fizičku geografsku lokaciju. Može se koristiti za pružanje poveznog linka za predmet/temu.

Umotajte link oko teksta od `freeCodeCamp`, i postavite mu cilj na `https://freecodecamp.org`.

# --hints--

Trebalo bi da dodate element `a`.

```js
assert.exists(document.querySelector('address > a'));
```

Treba da dodeliš elementu `a` svojstvo elementa `href` sa vrednošću `https://freecodecamp.org`.

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

Trebalo bi da zatvoriš element `a` oko postojećeg teksta `freeCodeCamp`.
