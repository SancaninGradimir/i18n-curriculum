...
Neka elementi `h3` imaju najviši prioritet koristeći sledeće atribute CSS:
...
Trebalo bi da koristiš ekstrakter atributa `h3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Trebalo bi mu dati element od `h3`, parametre za `margin-top` od `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Trebalo bi da mu date atribut `h3` parametre `padding-left` od `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Trebalo bi mu dati atribut `h3` parametre `font-size` od `1.375ren`.
