...
title: Dizajniranje kartice za blog post
challengeType: 25
...
U ovom laboratorijumu, veštaćete kako da stilizujete pozadine i ivice kreiranjem kartice za blog post.

**Cilj:** Ispunite priče korisnika ispod i učinite da svi testovi prođu kako biste završili laboratorijum.

**Priče korisnika:**

1. Trebalo bi da imate element `div` sa klasom `blog-post-card` za držanje svih vaših elemenata kartice.
2. Unutar elementa `.blog-post-card`, trebalo bi da imate element slike sa validnim atributom `alt` i tekstom, te klasom `post-img`. Možete koristiti `https://cdn.freecodecamp.org/curriculum/labs/cover-photo.jpg` za atribut `src` slike.
3. Trebalo bi da imate `div` sa klasom `post-content` unutar elementa `.blog-post-card` sa sledećim:
    - Element `h2` sa klasom `post-title` i tekstom za naslov vašeg blog posta.
    - Element `p` sa klasom `post-excerpt` i tekstom za rezime vašeg blog posta.
    - Element `a` sa klasom `read-more` sa tekstom `Read More`.
4. Trebalo bi da postavite boju pozadine elementa `body` na vrednost koja nije bela.
5. Trebalo bi da primenite sledeće stilove na element `.blog-post-card`:
    - Belu pozadinu.
    - Zaobnjeglone uglove.
    - Širinu po izboru.
    - Poravnanje teksta po izboru.
6. Element `.post-img` treba da bude stilizovan tako da slika popuni celu širinu kartice i ima donju ivicu (ili: donju granicu).
7. Element `.post-content` treba da bude stilizovan tako da postoji unutrašnji razmak (padding) u kartici.
8. Elementi `.post-title` i `.post-excerpt` treba da imaju boju teksta koja nije podrazumevana i margine sa svih strana.
9. Element `.read-more` treba da bude stilizovan kao dugme i da ima:
    - Boju teksta koja nije podrazumevana.
    - Boju pozadine.
    - Margine sa svih strana.
    - Unutrašnji razmak (Padding) sa svih strana.
    - Zaobnjeglone uglove.
    - Svojstvo `display` postavljeno na `inline-block`.
    - Efekat prelaska mišem (hover) koji menja boju pozadine.
