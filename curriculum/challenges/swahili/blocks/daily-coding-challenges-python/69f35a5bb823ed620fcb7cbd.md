...
Kada dobijete reč ili rečenicu, vratite ispravljenu verziju u kojoj svaka reč prati pravilo "i pre e osim nakon c".

- Ako reč sadrži `"ei"` i nije prethodno od `c`, zamenite ga sa `"ie"`.
- Ako reč sadrži `"ie"` i prethodi je `c`, zamenite ga sa `"ei"`.
