...
# --description--
Uzimajući u obzir kolonu sa punim brojevima i kolonu podataka sekvencijalnih karaktera, primenite te operacije za broj po broj od leve ka desnoj. Ponavljajte ove operacije kako je potrebno sve dok se ne koriste svi brojevi. Vratite konačan rezultat.

Na primer, razmatrajući `[1, 2, 3, 4, 5]` i `['+', '*']`, vrati rezultate procene `1 + 2 * 3 + 4 * 5` sa leve na desnu bez uzimanja u obzir standardnog reda operacija.

- Važeća opereta je `+`, `-`, `*`, `/`, na `%`.
