id: 699c8e045ee7cb94ed2322d7
title: "Izazov 216: Dan Pi"
challengeType: 29
dashedName: challenge-216
---

# --description--

Srećan dan Pi (π)!

Ako dobijete celobrojni broj (`n`), gde je `n` između 1 i 1000 (uključujući), vratite `n`-ti decimalni znak od π.

- Uverite se da vraćate broj, a ne niz karaktera.

π sa prvih pet decimalnih mesta je 3.14159. Na primer, ako dobijete `5`, vratite `9`, peti decimalni znak.

Možda ćete morati pronaći prvi 1000 decimalnih znaka π negde.

# --hints--

`get_pi_decimal(5)` bi trebalo da vrati `9`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(5), 9)`)
}})
```

`get_pi_decimal(10)` bi trebalo da vrati `5`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(10), 5)`)
}})
```

`get_pi_decimal(22)` bi trebalo da vrati `6`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(22), 6)`)
}})
```

`get_pi_decimal(39)` bi trebalo da vrati `7`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(39), 7)`)
}})
```

`get_pi_decimal(76)` bi trebalo da vrati `2`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(76), 2)`)
}})
```

`get_pi_decimal(384)` bi trebalo da vrati `4`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(384), 4)`)
}})
```

`get_pi_decimal(601)` bi trebalo da vrati `0`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(601), 0)`)
}})
```

`get_pi_decimal(1000)` bi trebalo da vrati `9`.

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_pi_decimal(1000), 9)`)
}})
```

# --seed--

## --seed-contents--

```py
def get_pi_decimal(n):

    return n
```

# --solutions--

```py
def get_pi_decimal(n):
    pi = ""

    return int(pi[n])
```