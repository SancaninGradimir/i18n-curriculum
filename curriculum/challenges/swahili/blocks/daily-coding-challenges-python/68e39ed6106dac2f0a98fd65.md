...
Dato je niz niz znakova poruke, niz znakova tajnog ključa i broj potpisa, utvrdite da li je potpis validan koristeći ovu metodu kodiranja:

- Slova u poruci i tajnom ključu imaju ove vrednosti:
  - `a` do `z` imaju vrednosti od `1` do `26` respektivno.
  - `A` do `Z` imaju vrednosti od `27` do `52` respektivno.
- Sva druga slova nemaju vrednost.
- Izračunajte potpis sabiranjem ukupne vrednosti poruke i ukupne vrednosti tajnog ključa.

Na primer, ako su dati poruka `"foo"` i tajni ključ `"bar"`, potpis će biti `57`:
