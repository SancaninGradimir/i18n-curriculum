...
# --description--
Dato je niz znakova; odredite da li je važeća e-mail adresa koristeći sledeće kriterijume:

- Mora imati samo jedan karakter `@`.
- Interni deo (pre `@`):
  - Može sadržati samo slova (`a-z`, `A-Z`), brojeve (`0-9`), tačke (`.`), znak donjeg crteža (`_`) ili znak razdvajanja (`-`).
  - Ne sme počinjati niti završavati tačkom.
- Domen deo (nakon `@`):
  - Mora imati najmanje jednu tačku.
  - Mora se završavati tačkom praćenom najmanje dva slova.
- Ni interni deo ni domen deo ne sme sadržati dve uzastopne tačke.
