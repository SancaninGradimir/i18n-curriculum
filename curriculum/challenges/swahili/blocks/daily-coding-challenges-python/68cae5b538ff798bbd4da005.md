...
title: "Challenge 67: Validator za e-poštu"
challengeType: 29
...
Data niz karaktera, utvrdite da li je validna adresa e-pošte koristeći sledeće kriterijume:

- Mora sadržati samo jedan znak `@`.
- Lokalni deo (pre `@`):
  - Može sadržati samo slova (`a-z`, `A-Z`), brojeve (`0-9`), tačku (`.`), donju crtu (`_`) ili znak zareza (`-`).
  - Ne sme počinjati niti završavati sa tačkom.
- Domen deo (nakon `@`):
  - Mora sadržati najmanje jednu tačku.
  - Mora završavati sa tačkom praćenom najmanje dva slova.
- Lokalni deo niti domen deo ne sme sadržati dve uzastopne tačke.
