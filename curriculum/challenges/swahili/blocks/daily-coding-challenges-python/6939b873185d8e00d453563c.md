...
Izvučeni niz znakova linka u Markdownu, vratite identičan niz znakova u HTML.

Markdown slika ima sledeći format: `"[link_text](link_url)"`. Vratite niz znakova HTML taga `a` sa `href` postavljenim na `link_url` i `link_text` kao sadržaj tog taga.
