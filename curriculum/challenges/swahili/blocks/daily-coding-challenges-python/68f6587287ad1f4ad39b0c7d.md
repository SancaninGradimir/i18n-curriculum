...
# --description--
Ako vam je data matrica (redovi redova pojedinačnih slova) i reč koju treba pronaći, vratite početne i krajnje koordinate tog teksta u matrici.

- Generisana matrica je popunjena sa svim malim slovima (`a-z`).
- Pretražvana reč će se pojaviti samo jednom u matrici.
- Pretražvana reč će se pojaviti u pravoj liniji u jednom od sledećih pravaca:
  - od leve ka desnoj
  - od desne ka levoj
  - od gore ka dole
  - od donekle ka gore
...
Gde `[0, 1]` su početne brojeve za `"c"` (u početku reči), i `[2, 1]` su krajnje brojeve za `"t"` (na kraju reči).
