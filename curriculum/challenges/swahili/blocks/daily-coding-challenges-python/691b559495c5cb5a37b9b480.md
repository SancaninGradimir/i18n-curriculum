...
Kada dobijete težinu u funtama kao broj, vratite niz znakova `"(lbs) pounds equals (kgs) kilograms."`.

- Zamijenite `"(lbs)"` sa ulaznim brojem.
- Zamijenite `"(kgs)"` s vrijednošću konvertovanom u kilograma, zaokruženom na dvije decimale i uvijek uključujući dvije decimalne točke u vrijednosti.
