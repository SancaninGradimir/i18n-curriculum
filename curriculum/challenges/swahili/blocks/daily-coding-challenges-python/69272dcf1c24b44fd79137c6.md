...
Ako vam je dat niz znakova koji može sadržavati tekst u italiku u Markdown-u, vratite odgovarajući niz HTML znakova.

- Italicki tekst u Markdown-u je bilo koji tekst koji počinje i završava sa jednim zvjezdicom (`*`) ili jednim donjom crtom (`_`).
- Nisu dozvoljeni razmaci između teksta i zvjezdice ili donje crte, ali razmaci mogu postojati unutar samog teksta.
- Zamijenite sve instance italika HTML `i` tagovima i vratite niz znakova.

Na primjer, ako primite `"*This is italic*"`, vratite `<i>This is italic</i>`.

Napomena: Konzola možda neće prikazati HTML tagove unutar niza tokom snimanja poruke. Provjerite konzolu pretraživača da vidite zapis sa ugrađenim tagovima.
