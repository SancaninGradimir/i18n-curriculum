...
Kada dobijete niz slova, vratite poruku koja sadrži brojač koliko slova i koliko brojeva se nalaze u njemu.

- Slova su `A-Z` i `a-z`.
- Brojevi su `0-9`.
- Ignorišite sva druga slova.

Vratite `"The string has X letters and Y numbers."`, gde je `"X"` brojač slova, a `"Y"` brojač brojeva. Ako je bilo koji brojač 1, koristite jedninu za tu stavku. Primer: `"1 letter"` umesto `"1 letters"` i `"1 number"` umesto `"1 numbers"`.
