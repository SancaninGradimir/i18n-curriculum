...
# --description--
Uzimajući u obzir red podataka o lekovima i niz znakova koji predstavlja trenutno vreme, vrati sledeći lek koji treba da se uzme i preostalo vreme do uzimanja.

- Svaki lek je u formatu `[name, lastTaken]`, gde `name` je naziv leka i `lastTaken` je vreme poslednje uzimanja.
- Sve navedeno vreme će biti u formatu `"HH:MM"` (sat 24).
...
| Mergeflictamine | Na svakih 4 sata |

Vrati niz slova u formatu `"{name} in Hh Mm"`. Na primer, `"Debuggamanizole in 2h 0m"` ili `"Deployxitrin in 1h 5m"`.
