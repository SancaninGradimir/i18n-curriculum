...
Given an array of medication data and a string representing the current time, return the next medication needed and the remaining time until it's due.

- Each medication is in the format `[name, lastTaken]`, where `name` is the drug name and `lastTaken` is the time it was last taken.
- All times provided will be in the format `"HH:MM"` (24-hour clock).
...
Return the string in the format `"{name} in Hh Mm"`. For example, `"Debuggamanizole in 2h 0m"` or `"Deployxitrin in 1h 5m"`.
