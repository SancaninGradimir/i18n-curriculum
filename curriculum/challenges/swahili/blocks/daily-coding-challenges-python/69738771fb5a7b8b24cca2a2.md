...
Danas je dan Groundhoga, kada groundhog predviđa vremensku prognozu na osnovu toga da li vidi svoju senku ili ne.

Za vrednost koja predstavlja izgled groundhoga, vratite tačnu predviđanju:

- Ako je zadata vrednost booleanski tip `True` (groundhog je video svoju senku), vratite `"Looks like we'll have six more weeks of winter."`.
- Ako je vrednost booleanski tip `False` (groundhog nije video svoju senku), vratite `"It's going to be an early spring."`.
