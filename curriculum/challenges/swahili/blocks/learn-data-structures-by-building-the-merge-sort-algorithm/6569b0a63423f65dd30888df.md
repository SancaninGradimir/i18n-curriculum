...
Ukloni glavnu reč `pass`. Za deo koda ciklusa `while`, postavite vrednost `left_part[left_array_index]` na `array[sorted_index]`.

# --hints--

Trebalo bi da imaš `array[sorted_index] = left_part[left_array_index]` unutar tela petlje. Ne zaboravi da ukloniš glavnu reč `pass` unutar tela petlje.
...
Ne bi trebalo da imaš `pass` u kola `while`.
