...
Sada je vreme da ažuriramo rezultate i poruku o rezultatima runde.

Kreiraj funkciju `showResults` sa parametrom nazvanim `userOption`.

Unutar vaše funkcije `showResults`, `roundResultsMsg` treba da se ažurira sa rezultatom runde.

Zatim, `playerScoreSpanElement` i `computerScoreSpanElement` takođe treba da se ažuriraju kako bi prikazali ažurirane bodove igrača i računara.

Zapamtite, redija ovde ima značaj. Prvo ćete morati da ažurirate `roundResultsMsg`, zatim `playerScoreSpanElement`, i na kraju `computerScoreSpanElement`, jer će se `roundResultsMsg` koristiti za određivanje bodova.
