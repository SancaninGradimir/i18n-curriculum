...
Sledeći korak je da priložite slušače događaja svakom od dugmadi koja ste kreirali u prethodnom koraku.

Počnite kreiranjem slušača događaja za dugme `rockBtn`. Kada se klikne na dugme `rockBtn`, trebalo bi da pozovete funkciju `showResults` sa argumentom `"Rock"`.

Zatim, kreirajte slušača događaja za dugme `paperBtn`. Kada se klikne na dugme `paperBtn`, trebalo bi da pozovete funkciju `showResults` sa argumentom `"Paper"`.

Konačno, kreirajte slušača događaja za dugme `scissorsBtn`. Kada se klikne na dugme `scissorsBtn`, trebalo bi da pozovete funkciju `showResults` sa argumentom `"Scissors"`.

Sada, trebalo bi da možete kliknuti na bilo koje od dugmadi i videti rezultate za svaku rundu.
