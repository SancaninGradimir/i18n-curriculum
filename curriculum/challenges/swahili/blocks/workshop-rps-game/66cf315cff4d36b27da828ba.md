...
U ovom koraku, fokusiraćete se na određivanje da li je igrač osvojio rundu.

Kreirajte funkciju `hasPlayerWonTheRound` sa dva parametra, nazvana `playerChoice` i `computerChoice`. Ovi parametri će uzimati string vrednosti bilo od `"Rock"`, `"Paper"` ili `"Scissors"`, predstavljajući izbore koje su napravili igrač i računar.

Funkcija bi trebalo da vrati `true` ako je igrač osvojio rundu, i `false` ako je igrač izgubio ili izjednačio rundu.

Evo kriterijuma za pobedu igrača u rundi:

- Ako igrač izabere `"Rock"` i računar izabere `"Scissors"`
- Ako igrač izabere `"Scissors"` i računar izabere `"Paper"`
- Ako igrač izabere `"Paper"` i računar izabere `"Rock"`
