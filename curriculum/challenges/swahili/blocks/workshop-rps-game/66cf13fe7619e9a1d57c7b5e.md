...
Sljedeći korak je izgradnja funkcionalnosti koja će generisati nasumičan izbor za računar.

Kreirajte funkciju nazvanu `getRandomComputerResult` koja vraća nasumičan izbor iz niza `options`.

*Savet*: Ne zaboravite da ste u ranijem odeljku o osnova JavaScripta naučili kako da koristite `Math.random()` i `Math.floor()`.
