...
Sada je vreme da se dobiju rezultati runde. Dopunite funkciju `getRoundResults`.

Ako pobediš rundu, ažuriraj `playerScore` sa `1` i vrati poruku `"Player wins! [player's choice] beats [computer's choice]"`.

Ako računar i igrač izaberu istu opciju, vrati poruku `"It's a tie! Both chose [player's choice]"`.

Ako pobedi računar rundu, ažuriraj `computerScore` sa `1` i vrati poruku `"Computer wins! [computer's choice] beats [player's choice]"`.

`[computer's choice]` treba zameniti sa `computerResult`, dok se `[player's choice]` treba zameniti sa `userOption`.
