...
Za poslednji deo radionice, fokusiraćete se na izradu funkcionalnosti poruke pobednika i dugmeta za resetovanje.

Počnite kreiranjem varijable nazvane `winnerMsgElement` koja će skladištiti element sa ID-om `winner-msg`. 

Zatim, kreirajte varijablu nazvanu `optionsContainer` koja će skladištiti element sa klasom `options-container`. 

Konačno, kreirajte varijablu nazvanu `resetGameBtn` koja će skladištiti element sa ID-om `reset-game-btn`.
