...
title: Napravite stranicu putničke agencije
challengeType: 25
...
1. Trebalo bi da imate deklaraciju `DOCTYPE`.
2. Trebalo bi da imate element `html` sa `lang` podešenim na `en`.
3. Trebalo bi da imate element `head` koji sadrži prazan (void) element `meta` sa `charset` podešenim na `utf-8` i element `title` sa tekstom `Travel Agency Page`.
4. Trebalo bi da imate tag `meta` u vašem elementu `head` koji sadrži kratak opis vaše veb stranice za SEO.
5. Trebalo bi da imate element `h1` za predstavljanje vaših destinacija za putovanja.
6. Trebalo bi da imate pasus ispod elementa `h1` koji uvodi mogućnosti putovanja.
7. Trebalo bi da imate element `h2` sa tekstom `Packages`.
8. Trebalo bi da imate element `p` koji ukratko predstavlja razne pakete.
9. Trebalo bi da imate neuređenu listu sa dva stavke liste. Dve stavke liste treba da imaju tekst `Group Travels` i `Private Tours`, respektivno. Tekst svake stavke liste treba da bude zatvoren anhkornim elementom.
10. Trebalo bi da imate element `h2` sa tekstom `Top Itineraries`.
11. Trebalo bi da imate najmanje tri `figure` elementa, svaki sa anhkornim elementom i elementom `figcaption`.
12. Tri anhkorna elementa treba da imaju element `img` sa odgovarajućim atributom `alt` i atributom `src` podešenim na validnu sliku kao njihov sadržaj. Možete koristiti `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`, `https://cdn.freecodecamp.org/curriculum/labs/alps.jpg` i `https://cdn.freecodecamp.org/curriculum/labs/sea.jpg` ako želite.
13. Svi vaši pet anhkornih elemenata treba da imaju atribut `href` sa vrednošću `https://www.freecodecamp.org/learn` i atribut `target` sa vrednošću `_blank`.
