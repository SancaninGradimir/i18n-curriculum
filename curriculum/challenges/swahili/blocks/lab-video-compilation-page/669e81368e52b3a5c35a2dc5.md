...
title: Izgraditi stranicu za kompilaciju videa
challengeType: 25
...
1. Trebalo bi da imate element `main` kao jedini dete elementa `body`.
1. Trebalo bi da imate element `h1` sa temom vaše stranice.
1. Ispod vašeg elementa `h1` trebalo bi da imate pasus koji uvodi temu vaše stranice.
1. Ispod prvog pasusa trebalo bi da imate tri elementa `section`.
1. Svakoj sekciji treba da nedostaje element `h2`, pasus i element `iframe`, u ovom redosledu.
1. Tri elementa `iframe` trebalo bi da imaju atribut `src` podešen na validno video.
1. Svakom elementu `iframe` takođe treba da ima atribut `title` za opis ugrađenog sadržaja, kao i atribut `height` i atribut `width` za podešavanje elemenata na odgovarajuću veličinu.
