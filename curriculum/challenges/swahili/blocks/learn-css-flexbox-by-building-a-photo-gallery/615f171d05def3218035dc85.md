...
Uklonite ivicu sa vašeg elementa tela, postavite `font-family` na `sans-serif`, i dodelite `background-color` sa vrednošću `#f5f6f7`.
...
Tvoj selektor za `body` mora imati svojstvo `margin` postavljeno na vrednost 0.
...
Vaš selektor za `body` mora imati svojstvo `font-family` koje je postavljeno na vrednost `sans-serif`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily === 'sans-serif');
```

Tvoj selektor za `body` mora imati svojstvo `background-color` postavljeno na vrednost `#f5f6f7`.
