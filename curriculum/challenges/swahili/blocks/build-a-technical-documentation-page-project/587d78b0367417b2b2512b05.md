...
title: Napravi stranicu tehnicke dokumentacije
challengeType: 14
...
**Cilj:** Napravi aplikaciju koja funkcionalno odgovara ovoj: <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a>. **Nemoj kopirati ovaj demo projekat**.

**Korisnicke price:**

1. Unaweza kuona kipengele cha `main` chenye `id="main-doc"` kinachohusiana nacho, ambacho kina maudhui makuu ya ukurasa (nyaraka za kiufundi)
1. Ndani ya kipengele cha `#main-doc`, unaweza kuona vipengele kadhaa vya `section`, kila kikiwa na darasa la vitu la `main-section`. Lazima kuwe na angalau tano
1. Kipengele cha kwanza ndani ya kila `.main-section` kinapaswa kuwa kipengele cha `header` chenye maandishi yanayoelezea mada ya sehemu hiyo
1. Kila kipengele cha `section` chenye darasa la vitu la `main-section` kinapaswa pia kuwa na `id` inayolingana na maandishi ya kila `header` kilichomo ndani yake. Nafasi zozote zinapaswa kubadilishwa kuwa alama za mstari chini (underscore) (mfano: Sehemu yenye kichwa "JavaScript and Java" inapaswa kuwa na `id="JavaScript_and_Java"` inayolingana)
1. Vipengele vya `.main-section` vinapaswa kuwa na angalau vipengele kumi vya `p` kwa jumla (si kila moja)
1. Vipengele vya `.main-section` vinapaswa kuwa na angalau vipengele vitano vya `code` kwa jumla (si kila moja)
1. Vipengele vya `.main-section` vinapaswa kuwa na angalau vipengele vitano vya `li` kwa jumla (si kila moja)
1. Unaweza kuona kipengele cha `nav` chenye `id="navbar"` kinacholingana nacho
1. Kipengele cha navbar kinapaswa kuwa na kipengele kimoja cha `header` chenye maandishi yanayoelezea mada ya nyaraka za kiufundi
1. Zaidi ya hayo, navbar inapaswa kuwa na vipengele vya viungo (`a`) vyenye darasa la vitu la `nav-link`. Lazima kuwe na kimoja kwa kila kipengele chenye darasa la vitu la `main-section`
1. Kipengele cha `header` ndani ya `#navbar` kinapaswa kuja kabla ya vipengele vyovyote vya viungo (`a`) ndani ya navbar
1. Kila kipengele chenye darasa la vitu la `nav-link` kinapaswa kuwa na maandishi yanayolingana na maandishi ya `header` ndani ya kila `section` (mfano: ikiwa una sehemu/kichwa cha "Hello world", navbar yako inapaswa kuwa na kipengele chenye maandishi "Hello world")
1. Unapobofya kipengele cha navbar, ukurasa unapaswa kuhamia sehemu inayolingana ya kipengele cha `#main-doc` (mfano: Unapobofya kipengele cha `.nav-link` chenye maandishi "Hello world", ukurasa unahamia kipengele cha `section` chenye kitambulisho hicho na kichwa kinacholingana)
1. Kwenye vifaa vya ukubwa wa kawaida (kompyuta za mezani, kompyuta za dawati), kipengele chenye `id="navbar"` kinapaswa kuonyeshwa upande wa kushoto wa skrini na kila mara kiwe kinaonekana kwa mtumizi
1. Nyaraka zako za kiufundi zinapaswa kutumia angalau media query moja
...
Treba da imas `main` element sa `id` vrednoscu `main-doc`.
...
Treba da imas najmanje pet `section` elemenata sa klasom `main-section`.
...
Svi `.main-section` elementi treba da budu `section` elementi.
...
Unapaswa kuwa na vipengele vya angalau tano vya `.main-section` ambavyo ni mzao wa `#main-doc`.
...
Mtoto wa kwanza wa kila `.main-section` anapaswa kuwa kipengele cha `header`.
...
Hakuna kipengele cha `header` kinachopaswa kuwa tupu.
...
Vipengele vyote vya `.main-section` vinapaswa kuwa na `id`.
...
Kila `.main-section` inapaswa kuwa na `id` inayolingana na maandishi ya mtoto wake wa kwanza, ambapo nafasi zozote katika maandishi ya mtoto zinabadilishwa kuwa alama za mstari chini (`_`) kwa ajili ya kitambulisho.
...
Unapaswa kuwa na vipengele vya angalau 10 vya `p` (kwa jumla) ndani ya vipengele vyako vya `.main-section`.
...
Unapaswa kuwa na vipengele vya angalau tano vya `code` ambavyo ni mzao wa vipengele vya `.main-section`.
...
Unapaswa kuwa na vipengele vya angalau tano vya `li` ambavyo ni mzao wa vipengele vya `.main-section`.
...
`#navbar` yako inapaswa kuwa na kipengele kimoja tu cha `header` ndani yake.
...
Unapaswa kuwa na kipengele kimoja cha `a` chenye darasa la vitu la `nav-link`.
...
Vipengele vyote vya `.nav-link` vinapaswa kuwa vipengele vya tagi ya nanga (`a`).
...
Vipengele vyote vya `.nav-link` vinapaswa kuwa ndani ya `#navbar`.
...
Unapaswa kuwa na idadi sawa ya vipengele vya `.nav-link` na `.main-section`.
...
Kipengele cha `header` ndani ya `#navbar` kinapaswa kuja kabla ya vipengele vyovyote vya viungo (`a`) pia ndani ya `#navbar`.
...
Kila `.nav-link` inapaswa kuwa na maandishi yanayolingana na maandishi ya `header` ya `section` inayohusiana nayo (mfano: ikiwa una sehemu/kichwa cha "Hello world", `#navbar` yako inapaswa kuwa na `.nav-link` yenye maandishi "Hello world").
...
Kila `.nav-link` inapaswa kuwa na sifa ya `href` inayounganisha na `.main-section` inayolingana nayo (mfano: Unapobofya kipengele cha `.nav-link` chenye maandishi "Hello world", ukurasa unahamia kipengele cha `section` chenye kitambulisho hicho).
...
`#navbar` yako inapaswa kila mara kuwa kwenye kingo ya kushoto ya dirisha.
...
Mradi wako wa nyaraka za kiufundi unapaswa kutumia angalau media query moja.
