...
Kreiraj novi selektor za `25%` između vaših selektora od `0%` i `50%`. Postavi svojstvo `background-color` za ovaj novi selektor i postavi ga na `yellow`.

# --hints--

Trebaće ti da napraviš novi preklopnik za `25%` u svom pravilu `@keyframes cabins`.
...
Tvoj selektor za `25%` treba da bude između tvojih selektora `0%` i `50%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

Tvoj selektor za `25%` mora imati svojstvo `background-color` postavljeno na `yellow`.
