...
Kreiraj drugo pravilo za `@keyframes` sa imenom `cabins`. Koristi svojstva slična onima od `@keyframes wheel`, preuzimajući pravila iz `0%` i `100%`, ali postavi svojstvo `transform` selektora `100%` na `rotate(-360deg)`.
...
Tvoj novi zakon `@keyframes` treba da se zove `cabins`.
...
Tvoj selektor za `100%` mora da dođe nakon selektora za `0%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '100%')
```

Vaš selektor za `100%` mora imati svojstvo `transform` postavljeno na `rotate(-360deg)`.
