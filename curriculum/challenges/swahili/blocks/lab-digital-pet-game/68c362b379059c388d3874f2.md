...
title: Izgradite digitalni pet game
challengeType: 25
...
Aplikacija bi trebalo da ima dva prikaza (views). Zahtevi za svaki prikaz obuhvaćeni su preostalim pričama korisnika:

Prvi prikaz:

- Treba da sadrži formu koja prihvata jedan tekstualni unos.
- Treba da traži od korisnika da imenuje svoje kućno ljubimce.
- Treba da sadrži dugme za slanje koje će pokrenuti igru.
- Treba da nestane nakon slanja.

Drugi prikaz:

- Treba da se pojavi nakon slanja prvog prikaza.
- Treba da sadrži tri `<button>` elementa.
- Svaki element dugmeta treba da se odnosi na jednu od sledećih tri akcije: `[EAT, PLAY, SLEEP]`.

  - Dugme `EAT` treba da smanji vrednost kućnih ljubimaca `Hunger`, i poveća vrednost kućnih ljubimaca `Energy`.
  - Dugme `PLAY` treba da smanji vrednost kućnih ljubimaca `Energy`, i poveća vrednost kućnih ljubimaca `Happiness`.
  - Dugme `SLEEP` treba da poveća vrednost kućnih ljubimaca `Hunger`, i poveća vrednost kućnih ljubimaca `Energy`.

- Treba vizuelno pratiti tri svojstva kućnog ljubimca. To su: `['Hunger', 'Happiness', 'Energy']`.
- Svaki element koji prati svako od tri svojstva treba da:

  - Sadrži tekst relevantan za to svojstvo, bez obzira na veličinu slova, npr., `hunger`, `Hunger`, `hUnGeR`, su sve validni.
  - NE sme da sadrži tekst relevantan za druga svojstva, tj. element za praćenje `Hunger` ne sme da sadrži tekst `Happiness`.
  - Sadrži jasan numerički prikaz trenutne vrednosti tog svojstva.

- Svaki `Action` dostupan kućnom ljubimcu treba da bude obuhvaćen enumeratorom.
- Svaki `Action` `button` treba da sadrži po jedan od sledećih `id`'s relevantnih za akciju tog dugmeta: `['eat-action', 'play-action', 'sleep-action']`.
- Kućni ljubimac bi trebalo da ima sledeće raspoloženja, obuhvaćena `PetMood` enumeratorom: `['Happy', 'Excited', 'Content', 'Sad', 'Tired', 'Sick', 'Hungry']`.

  - Kućni ljubimac treba da bude `Hungry` ako je `Hunger > 70`.
  - Kućni ljubimac treba da bude `Tired` ako je `Energy < 30`.
  - Kućni ljubimac treba da bude `Sad` ako je `Happiness < 30`.
  - Kućni ljubimac treba da bude `Excited` ako su `Happiness > 80` i `Energy > 70`.
  - Kućni ljubimac treba da bude `Happy` ako je `Happiness > 60`.
  - U suprotnom, kućni ljubimac bi trebalo da bude `Content`.

- Kućni ljubimac bi trebalo da ima određeni broj slika ili emoji vezanih za svaki `Mood`.

  - Ovi emoji ili slike treba da budu vezani za trenutno `PetMood`, sa `Record`.
  - Ključ svakog unosa `Record` bi trebalo da bude relevantna vrednost enum-a `PetMood`.
