...
Pre nego što dodate bilo kakav novi sadržaj, trebalo bi da koristite element `section` za razdvajanje sadržaja slika mačaka od budućeg sadržaja.

Element `section` se koristi za definisanje sekcija u dokumentu, kao što su poglavlja, zaglavlja, podnožja ili bilo koje druge sekcije dokumenta. To je semantički element koji pomaže sa SEO-om i pristupačnošću.
...
Uzmite svoj element `h2`, dva elementa `p`, i element sidra (anchor) (`a`) i ugnježđujte ih u elementu `section`.
