...
Element za natpis slike (`figcaption`) koristi se za dodavanje podpisa koji opisuje sliku sadržanu u elementu `figure`. 

Evo primera elementa `figcaption` sa podpisom `A cute cat`:
...
Nakon slike ugnježdenoj u element `figure`, dodajte element `figcaption` sa tekstom postavljenim na:
