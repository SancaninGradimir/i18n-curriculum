...
HTML <dfn>attributes</dfn> su specijalne reči koje se koriste unutar otvarajućeg taga elementa za kontrolisanje ponašanja tog elementa. Atribut ``src`` u elementu ``img`` specificira URL slike (gde je slika).

Evo primera elementa ``img`` sa atributom ``src`` koji ukazuje na logo freeCodeCamp:

```html
<img src="https://cdn.freecodecamp.org/platform/universal/fcc_secondary.svg">
```

Unutar postojećeg elementa ``img``, dodajte atribut ``src`` sa ovim URL-om:
