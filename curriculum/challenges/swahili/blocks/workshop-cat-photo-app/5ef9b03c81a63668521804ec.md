...
Napomena da sadržaj cele stranice je ugnježdjen unutar elementa `html`. Element `html` je koreni element HTML stranice i obuhvata sav sadržaj na stranici.

Takođe možete specificirati jezik vaše stranice dodavanjem atributa `lang` elementu `html`. 

Dodajte atribut `lang` sa vrednošću `en` na otvarajući tag `html` kako biste specificirali da je jezik stranice engleski.
