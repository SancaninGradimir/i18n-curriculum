...
Elementi `thead` i `tbody` se koriste za ukazivanje na koji deo vaše tabele je zaglavlje, a koji deo sadrži primarne podatke ili sadržaj.

Dodajte `thead` i `tbody` u vaš prvi `table`, ispod elementa `caption`.
