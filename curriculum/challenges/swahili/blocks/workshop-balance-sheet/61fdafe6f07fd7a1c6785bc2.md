...
Konačno, morate izvući ove skrivene elemente iz dokumentovnog protoka. Dodajte selektoru `span[class~="sr-only"]` svojstvo `position` postavljeno na `absolute`, svojstvo `padding` postavljeno na `0`, i svojstvo `margin` postavljeno na `-1px`. Ovo će osigurati da nisu samo više vidljivi, već da nisu ni u pogledu stranice.
