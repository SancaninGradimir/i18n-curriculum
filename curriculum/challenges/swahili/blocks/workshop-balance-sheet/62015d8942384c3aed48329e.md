...
Omotali ste svoju tabelu u sekcijski element – sada možete da ga stilizujete kako biste vašoj tabeli dali granicu. Kreirajte selektor `section`, i dodelite mu svojstvo `max-width` postavljeno na `40rem` za responsivan dizajn. Postavite svojstvo `margin` na `0 auto` da biste ga centrirali, i postavite svojstvo `border` na `2px solid #d0d0d5`.
