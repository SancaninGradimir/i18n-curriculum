...
Element `tr` se koristi za označavanje redova tabele. Dodajte element `tr` unutar vašeg elementa `thead`. U vaš novi element `tr`, dodajte element `td`, praćen tri elementa `th`.

Element `td` označava ćeliju podataka, dok element `th` označava zaglavnu ćeliju.
