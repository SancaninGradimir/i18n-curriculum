...
U vašem četvrtom `tr` elementu, dodajte `th` element sa tekstom `Total Assets`. Omotajte tekst `Assets` u `span` element sa atributom `class` postavljenim na `sr-only`.

Nakon toga, dodajte tri `td` elementa sa sledećim tekstom (u redu): `$579`, `$736`, `$809`. Dajte trećem `td` elementu atribut `class` postavljen na `current`.
