...
Selektor `[attribute="value"]` cilja bilo koji element koji ima atribut sa određenom vrednošću. Kreirajte selektor `tr[class="total"]` da cilja specifično vaše `tr` elemente sa klasom `total`. Dajte mu donju ivicu od `4px double #0a0a23` i učinite font podebljanim.
