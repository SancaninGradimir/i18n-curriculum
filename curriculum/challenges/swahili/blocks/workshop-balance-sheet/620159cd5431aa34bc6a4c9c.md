...
Ciljate svoj fleks kontejner sa selektorom `h1 .flex`. Dajte mu svojstvo `display` postavljeno na `flex` da biste omogućili flexbox raspored. Zatim postavite svojstvo `flex-direction` na `column-reverse` - ovo će prikazati ugnježđene elemente odozdo prema gore. Na kraju, postavite svojstvo `gap` na `1rem` da biste stvorili razmak između elemenata.
