...
Unutar vašeg `thead`, kreirajte element `tr`. U njemu, dodajte element `td` i tri elementa `th`. Unutar svakog od elemenata `th`, dodajte element `span` sa `class` podešenim na `sr-only` i sledeći tekst, po redu: `2019`, `2020`, i `2021`.
