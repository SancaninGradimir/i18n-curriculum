...
U vašem drugom `tr` elementu, dodajte `th` element sa tekstom `Checking Our primary transactional account.`. Umotajte taj tekst, osim za `Checking `, u `span` element sa atributom `class` postavljenim na `description`.

Nakon toga, dodajte tri `td` elementa sa sledećim tekstom (u redu): `$54`, `$56`, `$53`. Trećem `td` elementu dodelite atribut `class` postavljen na `current`.
