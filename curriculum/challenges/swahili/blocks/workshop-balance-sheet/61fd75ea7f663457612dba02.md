...
U svakom vašem novom `th` elementu, ugnježdi `span` element sa atributom `class` postavljenim na `sr-only year`. Daj im sledeći tekst (u redosledu): `2019`, `2020` i `2021`.

Daj vašem trećem `th` elementu atribut `class` postavljen na `current`.

Ostavite `td` element prazan. Ovaj element postoji samo da bi osigurao da vaša tabela ima raspored od četiri kolone i povezao zaglavlja sa odgovarajućim kolonama.
