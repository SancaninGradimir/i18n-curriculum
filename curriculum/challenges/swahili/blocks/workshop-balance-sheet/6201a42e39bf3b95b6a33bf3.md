...
Kreiraj `tr.data th .description` selektor za ciljanje elemenata sa `class` podešenim na `description` koji se nalaze unutar vaših `th` elemenata u redovima tabele `.data`. Daj im blok prikaz (block display), učini tekst italik sa normalnom težinom, i pozicioniraj ih sa `padding` podešenim na `1rem 0 0.75rem` i desnim margina od `-13.5rem`.
