...
Stilizujte tekst unutar vašeg elementa `#years` kreiranjem selektora `#years span[class]`. Sintaksa `span[class]` će ciljati bilo koji element `span` koji ima postavljen atribut `class`, bez obzira na vrednost atributa.

Dajte vašem novom selektoru font `bold`, širinu od `4.5rem` i tekst poravnat sa desne strane.
