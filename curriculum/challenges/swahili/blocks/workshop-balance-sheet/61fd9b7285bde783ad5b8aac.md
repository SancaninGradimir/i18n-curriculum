...
Unutar `tbody`, dodajte `tr` sa `class` podešenim na `total`. Tamo, dodajte `th` sa tekstom `Total Net Worth`, i obuhvatite `Net Worth` u `span` sa `class` podešenim na `sr-only`.

Zatim dodajte tri `td` elementa, dajući trećem `class` podešenom na `current`, i dajući svakom sledeći tekst: `$-171`, `Within the `tbody`, add a `tr` with the `class` set to `total`. In that, add a `th` with the text `Total Net Worth`, and wrap `Net Worth` in a `span` with the `class` set to `sr-only`.

Then add three `td` elements, giving the third a `class` set to `current`, and giving each the following text: `$-171`, `$136`, `$334`.36`, `$334`.
