...
Pseudo-selektor `:nth-of-type()` se koristi za ciljanje specifičnih elemenata na osnovu njihovog redosleda među braćama istog tipa. Koristite ovaj pseudo-selektor za ciljanje trećeg elementa `td` unutar vaših redova tabele `total`. Dajte mu desni padding od `0.5rem`.
