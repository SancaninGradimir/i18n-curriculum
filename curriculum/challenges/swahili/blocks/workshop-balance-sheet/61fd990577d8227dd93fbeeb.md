...
U vašem četvrtom `tr` elementu, dodajte `th` element sa tekstom `Total Liabilities`. Omotajte tekst `Liabilities` u `span` element sa atributom `class` postavljenim na `sr-only`.

Nakon toga, dodajte tri `td` elementa sa sledećim tekstom (u redu): `$750`, `$600`, `$475`. Dajte trećem `td` elementu atribut `class` postavljen na `current`.
