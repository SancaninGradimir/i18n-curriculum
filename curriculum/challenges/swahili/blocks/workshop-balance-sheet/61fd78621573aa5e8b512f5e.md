...
U vašem trećem `tr` elementu, dodajte `th` element sa tekstom `Savings Funds set aside for emergencies.`. Omotajte taj tekst, osim za `Savings `, u `span` element sa atributom `class` postavljenim na `description`.

Nakon toga, dodajte tri `td` elementa sa sledećim tekstom (u redu): `$500`, `$650`, `$728`. Trećem `td` elementu dodelite atribut `class` postavljen na `current`.
