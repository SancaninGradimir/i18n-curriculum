...
1. Trebalo bi da imate `div` sa atributom `class` sa vrednošću `business-card` koji će sadržati sve ostale elemente. 
1. Unutar elementa `.business-card`, treba da postoji element `img` sa atributom `class` sa vrednošću `profile-image`. Možete postaviti izvor slike na `https://cdn.freecodecamp.org/curriculum/labs/flower.jpg` ako želite. Treba da postoji `alt` sa smislenim opisom.
1. Unutar elementa `.business-card`, nakon elementa `img`, trebalo bi da imate tri `p` elementa sa atributom `class` koji, respektivno, ima vrednost `full-name`, `designation` i `company`.
1. Prvi `p` element treba da sadrži vaše ime.
1. Drugi `p` element treba da sadrži vašu titulu/funkciju.
1. Treći `p` element treba da sadrži naziv vaše kompanije. 
1. Ispod trećeg __INLINE_
