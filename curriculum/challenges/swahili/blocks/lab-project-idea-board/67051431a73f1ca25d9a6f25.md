...
title: Izgradite tablu sa idejama za projekat
challengeType: 26
...
1. Treba definisati objektnu konstantu pod nazivom `projectStatus` sa tri ključa: `PENDING`, `SUCCESS` i `FAILURE`. Svakom statusu treba dodeliti objekat sa ključem `description` sa vrednostima `Pending Execution`, `Executed Successfully` i `Execution Failed`, respektivno.
2. Treba definisati klasu pod nazivom `ProjectIdea` sa konstruktorom koji prima `title` i `description` kao parametre. Inicijalizovati svojstva `title` i `description` pruženim parametrima. Klasa takođe treba da ima svojstvo pod nazivom `status` koje se po defaultu postavlja na vrednost `projectStatus.PENDING`.
3. Treba definisati metodu pod nazivom `updateProjectStatus` unutar klase `ProjectIdea`. Ova metoda treba da prihvati parametar `newStatus` i ažurira svojstvo `status` na zadatu vrednost.
4. Treba definisati klasu `ProjectIdeaBoard` sa konstruktorom koji prima `title` i inicijalizuje prazan niz pod nazivom `ideas` za čuvanje instanci klase `ProjectIdea`.
5. Treba definisati metodu pod nazivom `pin` unutar klase `ProjectIdeaBoard` koja prima instancu klase `ProjectIdea` i dodaje tu instancu u niz `ideas`.
6. Treba definisati metodu pod nazivom `unpin` unutar klase `ProjectIdeaBoard` koja prima instancu klase `ProjectIdea` i uklanja je iz niza `ideas`.
7. Treba definisati metodu pod nazivom `count` koja vraća broj projektnih ideja u datoem nizu `ProjectIdeaBoard`.
8. Treba definisati metodu pod nazivom `formatToString` koja vraća ime projektnih ideja, njihov opis i status u formatu:
