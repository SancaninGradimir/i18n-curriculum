...
Na kraju, ispišite rezultate za trenutni čvor.

Vaš izlaz bi trebalo da prikaže dve stvari: rastojanje od `start_node` do `node_no` i putanju koja je korišćena za dolazak tamo.

Koristite `f-string` da formatirate izlaz i prikažete kako rastojanje, tako i punu putanju koristeći format:
...
- `[starting node]` je `start_node`
- `[node number]` je `node_no`
- `[distance]` je rastojanje od `start_node` do `node_no`
- `[path]` je putanja koja je korišćena za dolazak do `node_no`
