...
U ovom trenutku, želite da prikažete rezultate samo za čvorove koji nisu početni čvor i koji su dostiživi od njega.

Dodajte uslov koji proverava da li je `node_no` jednako `start_node` **ili** da li je `distances[node_no]` jednako `INF`.

Ako je bilo koji uslov tačan, koristite `continue` da preskočite na sledeću iteraciju petlje.
