...
Ako je uslov koji ste upravo dodali istinit, to znači da je trenutni čvor najbolja neposeta opcija koju ste do sada pronašli i morate ažurirati svoje varijable kako biste to odrazili.

Unutar bloka `if`, ažurirajte `min_distance` na `distances[node_no]` i postavite `current` na `node_no`.
