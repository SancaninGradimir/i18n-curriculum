...
Sada kada ste izračunali novu moguću udaljenost do suseda, proverite da li je bolja od one trenutno sačuvane u listi `distances`. Ako jeste, ažurirajte udaljenost.

Unutar postojećeg bloka `if`, dodajte izjavu `if` koja proverava da li je `new_distance` manje od `distances[node_no]`.

Unutar ovog novog uslovnog bloka, ažurirajte `distances[node_no]` da sačuva `new_distance`.
