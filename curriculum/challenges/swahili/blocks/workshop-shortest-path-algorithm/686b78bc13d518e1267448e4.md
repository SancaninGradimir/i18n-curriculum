...
Pored praćenja udaljenosti, morate pratiti i stvarne staze pređene za dolazak do svakog čvora.

Kreiraćete listu gde svaki unos skladišti putanju pređenu za dolazak do tog čvora. Inicijalno, staza svakog čvora će sadržati samo njega samog.  

List razumevanja pruža koncizan način za kreiranje lista. Na primer:

```py
[x * 2 for x in range(3)]
```

Kreirajte varijablu pod nazivom `paths` i inicijalizujte je korišćenjem list razumevanja koji kreira listu koja sadrži `[node_no]` za svaki `node_no` u `range(n)`.
