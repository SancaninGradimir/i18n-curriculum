...
Kada algoritam završi rad, morate odlučiti za koje čvorove da prikažete rezultate.

Ako je dostavljen određeni `target_node`, prikazaćete samo udaljenost i put do tog čvora. U suprotnom, prikazaćete rezultate za sve čvorove.

Nakon što se spoljašnja petlja `for _ in range(n):` završi, kreirajte varijablu pod nazivom `targets`. Koristite uslovni izraz da joj dodelite `[target_node]` ako `target_node` nije `None`, u suprotnom dodelite `range(n)`.
