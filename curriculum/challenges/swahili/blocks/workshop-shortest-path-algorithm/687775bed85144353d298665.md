...
Sada kada ste izabrali i označili čvor kao posetić, vreme je da pogledate sve njegove susede kako biste videli da li možete pronaći kraće staze do njih.

After the line `visited[current] = True`, add a `for` loop that iterates through `node_no` in `range(n)`.

Unutar ove petlje, kreirajte varijablu `distance` i postavite je na `matrix[current][node_no]`. Ovo će vam dati rastojanje od trenutnog čvora do susednog čvora.
