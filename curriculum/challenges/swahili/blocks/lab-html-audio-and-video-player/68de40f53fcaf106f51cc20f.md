...
title: Napravite HTML audio i video plejer
challengeType: 25
...
1. Trebalo bi da imate `h1` element za glavni naslov stranice.
2. Trebalo bi da imate dva `section` elementa.
3. Unutar prvog `section` elementa, trebalo bi da imate `h2` element za naslov reprodukovanog videa.
4. Ispod `h2` elementa, trebalo bi da imate `video` element sa atributima `controls` i `width`. Atribut `width` treba da bude postavljen na `640`.
5. Unutar `video` elementa, trebalo bi da imate `source` element sa atributom `src` koji ukazuje na video fajl i atributom `type`.
   - Možete koristiti `https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`.
6. Unutar drugog `section` elementa, trebalo bi da imate `h2` element za naslov reprodukovane pesme.
7. Ispod `h2` elementa, trebalo bi da imate `audio` element sa atributima `controls` i `loop`, i atributom `src` koji ukazuje na audio fajl.
