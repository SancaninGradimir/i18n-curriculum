...
Sada ćete implementirati glavnu BFS petlju. Kreirajte `while` petlju koja se nastavlja dok `queue` nije prazan (dok `queue` ne evaluira na `True`).

Unutar petlje, ispišite `queue`.
