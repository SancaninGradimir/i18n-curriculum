...
Da biste označili polja za čekiranje, dodajte atribut `checked` na polje za čekiranje `input`. Koristite metodu `includes` da proverite da li je trenutni `power` u nizu `powers`.

`onChange` će biti zasebna funkcija, postavite ga na `handlePowersChange` za sada. Ovo će dovesti do greške koju ćete popraviti u sledećem koraku.
