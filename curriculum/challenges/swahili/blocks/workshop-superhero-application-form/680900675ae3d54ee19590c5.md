...
Vreme je da kreiramo etikete i ulaze. U formularu, kreirajte `label` sa tekstom `Hero Name` i elementem `input` tipa `text` unutar njega.

Povežite `input` za `Hero Name` sa stanjem varijablom `heroName` dodeljivanjem atributa `value`, postavljenog na `heroName`.
