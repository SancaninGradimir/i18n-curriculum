...
Sledeći `input` je polje za čekiranje za označavanje svih superherojskih moći koje se primenjuju na korisnika.

Kreirajte element `label` sa `className` od `section` i `column`, i tekstom od `List your powers (select all that apply):`.
