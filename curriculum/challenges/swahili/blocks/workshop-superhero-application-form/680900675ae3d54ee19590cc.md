...
Vrednosti `powerSourceOptions` sada se pojavljuju kao opcije za element `select`.

Sada, koristite `powerSource` kao vrednost za element `select`. Zatim, postavite rukovatelja (handler) `onChange` koristeći strelicastu funkciju koja uzima `e` kao parametar i implicitno vraća `setPowerSource(e.target.value)`.
