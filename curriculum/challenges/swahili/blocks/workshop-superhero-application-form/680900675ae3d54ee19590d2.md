...
Kreiraj element `button` sa `className` od `submit-btn` i `type` od `submit`. Daj dugmetu tekst `Join the League`.

Nakon toga, pokušajte da pošaljete formular i vidite šta se dešava.
