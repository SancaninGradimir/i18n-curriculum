...
Uklonite tekst `Hello` unutar `label`, zatim kreirajte `input` tipa `checkbox` i `value` od `power`.

Na kraju, prikažite vrednosti iz niza pored svakog čekboksa kreiranjem elementa `span` odmah pre zatvarajućeg `label` sa tekstualnim sadržajem `{power}`.
