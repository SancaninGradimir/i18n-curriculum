...
Primetili ste da možete poslati formular bez popunjavanja polja?

Da biste to sprečili, dodajte atribut `disabled` da onemogućite dugme za slanje ako su `heroName`, `realName` ili `powerSource` lažni (falsy), ili ako je dužina `powers` jednaka `0`.

Sada će dugme biti onemogućeno ako nešto nije popunjeno. Popunite formular sa informacijama o vašem superheroju i pošaljite ga da biste ušli u ligu.

Time je vaš obrazac za prijavu superjunaka završen.
