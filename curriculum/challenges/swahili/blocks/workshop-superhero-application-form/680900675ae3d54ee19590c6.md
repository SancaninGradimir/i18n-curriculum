...
Možda ćete primetiti da ne možete kucati u `input`. Ako otvorite konzolu, videćete upozorenje.

Da biste uklonili upozorenje i popravili problem sa tipiziranjem unosa, koristite događaj `onChange` da ažurirate `heroName`. Vrednost za događaj `onChange` treba da bude strelica funkcija sa `e` kao parametrom. Strelica funkcija bi trebalo implicitno da vrati `setHeroName(e.target.value)`.
