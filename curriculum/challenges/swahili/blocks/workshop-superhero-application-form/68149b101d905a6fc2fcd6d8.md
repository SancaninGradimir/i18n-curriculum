...
Ispod destrukturiranih `value` i `checked`, ažurirajte listu proverenih snaga.

Pozovite funkciju `setPowers`, koja prima niz kao argument. Koristite ternarni operator da proverite da li je `checked` tačno. Ako jeste, proširite postojeći niz `powers` u novi niz i dodajte mu `value`. Ako nije tačno, filtrirajte `value` iz `powers` pomoću `powers.filter(p => p !== value)`.
