...
U vašem `select` elementu, morate kreirati opcije koje korisnik može izabrati kao izvor napajanja. `select` treba da ima sedam opcija navedenih ispod:
...
Prva opcija (`Select one`) treba da ima prazan string kao svoj atribut `value` (tj. `value=''`). Ovo služi kao zamena u korisničkom interfejsu (UI).
