...
Kreiraj još jedan `label` sa tekstom `Real Name` i `input` tipa `password` unutar njega. Daj `input` `value` od `realName`.

Zatim koristi događaj `onChange` da ažuriraš `realName` na isti način kao što si ažurirao `heroName`.
