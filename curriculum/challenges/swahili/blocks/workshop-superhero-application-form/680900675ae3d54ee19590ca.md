...
Da biste uzeli u obzir setter funkciju, moraćete da priložite atribut `value` i događaj `onChange` na `select`, a atribut `value` svakom elementu `option`.

To je dug proces, zar ne? Zato je najbolji način za rešavanje toga da imate niz opcija vrednosti i da ga mapirate kako biste kreirali elemente `option`.

Niz `powerSourceOptions` je dodan na vrh vaše komponente. Pogledajte ga.

Trenutno, uklonite sve opcije unutar elementa `select`, osim prve. U sledećem koraku ćete mapirati niz.
