...
Biće šest polja za čekiranje, pa je najbolje kreirati niz za njih i iterirati kroz njih kako bi se prikazali kao polja za čekiranje. Za to, dostavljen vam je niz ``powersOptions``.

Koristite strelicastu funkciju da iterirate kroz niz ``powersOptions`` koristeći parametar ``power``. Unutar mape, kreirajte element ``label`` sa ``key`` od ``power``, i tekstom ``Hello``.

Nakon toga, trebalo bi da vidite šest tekstova ``Hello``. Elemente niza ćete popuniti u sledećem koraku.
