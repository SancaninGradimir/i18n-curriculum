...
U ovoj radionici, naučićete kako da radite sa formularima u React-u kreiranjem obrasca za prijavu superjunaka.

Dostavljen je neki osnovni kod (boilerplate), samo treba da napravite ostatak React komponente.

Počnite vraćanjem elementa `div` sa `className` od `form-wrap`. Unutar `div`, kreirajte element `h2` sa tekstom `Superhero Application Form`. Ispod toga, kreirajte element `p` sa tekstom `Please complete all fields`.
