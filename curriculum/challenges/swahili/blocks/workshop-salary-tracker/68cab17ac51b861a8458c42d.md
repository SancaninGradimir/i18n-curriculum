...
Konačno, sada kada ste završili ``salary`` setter, možete ga pozvati unutar ``level`` settera.

Izmenite liniju ``self._salary = Employee._base_salaries[new_level]`` tako da poziva ``salary`` setter umesto da direktno modifikuje atribut ``_salary``.

Time se radionica za praćenje plate završava.
