...
Sada je vreme da dodamo neku validaciju metodi `__init__`. Na početku metode, kreiraj izjavu `if` koja proverava da li su ni `name` niti `level` instance od `str`.

Unutar `if` izjave, baci/izbaci `TypeError` sa porukom `'name' and 'level' attribute must be of type 'str'.`
