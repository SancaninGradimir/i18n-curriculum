...
Kao što ste naučili u prethodnoj lekciji, setter nudi način za kontrolisanje načina na koji se atribut može modifikovati. Da bi se osiguralo da je `new_name` pravi tip, kreirajte izjavu `if` koja će baciti `TypeError` sa porukom `'name' must be a string.` kada `new_name` nije instanca od `str`.
