...
Da bi se osiguralo da je `new_level` pravi tip, kreirajte izjavu `if` koja će baciti `TypeError` sa porukom `'level' must be a string.` kada `new_level` nije instanca od `str`.
