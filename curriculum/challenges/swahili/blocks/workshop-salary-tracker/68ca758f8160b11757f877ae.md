...
Na kraju, kreirajte četvrti `if` izjava koja pokreće `ValueError` sa porukom `Cannot change to lower level.` kada je osnovna plata novog nivoa manja od osnovne plate trenutnog nivoa.

Koristite `hasattr(self, '_level')` da biste izbegli `AttributeError` tokom inicijalizacije.
