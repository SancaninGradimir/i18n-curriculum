...
Sada ćete kreirati seter za atribut `_level`. Kreirajte metodu pod nazivom `level` sa parametrima `self` i `new_level`. Dekorirajte metodu sa `@level.setter`.

Unutar metode, postavite `self._level` na `new_level`.
