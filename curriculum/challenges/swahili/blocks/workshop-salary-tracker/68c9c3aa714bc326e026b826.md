...
Novi nivo ne može biti postavljen bez provere da li je validan nivo. Nakon provere `isinstance`, kreirajte izjavu `if` koja će baciti `ValueError` kada `new_level` nije ključ u `Employee._base_salaries`.

Za poruku o grešci, koristite `Invalid value '{new_level}' for 'level' attribute.`, gde treba zameniti `{new_level}` argumentom prosleđenim seteru.
