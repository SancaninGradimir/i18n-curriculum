...
Sada kada je kreiran seter `salary`, ažurirajte `__init__` da ga pozove umesto da direktno dodeljuje na `self._salary`. Promenite `self._salary = Employee._base_salaries[level]` u `self.salary = Employee._base_salaries[level]`.
