...
Za dva `p` elementa koje ste upravo dodali, dodajte `dessert` kao vrednost atributa `class` prvog `p` elementa i vrednost `price` kao atribut za drugi `p` element, `class`.
