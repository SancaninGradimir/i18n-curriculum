...
Slika koju ste dodali nije horizontalno centrirana kao naslov `Coffee` iznad nje. Elementi `img` su "kao" ugrađeni elementi.

Da biste učinili da slika se ponaša kao blok elementi, poput naslova, kreirajte tip selektor `img`. Postavite svojstvo `display` na `block`, i koristite svojstva `margin-left` i `margin-right` da ga horizontalno centrirate.
