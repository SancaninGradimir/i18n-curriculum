...
Promena `margin-bottom` u `5px` izgleda odlično. Međutim, sada razmak između stavke menija `Cinnamon Roll` i drugog elementa `hr` ne odgovara razmaku između gornjeg elementa `hr` i naslova `Coffee`.

Dodajte više razmaka kreiranjem klase pod nazivom `bottom-line` koristeći `25px` za svojstvo `margin-top`.
