...
Endelea na ongeza kipengele kingine cha `hr` kati ya kipengele cha `main` na kipengele cha `footer`.

# --hints--

Treba da dodate drugi element za `hr`.

```js
assert.lengthOf(document.querySelectorAll('hr'), 2);
```

Tvoja nova komponenta `hr` treba da bude između komponente `main` i komponente `footer`.
