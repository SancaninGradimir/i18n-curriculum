...
Elementi `p` ugnježbeni su unutar elementa `article` sa atributom klase `item`. Možete stilizovati sve elemente `p` ugnježbene bilo gde u elementima sa klasom nazvanom `item` na ovaj način:

```css
.item p { }
```

Koristeći gore navedeni selektor, dodajte svojstvo `display` sa vrednošću `inline-block` kako bi elementi `p` ponašali više kao inline elementi.
