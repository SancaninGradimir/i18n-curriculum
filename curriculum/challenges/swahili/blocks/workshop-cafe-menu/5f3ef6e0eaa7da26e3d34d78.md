...
Možete dodati mali razmak između vaše menije i sadržaja, kao i sa strane, koristeći različite atribute za `padding`.

Daj klasu od `menu` `padding-left` i `padding-right` sa vrednošću jednaku `20px`.

# --hints--

Trebalo bi da postaviš atribut `padding-left` na `20px`.
...
Kipengele chako cha `.menu` kinapaswa kuwa na `padding-left` ya `20px`.
...
Vaš element `.menu` bi trebalo da ima `padding-right` od `20px`.
