...
Menjanje svojstava linka kada mišem pređe preko njih radi se korišćenjem `<dfn>pseudo-selector</dfn>` koji izgleda kao ``a:hover { propertyName: propertyValue; }``.

Promenite boju linka za podnožje ``Visit our website`` da bude ``brown`` kada korisnik pređe mišem preko njega.
