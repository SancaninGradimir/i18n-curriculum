...
Ako smanjite širinu pregleda stranice, primetićete da deo teksta na levoj strani počinje da se prelapa na sledeći red. To je zato što širina elemenata `p` na levoj strani može zauzeti samo `50%` prostora.

Pošto cene na desnoj strani imaju značajno manje karaktera, ažurirajte klasu `flavor` `width` u `75%` i klasu cene `width` u `25%`.
