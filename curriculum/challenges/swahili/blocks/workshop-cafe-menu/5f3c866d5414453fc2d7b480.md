...
Ispod postojećeg para (kafa/cena), dodajte sledeće kafe i cene koristeći elemente ``article`` sa dva ugnježdena elementa ``p`` unutar svakog.
...
Kao i pre, tekst prvog elementa ``p`` treba da sadrži ukus kafe, a tekst drugog elementa ``p`` treba da sadrži cenu.
