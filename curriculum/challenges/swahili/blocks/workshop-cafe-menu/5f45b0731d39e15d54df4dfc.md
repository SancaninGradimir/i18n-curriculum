...
Podrazumevana boja veze koja još nije kliknuta je obično plava. Podrazumevana boja veze koja je već poseta sa stranice je obično ljubičasta.

Da bi se `footer` veze učinile iste boje bez obzira na to da li je veza poseta, koristite tip selektor za element anker (`a`) i koristite vrednost `black` za svojstvo `color`.
