...
Možete koristiti element `hr` za prikaz razdelnika između sekcija sa različitim sadržajem.
...
Prvo, dodajte element `hr` između elementa `p` sa klasom `established` i prvog elementa `section`.

Napomena da je element `hr` prazan element.
