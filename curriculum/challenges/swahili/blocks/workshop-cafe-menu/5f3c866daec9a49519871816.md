...
`article` elementi često sadrže više elemenata koji imaju povezane informacije. U ovom slučaju, sadržaću ukus kafe i cenu za taj ukus.

Ugnjezdajte dva `p` elementa unutar vašeg `article` elementa. Tekst prvog treba da bude `French Vanilla`, a tekst drugog `3.00`.
