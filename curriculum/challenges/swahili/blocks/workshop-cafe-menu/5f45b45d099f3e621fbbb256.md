...
Menjate svojstva linka kada se klikne na njega koristeći `<dfn>pseudo-selector</dfn>` koji izgleda kao ``a:active { propertyName: propertyValue; }``.

Promenite boju linka u podnožju (footer) ``Visit our website`` na ``white`` kada se klikne.
