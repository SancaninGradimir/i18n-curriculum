...
Koristeći sinaksu strelice, dovršite callback funkciju za `forEach`. Unutar tela callback funkcije koristite operater dodeljivanja vrednosti da postavite `innerHTML` od `tasksContainer` na prazne placeholder markere.

# --hints--

Ne smijete mijenjati `taskData.forEach()` koji je prisutan sa oštećenom vrednošću.
...
Trebalo bi da koristiš sinaksu sa strelicom i priloži `innerHTML` i `tasksContainer`.

```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{.*\}\s*\)\s*=>\s*(\s*\{?\(\s*tasksContainer\.innerHTML\s*\)\s*\}?)?/)
```

Unapaswa kutumia mgawo wa thamani wa kuongeza kuweka `innerHTML` ya `tasksContainer` kuwa jozi tupu ya alama za kupigia.
