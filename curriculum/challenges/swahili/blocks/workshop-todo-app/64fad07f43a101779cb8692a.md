...
Možete poboljšati čitljivost i održivost koda refaktorisanjem slušaoca događaja `submit` u dve zasebne funkcije. Prvu funkciju možete koristiti za dodavanje ulaznih vrednosti u `taskData`, dok je druga funkcija odgovorna za dodavanje zadataka u DOM.

Koristite strelicasti sintaks (arrow syntax) da kreirate funkciju `addOrUpdateTask`. Zatim premestite varijablu `dataArrIndex`, objekat `taskObj` i izjavu `if` u funkciju `addOrUpdateTask`.
