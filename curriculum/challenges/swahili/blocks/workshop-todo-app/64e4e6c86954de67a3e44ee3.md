...
Potrebno je da pristupite više elemenata sa metodom `getElementById()`. Ovog puta vam trebaju elementi `close-task-form-btn`, `add-or-update-task-btn` i `cancel-btn`. Sačuvajte ih u varijable `closeTaskFormBtn`, `addOrUpdateTaskBtn` i `cancelBtn`.
