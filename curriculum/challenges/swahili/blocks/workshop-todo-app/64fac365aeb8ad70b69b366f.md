...
Unutar funkcije `reset`, postavite svaku vrednost `titleInput`, `dateInput`, `descriptionInput` na prazan string.

Takođe, koristite `classList` da prebacujete klasu `hidden` na `taskForm` i postavite `currentTask` na prazan objekat. To je zato što u ovom trenutku, `currentTask` će biti popunjeno zadatkom koji je korisnik možda dodao.
