...
Kao što ste uradili u funkciji `deleteTask`, morate pronaći indeks zadatka koji treba da se izmeni.

Kreirajte varijablu `dataArrIndex`. Za njenu vrednost, koristite metod `findIndex()` na `taskData`. Prosledite `item` kao parametar njegovoj funkciji povratne pozive i proverite da li je `id` od `item` jednak `id` od `parentElement` od `buttonEl`.
