...
Kao što ste se setili iz prethodnih lekcija, `localStorage` je deo Web Storage API-ja koji omogućava da podaci ostanu trajni čak i nakon zatvaranja prozora pretraživača ili osvežavanja stranice.

Evo podsetnika kako postaviti stavku u `localStorage`:

```js
localStorage.setItem("key", value); // value could be string, number, or any other data type
```

Omogućili smo vam niz tipa `myTaskArr`. Koristite metodu `setItem()` da ga sačuvate ključem `data`.
