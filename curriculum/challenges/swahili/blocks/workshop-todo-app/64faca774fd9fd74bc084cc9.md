...
Trebalo bi da prikažete dugmad `Cancel` i `Discard` korisniku samo ako postoji neki tekst u poljima za unos.

Počnite sa uklanjanjem linije `confirmCloseDialog.showModal();`.

Zatim unutar slušaoca događaja `closeTaskFormBtn`, napišite neku logiku koja proverava da li postoji vrednost u polju `titleInput`, `dateInput` **ili** polje `descriptionInput`.

Ako bilo koje od tih polja za unos ima vrednost, onda koristite metod `showModal()` na `confirmCloseDialog`. U suprotnom, ako nema promena, pozovite funkciju `reset()` da obriše polja za unos i sakrije modal formulara.
