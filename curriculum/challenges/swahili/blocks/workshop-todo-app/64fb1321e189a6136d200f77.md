...
Zadatak koji treba da se izmeni je sada u `currentTask` objektu. Postavite ga za uređivanje unutar polja za unos postavljanjem vrednosti `titleInput` na `currentTask.title`, `dateInput` na `currentTask.date`, i `descriptionInput` na `currentTask.description`.
