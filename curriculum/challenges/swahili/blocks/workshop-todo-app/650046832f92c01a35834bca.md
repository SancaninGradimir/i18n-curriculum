...
Takođe želite da obrisani zadatak bude uklonjen iz lokalnog skladišta. Za ovo, ne treba vam metode `removeItem()` niti `clear()`. Pošto već koristite `splice()` da uklonite obrisani zadatak iz `taskData`, sve što sada treba da uradite je ponovo sačuvati `taskData` u lokalno skladište.

Koristite `setItem()` da ponovo sačuvate niz `taskData`. Prosledite `data` kao ključ i uverite se da je `taskData` konvertovan u string pre čuvanja.
