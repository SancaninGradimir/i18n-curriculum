...
Morate prvo pronaći indeks zadatka koji želite obrisati.

Kreirajte varijablu `dataArrIndex` i postavite njenu vrednost koristeći metodu `findIndex()` na nizu `taskData`. Prođite `item` kao parametar za strelicu funkciju povratne pozive (arrow callback function), a unutar povratne pozive, proverite da li je `id` od `item` jednak `id` od `parentElement` od `buttonEl`.
