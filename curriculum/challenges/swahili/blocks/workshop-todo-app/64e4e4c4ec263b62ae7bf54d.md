...
U ovom projektu, naučićete kako `localStorage` radi u JavaScriptu gradeći aplikaciju za zadatke (Todo). <dfn>LocalStorage</dfn> je funkcija za skladištenje na vebu u JavaScriptu koja vam omogućava da sačuvate podatke tako što ih čuvate kao par **ključ:vrednost**.

HTML i CSS za ovaj projekat obezbeđeni su za vas. Pogledajte fajlove kako biste se upoznali sa njima.

Počnite pristupom elementima `task-form`, `confirm-close-dialog` i `open-task-form-btn` pomoću metode `getElementById()`. Sačuvajte ih u varijable `taskForm`, `confirmCloseDialog` i `openTaskFormBtn`.
