...
Sada kada ste radili na otvaranju i zatvaranju modala, vreme je da dobijete vrednosti iz polja za unos, sačuvate ih u niz ``taskData`` i prikažete ih na stranici.

Da biste počeli, dodajte ``submit`` slušalicu događaja (event listener) na vaš element ``taskForm`` i prosledite ``e`` kao parametar vaše strelice funkcije. Unutar vitičastih zagrada, koristite metodu ``preventDefault()`` da sprečite pregledač od osvežavanja stranice nakon slanja forme.
