...
Sada bi trebalo da sačuvate stavke zadataka u lokalno skladište kada korisnik dodaje, ažurira ili uklanja zadatak.

Unutar funkcije `addOrUpdateTask`, koristite `setItem()` da sačuvate zadatke sa ključem `data`, a zatim prosledite niz `taskData` kao njegov argument. Osigurajte da stringifikujete `taskData`.

Ovo bi očuvalo podatke kada korisnik doda ili ažurira zadatke.
