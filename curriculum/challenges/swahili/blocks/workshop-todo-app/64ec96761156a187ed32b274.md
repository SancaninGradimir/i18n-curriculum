...
Da biste omogućili upravljanje zadacima, morate uključiti dugme za brisanje i dugme za uređivanje za svaki zadatak.

Kreirajte dva `button` elementa sa atributom `type` podešenim na `button` i atributom `class` podešenim na `btn`. Podesite tekst prvog dugmeta na `Edit` i tekst drugog dugmeta na `Delete`.
