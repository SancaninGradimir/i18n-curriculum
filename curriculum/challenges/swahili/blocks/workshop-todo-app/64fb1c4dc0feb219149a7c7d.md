...
Ako korisnik pokušava da izmeni zadatak, ali odluči da ne napravi nikakve promene pre zatvaranja forme, nije potrebno prikazivati modal sa dugmićima `Cancel` i `Discard`.

Unutar slušaoca događaja `closeTaskFormBtn`, koristi se `const` za kreiranje druge varijable pod nazivom `formInputValuesUpdated`. Proveri da li je korisnik napravio promene dok pokušava da izmeni zadatak proveravanjem da vrednost `titleInput` **nije jednaka** `currentTask.title`, ili da vrednost `dateInput` **nije jednaka** `currentTask.date`, ili da vrednost `descriptionInput` **nije jednaka** `currentTask.description`.
