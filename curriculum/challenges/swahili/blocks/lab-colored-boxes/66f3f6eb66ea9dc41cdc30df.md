...
title: Dizajnirajte skup bojenih kutija
challengeType: 25
...
U ovom laboratorijumu, veštaćete se korišćenjem CSS boja dizajniranjem kutija.

**Cilj:** Ispunite priče korisnika ispod i učinite da svi testovi prođu kako biste završili laboratorijum.

**Priče korisnika:**

1. Trebalo bi da postavite boju pozadine za `body` na `#f4f4f4`.
2. Trebalo bi da imate `div` sa klasom `color-grid` za držanje svih vaših elemenata boja.
3. Trebalo bi da imate pet `div` elemenata unutar `.color-grid` elementa.
4. Pet `div` elemenata trebalo bi da imaju klasu `color-box` i `color#`, gde je `#` broj reda tog `div`. Na primer: `color1` za prvi `div`, `color2` za drugi, i tako dalje.
5. Klasa `.color-box` trebalo bi da ima postavljene `width` i `height` kako bi vaši `div` elementi bili vidljivi na stranici.
6. Element `.color1` trebalo bi da ima `background-color`, koji koristi heksadecimalnu vrednost boje.
7. Element `.color2` trebalo bi da ima `background-color`, koji koristi RGB vrednost boje.
8. Element `.color3` trebalo bi da ima `background-color`, koji koristi unapred definisanu (rečnu) vrednost boje.
9. Element `.color4` trebalo bi da ima `background-color`, koji koristi HSL vrednost boje.
10. Element `.color5` trebalo bi da ima postavljeno `background-color`.
