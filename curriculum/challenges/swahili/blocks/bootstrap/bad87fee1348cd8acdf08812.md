...
Bootstrap ima sopstvene stilove za elemente `button` koji izgledaju bolje od standardnih HTML elemenata.

Kreiraj novi element `button` ispod svoje velike slike malog mačka. Specifično, koristi klase `btn` i `btn-default`, zajedno sa tekstom `Like`.

# --hints--

Trebalo bi da kreiraš novi element `button` sa tekstom `Like`.
...
Tvoje novo dugme mora imati dve klase: `btn` i `btn-default`.
