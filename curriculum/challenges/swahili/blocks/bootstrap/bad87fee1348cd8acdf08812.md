...
Bootstrap ima svoje stilove za `button` elemente koji izgledaju bolje od podrazumevanog HTML izgleda.

Napravi novi `button` element ispod velike slike macke. Dodeli mu klase `btn` i `btn-default`, kao i tekst `Like`.

# --hints--

Treba da napravis novi `button` element sa tekstom `Like`.
...
Novo dugme treba da ima dve klase: `btn` i `btn-default`.
