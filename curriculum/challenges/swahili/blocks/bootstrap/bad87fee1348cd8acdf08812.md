...
Bootstrap ima stilove za elemente `<button>` koji izgledaju bolje od standardnih HTML elemenata.

Kreiraj novi element `<button>` ispod svoje velike slike mačke. Dodeli mu klase `btn` i `btn-default`, kao i tekst "Like".

# --hints--

Treba da kreiraš novi element `<button>` sa tekstom "Like".
