...
title: Daj svakom elementu jedinstveni ID
challengeType: 0
forumTopicId: 18191
...
Takođe ćemo koristiti jQuery za ciljanje svakog dugmeta njegovim jedinstvenim ID-em.

Dajte svakom svom dugmetu jedinstveni ID, počevši od `target1` i završavajući sa `target6`.

Uverite se da su `target1` do `target3` unutar `#left-well`, a `target4` do `target6` unutar `#right-well`.

# --hints--

Jedno dugme (`<button>`) mora imati ID `target1`.
...
Jedno dugme (`<button>`) mora imati ID `target2`.
...
Jedno dugme (`<button>`) mora imati ID `target3`.
...
Jedno dugme (`<button>`) mora imati ID `target4`.
...
Jedno dugme (`<button>`) mora imati ID `target5`.
...
Jedno dugme (`<button>`) mora imati ID `target6`.
