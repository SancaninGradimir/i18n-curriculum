...
Sledeći korak je da promenite liniju koda koja poziva metod `json` tako da umesto njega koristi operator `await`.

Na primer, dato ovaj kod:
...
Zatim, sačuvajte rezultat u vašem `authorDataArr`, i premestite liniju koda koja poziva `displayAuthors` tako da sledi odmah nakon vaše nove `await` naredbe.
Na kraju, obrišite preostu `then` naredbu i njen blok koda jer više ne služi nijednoj svrsi.

Primeći ćete `SyntaxError` u konzoli u ovom trenutku. Sledeći korak će se baviti refaktorisanjem metode `catch` kako bi se rešila ova greška.
