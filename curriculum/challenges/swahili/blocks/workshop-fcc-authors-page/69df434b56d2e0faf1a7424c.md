...
Sada kada je vaša fCC Authors Page potpuno funkcionalna, hajde da je refaktorišemo kako bismo poboljšali njenu čitljivost korišćenjem `async` i `await` umesto metode lančevanja obećanja (promise chaining method) `then`.

Zapamtite da biste koristili operator `await` za čekanje funkcije koja vraća `Promise`, morate obaviti poziv funkcije unutar `async` funkcije (ako nije u glavnom telu modula).

Pošto vaš `fetch` poziv nije definisan ni u asinhronoj funkciji ni u glavnom telu modula, ne možete koristiti `await` pre popravljanja toga.

Obuhvatite `fetch` izjavu i ceo niz lančanih then i catch metoda kao celinu u novu asinhronu strelicastu funkciju nazvanu `initialFetch` koja ne prima argumente. 

Primer deklaracije `async` funkcije je:
