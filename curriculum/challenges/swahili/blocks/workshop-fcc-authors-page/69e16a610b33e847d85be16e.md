...
U ovom trenutku, ponašanje aplikacije ne bi trebalo da se promenilo. Kod i dalje preuzima fajl sa CDN-a, a zatim obrađuje povratne podatke u `JSON`. Kada je to gotovo, on čuva te podatke u varijablu `authorDataArr` i zatim poziva `displayAuthors`.

Izmenite poziv na `fetch` tako da koristi operator `await`. Sačuvajte rezultat u novu varijablu `const` nazvanu `res`. Nova varijabla bi trebalo lokalno definisati unutar `initialFetch`.
...
Možda ćete videti `SyntaxError`. To se za sada može ignorisati.
