...
Sada je vreme da pozovete funkciju `displayAuthors`. Ali ponovo, ne želite popuniti stranicu sa svim autorima odjednom. Umesto toga, možete izvući deo autora sa varijablama `startingIndex` i `endingIndex`. Najbolji metod za ovo je metoda niza (array method) `.slice()`.

Prvo, uklonite izjavu za konsolu koja prikazuje `authorDataArr`. Zatim, pozovite funkciju `displayAuthors` sa nizom `authorDataArr` i `.slice()`. Koristite varijablu `startingIndex` za početnu tačku, a varijablu `endingIndex` za krajnju tačku.
