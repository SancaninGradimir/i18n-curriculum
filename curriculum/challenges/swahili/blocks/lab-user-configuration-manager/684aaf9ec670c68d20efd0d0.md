...
title: Izgradite menadžer konfiguracije korisnika
challengeType: 27
...
U ovom laboratorijumu, kreiraćete Menadžera konfiguracije korisnika (User Configuration Manager) koji omogućava korisnicima upravljanje svojim podešavanjima kao što su tema, jezik i obaveštenja. Implementovaćete funkcije za dodavanje, ažuriranje, brisanje i pregled podešavanja korisnika.

**Cilj:** Ispunite priče korisnika (user stories) ispod i učinite da svi testovi prođu kako biste završili laboratorijum.

**Priče korisnika:**

1. Trebalo bi da definišete funkciju pod nazivom `add_setting` sa dva parametra koja predstavljaju rečnik podešavanja i tupel koji sadrži par ključ-vrednost

1. Funkcija `add_setting` treba da:
    
   - Pretvori ključ i vrednost u mala slova (lowercase).
   - Ako postoji podešavanje sa ovim ključem, vrati `Setting '[key]' already exists! Cannot add a new setting with this name.`
   - Ako podešavanje sa ovim ključem ne postoji, dodaj par ključ-vrednost u dati rečnik podešavanja i vrati `Setting '[key]' added with value '[value]' successfully!`.
   - Poruke koje se vraćaju treba da imaju ključ i vrednost u mala slova (lowercase).

1. Trebalo bi da definišete funkciju pod nazivom `update_setting` sa dva parametra koja predstavljaju rečnik podešavanja i tupel koji sadrži par ključ-vrednost.

1. Funkcija `update_setting` treba da:
       
   - Pretvori ključ i vrednost u mala slova (lowercase).
   - Ako postoji podešavanje sa ovim ključem, ažuriraj njegovu vrednost u dati rečnik podešavanja i vrati: `Setting '[key]' updated to '[value]' successfully!`
   - Ako podešavanje sa ovim ključem ne postoji, vrati `Setting '[key]' does not exist! Cannot update a non-existing setting.`
   - Poruke koje se vraćaju treba da imaju ključ i vrednost u mala slova (lowercase).


1. Trebalo bi da definišete funkciju pod nazivom `delete_setting` sa dva parametra koja predstavljaju rečnik podešavanja i ključ.

1. Funkcija `delete_setting` treba da:
    
   - Pretvori prosleđeni ključ u mala slova (lowercase).
   - Ako postoji podešavanje sa ovim ključem, ukloni par ključ-vrednost iz dati rečnik podešavanja i vrati `Setting '[key]' deleted successfully!`
   - Ako podešavanje sa ovim ključem ne postoji, vrati `Setting not found!`
   - Poruke koje se vraćaju treba da imaju ključ u mala slova (lowercase).

1. Trebalo bi da definišete funkciju pod nazivom `view_settings` sa jednim parametrom koji predstavlja rečnik podešavanja.

1. Funkcija `view_settings` treba da:
   
   - Vrati `No settings available.` ako je dati rečnik podešavanja prazan. 
   - Ako rečnik sadrži bilo koja podešavanja, vrati string koji prikazuje ta podešavanja. String treba da počinje sa `Current User Settings:` praćen parovima ključ-vrednost, svaki na novoj liniji i sa kapitalizovanim ključem. Na primer, `view_settings({'theme': 'dark', 'notifications': 'enabled', 'volume': 'high'})` bi trebalo da vrati:
...
1. Za testiranje koda, trebalo bi da kreirate rečnik pod nazivom `test_settings` za skladištenje nekih preferencija konfiguracije korisnika.
