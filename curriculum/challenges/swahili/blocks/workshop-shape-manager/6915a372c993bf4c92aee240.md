...
Sada, kreiraj funkciju `initializeApp` i ostavi je praznu za sada.

Ovo će inicijalizovati celu aplikaciju kada se DOM učita. Dakle, to je funkcija koju ćeš uskoro proslediti u događaj `DOMContentLoaded`, koji će pokrenuti aplikaciju.

# --hints--

You should create an `initializeApp` function.

```js
assert.isFunction(initializeApp)
```
