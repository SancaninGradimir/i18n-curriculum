...
Dodajte događaj `DOMContentLoaded` na korenski dokument i prosledite `initializeApp` kao funkciju povratnog poziva (callback) za izvršavanje. Ovo osigurava da se sva inicijalizacija dešava tek nakon što je DOM potpuno učitan.

# --hints--

You should add the `DOMContentLoaded` event to the root `document`.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

You should reference `initializeApp` as the function to run in your event listener.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["'],\s*initializeApp\);?/)
```
