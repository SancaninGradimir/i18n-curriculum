...
U petlji, podesiti događaj `oninput` svakog elementa `input` na funkciju `handleInput`.

Time je radionica za upravljanje oblicima završena!

# --hints--

You should set `input.oninput` to the `handleInput` function.

```js
assert.match(code, /input\.oninput\s*=\s*handleInput;?/)
```
