...
Potrebno vam je da bezbedno pristupite elementu `select` i zaštitite se od vrednosti `null`. Za to, kreirajte konstantu `val` sa vrednošću `e.currentTarget` i tipizujte je kao `HTMLSelectElement` koristeći ključnu reč `as`.

Pod konstantom, koristite izjavu `if` da proverite da li je `val` lažno (falsy) negiranjem ga. U bloku `if`, vratite `target value not found`.

# --hints--

You should have a constant called `val`.

```js
assert.match(code, /const\s+val/)
```

Your `val` constant should have a `e.currentTarget` value typed `HTMLSelectElement` using the `as` keyword.
