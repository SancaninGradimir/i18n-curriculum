...
Unutar funkcije `handleShapeSelect`, pozovite metod `.preventDefault()` na `e`, i pozovite funkciju `clearInputFields()` za brisanje polja unosa.

# --hints--

You should call `.preventDefault()` on `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

You should call the `clearInputFields` function.

```js
assert.match(handleShapeSelect.toString(), /clearInputFields\(\);?/)
```
