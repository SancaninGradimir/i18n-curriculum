...
Sada, pozovite funkciju ``chooseShape()`` sa ``val.value`` da prikažete ispravna polja za unos, zatim pozovite ``updateResult()`` da ponovo izračunate i prikažete površinu.

# --hints--

You should call the `chooseShape` function with `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

You should call the `updateResult()` function.

```js
assert.match(handleShapeSelect.toString(), /updateResult\(\);?/)
```
