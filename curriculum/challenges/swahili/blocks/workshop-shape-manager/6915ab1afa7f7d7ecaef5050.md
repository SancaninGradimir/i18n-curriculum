...
Da biste konačno prikazali proračune, prođite kroz objekat `propertyInputs` koristeći petlju `for...of` sa `Object.entries()`. Možete ignorisati ključ korišćenjem zareza pre `input` u deklaraciji petlje.

# --hints--

You should create a `for...of` loop that destructures each entry from `Object.entries(propertyInputs)` by skipping the key and keeping only `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Your `for...of` loop should iterate over `Object.entries(propertyInputs)`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of\s+Object\.entries\(propertyInputs\)\)\s*\{\s*\}/)
```
