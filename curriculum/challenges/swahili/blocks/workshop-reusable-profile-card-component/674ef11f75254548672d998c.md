...
U ovom projektu, naučićete kako da radite sa propovima kreiranjem ponovo upotrebljive komponente za profilnu karticu.

Počnite korišćenjem ključne reči `export` da kreirate funkcionalnu komponentu `Card` sa `name`, `title` i `bio` kao propovima. Ne zaboravite da proverite da li su sve u zagradama (curly braces). Na taj način, dekonstruišete ih umesto da pristupate njima iz `props`.

Takođe, vratite par zagrada sa praznim stringom unutra za sada.
