...
Kao što ste se setili iz ranijih lekcija, možete koristiti metod `map` za iteraciju kroz različite objekte i dinamično prikazivanje komponenti na ovaj način:
...
Koristite metod `map()` za prikazivanje jedne `Card` iz niza. Prođite (ili Prosledite) `name`, `title` i `bio` kao props komponenti `Card`.
