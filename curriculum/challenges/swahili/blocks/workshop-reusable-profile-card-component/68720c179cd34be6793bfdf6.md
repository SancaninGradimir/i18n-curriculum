...
Iako vaša aplikacija radi kako je očekivano, obično ne biste ručno dodavali više `Card` komponenti na ovaj način.

Uklonite tri `<Card />` komponente unutar elementa `<div className="flex-container">`. U sledećim koracima, kreiraćete niz podataka i koristićete funkciju `map()` za renderovanje `Card` komponenti, što je uobičajniji pristup u Reactu.
