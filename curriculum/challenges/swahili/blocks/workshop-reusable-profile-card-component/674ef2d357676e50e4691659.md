...
Unutar elementa `div`, kreiraj element `h2` i interpoliraj svojstvo `name` kao njegov tekst.

Takođe, unutar `div`, kreiraj paragraf sa `className` od `card-title` za interpoliranje svojstva `title`, i još jedan paragraf za interpoliranje svojstva `bio`.
