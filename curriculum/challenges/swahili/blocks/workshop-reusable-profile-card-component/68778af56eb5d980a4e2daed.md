...
Umesto postavljanja propova za svaki pojedinačni `Card` komponent, trebalo bi da kreirate niz podataka profila i koristite te podatke za dinamičko renderovanje `Card` komponenti.

Počnite kreiranjem niza `profiles` unutar `App` komponente.

Dodajte jedan objekat u niz za `Mark`, sa sledećim vrednostima:
