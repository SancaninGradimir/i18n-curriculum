...
Jedno poslednje malo poboljšanje koje možete dodati je da takođe prikažete indeks na kojem se cilj `value` nalazi.

Dakle, umesto da vraćate samo `pathToTarget` u izjavi `if`, dodajte *template* literalo sa porukom `Value found at index ${mid}`.

Time, vaša radionica za binarnu pretragu algoritama je završena!
