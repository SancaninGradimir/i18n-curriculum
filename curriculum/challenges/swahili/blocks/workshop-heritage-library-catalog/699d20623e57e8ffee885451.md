...
Funkcija `parseCard` će uzeti sirovi katalog string i vratiti strukturirani objekat. Pre nego što izgradite svoju funkciju, postavite način da vidite njen izlaz dok radite.

Pozovite `parseCard` sa `rawCatalogCards[2]` kao argumentom, dodelite rezultat promenljivoj nazvanoj `cardResult`, i logujte `cardResult` u konzolu.

Trenutno `parseCard` nema telo, pa vraća `undefined`. Kako dodajete kod funkciji kroz sledeće korake, logovani izlaz će vam tačno pokazati šta funkcija vraća na svakoj fazi.
