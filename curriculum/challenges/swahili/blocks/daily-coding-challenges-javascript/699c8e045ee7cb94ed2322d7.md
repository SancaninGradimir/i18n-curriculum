id: 699c8e045ee7cb94ed2322d7
title: "Vežba 216: Dan Pi"
challengeType: 28
dashedName: challenge-216
---

# --description--

Srećan Dan Pi (π)!

Kada dobijete celo broju (`n`), gde je `n` između 1 i 1000 (uključujući), vratite `n`-tu decimalu π.

- Uverite se da vraćate broj, a ne niz karaktera.

Pi sa prvih pet decimala je 3.14159. Na primer, ako dobijete `5`, vratite `9`, petu decimalu.

Možda ćete morati pronaći prvu 1000 decimalu π na nekom mestu.

# --hints--

`getPiDecimal(5)` treba da vrati `9`.

```js
assert.strictEqual(getPiDecimal(5), 9);
```

`getPiDecimal(10)` treba da vrati `5`.

```js
assert.strictEqual(getPiDecimal(10), 5);
```

`getPiDecimal(22)` treba da vrati `6`.

```js
assert.strictEqual(getPiDecimal(22), 6);
```

`getPiDecimal(39)` treba da vrati `7`.

```js
assert.strictEqual(getPiDecimal(39), 7);
```

`getPiDecimal(76)` treba da vrati `2`.

```js
assert.strictEqual(getPiDecimal(76), 2);
```

`getPiDecimal(384)` treba da vrati `4`.

```js
assert.strictEqual(getPiDecimal(384), 4);
```

`getPiDecimal(601)` treba da vrati `0`.

```js
assert.strictEqual(getPiDecimal(601), 0);
```

`getPiDecimal(1000)` treba da vrati `9`.

```js
assert.strictEqual(getPiDecimal(1000), 9);
```

# --seed--

## --seed-contents--

```js
function getPiDecimal(n) {

  return n;
}
```

# --solutions--

```js
function getPiDecimal(n) {
  const pi = ""

  return Number(pi[n]);
}
```