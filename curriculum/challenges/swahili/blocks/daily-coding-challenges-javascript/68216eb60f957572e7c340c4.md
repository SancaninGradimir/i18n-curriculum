...
Uzimajući u obzir broj milja pretrčanih i vreme u formatu `"MM:SS"` (minute:sekunde) koje je potrebno za trčanje tih milja, vratite string koji pokazuje prosečno vreme potrebno za trčanje svake milje u formatu `"MM:SS"`.

- Dodajte vodeće nule kada je potrebno.

# --hints--

`milePace(3, "24:00")` treba da vrati `"08:00"`.

```js
assert.equal(milePace(3, "24:00"), "08:00");
```

`milePace(1, "06:45")` treba da vrati `"06:45"`.

```js
assert.equal(milePace(1, "06:45"), "06:45");
```

`milePace(2, "07:00")` treba da vrati `"03:30"`.

```js
assert.equal(milePace(2, "07:00"), "03:30");
```

`milePace(26.2, "120:35")` treba da vrati `"04:36"`.
