...
title: "Vežba 306: Ekstraktovanje HTML sadržaja"
challengeType: 28
...
Ako dobijete niz HTML karaktera, vratite običan tekstualni sadržaj bez svih tagova.

# --hints--

`extractContent('<p>hello world</p>')` bi trebalo da vrati `"hello world"`.

```js
assert.equal(extractContent('<p>hello world</p>'), "hello world");
```

`extractContent('<p>hello <span>world</span></p>')` bi trebalo da vrati `"hello world"`.

```js
assert.equal(extractContent('<p>hello <span>world</span></p>'), "hello world");
```

`extractContent('<a href="example.com">Click me</a>')` bi trebalo da vrati `"Click me"`.

```js
assert.equal(extractContent('<a href="example.com">Click me</a>'), "Click me");
```

`extractContent('<p><button onClick="learnToCode()">Learn</button> to <code>code<code> <br/>for <strong>free</strong> <br/>on <a href="https://freecodecamp.org/" target="_blank"><span class="highlight">freecodecamp</span>.org</a>')` bi trebalo da vrati `"Learn to code for free on freecodecamp.org"`.
