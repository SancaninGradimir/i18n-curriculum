...
Sada zamenite vrednost svojstva ``position`` vašeg ``.cat-head`` sa ``fixed``. Ostavite i ``top`` i ``left`` kako su.

Nakon toga, skrolujte gore i dole da vidite kako radi vrednost ``fixed``.
