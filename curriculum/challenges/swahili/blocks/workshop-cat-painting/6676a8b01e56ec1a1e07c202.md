...
Primećujete da sada imate beli kvadrat sa debelim granicama. Granice imaju dijagonalne ivice koje se mogu koristiti za kreiranje trouglova. Biće vam potrebne da napravite uši mački.

Unutar istog selektora klase prilagodite `height` i `width` na `0`. Postavite levu, desnu i gornju granicu na `transparent`.

Uklonite svojstvo `background-color`, pa bi trebalo da vidite trougao.
