---
id: 646c48df8674cf2b91020ecc
title: Korak 1
challengeType: 0
dashedName: step-1
demoType: onLoad
---

# --description--

Dostavljen vam je osnovni boilerplat. Počnite dodavanjem elementa `link` unutar vašeg elementa `head`. Za taj element `link`, postavite atribut `rel` na `stylesheet` i `href` na `./styles.css`.

# --hints--

You should have a `link` element.

```js
assert.exists(document.querySelector("head>link"));
```

Your `link` element should have a `rel` and `href` attribute set to `stylesheet` and `./styles.css`.

```js
const link = document.querySelector("head>link");
assert.equal(link?.getAttribute("rel"), "stylesheet");
const href = link?.getAttribute("data-href");
console.log(href)
assert.oneOf(href, ["./styles.css", "styles.css"]);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
--fcc-editable-region--
<head>
    <meta charset="UTF-8">
    <title>fCC Cat Painting</title>
</head>
--fcc-editable-region--
<body></body>
</html>
```

```css

```
