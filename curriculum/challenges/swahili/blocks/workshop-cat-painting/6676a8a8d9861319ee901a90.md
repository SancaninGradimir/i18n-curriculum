...
Sada možete početi da radite na ušima vaše mačke.

Očistite svoj pregledni kod uklanjanjem selektora `.cat-right-ear` i svih njegovih svojstava.

Koristeći selektor klase, date elementu `.cat-left-ear` levu i desnu ivicu od `35px solid transparent` svaka. Takođe, postavite donju ivicu na `70px solid #5e5e5e`.
