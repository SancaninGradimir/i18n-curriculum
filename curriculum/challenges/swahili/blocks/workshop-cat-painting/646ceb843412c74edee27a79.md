...
Koristeći selektor klase, daj elementu `.cat-right-ear` svojstva `height` i `width` postavljena na `100px`. Postavi `background-color` na `white`. Postavi levu i desnu granicu na `35px solid blue`. Postavi gornju i donju granicu na `35px solid red`.
