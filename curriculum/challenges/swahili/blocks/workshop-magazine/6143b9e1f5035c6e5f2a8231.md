...
U vašem `ul` elementu, kreirajte šest `li` elemenata. Dodajte `h4` element sa `class` podešenim na `list-subtitle` i `p` elementu svakom vašem `li` elementu.

Zatim dodelite elementima `h4` i `p` sledeći tekstualni sadržaj, po redu, pri čemu svaki `h4` koristi ono što je sa leve strane dvotačke, a svaki `p` koristi ono što je sa desne:
