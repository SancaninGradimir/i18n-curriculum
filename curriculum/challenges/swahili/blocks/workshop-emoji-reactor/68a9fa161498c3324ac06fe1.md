...
Vreme je da počnemo sa radom na drugim dugmićima. Preostali elementi `button` dodati su za vas u HTML fajl.

Za svako novo dugme (`#confused-btn`, `#sad-btn` i `#loving-btn`), izaberite ga koristeći `querySelector` sa njegovim ID-om, a zatim dodajte slušalicu za događaj klika koja poziva `updateCount` sa tim dugmetom.

Koristite imena promenljivih `confusedBtn`, `sadBtn` i `lovingBtn`.
