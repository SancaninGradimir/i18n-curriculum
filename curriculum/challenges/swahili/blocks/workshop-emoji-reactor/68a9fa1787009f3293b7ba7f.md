...
Verovatno ste primetili da vaš kod postaje prilično repetitivan! Postoji bolji način za rukovanje više sličnih elemenata.

Umesto da selektujete svako dugme pojedinačno, koristite `querySelectorAll(".emoji-btn")` da odaberete sva dugmeta sa klasom `emoji-btn` odjednom. Sačuvajte rezultat u varijablu nazvanu `btns`.
