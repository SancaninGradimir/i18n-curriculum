...
Možete učiniti kod reusabilnijim.

Kreirajte funkciju nazvanu `updateCount` koja prima element dugmeta kao parametar.

Ova funkcija bi trebalo da sadrži svu logiku za ažuriranje broja dugmeta: pronalaženje elementa sa brojem, parsiranje trenutnog broja, proveru da li je na maksimumu, inkrementiranje i ažuriranje prikaza.

Zatim ažurirajte vaš *event listener* povratni poziv (callback) da pozove `updateCount(happyBtn)` umesto da logika bude ugrađena (inline).
