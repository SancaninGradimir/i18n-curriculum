...
U prethodnim lekcijama ste naučili da `querySelectorAll` vraća `NodeList`, što je struktura slična nizu, i jedna od metoda koju ima `NodeList` jeste `forEach()`, a radi isto kao i `forEach` koji postoji na nizovima.

Iterirajte preko `btns` sa `forEach()` koji dodaje isti slušalicu događaja svakom dugmetu.

Koristite funkciju povratne pozive (callback) koja prima parametar `btn`, i unutar povratne pozive dodajte slušalicu događaja za klik koja poziva `updateCount(btn)`.

Time ste završili radionicu.
