...
Unutar elementa `button`, kreiraj element `span` koji sadrži `😊`.

Elementu `span` dodaj atribut `role` sa vrednošću `img` i atribut `aria-hidden` sa vrednošću `true`.

Zatim, kreiraj drugi element `span` sa `class` od `count` koji sadrži tekst `0/10`.
