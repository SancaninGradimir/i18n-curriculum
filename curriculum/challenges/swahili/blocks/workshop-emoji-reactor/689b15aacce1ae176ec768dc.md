...
Sada, morate dobiti prvi broj u tekstu `0/10`, na način koji će raditi čak i za `5/10` ili `10/10`.

Parsirajte tekstualni sadržaj elementa `.count` kako biste izvukli trenutni broj.

Pretvorite brojač iz niza karaktera u broj.

Zamenite prethodni `console.log(countEl.textContent)` sa vašim novim logom.

Logujte trenutni brojač u ovom formatu: `console.log("Current count:", currCount)`.

Savet: zapamtite da možete koristiti metod `split` za podelu niza karaktera na niz podnizova.
