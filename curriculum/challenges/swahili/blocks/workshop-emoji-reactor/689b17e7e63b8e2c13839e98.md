...
Sada je vreme da se implementira stvarna funkcionalnost brojanja.

Umesto samo logovanja trenutnog broja, morate ga povećati i ažurirati prikaz.

Uklonite `console.log`.

Proverite da li je trenutni prvi broj unutar `.count` već 10 (tekst je `10/10`), i ako jeste, ne radite ništa.

Ako je broj manji od 10, povećajte trenutni brojač za 1 i ažurirajte tekst elementa `.count`. Na primer, ako je trenutni brojač 2, treba ga povećati na 3, a tekst za element `.count` bi trebalo da bude `3/10`.
