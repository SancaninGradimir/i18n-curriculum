...
Da biste stilizovali dugme za slanje, možete koristiti `_attribute_` selektor, koji bira element na osnovu zadate vrednosti atributa. Evo primera:

```css
input[name="password"]
```

Prethodno bira elemente `input` sa vrednošću atributa `name` na `password`.

Sada, koristite atribut selektor da stilizujete dugme za slanje sa `display` od `block`, i `width` od `60%`.
