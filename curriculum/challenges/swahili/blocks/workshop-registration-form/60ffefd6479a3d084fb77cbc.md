...
Napravite `input` za uslove korišćenja `inline` dodavanjem odgovarajuće klase u HTML.
...
```js
assert.isTrue(document.querySelector('fieldset:nth-child(3) + a + label > input')?.classList?.contains('inline'));
```
