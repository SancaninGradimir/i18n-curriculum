...
Predaja forme sa izabranom opcijom ne bi poslala korisnu vrednost na server. Zbog toga, svaki element `option` zahteva atribut `value`. Bez ovog atributa, tekstualni sadržaj `option` će biti poslat na server.

Dajte prvom `option` atribut `value` od `""`, a naknadnim elementima `option` dodelite atribute `value` od `1` do `4`.
