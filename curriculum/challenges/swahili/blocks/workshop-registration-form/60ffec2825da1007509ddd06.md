...
Većina pregledača ubacuje sopstvene podrazumevane CSS svojstva i vrednosti za različite elemente. Ako pažljivo pogledate, možda ćete primetiti da je fajl `input` manji od drugih tekstualnih elemenata `input`. Podrazumevano, elementima `input`, u koji možete kucati, dodeljuje se `padding` od `1px 2px`.

Koristeći drugi atributni selektor, stilizujte `input` sa `type` od `file` da ima isti padding kao i ostali elementi `input`.
