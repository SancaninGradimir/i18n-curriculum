...
Želite da `select` element ostane sa belim pozadinom, ali sada ne dobija isti `min-height` kao i `input` i `textarea` elementi.

Premestite `min-height` svojstvo i vrednost tako da svi tri tipa elemenata imaju istu `min-height` vrednost, a `select` element i dalje ima belu pozadinu.
