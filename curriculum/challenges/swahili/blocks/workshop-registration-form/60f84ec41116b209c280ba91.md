...
Sa `type="password"` možete koristiti atribut `pattern` da definišete regularni izraz kojem lozinka mora odgovarati da bi se smatrala validnom.

Dodajte atribut `pattern` na element lozinke `input` da zahtevate podudarnost unosa: `[a-z0-5]{8,}`

Gore je regularni izraz koji odgovara osam ili više malih slova ili ciframa od `0` do `5`. Zatim, uklonite atribut `minlength` i isprobajte.
