...
U skladu sa najboljim praksama pristupačnosti, povežite elemente `input` i elemente `label` zajedno koristeći atribut `for`.

Koristite `first-name`, `last-name`, `email` i `new-password` kao vrednosti za odgovarajuće `id` atribute.
