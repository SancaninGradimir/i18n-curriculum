...
`textarea` izgleda veoma malo. Da biste mu dali početne dimenzije, možete dodati svojstva elementa `rows` i `cols`.

Povećajte početnu veličinu linija `3` i reda `30`.

# --hints--

Unapaswa kumpa `textarea` sifa ya kipengele `rows` yenye thamani `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Unapaswa kumpa `textarea` sifa ya kipengele `cols` yenye thamani `30`.
