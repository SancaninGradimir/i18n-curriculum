...
Tokom razvoja, važno je videti granice podrazumevanih opcija `fieldset`. Međutim, to čini da sadržaj izgleda previše razbijeno.

Uklonite `border`, samo iz gornjeg i donjeg razmaka `2rem` unutar svakog `fieldset`. Proverite da uklanjate `padding` sa leve i desne strane.

# --hints--

Možete koristiti vrednost `none` ili `0` da uklonite `border`.
...
Treba da uklonite `padding` sa leve i desne strane svakog `fieldset`.
