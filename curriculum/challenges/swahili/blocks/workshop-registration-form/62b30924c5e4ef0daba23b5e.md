...
Granica poslednjeg `fieldset` elementa izgleda malo ne na mestu. 

U prethodnim lekcijama, naučili ste kako da radite sa `last-of-type` CSS pseudo-klasom na ovaj način: 

```css
p:last-of-type { }
```

To će izabrati poslednji `p` element.

Kreirajte novi selektor koji cilja poslednji `fieldset` element i postavite njegov `border-bottom` na `none`.
