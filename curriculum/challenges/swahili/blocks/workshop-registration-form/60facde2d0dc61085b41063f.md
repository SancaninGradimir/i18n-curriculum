...
Element `textarea` funkcioniše kao element `input` tipa `text`, ali dolazi sa dodatnom prednošću mogućnosti prijema višelinijskog teksta, i početnog broja redova i kolona teksta.

Korisnici će moći da se registruju sa biografijom. Dodajte `label` sa tekstom `Provide a bio:` na kraju `fieldset`. Dodajte element `textarea` unutar elementa `label`. Napomena da `textarea` zahteva zatvarajući tag.
