...
Dodavanje padajućeg menija na formular je lako sa elementom `select`. Element `select` je kontejner za grupu elemenata `option`, a element `option` služi kao oznaka (labela) za svaku opciju padajućeg menija. Oba elementa zahtevaju zatvarajuće tagove.

Počnite dodavanjem elementa `select` ispod poslednjeg elementa `label`. Zatim ugnježđujte 5 elemenata `option` unutar elementa `select`.
