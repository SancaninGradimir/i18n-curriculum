...
Konačno, za dugme za slanje, želite da ga odvojite od `fieldset` iznad, i prilagodite mu širinu tako da nikada ne bude ispod `300px`.

Promenite svojstvo `margin` da uključuje `1em` na vrhu i dnu, ostavljajući desne i leve margine postavljene na `auto`. Zatim postavite širinu kao što je opisano iznad.
