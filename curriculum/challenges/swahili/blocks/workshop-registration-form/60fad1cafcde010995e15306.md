...
Kada se šalju forme, korisno je i dobra praksa da svakom podnesivom elementu dodelite atribut `name`. Ovaj atribut se koristi za identifikaciju elementa u predaji forme.

Osim dva `radio` ulaza (koja ste već nazvali), dodelite svakom podnesivom elementu jedinstven atribut `name` po vašem izboru.
