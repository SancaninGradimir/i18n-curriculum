...
Prelazimo na poslednji `fieldset`. Šta ako želite da dozvolite korisniku da učita profilnu sliku?

Pa, tip `input` `file` to čini upravo. Dodajte `label` sa tekstom `Upload a profile picture: `, i ugnježđite `input` koji prihvata učitavanje fajla.
