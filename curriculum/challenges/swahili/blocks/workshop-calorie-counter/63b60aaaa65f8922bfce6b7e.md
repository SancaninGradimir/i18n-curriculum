...
U vašem `span` elementu, kreirajte `label` element za `entry-dropdown` i dodelite mu tekst `Add food or exercise:`. Zatim kreirajte `select` element sa `id` podešenim na `entry-dropdown` i `name` podešenim na `options`. Ispod toga, dodajte `button` element sa `id` podešenim na `add-entry` i tekstom `Add Entry`.

Dodelite vašem `button` elementu atribut `type` podešen na `button` kako biste sprečili automatsko slanje forme.
