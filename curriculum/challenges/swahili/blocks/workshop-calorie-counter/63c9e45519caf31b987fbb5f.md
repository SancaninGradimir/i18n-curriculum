...
Vaša `getCaloriesFromInputs` funkcija će postaviti globalnu zastavicu greške `isError` na `true` ako je detektovan nevažeći unos. Dodajte izjavu `if` u vašu `calculateCalories` funkciju koja proverava istinitost vaše globalne zastavice greške, i ako je ona istinita onda koristite `return` da zaustavite izvršavanje funkcije.
