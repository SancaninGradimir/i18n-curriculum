...
Svaki unos će imati tekstualni unos za naziv unosa i numerički unos za kalorije. Da biste dobili broj unosa, možete upitati pomoću tekstualnih unosa.

Prođite string `input[type="text"]` u metod `querySelectorAll()`. Zapamtite da ćete morati da koristite jednostru navodnike za vaš string, kako biste unutar njega mogli koristiti dvostruke navodnike.

Ovo će vratiti `NodeList` svih tekstualnih unosa u formularu. Zatim možete pristupiti svojstvu `length` objekta `NodeList` da biste dobili broj unosa. Uradite ovo na istoj liniji.
