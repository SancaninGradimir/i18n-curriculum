...
Konačno, na novoj liniji nakon vašeg drugog `label`, kreirajte još jedan `input` element. Dodelite mu atribut `type` postavljen na `number`, atribut `min` postavljen na `0` (da bi se osiguralo da ne mogu dodati negativne kalorije), atribut `placeholder` postavljen na `Calories`, i atribut `id` koji odgovara atributu `for` vašeg drugog `label` elementa.
