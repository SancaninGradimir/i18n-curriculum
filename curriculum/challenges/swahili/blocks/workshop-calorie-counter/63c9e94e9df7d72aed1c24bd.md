...
Na kraju, morate učiniti da element `#output` vidljivim kako bi korisnik mogao da vidi vaš tekst. Vaša varijabla `output` je Element, koji ima svojstvo `classList`. Ovo svojstvo ima metod `.remove()`, koji prima niz (string) koji predstavlja klasu koju treba ukloniti sa elementa.
...
Koristite metod `.remove()` svojstva `classList` varijable `output` da uklonite klasu `hide`. Ne zaboravite da stavite reč `hide` unutar navodnika.
