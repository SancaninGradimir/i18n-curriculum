...
Zapamtite da `return` prekida izvršavanje funkcije. Nakon vašeg bloka `if`, morate obraditi logiku za slučaj kada je unos validan. Pošto vaša izjava `if` vraća vrednost, ne treba vam izjava `else`.

Koristite operator dodeljivanja sabiranja da biste dodali `currVal` vašem ukupnom iznosu `calories`. Biće vam potrebno koristiti konstruktor `Number` da pretvorite `currVal` u broj.

Konstruktor `Number` je funkcija koja pretvara vrednost u broj. Ako se vrednost ne može pretvoriti, vraća `NaN`, što znači "Not a Number". 
