...
Kada korisnici žele da izaberu tip obroka kako bi uneli svoje kalorijske brojeve, treba im biti prikazan padajući meni i dugme za dodavanje tipa obroka.

Počnite kreiranjem elementa `div` i dodeljujte mu atribut `class` sa vrednošću `controls`. Zatim, ugnježđite element `span` unutar ovog `div`.
