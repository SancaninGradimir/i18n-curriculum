...
Vaša funkcija mora da dohvati vrednosti sa unosa koje je korisnik dodao.

Deklariš varijablu `breakfastNumberInputs`, i dodeli joj vrednost pozivanjem `document.querySelectorAll()` sa selektorom `#breakfast input[type='number']`. Ovo će vratiti sve `number` ulaze koji se nalaze u elementu `#breakfast`.
