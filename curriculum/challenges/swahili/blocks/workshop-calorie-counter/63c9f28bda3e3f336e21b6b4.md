...
Da biste završili ovu funkciju, morate vratiti klasu `hide` na element `output`. Svojstvo `classList` ima metod `.add()`, koji je suprotan metodi `.remove()`. Prihvata niz znakova (string) koji predstavlja klasu koju treba dodati elementu.

Dodajte klasu `hide` vašem `output`.
