...
Sada morate da ciljate element `.input-container` unutar elementa koji ima vaš `targetId`. Deklarišite novu `targetInputContainer` varijablu i dodelite joj vrednost od `document.querySelector()`. Koristite konkatenaciju da razdvojite `targetId` i `'.input-container'` razmakom, i prosledite taj string na `querySelector()`.
