...
Koristeći istu sintaksu interpolacije, dodajte drugi element `p` sa tekstom `consumedCalories Calories Consumed` i treći sa tekstom `exerciseCalories Calories Burned`. Zapamtite da zamenite vaše varijable `consumedCalories` i `exerciseCalories` odgovarajućim vrednostima.
