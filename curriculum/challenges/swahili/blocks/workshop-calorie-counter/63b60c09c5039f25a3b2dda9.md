...
Kreiraj još jedan `div` element. Unutar njega, ugnjezdi `button` da `submit` formu. Ovaj dugme treba da ima tekst `Calculate Remaining Calories`.

Zatim dodaj `button` sa `id` podešenim na `clear` za brisanje forme (nemoj zaboraviti da mu dodaš atribut `type`, koji sprečava slanje forme). Ovo dugme treba da ima tekst `Clear`.
