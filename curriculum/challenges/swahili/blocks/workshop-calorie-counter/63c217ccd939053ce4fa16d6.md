...
Nakon vašeg `label` elementa, i na novoj liniji u vašem šablon tekstu, kreirajte `input` element. Dajte mu atribut `type` postavljen na `text`, atribut `placeholder` postavljen na `Name`, i atribut `id` koji odgovara atributu `for` vašeg `label` elementa.
