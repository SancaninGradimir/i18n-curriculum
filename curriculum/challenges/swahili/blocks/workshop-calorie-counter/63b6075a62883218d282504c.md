...
Vaš element `input` zahteva neke dodatne atribute. Dodajte mu `type` postavljeno na `number` da dozvoljava samo numeričke unose, atribut `min` sa vrednošću `0` da dozvoljava samo pozitivne brojeve, i `placeholder` postavljeno na `Daily calorie budget`.

Na kraju, označite element `input` kao `required`.
