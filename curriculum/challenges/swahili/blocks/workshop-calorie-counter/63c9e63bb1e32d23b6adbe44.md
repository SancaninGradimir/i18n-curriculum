...
Kada vam je potrebno da pretvorite niz u mala slova, možete koristiti metodu <dfn>toLowerCase()</dfn>. Ova metoda vraća vrednost niza koji je pozvao, konvertovanu u mala slova.
...
Vaš `output.innerHTML` string će zahtevati element `span`. Kreirajte ga i dodelite mu atribut `class` postavljen na varijablu `surplusOrDeficit`. Vaša varijabla `surplusOrDeficit` bi trebalo da se konvertuje u mala slova korišćenjem metode `toLowerCase()`.

Ne dodeljujte svom `span` još nikakav tekst.
