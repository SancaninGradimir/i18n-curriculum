...
Vaš drugi kvar nastaje ako dodate unos `Breakfast`, popunite ga, a zatim dodate drugi unos `Breakfast`. Videćete da su vrednosti koje ste dodali nestale.

To je zato što direktno ažurirate `innerHTML`, što ne čuva vaš ulazni sadržaj. Promenite dodelu vašeg `innerHTML` tako da umesto toga koristi metodu `insertAdjacentHTML()` iz `targetInputContainer`. Još ne prosleđujte argumente.
