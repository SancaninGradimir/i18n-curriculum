...
U vašoj formi, korisnici će moći da unesu broj koji predstavlja njihov dnevni budžet kalorija.

Kreirajte `label` element, dodelite mu atribut `for` postavljen na `budget` i tekst `Budget`, zatim kreirajte `input` element sa `id` podešenim na `budget`.
