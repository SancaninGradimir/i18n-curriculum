...
Metoda `insertAdjacentHtml` prima dva argumenta. Prvi argument je niz koji određuje poziciju umetnutog elementa. Drugi argument je niz koji sadrži HTML za ubacivanje.

Za prvi argument, prosledite niz `"beforeend"` da umetnete novi element kao poslednjeg deteta od `targetInputContainer`.

Za drugi argument, prosledite svoju varijablu `HTMLString`.
