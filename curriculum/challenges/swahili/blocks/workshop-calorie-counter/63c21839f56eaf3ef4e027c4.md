...
Kreiraj još jedan element `label` (na novoj liniji) na kraju vašeg `HTMLString`. Ovaj `label` treba da ima tekst `Entry # Calories`, koristeći vašu sintaksu za tekstualne literale da zameni `#` vrednošću `entryNumber`, a atribut `for` postaviti na `X-#-calories`, gde je `X` `value` od `entryDropdown`, a `#` je vrednost `entryNumber`.
