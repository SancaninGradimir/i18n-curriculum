...
Vaše izbornično meniju trebaju opcije za svaki od elemenata hrane i vežbanja `fieldset` koje ste kreirali u prethodnim koracima. Koristite element `option` da kreirate novu opciju za svaki `fieldset`. Atribut `value` svake opcije treba da bude `id` od `fieldset`, a tekst svake opcije treba da bude tekst `legend`.

Postavite opciju `Breakfast` kao opciju `selected`.
