...
Vaš izborni meni zahteva opcije za svaki artikl hrane i vežbe od `fieldset` koji ste kreirali u prethodnim koracima. Koristite funkciju `option` da kreirate novu opciju za svaki `fieldset`. Atribut `value` svake opcije treba da bude `id` od `fieldset`, a tekst svake opcije treba da bude tekst od `legend`.

Postavi opciju `Breakfast` kao opciju `selected`.

# --hints--

Treba da kreiraš pet elemenata `option` unutar svog elementa `select`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.length, 5);
```

`option` yako ya kwanza inapaswa kuwa na maandishi `Breakfast`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[0]?.textContent?.trim(), 'Breakfast');
```

`option` yako ya kwanza inapaswa kuwa na sifa ya `value` iliyowekwa kuwa `breakfast`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[0]?.value, 'breakfast');
```

`option` tvoja druga mora imati tekst `Lunch`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[1]?.textContent?.trim(), 'Lunch');
```

`option` Vaš drugi mora imati svojstvo od `value` postavljeno na `lunch`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[1]?.value, 'lunch');
```

`option` Treći vaš element treba da sadrži tekst `Dinner`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[2]?.textContent?.trim(), 'Dinner');
```

`option` tvoj treći mora imati svojstvo od `value` postavljeno na `dinner`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[2]?.value, 'dinner');
```

`option` četvrti deo treba da sadrži tekst `Snacks`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[3]?.textContent?.trim(), 'Snacks');
```

`option` Vaš četvrti mora imati svojstvo od `value` postavljeno na `snacks`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[3]?.value, 'snacks');
```

`option` peto mora da ima tekst `Exercise`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[4]?.textContent?.trim(), 'Exercise');
```

`option` tvoja peta mora imati svojstvo od `value` postavljeno na `exercise`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[4]?.value, 'exercise');
```

`option` Vaš prvi izbor bi trebalo da bude podrazumevano odabran.
