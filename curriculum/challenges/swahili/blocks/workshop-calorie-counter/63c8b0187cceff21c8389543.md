...
Vrednosti `NodeList` koje ćete proslediti u `list` sastojće se od `input` elemenata. Zato želite da pogledate atribut `value` svakog elementa.

Dodelite `item.value` promenljivoj tipa `const` nazvanoj `currVal`.
