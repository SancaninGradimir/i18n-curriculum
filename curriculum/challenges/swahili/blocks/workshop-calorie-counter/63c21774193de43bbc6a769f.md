...
Dajte vašem `label` elementu atribut `for` sa vrednošću `X-#-name`, gde je `X` `value` elementa `entryDropdown` i `#` je vrednost `entryNumber`. Zapamtite da se HTML atributi treba obaviti dvostrukim navodnicima.
