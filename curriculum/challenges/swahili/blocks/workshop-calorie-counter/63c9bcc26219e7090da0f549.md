...
U programiranju, `null` je namenjen da predstavlja odsustvo vrednosti. U ovom slučaju, ako korisnik unese nevažeći unos, želite da ih upozorite i zatim vratite `null` kako biste ukazali na to da je funkcija propala.

Još unutar vašeg `if` bloka, postavite `isError` na `true` i vratite `null`.
