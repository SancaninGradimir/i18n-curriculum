...
Ažurirajte prvi argument za `setTimeout` u `async` streličnu funkciju. Ovo vam omogućava da koristite `await` unutar odložene logike. Unutar te funkcije, kreirajte `try...catch` blok za rukovanje bilo kakvim potencijalnim greškama prilikom dohvaćanja podataka.
