...
Sada kada imate podatke sa API-ja, ažurirajte stanje `results` pozivom `setResults` i prosleđivanjem niza koji sadrži samo svojstvo `name` iz svakog voća u nizu `data`. Koristite metod `map()` za ovo.
