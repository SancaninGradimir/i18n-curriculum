...
Unutar bloka `try`, pozovite funkciju `fetch` sa URL-om `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}`, i sačuvajte rezultat u varijablu pod nazivom `response`. Pošto ste unutar funkcije `async`, trebalo bi da koristite `await` pre `fetch`.
