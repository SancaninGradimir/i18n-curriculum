...
Unutar elementa `#results`, koristite ternarni operator da proverite da li je dužina niza `results` veća od nule. Ako jeste, iterirajte kroz stavke i prikažite svaku u elementu `p` sa klasom `result-item`. Obavezno uključite atribut `key` i uz svaki pasus.

Ako je `results` prazan, renderujte element `p` sa porukom `No results found`.
