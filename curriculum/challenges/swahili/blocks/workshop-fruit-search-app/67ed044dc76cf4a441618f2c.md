...
U React-u, <dfn>kontrolisani ulazi</dfn> su standardan način za rukovanje podacima iz formulara, gde je vrednost unosa sinhronizovana sa stanjem komponente.
...
Vi ćete više naučiti o ovom obrascu u budućoj lekciji.

Da biste pratili šta korisnik unosi u polje za pretragu, kreirajte varijablu stanja (state variable) pod imenom `query` sa početnom vrednošću praznog stringa (`''`). Takođe definišite njenu odgovarajuću funkciju za postavljanje (setter function), `setQuery`, koristeći Hook `useState`.
