...
Sledeće, napravićete da vaša aplikacija dobije imena voća sa API-ja kada korisnik unese nešto u unos.

Prvo, dodajte ``useEffect`` hook tako da se izvršava svaki put kada se stanje ``query`` promeni. Pazite da prosledite streličnu funkciju kao prvi argument za ``useEffect``, i uključite ``query`` u niz zavisnosti (dependency array).
