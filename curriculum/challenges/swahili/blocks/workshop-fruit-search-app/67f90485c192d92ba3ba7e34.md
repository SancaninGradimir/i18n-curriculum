...
Unutar poziva funkcije (callback) `useEffect`, dodajte uslov `if` da proverite da li je `query.trim() === ''`. Ovo proverava da li je `query` prazan niz ili sadrži samo razmake. Ako jeste, pozovite `setResults([])` i vratite se rano.
