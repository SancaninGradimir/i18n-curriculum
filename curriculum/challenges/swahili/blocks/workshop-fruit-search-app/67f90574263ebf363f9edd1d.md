...
Ako `query` nije prazno, želite da dobijete voće koje odgovara unosu korisnika iz API-ja, ali samo nakon što korisnici prestanu da kucaju na kratko period kako biste izbegli prečesto pozivanje funkcije za dohvaćanje (fetch call).

Da počnete, nakon izjave `if`, pozovite `setTimeout` sa praznom strelicastom funkcijom i vrednošću kašnjenja od `700` kao argumentima, i sačuvajte ga u varijablu nazvanu `timeoutId`. Ovo vam omogućava da kasnije otkažete tajmer kada se efekat očisti.
