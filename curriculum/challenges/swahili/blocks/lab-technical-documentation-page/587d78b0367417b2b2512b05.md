...
title: Napravite stranicu tehničke dokumentacije
challengeType: 25
demoType: onClick
...
1. Možete videti element `main` sa odgovarajućim `id="main-doc"`, koji sadrži glavni sadržaj stranice (tehničku dokumentaciju).
1. Unutar elementa `#main-doc`, možete videti nekoliko elemenata `section`, svaki sa klasom `main-section`. Treba da ima najmanje pet.
1. Prvi element unutar svakog `.main-section` treba da bude element `header`, koji sadrži tekst koji opisuje temu tog odeljka.
1. Svaki element `section` sa klasom `main-section` takođe bi trebalo da ima `id` koji odgovara tekstu svakog `header` sadržanog u njemu. Sve razmake treba zameniti podvučnicama (npr. Sekcija koja sadrži naslov "JavaScript and Java" treba da ima odgovarajući `id="JavaScript_and_Java"`).
1. Elementi `.main-section` bi trebalo da sadrže najmanje deset elemenata `p` ukupno (ne svaki).
1. Elementi `.main-section` bi trebalo da sadrže najmanje pet elemenata `code` ukupno (ne svaki).
1. Elementi `.main-section` bi trebalo da sadrže najmanje pet stavki `li` ukupno (ne svaki).
1. Možete videti element `nav` sa odgovarajućim `id="navbar"`.
1. Element 'navbar' bi trebalo da sadrži jedan element `header`, koji sadrži tekst koji opisuje temu tehničke dokumentacije.
1. Pored toga, 'navbar' bi trebalo da sadrži elemente linka (`a`) sa klasom `nav-link`. Treba ga po jedan za svaki element sa klasom `main-section`.
1. Element `header` u `#navbar` mora da stoji pre bilo kojih elemenata linka (`a`) u 'navbar'-u.
1. Svaki element sa klasom `nav-link` bi trebalo da sadrži tekst koji odgovara tekstu `header` unutar svakog `section` (npr. ako imate sekciju/naslov "Hello world", vaš 'navbar' bi trebalo da ima element koji sadrži tekst "Hello world").
1. Kada kliknete na element 'navbar', stranica bi trebalo da navigira do odgovarajućeg odeljka elementa `#main-doc` (npr. Ako kliknete na element `.nav-link` koji sadrži tekst "Hello world", stranica se pomera ka elementu `section` sa tim id-om, i koji sadrži odgovarajući naslov).
1. Na uređajima standardne veličine (laptopi, desktop računari), element sa `id="navbar"` bi trebalo da bude prikazan na levoj strani ekrana i uvek da je vidljiv korisniku.
1. Vaša tehnička dokumentacija bi trebalo da koristi najmanje jedan medijski upit (media query).
