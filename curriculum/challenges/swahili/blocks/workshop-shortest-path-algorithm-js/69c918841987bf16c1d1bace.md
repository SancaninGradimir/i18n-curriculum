...
Sada kada znate da je čvor dostupan i nije početak, formatirajte njegov put u čitljiv niz znakova.

Niz `paths[nodeNo]` sadrži sekvencu indeksa čvorova posetao do dolaska do `nodeNo`. To možete pretvoriti u čitljiv string za čoveka, kao što je `0 -> 2 -> 3`, koristeći metodu `.join()`.

Unutar petlje `for...of`, nakon naredbe `if` sa `continue`, deklarišite varijablu pod imenom `path` i dodelite joj vrednost `paths[nodeNo].join(' -> ')`.
