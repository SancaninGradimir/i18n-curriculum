...
Sada, trebate ispisati rezultat za svaki dostižan čvor.

Nakon varijable `path`, dodajte poziv `console.log()` sa šablon literalom koji ispisuje:

```md
\n{startNode}-{nodeNo} distance: {distances[nodeNo]}\nPath: {path}
```

Na primer, ako je `startNode` `0` i ako je `nodeNo` `3`, izlaz bi trebalo da izgleda ovako:
