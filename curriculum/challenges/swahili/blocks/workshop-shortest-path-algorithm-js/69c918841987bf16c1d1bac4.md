...
Ako je uslov tačan, trenutni čvor je najbolji neposeta kandidat pronađen do sada. Morate ažurirati vaše praćenje varijable kako biste to odrazili.

Unutar izjave ``if``, ažurirajte ``minDistance`` u ``distances[nodeNo]`` i ``current`` u ``nodeNo``.
