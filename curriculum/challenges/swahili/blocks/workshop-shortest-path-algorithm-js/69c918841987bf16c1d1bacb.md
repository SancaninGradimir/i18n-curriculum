...
Kada glavni petlja završi, morate odlučiti za koje čvorove prikazati rezultate.

Ako je dostavljen određeni `targetNode`, prikažite rezultate samo za taj čvor. U suprotnom, prikažite rezultate za sve čvorove.

U JavaScriptu, možete dobiti sve indekse niza kao iterabil koristeći `[...Array(n).keys()]`.

Nakon spoljne petlje `for`, kreirajte varijablu pod nazivom `targets`. Koristeći ternarni izraz, ako `targetNode` nije striktno `null`, dodelite `[targetNode]`, u suprotnom dodelite `[...Array(n).keys()]`.
