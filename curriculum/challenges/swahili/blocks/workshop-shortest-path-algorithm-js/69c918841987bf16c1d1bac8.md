...
Pre nego što ažurirate udaljenosti, morate proveriti da li je sused vredan razmatranja. Mora postojati stvarna ivica do njega (`distance !== INF`) i još uvek ne sme biti poseta.

Unutar petlje suseda `for`, dodajte izjavu `if` koja proverava da li `distance` nije strogo jednako `INF` **i** da li `nodeNo` nije poseta.

Unutar bloka `if`, deklarišite varijablu `newDistance` dodeljenu vrednošću `distances[current] + distance`. Ovo je ukupna cena dolaska do suseda preko trenutnog čvora.
