...
Sada treba da proverite da li prolazak kroz trenutni čvor daje kraći put do suseda. Ako je `newDistance` bolji od onoga što je već sačuvano, onda ga treba ažurirati.

Unutar postojećeg bloka `if`, dodajte ugnježdanu izjavu `if` koja proverava da li je `newDistance` manje od `distances[nodeNo]`. Unutar tog ugnježđenog bloka `if`, ažurirajte `distances[nodeNo]` na `newDistance`.
