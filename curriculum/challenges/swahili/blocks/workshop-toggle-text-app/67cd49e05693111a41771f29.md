...
U prethodnim lekcijama, naučili ste kako da kreirate varijable stanja koristeći kuka ``useState`` i dekonstrukciju nizova:

```js
const [user, setUser] = useState("Jessica");
```

Konvencija za imenovanje varijabli stanja je korišćenje formata ``example`` i ``setExample``.

``useState`` vraća niz od dve vrednosti. Prva vrednost je trenutno stanje. Druga vrednost je funkcija za postavljanje koja će se koristiti za ažuriranje stanja. 

Kreirajte novu varijablu stanja i funkciju za postavljanje nazvane ``isVisible`` i ``setIsVisible``. Zapamtite da koristite sintaksu dekonstrukcije nizova, kao što je prikazano u primeru. 

Zatim dodelite kuka ``useState`` sa početnom vrednošću ``false``.
