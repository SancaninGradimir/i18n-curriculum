...
Sada je vreme da koristite svoju funkciju `handleToggleVisibility`.

U prethodnim lekcijama, naučili ste o Reactovom Sintetičkom sistemu događaja (Synthetic Event System) i kako da radite sa atributima kao što su `onClick` i `onSubmit`.

Zapamtite da možete proslediti referencu funkcije atributu `onClick` na ovaj način:
...
Ažurirajte svoj postojeći element `button` da dodate atribut `onClick` i prosledite referencu funkcije `handleToggleVisibility`. 

Sada, kada kliknete na dugme, videćete da se vidljivost teksta pasusa prebacuje između skrivenog i prikazanog na stranici.
