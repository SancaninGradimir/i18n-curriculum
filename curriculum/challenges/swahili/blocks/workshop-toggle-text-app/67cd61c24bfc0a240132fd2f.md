...
Bilo bi lepo dinamički prikazati reči `Show Message` ili `Hide Message` na osnovu stanja `isVisible`.

Ranije ste pregledali kako da radite sa ugrađenim uslovnim renderovanjem. Sada ćete primeniti sličnu logiku na tekst dugmeta.

Evo primera korišćenja ternarnog operatera za uslovni prikaz reči `Hide` i `Show`:

```jsx
<button>{isMenuVisible ? "Hide" : "Show"} Menu</button>
```

Za poslednji korak radionice, koristite ternarni operater za uslovni prikaz reči `Hide` ili `Show` za dugme poruke. Postavite uslov pre teksta `Message` tako da prikaže `Show Message` ili `Hide Message`.

I sa tom poslednjom promenom, vaša radionica je gotova! Testirajte dugme da vidite kako funkcioniše.
