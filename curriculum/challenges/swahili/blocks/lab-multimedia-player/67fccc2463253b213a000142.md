...
title: Izgraditi multimedijalni plejer
challengeType: 25
...
U prethodnim lekcijama, upoznati ste sa radom sa elementima `audio` i `video`. U ovom laboratorijumu, kreiraćete multimedijalni plejer koji će prikazivati audio trag `audio` i transkript `video`.

Za element `audio`, moraćete da uključite element `source` koji se koristi za specifikovanje medija koji se koriste. 
...
Element `source` takođe se može koristiti u elementu `video` na ovaj način:
...
**Cilj:** Ispunite sledeće korisničke priče i prođite sve testove kako biste završili laboratoriju.

**Korisničke priče:**

1. Treba vam element `h1` za glavni naslov stranice.
2. Treba vam tri `section` elementa.
3. Unutar prvog `section` elementa, treba vam element `h2` za naslov pesme koja se reprodukuje.
4. Ispod elementa `h2`, treba vam element `audio` sa atributom `controls` i atributom `aria-label`.
5. Unutar elementa `audio`, treba vam element `source` sa atributom `src` koji ukazuje na audio fajl i atributom `type`. Slobodni ste da koristite ovu audio URL ako želite: `https://cdn.freecodecamp.org/curriculum/js-music-player/sailing-away.mp3`
6. Unutar drugog `section` elementa, treba vam element `h2` za naslov videa koji se reprodukuje.
7. Ispod elementa `h2`, treba vam element `video` sa atributima `controls` i `width` i atributom `aria-label`.
8. Unutar elementa `video`, treba vam element `source` sa atributom `src` koji ukazuje na video fajl i atributom `type`. Slobodni ste da koristite ovaj video URL ako želite: `https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`
9. Ispod elementa `source`, treba vam element `track` sa atributom `src` koji ukazuje na fajl sa titlovima i atributima `kind`, `srclang` i `label`. Slobodni ste da koristite ovaj URL za titlove ako želite: `https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.vtt`
10. Unutar trećeg `section` elementa, treba vam element `h2` za naslov sekcije, npr. "Transkript".
11. Ispod elementa `h2`, treba vam element `p` sa transkriptom videa.
