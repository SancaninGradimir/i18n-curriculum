...
Unutar vaše `while` petlje, koristite `queue.shift()` da uklonite i dobijete prvi element iz reda. Ovo implementira ponašanje "prvi uđi, prvi izađe" (FIFO), karakteristično za BFS.

Destrukturirajte ovaj niz u tri varijable: `current`, `opensUsed` i `closesUsed`.
