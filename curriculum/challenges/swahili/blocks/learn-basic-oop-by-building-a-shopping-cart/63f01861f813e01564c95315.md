...
Unutar `div`, dodaj dva elementa od `p`. Postavi tekst drugog elementa od `p` na vrednost izmene `price`.

# --hints--

Trebalo bi dodati dva elementa `p` unutar vašeg elementa `div`.
...
Tvoj drugi element za `p` treba da ima napomene o promenama za `price`.
