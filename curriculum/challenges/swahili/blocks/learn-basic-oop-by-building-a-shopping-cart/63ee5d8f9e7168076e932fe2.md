...
Nakon vašeg elementa sa `h2`, kreiraj dva elementa od `p`. Dodaj prvi element `class` za `dessert-price`, i postavi tekst na dolar znak `"$"` praćen vrednošću parametra `price`. Dodaj drugi element `class` za `product-category`, i tekst `"Category: "` praćen vrednošću parametra `category`.

# --hints--

Trebalo bi vam kreirati dva elementa za `p`.
...
Vaši elementi od `p` treba da dolaze nakon vašeg elementa od `h2`.
...
Vaš prvi element od `p` mora imati `class` za `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Vaš prvi element od `p` mora imati dolar znak `"$"`, praćen vrednošću parametra `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Tvoj drugi element od `p` treba da ima `class` od `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Tvoja druga sekcija za `p` treba da sadrži tekst `"Category: "` praćen vrednošću parametra `category`.
