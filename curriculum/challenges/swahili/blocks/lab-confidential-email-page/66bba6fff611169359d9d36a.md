...
title: Izgradite stranicu za poverljive e-mailove
challengeType: 25
...
1. Trebalo bi da imate element `main` sa `id` od `email`.
1. Vaš element `#email` treba da ima `padding` od `50px`, gornji margina od `50px`, `width` od `500px`, i `border` koji je `2px`.
1. Ukrštena širina vašeg elementa `#email`, uključujući paddinge i ivice, treba da bude `500px`.
1. Trebalo bi da imate dva `div` elementa, jedan sa id-om `confidential` i drugi sa id-om `top-secret`, unutar vašeg `#email` elementa.
1. Vaši `#confidential` i `#top-secret` elementi treba da imaju `display` od `inline-block`.
1. Vaši `#confidential` i `#top-secret` elementi treba da imaju `padding`, levu marginu i `border`.
1. Element `#confidential` treba da ima tekst `CONFIDENTIAL`.
1. Element `#top-secret` treba da ima tekst `TOP SECRET`.
1. Vaši `#confidential` i `#top-secret` elementi bi trebalo rotirati pomoću CSS transformacije.
1. Trebalo bi da imate najmanje tri paragrafa unutar vašeg `#email` elementa.
1. Trebalo bi da imate najmanje tri `span` elementa sa klasom `blurred`, unutar paragraf elemenata.
1. Treba vam selektor `blurred` koji će zamutiti (blur) element `3px` korišćenjem CSS filtera.
