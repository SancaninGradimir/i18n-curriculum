...
title: Izgradite predskagač sreće
challengeType: 26
...
U ovom laboratorijumu, definišite ćete pet srećnih predviđanja i nasumično izabrati jedno od njih da prikažete predviđenje korisniku.

**Cilj:** Ispunite sledeće korisničke priče i prođite sve testove kako biste završili laboratorijum.

**Korisničke priče:**

1. Trebalo bi da inicijalizujete pet promenljivih `fortune1`, `fortune2`, `fortune3`, `fortune4` i `fortune5` sa string vrednošću po izboru. Možete koristiti opcije ispod ako želite:
...
2. Trebalo bi da izaberete nasumičan broj između 1 i 5, uključujući oba broja, i dodelite ga promenljivoj `randomNumber`.

3. Trebalo bi da kreirate promenljivu `selectedFortune` i dodelite odgovarajuće predviđanje na osnovu ovih pravila:

   - Ako je `randomNumber` jednako 1, dodelite vrednost `fortune1` promenljivoj `selectedFortune`.
   - Ako je `randomNumber` jednako 2, dodelite vrednost `fortune2` promenljivoj `selectedFortune`.
   - Ako je `randomNumber` jednako 3, dodelite vrednost `fortune3` promenljivoj `selectedFortune`.
   - Ako je `randomNumber` jednako 4, dodelite vrednost `fortune4` promenljivoj `selectedFortune`.
   - Ako je `randomNumber` jednako 5, dodelite vrednost `fortune5` promenljivoj `selectedFortune`.

4. Trebalo bi da logujete (ili: Ispišete) `selectedFortune` na konzolu.
