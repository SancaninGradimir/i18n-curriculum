...
title: Napravi product landing page
challengeType: 14
...
**Cilj:** Napravi aplikaciju koja funkcionalno odgovara ovoj: <a href="https://product-landing-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://product-landing-page.freecodecamp.rocks</a>. **Nemoj kopirati ovaj demo projekat**.

**Korisnicke price:**

1. Tvoja landing stranica treba da ima `header` element sa `id="header"`
1. Treba da postoji slika unutar `header` elementa sa `id="header-img"` (logo je dobar izbor)
1. Unutar elementa `#header` treba da postoji `nav` element sa `id="nav-bar"`
1. Treba da postoje bar tri klikabilna elementa unutar `nav` elementa, svaki sa klasom `nav-link`
1. Kada kliknes na `.nav-link` u okviru `nav` elementa, treba da te odvede na odgovarajucu sekciju landing stranice
1. Treba da mozes da vidis ugradjeni video proizvoda sa `id="video"`
1. Tvoja landing stranica treba da ima `form` element sa `id="form"`
1. Unutar forme treba da postoji `input` polje sa `id="email"` u koje mozes uneti email adresu
1. Polje `#email` treba da ima placeholder tekst koji korisniku objasnjava cemu sluzi
1. Polje `#email` treba da koristi HTML5 validaciju kako bi proverilo da je unet tekst email adresa
1. Unutar forme treba da postoji submit `input` dugme sa `id="submit"`
1. Kada kliknes na `#submit`, email treba da se posalje na staticku stranicu (koristi ovaj URL: `https://www.freecodecamp.com/email-submit`)
1. Navbar treba da bude uvek na vrhu viewport-a
1. Tvoja landing stranica treba da ima bar jedan media query
1. Tvoja landing stranica treba da koristi CSS Flexbox bar jednom
...
Treba da imas `header` element sa `id` vrednoscu `header`.
...
Treba da imas `img` element sa `id` vrednoscu `header-img`.
...
Tvoj `#header-img` treba da bude potomak elementa `#header`.
...
`#header-img` yako inapaswa kuwa na sifa ya kipengele ya `src`.
...
Thamani ya `#header-img` ya `src` inapaswa kuwa URL halali (inaanza na `http`).
...
Unapaswa kuwa na vipengele vya `.nav-link` vitatu angalau ndani ya `#nav-bar`.
...
Kila kipengele cha `.nav-link` kinapaswa kuwa na sifa ya kipengele ya `href`.
...
Kila kipengele cha `.nav-link` kinapaswa kuunganisha na kipengele kinacholingana kwenye ukurasa wa kutua (kina `href` yenye thamani ya kitambulisho cha kipengele kingine, mfano `#footer`).
...
`#video` yako inapaswa kuwa na sifa ya kipengele ya `src`.
...
`#email` yako inapaswa kuwa na sifa ya kipengele ya `placeholder` yenye maandishi ya kiwakilishi.
...
`#email` yako inapaswa kutumia uthibitisho wa HTML5 kwa kuweka `type` kuwa `email`.
...
`#form` yako inapaswa kuwa na sifa ya kipengele ya `action` yenye thamani ya `https://www.freecodecamp.com/email-submit`.
...
`#email` yako inapaswa kuwa na sifa ya kipengele ya `name` yenye thamani ya `email`.
...
`#nav-bar` yako inapaswa kuwa kila wakati juu ya eneo la kuona.
...
Ukurasa wako wa kutua wa bidhaa unapaswa kutumia angalau media query moja.
...
Ukurasa wako wa kutua wa bidhaa unapaswa kutumia CSS Flexbox angalau mara moja.
