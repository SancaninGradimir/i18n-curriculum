...
Sada je vreme da dodamo elemente forme za prikupljanje informacija o roditelju.

Počnite dodavanjem elementa `label` sa tekstom `Parent/Guardian Name: ` i atributom `for`, podešenim na `"parent-name"`.

Zatim, ispod vašeg elementa `label`, dodajte element `input` sa atributom `type`, podešenim na `"text"`. Atributi `name` i `id` treba da budu podešeni na `"parent-name"`. Atribut `placeholder` treba da bude podešen na `"E.g., Nancy Doe"`. Napokon, vaš `input` mora biti obavezan.
