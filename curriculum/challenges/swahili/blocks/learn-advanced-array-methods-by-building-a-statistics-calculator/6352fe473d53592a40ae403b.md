...
U vašem povratnom pozivu od `.reduce()` sa ničim, deklarisati parametar `difference` i postaviti mu vrednost na `el` manje `mean`. Zatim deklarisati parametar `squared`, i postaviti mu vrednost na `difference` sa moći od `2`. Na kraju, vratiti vrednost `acc` sumirano sa `squared`.

# --hints--

Vaš povratni poziv za `reduce` treba da ima parametar `difference`.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

Vaš parametar za `difference` mora biti postavljen na vrednost od `el`, manja od `mean`.
...
Tvoj povratni poziv za `reduce` mora imati parametar `squared`.
...
Tvoj parametar za `squared` treba da se postavi na vrednost `difference` sa potencijom od 2.
...
Vaš povratni poziv `reduce` treba da vrati vrednost `acc` dodatu sa `squared`.
...
Ne smete da uklonite početnu vrednost `0` iz vašeg puta/načina `.reduce()`.
