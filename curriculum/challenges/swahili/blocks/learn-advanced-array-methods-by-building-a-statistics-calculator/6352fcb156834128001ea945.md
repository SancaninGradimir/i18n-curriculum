...
Sledeće, morate izračunati zbir razlika raspoređenih u kvadrat.

Deklariši parametar `sumSquaredDifferences`, i postavi mu vrednost na `squaredDifferences.reduce()`. Za povratni poziv, vrati zbir od `acc` i `el`. Zapamti da postavi inicijalnu vrednost na `0`.

# --hints--

Trebalo bi da imaš kriterijum od `sumSquaredDifferences`.

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=/);
```

Vaš parametar za `sumSquaredDifferences` mora koristiti metodu `squaredDifferences.reduce()`.

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(/);
```

Vaš parametar za `sumSquaredDifferences` treba da koristi parametre `acc` i `el` u funkciji povratnog poziva.

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)/);
```

Vaš poziv za povratak `reduce` trebao bi vratiti zbir od `acc` i `el`.
...
Nema početne vrednosti za `reduce`.
