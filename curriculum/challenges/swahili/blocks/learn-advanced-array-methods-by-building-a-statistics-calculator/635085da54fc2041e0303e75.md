...
Konačno, potrebno je da vratiš vrednost za `mean`.

# --hints--

Tvoja funkcija za `getMean` treba da koristi ključnu reč `return`.
...
Tvoja funkcija za `getMean` treba da vrati vrednost od `mean`.
...
Deklaracija za `return` mora da sledi nakon deklaracije o promenama `mean`.
