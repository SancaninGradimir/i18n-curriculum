...
Zabavna činjenica: Pingvini ne mogu da lete bez krila.

Unutar `.penguin-body`, pre elemenata `.foot`, dodajte dva `div` elementa, svaki sa `class` od `arm`. Dajte prvom `.arm` `class` od `left`, a drugom `.arm` `class` od `right`.
