...
Ispod elementa `.chin`, dodajte dva `div` elementa, svaki sa `class` od `eye`. Takođe, prvom `.eye` elementu dodelite/dajte `class` od `left`, a drugom `.eye` elementu `class` od `right`.
