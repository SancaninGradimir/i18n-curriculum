...
Ispod elementa `.eye.right`, dodajte dva `div` elementa, svaki sa `class` od `blush`. Takođe, prvom `.blush` elementu dodelite/dajte `class` od `left`, a drugom `.blush` elementu `class` od `right`.

# --hints--

You should add two `div` elements within `.penguin-head`. Expected --fcc-expected-- `.penguin-head > div` elements, but found --fcc-actual--.
