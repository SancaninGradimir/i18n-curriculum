...
Promeni ponašanje elementa `.penguin` u `transition` tokom transformacije tako da ima trajanje od `1s`, funkciju vremena od `ease-in-out` i kašnjenje od `0ms`.
...
```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionProperty, 'transform');
```
