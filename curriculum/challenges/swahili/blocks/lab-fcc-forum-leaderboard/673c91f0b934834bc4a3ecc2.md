...
title: Izgradite bảng za rangiranje fCC Foruma
challengeType: 25
...
U ovom laboratorijumu, kreiraćete bảngualan (leaderboard) za forum freeCodeCamp koji prikazuje najnovije teme, korisnike i odgovore sa [freeCodeCamp forum](https://forum.freecodecamp.org/). HTML, CSS i deo JS dostavljeni su vam. Slobodno ih istražite.

API krajnji tačka vraća JSON u sledećem obliku:
...
Napomena da su vrednosti `avatar_template` relativne staze: počinju sa `/` i sadrže placeholder `{size}` koji treba zameniti veličinom u pikselima. Konstanta `avatarUrl` drži osnovnu URL adresu koju treba dodati kada je putanja relativna.

**Cilj:** Ispunite korisničke priče ispod i prođite sve testove kako biste završili laboratorijum.

**Korisničke priče:**

1. Trebalo bi da imate funkciju pod nazivom `timeAgo` koja prima vremenski pečat u ISO 8601 formatu kao argument.
1. Funkcija `timeAgo` treba da izračuna razliku vremena između vremena prosleđenog kao argument i trenutnog vremena i vrati:
   - `xm ago` (`x` predstavlja minute) ako je količina minuta koja je prošlo manja od `60`.
   - `xh ago` (`x` predstavlja sate) ako je količina sati koja je prošlo manja od `24`.
   - `xd ago` (`x` predstavlja dane) u suprotnom.
1. Trebalo bi da imate funkciju pod nazivom `viewCount` koja prima broj pregleda posta kao argument.
1. Ako je vrednost pregleda prosleđena kao argument veća ili jednaka `1000`, funkcija `viewCount` treba da vrati string sa vrednošću pregleda podeljenom sa `1000`, zaokruženom na najbliži ceo broj i priloženo slovo `k`. U suprotnom, treba da vrati vrednost pregleda.
1. Trebalo bi da imate funkciju pod nazivom `forumCategory` koja prima ID izabrane kategorije kao argument.
1. Funkcija `forumCategory` treba da proveri da li je ID izabrane kategorije svojstvo objekta `allCategories` i treba da vrati string koji sadrži anker element sa:
   - tekstom ključa `category` iz izabrane kategorije.
   - klasom `category` praćenom svojstvom `className` iz izabrane kategorije.
   - `href` formiranim dodavanjem `<className>/<id>` na `forumCategoryUrl` (napomena: `forumCategoryUrl` već završava sa `/`, pa nemojte dodavati dodatni separator `/`), gde je `<className>` svojstvo izabrane kategorije, a `id` argument prosleđen funkciji `forumCategory`.
1. Ako objekat `allCategories` nema ID izabrane kategorije kao svoje svojstvo, `category` treba da bude indikovan kao
