...
title: Napravite konverter iz Markdowna u HTML
challengeType: 25
...
Markdown je jezik za označavanje (markup language) koji se koristi za dodavanje elemenata formatiranja u dokumente običnog teksta. Za ovaj laboratorijski zadatak, sav HTML i CSS su vam obezbedjeni. Koristićete JavaScript da dovršite aplikaciju Markdown to HTML Converter tako da može da obradi konverziju osnovnih Markdown konstrukcija u HTML elemente.

**Napomena:** Konačni rezultat neće biti sveobuhvatni konverter iz Markdown-a u HTML, ali možete mu dodati dodatne funkcionalnosti ako želite.

**Cilj:** Ispunite priče korisnika (user stories) ispod i prođite sve testove kako biste završili laboratorijski zadatak.

**Priče korisnika:**

1. Trebalo bi da imate funkciju pod nazivom `convertMarkdown` koja ne prima parametre.
1. Funkcija `convertMarkdown` treba da koristi regularne izraze za konverziju Markdown unosa iz `#markdown-input` u HTML i treba da vrati string koji sadrži HTML kod.
1. Funkcija `convertMarkdown` treba da konvertuje naslove nivoa jedan, dva i tri u odgovarajuće elemente `h1`, `h2` i `h3`. Naslov u markdown-u je označen sa onoliko `#` karaktera koliko je njegov nivo, praćen razmakom i tekstom naslova. `#` karakteri treba da se nalaze na početku linije: mogu postojati razmaci, ali ne i drugi karakteri pre njih.
1. Funkcija `convertMarkdown` treba da konvertuje podebljani tekst u elemente `strong`. Podebljan tekst u markdown-u je označen ili par dvostrukih zvezdica ili par dvostrukih podznakova koji okružuju tekst.
1. Funkcija `convertMarkdown` treba da konvertuje kurzivni tekst u elemente `em`. Kurzivni tekst u markdown-u je označen ili par zvezdica ili par podznakova koji okružuju tekst.
1. Funkcija `convertMarkdown` treba da konvertuje slike u elemente `img`. Slika u markdown-u je označena sa `![alt-text](image-source)`, gde je `alt-text` vrednost atributa `alt` i `image-source` je vrednost atributa `src`.
1. Funkcija `convertMarkdown` treba da konvertuje linkove u elemente sidra (anchor elements). Link u markdown-u je označen sa `[link text](URL)`, gde je `link text` tekst koji se nalazi unutar tagova sidra, a `URL` je vrednost atributa `href`.
1. Funkcija `convertMarkdown` treba da konvertuje citate u elemente `blockquote`. Citat u markdown-u je označen sa `>`, praćen razmakom i tekstom cita. `>` karakter treba da se nalazi na početku linije: mogu postojati razmaci, ali ne i drugi karakteri pre njih.
1. Kada unesete tekst unutar `#markdown-input`, sirovi HTML kod koji vraća `convertMarkdown` treba da se prikaže unutar `#html-output`.
1. Kada unesete tekst unutar `#markdown-input`, HTML kod koji vraća `convertMarkdown` treba da se renderuje unutar `#preview`.

Napomena: za ovaj projekat trebali biste raditi sa događajem `input`.

Evo tabele koja sadrži sve markdown koje `convertMarkdown` bi trebalo da može obraditi i očekivani HTML nakon konverzije:
...
**Napomena:** Obavezno povežite svoj JavaScript fajl u vašem HTML-u.
