...
Koristite metodu `classList` i `remove()` da uklonite klasu `playing` iz `playButton`, jer će pesma biti pauzirana na ovom mestu.

Da biste konačno pauzirali pesmu, koristite metodu `pause()` na promenljivoj `audio`. `pause()` je metoda Web Audio API-ja za pauziranje muzičkih fajlova.
