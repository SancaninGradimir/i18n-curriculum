...
Preostale pesme su dodate u niz `allSongs` za vas.

U prethodnim lekcijama, naučili ste o <dfn>Web Audio API</dfn> i kako ga koristiti za puštanje pesama. Svi savremeni pretravači podržavaju Web Audio API, koji vam omogućava generisanje i obradu zvuka u web aplikacijama.

Kreirajte varijablu pod nazivom `audio` i postavite je jednako `new Audio()`. Ovo će kreirati novi HTML5 `audio` element.
