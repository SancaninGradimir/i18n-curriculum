...
Pre nego što počnete sa puštanjem sledeće i prethodne pesme, morate dobiti indeks svake pesme u svojstvu `songs` od `userData`. Za to možete koristiti nizovu metodu `indexOf()`, koja vraća prvi indeks na kojem se određeni element može pronaći u nizu, ili `-1` ako element nije prisutan.
...
Kreirajte funkciju pod nazivom `getCurrentSongIndex` koja vraća indeks na kojem se `userData.currentSong` može pronaći u `userData.songs` i `-1` kada je trenutna pesma `null`.
