...
Zatim, koristite svojstvo `classList` i metodu `add()` da dodate klasu `playing` elementu `playButton`. Ovo će potražiti klasu `playing` u CSS fajlu i dodati je elementu `playButton`.

Zatim, postavite `userData.currentSong` na `song`, a da biste konačno pustili pesmu, koristite metodu `play()` na promenljivoj `audio`. `play()` je metoda iz Web Audio API-ja za puštanje mp3 fajla.
