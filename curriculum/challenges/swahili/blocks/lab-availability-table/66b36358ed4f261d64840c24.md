...
title: Izgradite tabelu dostupnosti
challengeType: 25
...
1. Trebalo bi da imate tabelu sa najmanje tri kolone i pet redova, uključujući zaglavlja.
1. Ćelije u prvom redu treba da budu zaglavlja tabele koja sadrže dane u nedelji.
1. Ćelije u prvoj koloni treba da budu zaglavlja tabele sa `class` od `time` i treba da sadrže vrednosti vremena kao tekst.
1. Sve ćelije podataka treba da imaju `class` od `available-#`, gde je `#` broj od `0` do `5` koji određuje broj dostupnih ljudi u tom trenutku.
1. U vašem selektoru `:root`, trebalo bi da definišete šest CSS promenljivih pod nazivom `--color#`, gde je `#` broj od `0` do `5`, i dodelite im svaku vrednost boje. Koristite ove promenljive za postavljanje pozadinske boje odgovarajućih elemenata `.available-#`.
1. U vašem selektoru `:root`, trebalo bi da definišete CSS promenljive pod nazivom `--solid-border` i `--dashed-border`. Koristite ove promenljive za stilizovanje donjih ivica vaših ćelija podataka, menjajući između čvrstih i isprekidane ivice zavisno od reda. Dajte redovima ili klasu `sharp` ili `half` da ih stilizujete.
1. Trebalo bi da imate element `div` sa `id` od `legend`. On treba da sadrži element `span` sa tekstom `Availability` i element `div` sa `id` od `legend-gradient`.
1. Trebalo bi da date elementu `#legend-gradient` linearni gradijent koji prelazi između svih boja od `--color0` do `--color5` sa oštrim linijama.

**Napomena:** Obavezno povežite svoj stilski list u vašem HTML-u da biste primenili CSS.
