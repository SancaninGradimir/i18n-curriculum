...
Da biste prikazali pesme u korisničkom interfejsu (UI), moraćete da kreirate funkciju.

Koristi sintaksu funkcije strelice za kreiranje funkcije `renderSongs` koja prima `array` kao svoj argument.

# --hints--

Trebalo bi da koristiš `const` za kreiranje praznog funkcionera koji se zove `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` treba da bude funkcija.

```js
assert.isFunction(renderSongs)
```

Vaša funkcija `renderSongs` bi trebalo da koristi sintaksu strelične funkcije.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

Vaša funkcija `renderSongs` treba da primi `array` kao parametar.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

Tvoja funkcija za `renderSongs` treba da bude prazna.
