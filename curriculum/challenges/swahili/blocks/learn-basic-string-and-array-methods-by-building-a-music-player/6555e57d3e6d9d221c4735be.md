...
Pritisnite dugme za mešanje, želite da učinite `currentSong` praznim i da `songCurrentTime` postane `0`.

Postavi `userData.currentSong` u `null` i `userData.songCurrentTime` u `0`.

*Napomena*: Ne je preporučljivo koristiti sekvencijalno povezivanje za ovaj korak jer to eksplicitno postavlja svojstva `currentSong` i `songCurrentTime` na `null` i `0` odgovarajuće.
