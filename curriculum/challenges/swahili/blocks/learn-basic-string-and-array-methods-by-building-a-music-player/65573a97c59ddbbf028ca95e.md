...
Na kraju, treba da ponovo prikaže pesme, ažurira tekst dostupnog dugmeta za reprodukciju i ukloni dugme za resetovanje iz liste pesama. Takođe je potrebno ukloniti `resetButton` iz DOM.

Pozakrijte funkciju `renderSongs()` sa `sortSongs()` kao argumentom ili ponovo prikažite pesme po abecednom redosledu.

Pozovite funkciju `setPlayButtonAccessibleText()` da ažurira dostupni tekst dugmeta za reprodukciju.

Uklonite dugme za ponovno postavljanje sa liste pesama pozivom rute `remove()` na parametru `resetButton`.

**Napomena**: Sada možete pokušati da uklonite sve pesme kako biste videli šta se dešava.

# --hints--

Ne modifikujte postojeći event listener niti njegov sadržaj.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Trebalo bi da pozoveš funkciju `renderSongs` sa `sortSongs()`.
...
Trebalo bi da koristiš metod `remove()` za uklanjanje `resetButton` iz DOM.
