...
Da bi se obradili podaci o trenutnoj pesmi i pratilo njeno vreme reprodukcije, kreiraj parametre za `currentSong` i `songCurrentTime`. Postavi njihove vrednosti na `null` i `0`, po odvojeno.

# --hints--

Ne možete menjati nešto od sadržaja `userData` koje se nalazi unutra.

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

Vaš centar za `userData` mora imati ključ od `currentSong` postavljen na `null`.

```js
assert.propertyVal(userData, "currentSong", null)
```

Vaš centar za `userData` mora imati ključ od `songCurrentTime` postavljen na `0`.
