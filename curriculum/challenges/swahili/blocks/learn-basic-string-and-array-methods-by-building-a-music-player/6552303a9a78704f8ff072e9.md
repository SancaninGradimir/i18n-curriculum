...
Sada je vreme da se isproba dugme za zaustavljanje.

Dodajte slušača događaja za `"click"` na element `pauseButton`, zatim prosledite `pauseSong` kao drugi argument slušača događaja. Ovo je funkcija koju će slušač događaja izvršiti.

Isprobajte svoju aplikaciju klikom prvo na dugme za reprodukciju, a zatim na dugme za zaustavljanje. Trebalo bi da vidite da sve radi kako je očekivano.

# --hints--

Treba ti da pozoveš putanju `addEventListener()` na svoj parametar `pauseButton`.
...
Trebalo bi da prosledite `pauseSong` kao drugi argument vaše `addEventListener`.
