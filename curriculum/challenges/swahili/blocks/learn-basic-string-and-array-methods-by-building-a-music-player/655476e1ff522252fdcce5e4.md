...
Dodaj element `else` u izjavu `if`. Unutar elementa `else`, pozovi funkciju `getCurrentSongIndex()` i postavi njenu vrednost u konstantu nazvanu `currentSongIndex`.

# --hints--

Ne možete promeniti postojeći izraz `if`.
...
Trebalo bi da dodaš `else` na postojeći izraz `if`.
...
Trebalo bi da postaviš stabilizator `currentSongIndex` unutar elementa `else`, i to je `getCurrentSongIndex()`.
