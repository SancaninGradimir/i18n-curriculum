...
Sada ste kreirali dugme, morate mu dodeliti tekstualnu vrednost. Da biste to uradili, potrebno je da koristite metodu ``createTextNode()`` iz `DOM`.

Metoda `createTextNode()` se koristi za kreiranje tekstualnog čvora. Da biste je koristili, pozivate je i dajete joj tekst kao niz karaktera:

```js
document.createTextNode("your text")
```

Takođe ga možete postaviti kao parametar:

```js
const myText = document.createTextNode("your text")
```

Koristi putanju `createTextNode()` za kreiranje teksta `"Reset Playlist"`, zatim postavi njegovu vrednost u parametar `resetText`.
...
Trebalo bi da koristiš `document.createTextNode()` za kreiranje teksta `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Trebalo bi da postaviš tekstualni čvor koji si kreirao na parametar `resetText`.

```js
assert.match(code, /const\s+resetText\s*=\s*document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```
