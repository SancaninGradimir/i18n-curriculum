...
Dodajte treći stavak sa sledećim karakteristikama i vrednostima:
...
Tvoj red `allSongs` bi trebalo da ima nešto kao njegova treća vrednost.

```js
assert.isObject(allSongs[2]);
```

Treći element u vašem redosledu `allSongs` treba da ima atribut `id` postavljen na broj `2`.

```js
assert.equal(allSongs[2].id, 2);
```

Treći element u vašem redu od `allSongs` treba da ima atribut `title` postavljen za sekvencu karaktera `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

Treći element u vašem nizu `allSongs` treba da ima atribut `artist` postavljen za niz karaktera `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

Treći element u vašem nizu `allSongs` treba da ima atribut `duration` postavljen za niz karaktera `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

Treće polje u vašem nizu `allSongs` mora imati atribut `src` postavljen za niz znakova `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
