...
Da biste pozvali funkciju sa imenom, možete se na tu funkciju referisati po njenom imenu.

```js
exampleArrowFunction();
```

Ispod vaše definicije funkcije `printGreeting`, koristiće tu funkciju. Otvori konzolu da vidiš rezultate.

# --hints--

Trebalo bi da pozoveš svoju funkciju `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

Poziv tvoje funkcije za `printGreeting` treba da bude ispod definicije funkcije `printGreeting`.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

Poziv vaše funkcije za `printGreeting` treba da ispusti `Hello there!` na konzolu.
