...
Pronađi elemente `#next`, `#previous` i `#shuffle` iz fajla HTML.

Postavite njihove vrednosti u parametre nazvane `nextButton`, `previousButton`, i `shuffleButton`, u tom redosledu.

# --hints--

Trebalo bi da imaš kriterijum nazvan `nextButton`.

```js
assert.isDefined(nextButton);
```

Trebalo bi da koristite `document.getElementById()` da biste dobili element `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Treba da postavite element `#next` u parametar `nextButton`.

```js
assert.equal(nextButton, document.getElementById('next'));
```

Trebalo bi da imaš parametar nazvan `previousButton`.

```js
assert.isDefined(previousButton);
```

Trebalo bi da koristiš `document.getElementById()` da dobiješ funkciju `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Treba da staviš element `#previous` u parametar `previousButton`.

```js
assert.equal(previousButton, document.getElementById('previous'));
```

Trebalo bi da imaš parametar nazvan `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

Trebalo bi da koristite `document.getElementById()` da biste dobili funkciju `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Treba da staviš element `#shuffle` u parametar `shuffleButton`.
