...
Trenutno, `songsHTML` je niz podataka. Ako pokušate da ga prikažete kako jeste, videćete pesme razdvojene zarezima. Ovo nije rezultat koji želite jer želite da prikažete te pesme kao listu. Da biste ovo popravili, moraćete da spojite niz podataka u jednu sekvencu karaktera koristeći metodu <dfn>join()</dfn>.

Metoda `join()` se koristi za spajanje svih elemenata niza podataka u jednu sekvencu karaktera. Prihvata opcionalni parametar nazvan `separator`, koji se koristi za razdvajanje svakog elementa niza podataka. Primer:
...
Spoji put `join()` sa svojim putem `map()` i prođi niz praznih karaktera kao popunjavanje.

Da biste spojili više putanja zajedno, možete pozvati putanju `join()` u rezultatima putanje `map()`. Primer:
...
Trebalo bi dodati `join("")` u postojeći kod.
