...
Unutar funkcije `playPreviousSong`, dodaj deklaraciju `if` sa uslovom `userData?.currentSong === null`. Ovo će proveriti da li se trenutno reprodukuje neka pesma. Ako nema, izlazi iz funkcije koristeći `return`.

Unutar bloka od `else`, kreiraj konstantu nazvanu `currentSongIndex` i postavi vrednost `getCurrentSongIndex()`.

# --hints--

Ne smijete mijenjati funkciju `playPreviousSong` koja se nalazi i sadrži u njemu.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Trebalo bi da imaš deklaraciju `if` sa uslovom `userData?.currentSong === null` unutar svoje funkcije `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Treba da imaš `return` unutar bloka deklaracije `if`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Izjava za `if` mora da ima blok od `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Morate pozvati `getCurrentSongIndex` i postaviti vrednost `currentSongIndex` unutar bloka `else` vaše deklaracije `if`.
