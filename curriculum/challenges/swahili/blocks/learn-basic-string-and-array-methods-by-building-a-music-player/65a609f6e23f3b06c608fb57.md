...
Metoda `sort()` prihvata funkciju povratnog poziva za poređenje koja određuje redosled sortiranja.

U ovom primeru, prvi uslov `(a.name < b.name)` proverava da li je ime prvog voća manje od imena drugog voća. U tom slučaju, prvo voće se raspoređuje pre drugog voća.

Sekvenca znaka se poredi po abecednom redosledu, što znači da se poređuje znak po znak. Na primer, `"Apples"` je manje od `"Bananas"` jer `"A"` dolazi pre `"B"` u abecedi.

Razlog zašto ovaj primer vraća broj je zato što funkcija `sort()` očekuje da vrati broj. Ako se vrati negativan broj, prvi element će biti postavljen pre drugog elementa.
...
Unutar svoje callback funkcije, dodajte deklaraciju `if` ili proverite da li je `a.title` veće od `b.title`. Ako jeste, vratite `-1`.

# --hints--

Treba da imaš deklaraciju `if` unutar svoje funkcije za povratni poziv.

```js
assert.match(code, /if\s*\(/);
```

Vaša izjava za `if` treba da proveri da li `a.title` je manje od `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

Vrednost za `if` treba da vrati `-1` ako `a.title` je manje od `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)\s*{\s*return\s*-1\s*;?\s*}|if\s*\(\s*a\.title\s*<\s*b\.title\s*\)\s*return\s*-1\s*;?/);
```
