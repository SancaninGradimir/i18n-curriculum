...
Kada pažljivo proverite, videćete da pesma koja se trenutno reprodukuje nije istaknuta na listi pesama, pa ne znate koja je pesma koja se reprodukuje. Ovo možete popraviti kreiranjem funkcije koja ističe bilo koju reprodukovanu pesmu.

Ako se oslanjate na sinaksis sa strelicom, kreirajte funkciju za `highlightCurrentSong`. Unutar te funkcije, koristite `querySelectorAll` da dohvatite svojstvo od `.playlist-song` i postavite njegovu vrednost u varijablu `playlistSongElements`.

# --hints--

Trebalo bi da koristiš `const` za kreiranje funkcije `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` Trebalo bi da bude funkcija.

```js
assert.isFunction(highlightCurrentSong)
```

Vaša funkcija za `highlightCurrentSong` treba da koristi strelicastu sintaksu.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Treba da kreirate konstantu `playlistSongElements` unutar svoje funkcije `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Trebalo bi da koristiš metodu `querySelectorAll()` za odabir elementa `.playlist-song` i postaviš njegovu vrednost u konstantu `playlistSongElements`.
