...
Dodajte oslušač događaja za `"click"` na element `shuffleButton`. Da bi funkcija radila, uključite skriptu/funkciju `shuffle`.

**Napomena**: Ne treba vam povratni poziv unutar ovog slušaoca događaja. Takođe ne treba da pozovete funkciju `shuffle`, samo pitajte njegov identifikator.

# --hints--

Trebalo bi da povežete putanju `addEventListener()` sa `shuffleButton`.
...
Treba da prosledite `shuffle` kao drugu vrednost vašeg event listener-a za događaj `"click"`.
