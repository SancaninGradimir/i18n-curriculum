...
Unutar funkcije, dobijte reference na komponente od HTML koje su odgovorne za prikazivanje naslova pesme i izvođača.

Dohvati elemente iz `#player-song-title` i `#player-song-artist` pomoću `getElementById()`. Postavi njihove vrednosti u parametre `playingSong` i `songArtist` odgovarajuće.

# --hints--

Ne smijete mijenjati funkciju `setPlayerDisplay` koja je locirana i sadržana unutar nje.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Trebalo bi da koristite `document.getElementById()` da biste dobili element `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Trebalo bi da staviš element `#player-song-title` u parametar `playingSong`. Ne zaboravi da koristiš `const` za deklarisanje tog parametra.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Trebalo bi da koristite `document.getElementById()` da biste dobili funkciju/element `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Treba da staviš element `#player-song-artist` u parametar `songArtist`. Ne zaboravi da koristiš `const` za deklarisanje tog parametra.
