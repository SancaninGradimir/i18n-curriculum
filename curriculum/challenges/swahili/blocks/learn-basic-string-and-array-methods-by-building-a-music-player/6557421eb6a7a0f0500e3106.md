...
Unutar elementa dugmeta u funkciji `renderSongs`, dodajte svojstvo elementa `onclick`. Za vrednost, pozovite funkciju `deleteSong` i unesite `song.id`.

# --hints--

Trebalo bi da dodaš atribut elementa `onclick` na dugme za brisanje i prođeš preko `"deleteSong(${song.id})"`.
