...
Sledeće, trebaće vam da pronađete sledeću pesmu u spisku pesama. Stoga, trebaće vam da dobijete šablon trenutne pesme i zatim dodate `1` u njega.

Napravite komponentu nazvanu `nextSong` i postavite je unutar `userData?.songs[currentSongIndex + 1]`.

Konačno, pozovi funkciju `playSong` i prosledi `nextSong.id` kao argument.

# --hints--

Trebalo bi da promenite postojeću deklaraciju `if`, deo `else`, sa sadržajem.
...
Treba ti postaviti `userData?.songs[currentSongIndex + 1]` u polje `nextSong`.
...
Trebalo bi da pozoveš funkciju ``playSong`` za ``nextSong.id``.
