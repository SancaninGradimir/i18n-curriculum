...
Kreiraj drugi element za `button` sa klasom stavki `playlist-song-delete` i svojstvom elementa `aria-label` postavljeno na `Delete` kombinovano sa `song.title`. Za sadržaj ikone za brisanje, zalepite sledeći SVG:
...
Trebalo bi da kreirate element za `button` sa klasom `playlist-song-delete` kao svojstvo njegovog prvog elementa i njegova vrednost.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Vaš dugme element treba da ima atribut elemenata `aria-label` sa vrednošću `Delete ${song.title}` kao drugi atribut elementa i njegova vrednost.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Trebalo bi da zalepiš sadržaj `svg` koji je dobijen kao sadržaj tvog elementa `button`.
