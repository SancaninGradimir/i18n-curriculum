...
Sledeće, koristite atribut `classList` i metodu `add()` da dodate klasu objekta `"playing"` na element `playButton`. Ovo će pronaći klasu objekta `"playing"` u fajlu CSS i dodati je na element `playButton`.

Da biste konačno pustili pesmu, koristite putanju `play()` u podešavanjima `audio`. `play()` je putanja od API mrežnog zvuka za reprodukciju MP3 fajla.

# --hints--

Trebalo bi da koristiš atribut `classList` sa metodom `add` za dodavanje klase objekta `"playing"` na `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Trebalo bi da koristiš putanju `play()` za izmene `audio`.
