...
Da bi dobili prethodnu pesmu, izvucite `1` iz `currentSongIndex` od `userData?.songs` i postavite njegovu vrednost u konstantu `previousSong`. Nakon toga, pozovite funkciju `playSong` i prosledite `previousSong.id` kao argument.
...
Treba da postaviš vrednost `userData?.songs[currentSongIndex - 1]` u konstantu `previousSong`.
...
Treba da pozoveš funkciju `playSong` sa `previousSong.id`.
