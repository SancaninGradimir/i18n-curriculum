...
U početnim fazama, naučili ste kako da koristite metodu ``sort()`` za sortiranje pesama po abecednom redosledu. Druge upotrebe funkcije za povratni poziv su pretvaranje niza podataka u nasumičnu sekvencu.

Jedan način da se pretvori red podataka u nasumičan je korišćenje `0.5` iz `Math.random()`, koji vraća slučajne vrednosti koje mogu biti pozitivne ili negativne. Ovo čini rezultate poređenja mešavinom pozitivnih i negativnih vrednosti, što uzrokuje nasumično raspoređivanje elemenata.
...
Koristi putanju `sort()` u redu podataka `userData?.songs`. Pozovi funkciju odgovora za tu putanju i vrati rezultat `Math.random() - 0.5`.


# --hints--

Ne smete da menjate postojeću funkciju za `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Trebalo bi da koristiš putanju `sort()` na `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Tvoja putanja ``sort()`` bi trebalo da ima funkciju povratne pozive koja koristi sintaksu strelice.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

Callback funkcija za vašu rutu/putanju ``sort()`` mora vratiti ``Math.random() - 0.5``.
