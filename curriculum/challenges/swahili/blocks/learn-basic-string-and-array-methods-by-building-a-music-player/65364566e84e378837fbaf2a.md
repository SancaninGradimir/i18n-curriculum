...
U prethodnim koracima, implementacija funkcionalnosti za pevanje pesme je završena. Sada morate dodati funkcionalnost na dugme za reprodukciju kako bi moglo da pjeva trenutnu pjesmu kada se klikne.

Koristi putanju `addEventListener()` i pitaj događaj `"click"` kao prvi argument, a praznu funkciju sa sintaksom strelice kao drugi argument, na primer `() => {}`.

# --hints--

Treba ti nazvati putanju `addEventListener()` u svoj parametar `playButton`.

```js
assert.match(code, /playButton\.addEventListener\(/)
```

Vaš slušalac događaja bi trebalo da prati/sluša događaj `"click"`.

```js
assert.match(code, /playButton\.addEventListener\(\s*('|"|`)click\1/)
```

Trebalo bi da koristiš strelčastu sintaksu za prosleđivanje praznog *callback* funkcionera svom *event listener*-u. Ne zaboravi takođe da ti trebaju prazne vitičaste zagrade.
