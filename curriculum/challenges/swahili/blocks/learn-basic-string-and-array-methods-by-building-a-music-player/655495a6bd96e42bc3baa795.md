...
Sledeće, morate prikazati naslov trenutne pesme i izvođača u interfejsu plejera. Koristite `const` i sinaksu sa strelicom da kreirate praznu funkciju za `setPlayerDisplay`.

# --hints--

Trebalo bi da koristiš `const` za kreiranje prazne funkcije sa imenom `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` Trebalo bi da bude funkcija.

```js
assert.isFunction(setPlayerDisplay)
```

Tvoja funkcija za `setPlayerDisplay` bi trebalo da koristi streličnu sintaksu.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Tvoja funkcija za `setPlayerDisplay` bi trebalo da bude prazna.
