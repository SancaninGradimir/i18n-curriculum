...
Metoda ``map()`` se koristi za iteriranje kroz niz podataka i vraćanje novog niza podataka. Korisno je kada želite kreirati novi niz podataka na osnovu vrednosti postojećeg niza. Primer:
...
Proverite da metoda `map()` prima funkciju kao argument. Ovo se zove <dfn>callback funkcija</dfn>, koja je funkcija prosleđena drugoj funkciji kao argument. U gornjem primeru, callback funkcija je `(number) => number * 2`, i izvršava se za svaki element u redu `numbers`. Metoda `map()` zatim vraća novi red podataka sa rezultatima.

Proširi funkciju povratnog poziva putem ``map()``. Funkcija povratnog poziva treba da primi ``song`` kao parametar, koristi sintaksu strelične funkcije i da ima prazno telo.

# --hints--

Funkcija za povratni poziv vaše rute ``map()`` treba da koristi sintaksu strelične funkcije.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Tvoja putanja za `map()` treba da ima parametar `song` u svojoj funkciji odgovora (callback).

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(\s*song\s*\)|song)/)
```
