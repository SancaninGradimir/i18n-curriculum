...
Unutar funkcije `playSong`, pozovi funkciju `highlightCurrentSong`.

Nakon toga, pokušajte da pritisnete kontrolna dugmad kako biste videli kako funkcionalnost `highlightCurrentSong` radi.

# --hints--

Trebalo bi da pozovete funkciju `highlightCurrentSong`.

```js
assert.match(code, /highlightCurrentSong\(\s*\)/)
```
