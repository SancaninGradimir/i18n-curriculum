...
Unutar funkcije `playNextSong`, napravi izjavu `if` koja proverava da li `currentSong` od `userData` je potpuno jednako sa `null`. Ovo će proveriti da li trenutno nije reprodukovana pesma u objektu `userData`.

Ako je uslov tačan, pozovi funkciju `playSong` sa `id` prve pesme u redovima podataka `userData?.songs` kao argument.

# --hints--

Ne možete promeniti funkciju `playNextSong` koja se nalazi i sadržaj unutar nje.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Treba da napraviš izraz za `if` sa stanjem `userData?.currentSong === null`.

```js
const splitter = code.split('const getCurrentSongIndex = () => userData?.songs.indexOf(userData?.currentSong);')

assert.match(splitter[0], /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*/)
```

Treba da pozoveš funkciju `playSong` sa `userData?.songs[0].id` unutar izraza `if`.
