...
Pre nego što počnete da pevate pesmu, morate osigurati da počinjete od početka. Ovo se može postići korišćenjem svojstva ``currentTime`` na objektu ``audio``.

Dodaj izjavu za `if` ili proveri da li je `userData?.currentSong` nevažeći *ILI* da li `userData?.currentSong.id` nije u potpunosti jednak `song.id`. Ovo stanje će proveriti da li trenutno nije peva pesma ili da je trenutna pesma drugačija od one koja se očekuje.

Unutar `if`, postavi svojstvo `currentTime` objekta `audio` na `0`.

# --hints--

Treba da kreiraš deklaraciju za `if` sa statusom `userData?.currentSong === null || userData?.currentSong.id !== song.id`. Nemoj zaboraviti da uključiš sekvencijalno povezivanje.
...
Trebalo bi da staviš `audio.currentTime` unutar svog bloka `if`, a to je `0`.
