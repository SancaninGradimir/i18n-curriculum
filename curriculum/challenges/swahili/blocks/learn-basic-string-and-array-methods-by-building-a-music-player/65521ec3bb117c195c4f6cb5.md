...
Da bi se sačuvao trenutno vreme pesme prilikom napuštanja, postavite vrednost `songCurrentTime` objekta `userData` na `currentTime` promenljive/izmene `audio`.

*Napomena*: Ne treba koristiti serijsku vezu za ovaj korak jer `userData.songCurrentTime` neće biti `null` ili `undefined` u ovom trenutku.

# --hints--

Ne možete promeniti funkciju `pauseSong` koja postoji i sadrži unutar sebe.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Trebalo bi da postaviš vrednost `audio.currentTime` za `userData.songCurrentTime`.
