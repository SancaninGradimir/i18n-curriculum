...
Sada si kreirao `resetButton`, moraš da postaviš vrednost svojstva elementa `id` i `aria-label`. JavaScript pruža parametre `id` i `ariaLabel` koje ti treba za ovo.

Na primer, `element.id` će postaviti atribut elementu `id`, a `element.ariaLabel` će postaviti atribut elementu `aria-label`. Oba prihvataju svoje vrednosti kao string karaktera.

Postavi svojstvo elementa `id` od elementa `resetButton` na `"reset"`, i svojstvo njegovog elementa `aria-label` na `"Reset playlist"`.
...
Treba da koristiš `resetButton.id` za kreiranje atributa elementa `id` sa imenom `"reset"` za `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Trebalo bi da koristiš `resetButton.ariaLabel` za kreiranje atributa elementa `"aria-label"` sa imenom `"Reset playlist"` za `resetButton`.
