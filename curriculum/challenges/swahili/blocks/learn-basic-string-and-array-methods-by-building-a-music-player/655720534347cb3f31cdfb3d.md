...
Pre brisanja pesme, moraš proveriti da li je pesma trenutno reprodukovana. Ako se reprodukuje, moraš zaustaviti pesmu i pustiti sledeću pesmu sa liste pesama.

Koristite funkciju `if` da proverite da li je `userData?.currentSong?.id` jednako `id` pesme koju želite da izbrišete.

# --hints--

Treba da kreiraš prazan izraz (ili: string) sa uslovom `userData?.currentSong?.id === id`, i to je `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*\}/)
```
