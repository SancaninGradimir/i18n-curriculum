...
Dodajte drugi element sa sledećim ključevima i vrednostima:
...
Vaš red za `allSongs` bi trebalo da ima dve vrednosti.

```js
assert.equal(allSongs.length, 2);
```

Vaš red `allSongs` bi trebalo da ima nešto poput njegove druge vrednosti.

```js
assert.isObject(allSongs[1]);
```

Drugi element u vašem redu od `allSongs` treba da ima atribut `id` postavljen za broj `1`.

```js
assert.equal(allSongs[1].id, 1);
```

Drugi element u vašem redu od `allSongs` treba da ima svojstvo `title` postavljeno za sekvencu znakova `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Drugi element u vašem nizu od `allSongs` mora imati atribut `artist` postavljen za niz karaktera `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Drugi element u vašem redu od `allSongs` mora imati svojstvo `duration` postavljeno za niz znakova `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Drugi element u vašem redu od `allSongs` treba da ima svojstvo `src` postavljeno za sekvencu karaktera `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
