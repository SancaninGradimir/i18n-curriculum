...
Da biste osigurali da se prikaz plejera ažurira svaki put kada počne pustati nova pesma, pozovite funkciju `setPlayerDisplay()` unutar funkcije `playSong()`.

Sada bi trebalo da vidiš naslov pesme i izvođača koji se pojavljuju na predstavi.

# --hints--

Trebalo bi da pozovete funkciju `setPlayerDisplay` unutar vaše funkcije `playSong`.
