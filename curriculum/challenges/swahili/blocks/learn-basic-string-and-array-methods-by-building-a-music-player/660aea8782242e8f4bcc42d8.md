...
Kao i standardne funkcije, strelične funkcije mogu primiti mnogo parametara.

Ovo je primer funkcije sa strelicom koja ima naziv i jedan parametar:
...
Ako funkcija ima samo jedan parametar, možete ukloniti standardne zagrade koje okružuju listu parametara kao ovde:
...
Napravi novu funkciju strelice sa imenom `printMessage` koja ima jedan parametar nazvan `org`. Unutar tela te funkcije, dodaj deklaraciju konzole. Unutar te deklaracije na konzoli, dodaj niz znakova template literal-a `${org} is awesome!`.

Unutar vaše funkcije `printMessage`, pozovite tu funkciju i prosledite niz znakova `"freeCodeCamp"` kao argument.

Otvori konzolu ili vidi rezultate.

# --hints--

Trebalo bi da imaš funkciju `printMessage`.

```js
assert.isFunction(printMessage);
```

Trebalo bi da koristiš sintaksu strelične funkcije.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Vaša funkcija za `printMessage` mora imati parametar nazvan `org`.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?org?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/)

```

Vaša funkcija za `printMessage` treba da ima konzolnu deklaraciju.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Vaša konzolna izjava bi trebalo da uključuje niz karaktera template literal-a `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Trebalo bi da pozoveš funkciju `printMessage` i proslediš sekvencu znakova `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

Poziv vaše funkcije treba da bude unutar definicije vaše funkcije `printMessage`.

```js
const functionCall = code.indexOf("printMessage(");
const functionDefinition = code.indexOf("printMessage =");

assert.isBelow(functionDefinition, functionCall);
```

Poziv funkcije za `printMessage` treba da ispusti `freeCodeCamp is awesome!` u konzolu.
