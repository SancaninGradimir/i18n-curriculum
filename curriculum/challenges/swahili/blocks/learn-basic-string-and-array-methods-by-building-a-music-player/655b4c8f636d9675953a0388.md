...
Ako nema sledeće pesme u plejlisti, koristi deo `else` da modifikuje ključ `currentSong` od `userData` u `null`, i njegov atribut/svojstvo `songCurrentTime` u `0`.

# --hints--

Ne smete da menjate izjavu `if` koja je prisutna i njen sadržaj.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Vaša izjava o `if` mora da sadrži deo `else`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Trebalo bi da koristiš dot notaciju kako bi postavio svojstvo `currentSong` od `userData` na `null` u delu `else` tvog deklaracije za `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Trebalo bi da koristite tačkasto pisanje da postavite svojstvo za `songCurrentTime` od `userData` na `0` u delu vaše deklaracije `else` `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*userData\.songCurrentTime\s*=\s*0\s*;?\s*\}\s*;?/)
```
