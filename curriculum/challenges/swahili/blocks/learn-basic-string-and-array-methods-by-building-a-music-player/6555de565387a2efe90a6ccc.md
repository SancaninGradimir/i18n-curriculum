...
Funkcija `setPlayButtonAccessibleText` postavlja svojstvo elementa `aria-label` za trenutnu pesmu, ili za prvu pesmu u listi pesama. I ako je lista pesama prazna, on će postaviti `aria-label` na `"Play"`.

Koristite putanju `setAttribute` na elementu `playButton` da postavite svojstvo elementa nazvano `"aria-label"`. Koristeći trodimenzionalni kalkulator uslova, postavite vrednost svojstva elementa na `Play ${song.title}` ili `"Play"` ako `song?.title` nije dostupan.

Ne zaboravi da ovde unosiš vrednost, stoga moraš koristiti backtick znakove (backticks).

# --hints--

Ne možete menjati funkciju `setPlayButtonAccessibleText` koja se nalazi unutar nje.
...
Treba ti da koristiš putanju `setAttribute()` na `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` treba da bude prva vrednost vaše putanje za `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Tvoja putanja ``setAttribute`` bi trebalo da ima ``song?.title ? `Play ${song.title}` : "Play"`` kao drugi argument.
