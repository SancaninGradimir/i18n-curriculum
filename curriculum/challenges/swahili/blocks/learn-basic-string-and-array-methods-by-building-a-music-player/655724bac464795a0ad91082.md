...
Zatim, morate proveriti da li je lista pesama prazna. Ako je prazna, trebalo bi da resetujete stavku `userData` na njeno originalno stanje.

Koristi izraz `if` da proveri da li `userData?.songs` ima dužinu od `0`.

# --hints--

Trebalo bi da kreirate izraz od `if` prazan sa uslovom `userData?.songs.length === 0`.

```js
assert.match(code, /if\s*\(\s*userData\?\.songs\.length\s*===\s*0\s*\)\s*\{\s*\}/)
```
