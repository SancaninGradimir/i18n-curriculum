...
Koristi `const` i sintaksu strelice da kreiraš praznu funkciju za `playPreviousSong`.

# --hints--

Trebalo bi da koristite `const` za kreiranje neupoređenog funkcionera sa imenom `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` Trebalo bi da bude funkcija.

```js
assert.isFunction(playPreviousSong)
```

Tvoja funkcija za `playPreviousSong` treba da koristi streličnu sintaksu.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Tvoja komponenta za `playPreviousSong` bi trebalo da bude prazna.
