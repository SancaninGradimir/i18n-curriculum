...
Ako postoji jednačina, onda postavi `userData.currentSong` da bude `null` na `userData.songCurrentTime` da bude `0`.

Nakon toga, pozovi aktera `pauseSong()` da napusti igru i aktera `setPlayerDisplay()` da omogući prikaz igrača.
...
```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*.*\s*.*\s*.*\s*.*\s*\}/)
```

Trebalo bi da postaviš `userData.currentSong` na `null`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*.*\s*.*\s*\}/)
```

Treba da staviš `userData.songCurrentTime` umesto `0`.
