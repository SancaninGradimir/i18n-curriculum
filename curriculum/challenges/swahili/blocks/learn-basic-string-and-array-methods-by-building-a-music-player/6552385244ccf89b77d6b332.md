...
Treba da se izvrši funkcija reprodukcije sledeće i prethodne pesme. Za ovo, biće vam potreban modul `playNextSong` i `playPreviousSong`.

Koristite `const` i sintaksu strelice da kreirate praznu funkciju za `playNextSong`.

# --hints--

Trebalo bi da koristiš `const` za kreiranje neupoređenog funkcionera sa imenom `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` Trebalo bi da bude funkcija.

```js
assert.isFunction(playNextSong)
```

Tvoja funkcija ``playNextSong`` treba da koristi streličnu sintaksu.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Tvoja funkcija za `playNextSong` treba da bude prazna.
