...
Morate dobiti pesmu koja se trenutno reprodukuje ili prvu pesmu u plejlisti. Da biste to uradili, kreirajte konstantu `song` i koristite OR konstruktor (`||`) da je postavite kao trenutnu pesmu za `userData`, **ili** prvu pesmu u redovima podataka `userData?.songs`.

Ne zaboravite da koristite serijsko povezivanje.

# --hints--

Ne smete menjati `setPlayButtonAccessibleText` koji je prisutan i njegov sadržaj.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Trebalo bi da dobijete pristup `userData?.currentSong` ili `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Potrebno je postaviti vrednost `userData?.currentSong || userData?.songs[0]` u varijablu `song`.

```js
assert.match(code, /const\s+song\s*=\s*userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```
