...
Morate dodati `resetText` kao dete elementa `resetButton`, i takođe `resetButton` kao dete elementa `playlistSongs`. Za ovo postoji način da se koristi `appendChild()`.

`appendChild()` omogućava vam da dodate čvor ili element kao dete drugog elementa. U primeru prikazanom ispod, tekst "Click me" biće postavljen na dugme:
...
Upotrebite `appendChild()` da priložite `resetText` na polje `resetButton`, i `resetButton` na polje `playlistSongs`.

# --hints--

Trebalo bi da koristiš putanju `appendChild()` na `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Trebalo bi da unesete `resetText` kao vrednost za vaš prvi `appendChild()`.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Trebalo bi da koristiš putanju `appendChild()` na `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Trebalo bi da unesete `resetButton` kao vrednost vašeg drugog `appendChild()`.
