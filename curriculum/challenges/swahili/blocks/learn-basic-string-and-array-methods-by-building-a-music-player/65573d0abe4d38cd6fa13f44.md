...
Sada sve glavne funkcije rade kako treba. Jedini problem je što sljedeća pjesma se ne reprodukuje automatski kada završi trenutno slušana pjesma.

Da biste to popravili, možete postaviti slušalicu događaja koja detektuje kada pesma koja se trenutno reprodukuje završi. Slušalica događaja za `"ended"` je odgovarajuća za ovo. Aktivira se kada reprodukcija medija dođe do kraja.

Dodaj slušaoca događaja za element/svojstvo ``audio`` koji sluša događaj ``"ended"``. Koristi poziv za povratak koristeći strelicastu sintaksu i prazne zagrade.

# --hints--

Trebalo bi da povežete putanju `addEventListener()` sa vašim parametrom `audio`.

```js
assert.match(code, /audio\.addEventListener\(/)
```

Slušalac događaja koji je koristio vaš parametar za `audio` bi trebalo da sluša događaj `"ended"`.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1/)
```

Trebalo bi da koristiš strelčastu sintaksu za prosleđivanje poziva koji vraća prazno vašem slušaocu događaja `"ended"`.
