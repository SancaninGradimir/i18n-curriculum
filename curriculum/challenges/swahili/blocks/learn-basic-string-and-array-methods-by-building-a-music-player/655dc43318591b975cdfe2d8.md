...
Unutar reda podataka `allSongs`, kreiraj objekat sa sledećim svojstvima i vrednostima:
...
Red podataka `allSongs` treba da ima jednu vrednost.

```js
assert.equal(allSongs.length, 1);
```

Podatkovni red `allSongs` treba da ima nešto poput svoje prve vrednosti.

```js
assert.isObject(allSongs[0]);
```

Red podataka `allSongs` mora imati element sa karakteristikom `id`, definisan brojem `0`.

```js
assert.equal(allSongs[0].id, 0);
```

Red podataka `allSongs` treba da sadrži nešto sa svojstvom `title` postavljeno u sekvencu karaktera `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Redovi podataka `allSongs` bi trebalo da sadrže nešto sa karakteristikom `artist` postavljeno u sekvencu slova `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Redovi podataka `allSongs` bi trebalo da sadrže nešto sa karakteristikom `duration` definisanom nizom slova `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Redovi podataka `allSongs` bi trebali imati nešto sa karakteristikom `src` postavljeno u sekvencu slova `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
