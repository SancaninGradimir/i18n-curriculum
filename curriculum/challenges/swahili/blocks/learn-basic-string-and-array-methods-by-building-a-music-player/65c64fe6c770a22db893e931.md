...
Sada, nakon što imate listu pesama prikazanih na ekranu, biće dobro da ih sortirate po alfabetskom redosledu prema naslovu pesme.

Počnite sa kreiranjem strelične funkcije koja se zove `sortSongs`.

# --hints--

Trebalo bi da imaš funkciju koja se zove `sortSongs`.

```js
assert.isFunction(sortSongs);
```

Tvoja funkcija `sortSongs` bi trebalo da koristi sinaksu strelične funkcije.

```js
assert.match(code, /(const|let|var)\s+sortSongs\s*=\s*\(\s*\)\s*=>\s*{\s*}\s*;?/)
```
