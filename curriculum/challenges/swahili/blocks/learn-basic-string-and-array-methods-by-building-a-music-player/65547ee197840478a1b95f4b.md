...
Sada je vreme za testiranje funkcije `playNextSong`.

Dodajte listener događaja za `"click"` na element `nextButton`, a zatim prosledite `playNextSong` kao drugi argument vašem listeneru događaja. Ovo je funkcija koju će listener događaja izvršiti.

Testiraj svoju aplikaciju tako što ćeš prvo kliknuti na dugme za reprodukciju, a zatim na dugme „Nastavi“. Trebalo bi da vidiš da sve radi kako je očekivano.

# --hints--

Treba ti da pozoveš putanju `addEventListener()` u svoj parametar `nextButton`.
...
Trebalo bi da prosledite `playNextSong` kao drugi argument za vašu `addEventListener`.
