...
Potrebno je ponovo prikazati pesme, prikazujući ih isticanjem i dodavanjem teksta za pristupnost na dugme za reprodukciju jer će se lista pesama promeniti.

Izvršite poziv funkcije `renderSongs` sa nizom podataka `userData?.songs` kao argument, što će prikazati listu izmenjenih pesama.

Nakon toga, pozovi funkciju `highlightCurrentSong` da istakne trenutnu pesmu ako postoji, i takođe funkciju `setPlayButtonAccessibleText` za ispravljanje teksta pristupačnosti dugmeta za reprodukciju.

# --hints--

Ne smijete mijenjati funkciju `deleteSong` koja je locirana i sadržana u njemu.
...
Trebalo bi da pozovete funkciju ``renderSongs``, priloženo sa ``userData?.songs``.
