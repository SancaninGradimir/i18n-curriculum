...
Pre nego što počnete da reprodukujete sledeću i prethodnu pesmu, morate dobiti predglednik svake pesme u svojstvu `songs` od `userData`.

Započnite kreiranjem strelične funkcije pod nazivom `getCurrentSongIndex`.

# --hints--

Trebalo bi da imaš funkciju `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Tvoja funkcija za `getCurrentSongIndex` treba da koristi streličnu sintaksu.

```js
assert.match(code, /(let|const)\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>\s*{\s*}\s*/);
```
