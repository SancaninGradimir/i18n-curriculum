...
Metoda <dfn>find()</dfn> pronalazi prvi element unutar niza podataka koji zadovoljava uslove definisane u datoj funkciji povratne pozive. Ako nijedan element ne zadovoljava uslov, metoda vraća `undefined`.

U primeru ispod, metoda `find()` se koristi za dobijanje prvog broja većeg od 25:
...
Koristi `const` da kreira parametar nazvan `song` i postavi vrednost rezultata poziva metode `find()` u red podataka `userData?.songs`. Koristi `song` kao parametar za povratni poziv `find()` i proveri da li je `song.id` identično sa `id`.

Ovo će proći kroz red podataka za `userData?.songs`, tražeći pesmu koja odgovara `id` poslatu u funkciju `playSong`.

# --hints--

Trebalo bi da koristiš putanju `find` na `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Tvoja putanja ``find`` bi trebalo da ima ``song`` kao funkcionalni parametar za strelicu povratnog poziva.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Tvoj put za `find` bi trebalo da koristi strogo jednakost poređenje da proveri da li je `song.id` jednako `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Tvoja putanja za `find` treba da dobije fiksnu vrednost parametra `song`.
