...
Vaš muzički plejer mora da prati pesme, trenutno reprodukovanu pesmu i trajanje te pesme. Da biste to uradili, biće vam potrebno kreirati nešto za skladištenje ovih informacija.

Počnite korišćenjem ključne reči `let` za deklarisanje novog parametra koji se zove `userData` i postavite mu vrednost praznog objekta.

# --hints--

Trebalo bi da koristiš ključnu reč `let` za kreiranje parametra `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

Trebalo bi da postavite vrednost praznog objekta za vaš atribut `userData`.
