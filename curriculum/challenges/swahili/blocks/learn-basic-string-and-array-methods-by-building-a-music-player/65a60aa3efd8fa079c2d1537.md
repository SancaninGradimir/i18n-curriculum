...
Drugi uslov u ovom primeru proverava da li `a.name > b.name`. Ako je tako, funkcija vraća `1`, koji priprema prvi voćni deo nakon drugog voćnog dela.
...
Unutar vaše funkcije povratnog poziva (callback), dodajte još jednu deklaraciju od `if` da proverite da li je `a.title` veće od `b.title`. Ako jeste, vratite broj/vrednost `1`.

# --hints--

Trebalo bi da imaš drugi deklarativni deo `if` unutar svoje funkcije za odgovor.
...
Vaša izjava za `if` mora da proveri da li `a.title` je veće od `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

Vaša izjava za `if` treba da vrati broj `1` ako je `a.title` veći od `b.title`.
