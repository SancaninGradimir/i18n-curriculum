...
Treba vam da ispravite pesmu koja se trenutno reprodukuje zajedno sa izgledom elementa `playButton`.

Postavi vrednost `song` na svojstvo `currentSong` u objekt `userData`.

*Napomena*: Nije prikladno koristiti operator sekvencijalnog spajanja `?.` na ovom koraku jer `userData.currentSong` trenutno neće biti `null` ili `undefined`.

# --hints--

Treba da postaviš vrednost `song` za `userData.currentSong`.
