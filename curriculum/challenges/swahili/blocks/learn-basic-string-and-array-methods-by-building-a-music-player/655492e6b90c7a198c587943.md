...
Sada je potrebno ponovo dodati svojstvo elementa na trenutno reprodukovanu pesmu.

Napravi maticu za `if` sa listom uslova `songToHighlight`. Za tu maticu, koristi `setAttribute` u `songToHighlight` da premostiš `"aria-current"` i `"true"` kao prvi i drugi argument.

# --hints--

Trebalo bi da napravite deklaraciju od `if` sa uslovom `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Trebalo bi da koristiš putanju `setAttribute()` na `songToHighlight` unutar tvog izjave `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Trebalo bi da prosledite `"aria-current"` i `"true"` kao prvi i drugi argument vašeg puta/načina za `setAttribute`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(\s*('|")aria-current\1\s*,\s*('|")true\2\s*\)\s*;?\s*\}?/)
```
