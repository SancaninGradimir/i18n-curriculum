...
Unutar `map()`, dodajte izraz od `return` sa pozivnim oznakama gde ćete kombinovati sve komponente uključene u prikaz detalja pesme.

Unutar oznaka poziva, kreiraj element sa `li` koji ima svojstvo elementa `id` sa vrednošću `song-${song.id}` i svojstvo elementa `class` sa vrednošću `playlist-song`.

# --hints--

Trebalo bi da dodate izraz `return` sa praznim znakovima.

```js
assert.match(code, /return\s*`/)
```

Trebalo bi da napraviš etiketu `li` unutar oznaka za pozivanje.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Vaš label za otvaranje za `li` mora imati atribut elementa `id` postavljen na `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Tvoja početna vrednost za `li` mora imati svojstvo elementa `class` postavljeno na `playlist-song`.
