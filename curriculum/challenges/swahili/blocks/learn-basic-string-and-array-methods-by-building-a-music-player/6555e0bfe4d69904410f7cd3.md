...
Koristeći `const` sa sintaksom strelice kreiraće se prazna funkcija nazvana `shuffle`.

Ova funkcija je odgovorna za mešanje pesama u plejlisti i izvršavanje važnih ažuriranja upravljanja stanjem nakon mešanja.

# --hints--

Trebalo bi da koristiš `const` za kreiranje prazne funkcije nazvane `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` treba da bude funkcija.

```js
assert.isFunction(shuffle)
```

Vaša funkcija `shuffle` bi trebalo da koristi sintaksu strelice i bude bez parametara.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Tvoja funkcija za `shuffle` mora biti prazna.
