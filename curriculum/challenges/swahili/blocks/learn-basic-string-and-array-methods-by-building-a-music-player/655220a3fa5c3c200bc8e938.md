...
Koristi putanje `classList` i `remove()` da ukloniš klasu objekta `playing` iz `playButton`, jer će pesma privremeno ostati u ovom stanju.

Za potpunu pauzu pesme, koristite metodu `pause()` na parametru `audio`. `pause()` je Web Audio način rada API za privremeno pauziranje audio fajlova.

# --hints--

Ne možete promeniti funkciju `pauseSong` koja se nalazi i sadržaj unutar nje.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Trebalo bi da koristiš atribut `classList` na načinu `remove()` da ukloni klasu entiteta `"playing"` sa `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Trebalo bi da koristiš putanju `pause()` u svom parametru `audio`.
