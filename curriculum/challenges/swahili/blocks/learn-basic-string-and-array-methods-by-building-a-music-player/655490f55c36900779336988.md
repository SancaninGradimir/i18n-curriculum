...
Rudia preko `playlistSongElements` koristeći način/metodu `forEach`.

Metoda `forEach` se koristi za iteriranje kroz niz podataka i izvršavanje funkcije za svaki element u nizu podataka. Na primer, zamislite da imate niz brojeva i želite da svaki broj ispišete na konzolu.
...
Koristi putanju `forEach` u `playlistSongElements`. Prođi `songEl` kao parametar i koristi sintaksu strelice za dodavanje praznog povratnog poziva.

# --hints--

Trebalo bi da povežete `forEach()` sa `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Tvoj put za `forEach()` treba da ima parametar `songEl` i koristi semantičku sintaksu strelice.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

`forEach` Trebalo bi da ima prazne vitičaste zagrade.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*\}\s*\)\s*;?/)
```
