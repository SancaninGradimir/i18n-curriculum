...
Za vraćanje liste pesama u njeno originalno stanje, uključite `allSongs` unutar reda podataka i postavite mu vrednost za `userData.songs`.

*Napomena*: Ne treba ti serijsko povezivanje za `userData.songs` jer pesma neće biti `null` ili `undefined` u ovom trenutku.

# --hints--

Ne smete da menjate slušaoca postojećeg događaja i njegovog sadržaja.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*\)\s*;?/)
```

Trebalo bi da postaviš vrednost `[...allSongs]` za `userData.songs`.

```js
assert.match(code, /userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]/)
```
