...
Sada moraš da zaustaviš pesmu koja se trenutno svira.

Definiši funkciju `pauseSong` koristeći ključnu reč `const` i sintaksu strelične funkcije. Ta funkcija ne sme da prima nikakve parametre.

# --hints--

Trebalo bi da koristiš `const` za kreiranje prazne funkcije nazvane `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` Trebalo bi da bude funkcija.

```js
assert.isFunction(pauseSong)
```

Tvoja funkcija za `pauseSong` treba da koristi sinaksu sa strelicom.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Tvoja funkcija za `pauseSong` mora biti prazna.
