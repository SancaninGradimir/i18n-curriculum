...
Sada, došlo je vreme za dodavanje funkcije osvežavanja na `resetButton`. Ovo će vratiti pesme u plejlistu kada se klikne.

Dodajte slušač događaja klika na promene za `resetButton`. Proširi povratni poziv koristeći strelicastu sintaksu i ostavi ga prazan za sada.

# --hints--

Trebalo bi da koristiš putanju `addEventListener()` na `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

Slušalica za događaj `resetButton` treba da sluša događaj `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

Funkcija handler-a vašeg event listener-a mora da koristi arrow sintaksu i da ima prazne vitičaste zagrade.
