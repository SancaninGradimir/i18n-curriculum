...
U primeru, ako `a.name` je jednako `b.name`, onda funkcija vraća `0`. To znači da se ništa ne menja sa redosledom `a` i `b`, ostaje isto.
...
Na osnovu vaše deklaracije za `if`, vratite broj `0` ili ostavite redosled dve komponente nepromenjen.

# --hints--

Treba da vratiš broj `0` ispod svojih izjava od `if`.
