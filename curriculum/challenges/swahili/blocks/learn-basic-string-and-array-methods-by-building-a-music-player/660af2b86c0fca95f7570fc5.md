...
Ako funkcija sa strelicom vraća jednostavan matematički izraz, možete ukloniti glavnu reč `return` i vitičaste zagrade `{}`. Ovo se naziva implicitni povrat.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Ako vaša funkcija strelice sadrži mnogo linija koda unutar tela funkcije, onda morate koristiti ključnu reč `return` i vitičaste zagrade `{}`.
...
Promenite, ili ispravite, vašu funkciju `addTwoNumbers` ili uklonite glavnu reč `return` iz vitičastih zagrada `{}`. Vaša funkcija `addTwoNumbers` bi trebalo umesto toga da koristi nejasnu rekonstrukciju.

Otvorite konzolu ili da se uverite da i dalje dobijate tačne rezultate.

# --hints--

Vaša ispravljena funkcija `addTwoNumbers` bi trebalo da koristi indirektan metod oporavka.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*num1\s*\+\s*num2\s*;?/);
```
