...
Unutar callback funkcije, koristi `removeAttribute()` da ukloni svojstvo elementa `"aria-current"`. Ovo će ukloniti svojstvo elementa za svaku pesmu.

# --hints--

Ne smijete mijenjati `forEach()` koji je prisutan i njegov sadržaj.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Trebalo bi da koristiš putanju `removeAttribute()` na `songEl` i proslediš `"aria-current"`.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(?:\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*songEl\.removeAttribute\(\s*('|")aria-current\1\s*\)\s*;?\s*\}\s*\)\s*;?/)
```
