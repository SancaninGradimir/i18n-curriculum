...
Sada je vreme za implementaciju funkcije brisanja za listu pesama. Ovo će upravljati uklanjanjem pesme sa liste, rukovanjem drugim srodnim akcijama kada se pesma obriše, i kreiranjem dugmeta `Reset Playlist`.

Koristite `const` i sintaksu strelice da kreirate praznu funkciju `deleteSong`, prosleđujući `id` kao argument.

# --hints--

Treba da koristiš `const` za kreiranje prazne funkcije sa imenom `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` Trebalo bi da bude funkcija.

```js
assert.isFunction(deleteSong)
```

Tvoja funkcija za `deleteSong` bi trebalo da koristi strelnu sintaksu.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Tvoja funkcija ``deleteSong`` mora da primi parametar ``id``.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Tvoja funkcija za `deleteSong` bi trebalo da bude prazna.
