...
Primećujte da je slika albuma u HTML i pesme u redu podataka `userData.songs` promenjene. Zamenili smo originalne pesme sa kratkim pesmama koje možete koristiti za testiranje vaše aplikacije u sledećim koracima.

Sljedeće, morate proveriti da li postoji sledeća pesma za reprodukciju. Dobijte metadata trenutne pesme pozivom funkcije `getCurrentSongIndex()` i sačuvajte je u promenljivoj `currentSongIndex`.

Kasnije, definišite konstantu za `nextSongExists` koja drži vrednost tačno ili netačno od `true` ili `false`, u zavisnosti od toga da li postoji sledeća pesma.

# --hints--

Ne smete mijenjati *event listener* `ended` koji postoji i koji je unutar njega.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Morate kreirati konstantu za `currentSongIndex` i postaviti je jednako pozivu funkcije `getCurrentSongIndex`.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*const\s+currentSongIndex\s*=\s*getCurrentSongIndex\(\s*\)\s*;?\s*.*\s*\}\s*\)\s*;?/)
```

Trebalo bi da proverite postoji li sledeća pesma poređenjem `userData.songs.length` sa `currentSongIndex` i upisivanjem u varijablu `nextSongExists`. Ako je poslednji indeks niza podataka pesama (`userData.songs.length - 1`) veći od currentSongIndex, to znači da postoji sledeća pesma.

```js
const secondPart = code.split(`audio.addEventListener("ended", () => {`)[1];
const middlePart = secondPart.split("});")[0]
const codeOutputFirstTest = eval(middlePart + "\nnextSongExists;");
assert.strictEqual(codeOutputFirstTest, true);
const codeOutputSecondTest = eval(`\nfunction getCurrentSongIndex() { return 100; };\n` + middlePart + "\nnextSongExists;")
assert.strictEqual(codeOutputSecondTest, false);
```
