...
Kao i obične funkcije, strelične funkcije mogu vratiti vrednost.

Evo primera funkcije sa strelicom koja vraća rezultat množenja dve brojeve:
...
Kreiraj novi parametar nazvan `addTwoNumbers` i dodeli mu vrednost strelica funkcije. Ova strelica funkcija treba da prihvati dva parametra, koja se nazivaju `num1` i `num2`. Unutar tela funkcije, vrati matematički izraz za sabiranje `num1` i `num2`.

Ispod funkcije `addTwoNumbers`, dodajte konzolnu izjavu. Unutar te konzolne izjave, pozovite funkciju `addTwoNumbers` i prosledite brojeve `3` i `4` kao argumente.
...
Vaša funkcija za `addTwoNumbers` treba da koristi sinaksu strelične funkcije.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Funkcija za `addTwoNumbers` treba da ima dva parametra koja se zovu `num1` i `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Vaša funkcija `addTwoNumbers` treba da vrati matematički izraz za sabiranje `num1` i `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Trebalo bi da imate deklaraciju `console.log(addTwoNumbers(3, 4));`.

```js
assert.match(code, /console\.log\(addTwoNumbers\(3,\s*4\)\);?/);
```
