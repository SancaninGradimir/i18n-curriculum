...
Takođe bi trebalo da ponovo prikaže pesme, zaustavi trenutno reprodukovanu pesmu, postavi prikaz plejera i resetuje tekst dostupnog dugmeta za reprodukciju.

Pozovi funkciju za `renderSongs` i prosledi `userData?.songs` kao argument. Takođe, pozovi funkcije za `pauseSong`, `setPlayerDisplay` i `setPlayButtonAccessibleText`.

# --hints--

Ne modifikujte funkciju `shuffle` koja postoji i njen sadržaj.
...
Morate pozvati funkciju `renderSongs` uz `userData?.songs`.
