...
Kada se pesme prikazuju na stranici, treba da prikaže naslov, izvođača, trajanje svake pesme i dugme za brisanje.

U narednim koracima, naučićete kako da dodate HTML novo za svaku pesmu koristeći metodu <dfn>map()</dfn>. Ova metoda će biti detaljno objašnjena u sledećem koraku.

Počnite korišćenjem `const` da deklarišete parametar nazvan `songsHTML` i postavite vrednost `array.map()`.

# --hints--

Trebalo bi da imaš parametar `const` koji se zove `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

Trebalo bi da postaviš vrednost `array.map()` za svoj `songsHTML`.
