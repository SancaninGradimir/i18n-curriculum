...
Pošto će korisnici moći da mešaju i brišu pesme iz liste pesama, biće vam potrebno da kreirate kopiju reda podataka `allSongs` bez menjanja njegovog izvora. Ovde pomaže operater spread (`spread`).

Funkcija širenja (Spread) (`...`) omogućava vam da kopirate sve elemente iz jedne kolone podataka u drugu. Takođe se može koristiti za spajanje više redova podataka u jedan. U primeru ispod, `arr1` i `arr2` su prošireni unutar `combinedArr`:
...
Unutar objekta `userData` kreiraj svojstvo `songs`. Za vrednost, definisite/navucite `allSongs` unutar redova podataka.

# --hints--

Tvoj element za `userData` mora imati ključ nazvan `songs`.

```js
assert.property(userData, "songs");
```

Tvoja vrednost za `songs` mora biti `[...allSongs]`.
