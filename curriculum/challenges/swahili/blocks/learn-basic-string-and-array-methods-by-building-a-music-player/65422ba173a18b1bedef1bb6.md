...
Unutar funkcije hendlera slušaoca događaja, dodajte `if` ili proverite da li `userData?.currentSong` je `null`.

Unutar bloka `if`, pozovi funkciju `playSong()` za `id` prve pesme u redovima podataka `userData?.songs`. Ovo će osigurati da se prva pesma u plejlisti reprodukuje prvo.

# --hints--

Treba da kreiraš deklaraciju za `if` koja ima uslov `userData?.currentSong === null`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

Trebalo bi da pozoveš funkciju `playSong` za `userData?.songs[0].id` unutar svog bloka `if`.
