...
Unutar funkcije `playSong`, postavite svojstvo `audio.src` jednako `song.src`. Ovo opisuje audio element gde se dobijaju audio podaci za odabrani/izabranu pesmu.

Pia, stavi atribut za `audio.title` jednak `song.title`. Ovo objašnjava kako element zvuka treba da se prikaže kao naslov pesme.

# --hints--

Ne smete da menjate funkciju `playSong` koja je locirana i sadržana unutar nje.
...
Trebalo bi da staviš `audio.src` na mesto `song.src`.
...
Treba da staviš `audio.title` umesto `song.title`.
