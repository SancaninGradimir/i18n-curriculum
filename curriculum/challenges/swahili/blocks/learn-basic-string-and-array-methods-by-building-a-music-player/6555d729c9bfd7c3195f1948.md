...
`textContent` postavlja tekst čvora i omogućava postavljanjem ili dohvaćanjem tekstualni sadržaj elementa HTML.
...
Možete koristiti ternarni operator da dodelite vrednost tekstualnog sadržaja uslovno. Evo primera kako postaviti rezultat ternarnog operatora u parametar:

```js
const example = condition ? "I'm true" : "I'm false";
```

Koristite ternarni operater da proverite da li je `currentTitle` evaluirano kao istina. Ako jeste, postavite `playingSong.textContent` na `currentTitle`. U suprotnom, postavite prazan niz karaktera.

Nakon toga, koristi operaciju `ternary` da proveri da li `currentArtist` jeste istinska vrednost. Ako je tako, postavi `songArtist.textContent` na `currentArtist`. Inače, postavi ga na prazan niz znakova.

# --hints--

Trebalo bi da povežete atribut `textContent` sa `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Trebalo bi da koristite operater `ternary` da postavite atribut `textContent` za `playingSong` na `currentTitle` ili `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Treba da povežete atribut `textContent` sa `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Treba koristiti operaciju ``ternary`` za postavljanje svojstva ``textContent`` za ``songArtist`` na ``currentArtist`` ili ``""``.
