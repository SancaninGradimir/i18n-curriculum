...
Sada nakon što imate listu pesama prikazanih na ekranu, biće dobro ih sortirati po abecednom redosledu prema naslovu. Možete ručno ispraviti niz podataka `allSongs`, ali JavaScript ima funkciju niza podataka koju možete koristiti i koja se zove <dfn>sort()</dfn>.

Metoda ``sort()`` konvertuje elemente podatkovnog niza u string i raspoređuje ih prema njihovim vrednostima u kodiranju ``UTF-16``.
...
Unutar vaše funkcije za `sortSongs`, dodajte putanju `sort()` za `userData?.songs`.

# --hints--

Morate dodati putanju za `sort()` do `userData?.songs`.
