...
Da biste dobili indeks trenutne pesme, možete koristiti metodu <dfn>indexOf()</dfn>. Metoda niza podataka `indexOf()` vraća indeks prvog elementa koji se može pronaći u nizu podataka, ili `-1` ako element ne postoji.
...
Unutar vaše funkcije za `getCurrentSongIndex`, vrati `userData?.songs.indexOf()`. Za argument `indexOf()`, postavi na `userData?.currentSong`.

# --hints--

Trebalo bi da vratiš `userData?.songs.indexOf()` unutar svoje funkcije `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Trebalo bi da prođeš kroz `userData?.currentSong` na putu do `indexOf()`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(\s*userData\?\.currentSong\s*\)\s*;?|\s*{\s*return\s+userData\?\.songs\.indexOf\(\s*userData\?\.currentSong\s*\)\s*;?\s*})/)
```
