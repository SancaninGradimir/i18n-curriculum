...
Koristi putanju `filter()` za uklanjanje elementa pesme koji odgovara parametru `id` iz niza podataka `userData?.songs`.

Njia za `filter` samo čuva elemente redova podataka koji zadovoljavaju funkciji odgovora koja mu je prosleđena:
...
Koristi putanju `filter()` na `userData?.songs`. Prosledi `song` kao parametar funkcije za strelicu odgovora i koristi nejasno vraćanje da proveriš da li `song.id` nije potpuno jednako `id`. Stavi sve to u `userData.songs`.

*Napomena*: Ne bi trebalo koristiti opcionalno spajanje kada postavljate rezultat `userData?.songs.filter` u `userData.songs` jer redovi podataka za `allSongs` neće biti ni `undefined` ni `null` u tom trenutku.

# --hints--

Ne smete da menjate funkciju `deleteSong` koja se nalazi.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Trebalo bi da koristiš putanju `filter()` na `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

Funkcija povratnog poziva za vašu putanju ``filter()`` treba da koristi streličnu sintaksu i ima parametar ``song``.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Tvoj filtracioni put treba da vrati `song.id !== id` na nejasni način.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Treba da staviš putanju koju si koristio na `filter()` u `userData?.songs` na `userData.songs`.

```js
assert.match(code, /userData\.songs\s*=\s*userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```
