...
Dodajte deo `else`. Unutar segmenta `else`, pozovite funkciju `playSong` sa `id` trenutno reprodukovane pesme kao argumentom.

Ovo osigurava da će pesma koja se trenutno reprodukuje nastaviti sa puštanjem kada se klikne na dugme za reprodukciju.
...
Treba da dodaš deo `else` u svoj deklarativni deo `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}\s*else\s*\{/)
```

Treba da izvršite poziv funkcije ``playSong`` koristeći ``userData?.currentSong.id`` unutar bloka od ``else`` vaše deklaracije ``if``.
