...
Da bi se pesma pustila svaki put kada korisnik klikne na nju, dodajte atribut elementa `onclick` na prvi dugme element. Unutar `onclick`, pozovite funkciju `playSong` za `song.id`.

Nemoj zaboraviti da trebaš uneti vrednost za dolar znak ovde.

# --hints--

Trebalo bi da dodate svojstvo elementa `onclick` sa vrednošću `playSong(${song.id})` na element `button`.
