...
Treba vam da dobijete `id` za pesmu koja se trenutno svira. Za ovo, možete koristiti `userData?.currentSong?.id`.

Koristite `getElementById()` da dobijete `id` trenutno puštenu pesmu, a zatim koristite ključeve za dodavanje prefiksa `song-`. Postavite njenu vrednost u konstantu `songToHighlight`.

# --hints--

Ne smete menjati funkciju `highlightCurrentSong` koja je locirana i sadržana unutar njega.
...
Trebalo bi da koristiš `document.getElementById()` i pređeš `` `song-${userData?.currentSong?.id}` ``.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Trebalo bi da postaviš vrednost `getElementById()` u konstantu `songToHighlight`.
