...
Ako je lista pesama prazna, potrebno je kreirati element za `resetButton` i tekst za njega. Ovo dugme će biti vidljivo samo ako je lista pesama prazna.

`createElement()` je način za DOM koji možete koristiti da kreirate element dinamički koristeći JavaScript. Da biste koristili `createElement()`, pozivate ga, zatim prosledite ime oznake kao niz karaktera:
...
Takođe možete postaviti njegovu vrednost u parametar:

```js
const divElement = document.createElement('div')
```

U okviru vaše deklaracije za `if`, deklariš konstantu od `resetButton`, zatim koristi `createElement()` da kreiraš `"button"`.
...
Treba ti da koristiš način `createElement` za kreiranje `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Trebalo bi da staviš svoj novi element `button` na statičku `resetButton`.
