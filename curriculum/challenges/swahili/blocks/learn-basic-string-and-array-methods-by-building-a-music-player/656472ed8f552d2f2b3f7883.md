...
Dodajte funkciju `else` za rukovanje trenutnim vremenom reprodukcije pesme. Ovo vam omogućava da nastavite sa pesmom tamo gde ste stali.

Unutar elementa `else`, postavi svojstvo `currentTime` objekta `audio` sa vrednošću sačuvanom u `userData?.songCurrentTime`.

# --hints--

Ne smete da menjate izjavu od `if` koja postoji i njen sadržaj.
...
Trebalo bi da dodate element `else` u postojeći element `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*\)\s*\{\s*audio\.currentTime\s*=\s*0\s*;?\s*\}\s*else\s*{/)
```

Treba da staviš `audio.currentTime` u `userData?.songCurrentTime` unutar elementa `else` tvog `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\|\|\s*userData\?\.currentSong\.id\s*!==\s*song\.id\s*\)\s*\{\s*audio\.currentTime\s*=\s*0\s*;?\s*\}\s*else\s*{\s*audio\.currentTime\s*=\s*userData\?\.songCurrentTime\s*;?\s*\}/)
```
