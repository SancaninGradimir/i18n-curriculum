...
Unutar elementa od `button`, kreiraj još dva elementa za `span`.

Prvi element `span` treba da ima `class` od `playlist-song-artist`. Između oznaka `span`, dodaj `${song.artist}`.

Drugi element od `span` treba da ima `class` od `playlist-song-duration`. Između oznaka `span`, dodaj `${song.duration}`.

# --hints--

Ne smijete menjati element `button` koji postoji i koji sadrži sadržaj unutar sebe.
...
Treba da napraviš komponentu sa klasom objekta `playlist-song-artist` za `span`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Vaš element `span` sa klasom `playlist-song-artist` mora da unese `song.artist` kao što je sadržano u njemu.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Moraš da kreiraš komponentu za `span` sa klasom od `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Vaš element `span` sa klasom objekta `playlist-song-duration` treba da umetne/sadrži `song.duration` kao ono što je unutra.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*\$\{song\.duration\}<\/span>/)
```
