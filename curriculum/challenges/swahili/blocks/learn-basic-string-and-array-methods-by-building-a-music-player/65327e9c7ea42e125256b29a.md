...
Dodali smo preostale pesme u niz podataka `allSongs` za tebe.

U nastavku, naučićete o <dfn>Web Audio API</dfn> i kako ga koristiti za reprodukciju pesama. Svi moderni pretraživači podržavaju Web Audio API, što vam omogućava kreiranje i obradu zvuka u web aplikacijama.

Koristi `const` da kreiraš atribut nazvan `audio` i postavi mu vrednost `new Audio()`. Ovo će stvoriti novi HTML5 element `audio`.

# --hints--

Trebalo bi da koristiš ključnu reč `new` za kreiranje primera objekta `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Moraš da staviš vrednost `Audio` u konstantu nazvanu `audio`.
