...
Vreme je da počnemo izvršavati funkciju za puštanje prikazanih pesama.

Definišite funkciju `playSong` koristeći `const`. Ta funkcija treba da primi parametar `id` koji će predstavljati jedinstveni identifikator pesme koju želite da pustite.

# --hints--

Trebalo bi da koristiš `const` za kreiranje prazne funkcije pod nazivom `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` trebalo bi da bude funkcija.

```js
assert.isFunction(playSong)
```

Vaša funkcija za `playSong` mora da koristi strelicastu sintaksu.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Funkcija vašeg `playSong` mora da primi parametar `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Tvoja funkcija za `playSong` treba da bude prazna.
