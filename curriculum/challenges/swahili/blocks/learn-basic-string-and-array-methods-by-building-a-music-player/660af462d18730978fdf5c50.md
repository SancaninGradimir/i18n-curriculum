...
Sada je vreme da koristiš ono što si naučio o streličnim funkcijama u tvom projektu muzičkog plejera.

Počnite sa uklanjanjem funkcija za `printGreeting`, `printMessage`, i `addTwoNumbers`. Takođe, uklonite sve konzolne izjave i pozive funkcija.

# --hints--

Ne smete imati funkciju od `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

Ne smete imati funkciju `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

Ne smete imati funkciju `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

Treba da imaš poziv za radni proces `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

Ne sadrži poziv ka funkciji `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

Ne smete imati konzolnu poruku za `console.log(addTwoNumbers(3,4));`.

```js
assert.notMatch(code, /console\.log\(addTwoNumbers\(3,4\)\);?/);
```
