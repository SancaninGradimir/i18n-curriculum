...
Sledeće, kreiraj red podataka za skladištenje svih pesama.

Kreiraj prazan red podataka `allSongs`.

# --hints--

Trebalo bi da imaš parametar koji se zove `allSongs`.

```js
assert.isDefined(allSongs)
```

Trebalo bi da postaviš vrednost kolone podataka na svoj parametar `allSongs`.

```js
assert.isArray(allSongs)
```

Red vaših podataka bi trebalo da bude prazan.
