...
Dohvati pristup svojstvima za `userData?.currentSong?.title` i `userData?.currentSong?.artist`, i postavi njihove vrednosti u parametre `currentTitle` i `currentArtist`, respektivno.

# --hints--

Trebaće vam pristup `title` od `currentSong` iz objekta `userData`. Ne zaboravite da koristite sekvencijalno spajanje.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Morate postaviti vrednost `title` od `currentSong` u konstantu `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Trebalo bi da dobijete dostupnost `artist` od `currentSong` iz objekta `userData`. Ne zaboravite da koristite serijski konektor/povezivanje.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Trebalo bi da postaviš vrednost `artist` u `currentSong` u konstantu `currentArtist`.
