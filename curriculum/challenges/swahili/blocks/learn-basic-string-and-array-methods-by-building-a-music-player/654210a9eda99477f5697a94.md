...
Kreiraj element ``button`` sa klasom ``playlist-song-info``. Unutar ``button``, dodaj element ``span`` sa klasom ``playlist-song-title``, zatim postavi ``song.title`` kao tekst.

# --hints--

Trebalo bi da kreirate `button` sa klasom entiteta `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Treba da kreiraš element `span` unutar svog elementa `button`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Element vašeg `span` treba da ima klasu objekata `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Trebalo bi da postaviš `song.title` kao sadržaj svog elementa `span`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>\s*\$\{\s*song\.title\s*\}\s*<\/span>\s*<\/button>/)
```
