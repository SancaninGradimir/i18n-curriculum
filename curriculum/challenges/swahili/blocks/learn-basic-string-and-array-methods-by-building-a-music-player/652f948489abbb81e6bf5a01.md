...
U ovom projektu naučićete osnovne metode za nizove znakova i strukture podataka kako biste napravili program za reprodukciju muzike. Bićete u stanju da puštate, pauzirate, preskačete i miksujete pesme.

HTML i CSS za ovaj projekat su već pripremljeni za vas, pa možete biti pažljivi sa JavaScript.

Počnite tako što ćete doći do pristupa komponentama `#playlist-songs`, `#play`, i `#pause` koristeći metodu `getElementById()`. Postavite njihove vrednosti u parametre `playlistSongs`, `playButton` i `pauseButton` u istom redosledu.


# --hints--

Trebalo bi da imaš parametar nazvan `playlistSongs`.

```js
assert.isDefined(playlistSongs);
```

Trebalo bi da koristite `document.getElementById()` da biste dobili funkciju/element `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Treba da postaviš element `#playlist-songs` u parametar `playlistSongs`.

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Trebalo bi da imaš parametar nazvan `playButton`.

```js
assert.isDefined(playButton);
```

Trebalo bi da koristite `document.getElementById()` da biste dobili element `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Trebalo bi da staviš element `#play` u parametar `playButton`.

```js
assert.equal(playButton, document.getElementById('play'));
```

Trebalo bi da imaš parametar koji se zove `pauseButton`.

```js
assert.isDefined(pauseButton);
```

Trebalo bi da koristite `document.getElementById()` da biste dobili funkciju/element `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Morate postaviti element `#pause` u parametar `pauseButton`.
