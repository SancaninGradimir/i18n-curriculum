...
<dfn>Alternativna sekvenca</dfn> `|` se može koristiti za podudaranje teksta s leve ili desne strane od `|`. Na primer, regularni izraz `/yes|no/` će podudarati sa `yes` ili `no`.

Ažurirajte vašu `helpRegex` da odgovara tekstu s leve ili desne strane od `please help` ili `assist me`.
