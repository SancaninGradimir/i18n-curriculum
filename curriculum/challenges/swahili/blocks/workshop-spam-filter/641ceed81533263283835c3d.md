...
Umesto korišćenja metode `.match()`, možete koristiti metodu `.test()` regularnog izraza da biste testirali da li niz odgovara obrascu. Za razliku od `.match()`, `.test()` vraća booleanski vrednost koja ukazuje da li niz odgovara obrascu.

Ažurirajte svoju funkciju `isSpam()` da koristi metodu `.test()` na `helpRegex` kako bi testirala da li je `msg` podudarnost.

Zatim, pokušajte da unesete neke poruke na svoju stranicu i vidite rezultat.
