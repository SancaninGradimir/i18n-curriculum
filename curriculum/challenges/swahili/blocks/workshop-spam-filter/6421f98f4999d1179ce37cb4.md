...
Arrays imaju metod `some()`. Kao i metoda `filter()`, `some()` prima funkciju povratnika koja bi trebalo da uzme element niza kao argument. Metoda `some()` će vratiti `true` ako funkcija povratnik vrati `true` za barem jedan element u nizu.

Evo primera poziva metode `.some()` za proveru da li je bilo koji element u nizu veliko slovo.
...
Ažurirajte funkciju `isSpam()` da koristi metodu `some()`, koja će proveriti da li testiranje `msg` protiv bilo kog regularnog izraza u `denyList` vraća `true`.
