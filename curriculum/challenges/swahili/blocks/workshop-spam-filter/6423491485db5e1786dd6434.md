...
Vaš konačni regularni izraz će tražiti nizove poput `dear friend`. Deklariši `dearRegex` i dodeli mu regularni izraz koji će odgovarati nizu `dear friend`. Zapamti da ga učiniš neosetljivim na veličinu slova (case insensitive) i dodaš u svoj niz `denyList`.
