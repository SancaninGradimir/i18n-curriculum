...
Ako pokušate da unesete poruku `free money`, primetićete da ne odgovara vašem izrazu! To je zato što `\s` ne odgovara početku ili kraju teksta.

Da biste poklopili početak teksta, možete koristiti sidro (anchor) `^`. Ovo tvrdi da vaša podudarnost obrasca počinje na samom početku celog niza karaktera.

Zamenite svoj prvi karakter `\s` sa neuhvatljivom grupom koja odgovara `\s` ili `^`.
