...
A <dfn>capture group</dfn> je način definisanja dela izraza koji treba da bude uhvaćen i sačuvan radi kasnijeg referenciranja. Možete definisati grupu za hvatanje obavijanjem dela vašeg izraza zagradama. Na primer, `/h(i|ey) camper/` bi odgovarao ili `hi camper` ili `hey camper`, i hvatio bi `i` ili `ey` u grupi.

Između vaših cifara i teksta `dollars`, želite da uhvatite vrednosti pozicije.

Dodajte grupu za hvatanje vašem `dollarRegex` i koristite token `|` unutar nje da dozvolite `hundred`, `thousand`, `million` ili `billion` između vaših cifara i teksta `dollars`.
