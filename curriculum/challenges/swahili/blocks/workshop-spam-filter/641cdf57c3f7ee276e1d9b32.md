...
Nazad u vašem slušaocu događaja (event listener), morate ažurirati tekst elementa `result`. Možete koristiti ternarni operator da biste ovo zadatje ostvarili.

Evo primera dodeljivanja rezultata ternarnog operatera u tekstualni sadržaj elementa:

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

Nakon izjave `if`, ažurirajte tekst elementa `result` na osnovu istinitosti pozivanja `isSpam()` sa `messageInput.value` kao argumentom. Ako je tačno, postavite svojstvo `textContent` na element `result` na `"Oh no! This looks like a spam message."`. U suprotnom, postavite ga na `"This message does not seem to contain any spam."`.

Zatim postavite svojstvo `value` elementa `messageInput` na prazan niz znakova.
