...
Nastavljajući sa tom strukturom, kreiraj `fieldset` za `dinner` sa slično organizovanim internim elementima.

# --hints--

Unapaswa kuunda kipengele cha tatu cha `fieldset` ndani ya `form` yako.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]);
```

Tvoj treći element od `fieldset` treba da dođe nakon tvog drugog elementa od `fieldset`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.previousElementSibling?.tagName, "FIELDSET");
```

Vaš treći element od `fieldset` mora imati `id` postavljen na `dinner`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.id, "dinner");
```

Vaš treći element za `fieldset` treba da ima element `legend` unutar sebe.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('legend'));
```

Vaš novi element za `legend` treba da ima tekst od `Dinner`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.querySelector('legend')?.innerText, "Dinner");
```

Treći vaš element ``fieldset`` treba da sadrži element ``div`` unutar sebe.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('div'));
```

Tvoj novi element za `div` mora imati `class` postavljen na `input-container`.
