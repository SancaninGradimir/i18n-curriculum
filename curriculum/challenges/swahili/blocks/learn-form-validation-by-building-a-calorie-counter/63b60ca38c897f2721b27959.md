...
Vaš formular zahteva mesto za prikaz rezultata. Dodajte prazno polje `div` i dodelite `id` od `output` vrednostima `class` za `output` i `hide`.

# --hints--

Treba da dodaš element `div` nakon tvog `form`.

```js
assert.exists(document.querySelector('.container > div'));
```

Vaša nova funkcija za `div` mora imati `id` postavljeno na `output`.

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

Vaše novo polje za `div` treba da ima vrednosti od `class` za `output` na `hide`.
...
`div` Tvoj novi treba da bude prazan.
