...
Prateći isti format, izvršite pretragu za vaš unos `number` u poljima `#dinner`, `#snacks` i `#exercise`. Postavite njihove vrednosti u parametre prateći redosled imena poslednje dve faze.

# --hints--

Trebalo bi da objavite parametar `dinnerNumberInputs`.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

Treba ti da pozoveš `document.querySelectorAll()` za `#dinner input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Trebalo bi da postavite vrednost rezultata vašeg poziva ``document.querySelectorAll()`` u ``dinnerNumberInputs``.
...
Trebalo bi da objaviš parametar `snacksNumberInputs`.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

Moraš pozvati `document.querySelectorAll()` za `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Trebalo bi da postaviš vrednost rezultata tvog poziva `document.querySelectorAll()` u `snacksNumberInputs`.
...
Trebalo bi da objavite parametar `exerciseNumberInputs`.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

Trebalo bi da pozoveš `document.querySelectorAll()` za `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Treba da postavite vrednost rezultata vašeg poziva `document.querySelectorAll()` u `exerciseNumberInputs`.
