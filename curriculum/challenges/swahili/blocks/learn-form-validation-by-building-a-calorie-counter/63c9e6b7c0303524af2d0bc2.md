...
Dostavite tekst za `remainingCalories Calorie surplusOrDeficit` za vaš element `span`, koristeći unos vrednosti da promenite `remainingCalories` u `surplusOrDeficit` za odgovarajuće parametre.

# --hints--

`span` Vaše treba da sadrži tekst od `${remainingCalories} Calorie ${surplusOrDeficit}`.
