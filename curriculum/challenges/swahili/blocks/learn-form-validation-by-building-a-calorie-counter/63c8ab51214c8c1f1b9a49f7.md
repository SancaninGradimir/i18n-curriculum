...
U svojoj novoj funkciji, deklariraj parametar za `calories` i tip vrednosti `0`. Koristi `let` za deklarisanje, jer ćeš kasnije menjati njegovu vrednost.

# --hints--

Trebalo bi da objavite parametar `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Vaš parametar `calories` mora dobiti vrednost `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Trebalo bi da objavite svoj parametar `calories` za `let`.
