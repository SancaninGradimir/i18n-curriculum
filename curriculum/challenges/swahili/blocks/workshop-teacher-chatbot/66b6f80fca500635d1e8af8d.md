...
Sada je vreme da pristupimo poslednjem karakteru stringa.

Počnite dodavanjem još jedne `console` naredbe koja ispisuje poruku `Here is an example of accessing the last letter in the word [subject].` 

Zapamtite da zamenite `[subject]` stvarnom vrednošću varijable `subject` i koristite ispravnu sintaksu za template literale.
