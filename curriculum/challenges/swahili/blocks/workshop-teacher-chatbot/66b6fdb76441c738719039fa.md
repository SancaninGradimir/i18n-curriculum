...
U prethodnim lekcijama, naučili ste kako da pristupite poslednjem karakteru u nizu na ovaj način:
...
`string.length - 1` će vam uvek dati poslednji indeks broj za niz.

Kreirajte novu varijablu nazvanu `lastCharacter` i dodelite joj vrednost poslednjeg karaktera iz varijable `subject`.

Zatim, logujte vrednost varijable `lastCharacter` na konzolu.
