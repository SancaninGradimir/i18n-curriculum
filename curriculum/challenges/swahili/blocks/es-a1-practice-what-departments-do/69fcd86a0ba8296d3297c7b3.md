...
U ovoj vežbi, ćеćete da trenirate prepoznavanje odeljenja na osnovu aktivnosti koje obavljaju.

# --instructions--

Slušajte i dovršite rečenicu za odgovarajući odeljak.
...
Ovaj odeljak obučava tim.

# --explanation--

`El departamento de aprendizaje y desarrollo capacita al equipo` to znači "Odeljenje za razvoj i obuku uči tim".

Sve `apoyar` ("pomoći") i `capacitar` ("učenjem") su glagoli u trećem licu množine koji se završavaju sa `-ar`.

Prati isti model kao `desarrolla`, `verifica`, i `crea` iz prethodnih vežbi.
