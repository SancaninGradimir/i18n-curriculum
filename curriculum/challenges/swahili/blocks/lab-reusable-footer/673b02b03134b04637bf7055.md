...
title: Izgradite ponovno upotrebljivo podnožđe
challengeType: 25
...
1. Trebalo bi da eksportujete komponentu `Footer`.
2. Vaša `Footer` komponenta treba da vrati element `footer` koji sadrži sve ostale elemente.
3. Ваш елемент `footer` treba da ima:

   - Najmanje tri neuređene liste, svaka sa najmanje dva stavke liste.
   - Najmanje jedan paragrafski element sa simbolom autorskih prava `©`.
   - Najmanje tri linka sa vrednošću `href` postavljenu na `#` i sadržajem linka podešenim na ikonu ili tekst po izboru.
