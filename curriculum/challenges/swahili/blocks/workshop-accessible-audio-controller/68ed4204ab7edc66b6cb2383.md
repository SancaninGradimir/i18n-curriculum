...
Kao što ste se setili iz prethodnih lekcija, atribut `aria-labelledby` koristi se kada postoji postojeći tekst na stranici koji se može koristiti kao oznaka. U ovom slučaju, dodali ste atribut `id` vašim elementima `span`, koji će sada biti korišćeni ovde.

Dodajte atribut `aria-labelledby` elementu `input`, podešen na `volume-label volume-description`.
