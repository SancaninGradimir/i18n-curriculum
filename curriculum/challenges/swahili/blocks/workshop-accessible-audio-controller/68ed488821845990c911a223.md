...
Vaš pristupačan audio kontroler je skoro završen, ali postoji još jedna stvar koju treba dodati.

Dodajte element `button` sa atributom `type` postavljenim na `button` i sa tekstom `Mute` ispod elementa `div`.

I tom promenom, završili ste ovu radionicu!
