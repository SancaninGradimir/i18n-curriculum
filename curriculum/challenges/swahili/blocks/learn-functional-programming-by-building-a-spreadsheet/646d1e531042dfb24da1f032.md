...
Drugi argument putem `.replace()` ne bi trebalo da bude string (niz karaktera). Umesto toga, možete proslediti callback funkciju kako biste izvršili složeniju logiku na odgovarajućem stringu.

Funkcija poziva uzima nekoliko parametara. Prvo je odgovarajuća sekvenca karaktera. Prođite praznu funkciju za vaš poziv od `.replace()`, i obavezni parametar za `match`.

# --hints--

Trebalo bi da prosledite funkciju strelice kao drugi argument u vaš način za `.replace()`.
...
Vaša strelična funkcija mora da primi parametar `match`.
