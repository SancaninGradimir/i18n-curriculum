...
Možete brzo dobiti sve ćelije sa vaše stranice uz dobijanje elementa `#container` za njegov `id` i dobijanjem svojstva `children` rezultata. Prosledite to vašem pozivu `evalFormula()` kao drugi parametar.

# --hints--

Za drugi parametar vašeg poziva `evalFormula()`, treba da pozoveš metodu `.getElementById()` objekta `document`.
...
Trebalo bi da prosledite `container` kao argument vašem pozivu `.getElementById()`.
...
Unapaswa kupata sifa ya `children` ya matokeo ya wito wako wa `.getElementById()`.
