...
Sada morate proveriti da li `value` ne uključuje `id` elementa. Kreirajte uslov za `if` da to uradite.

# --hints--

Trebalo bi da kreirate deo `if`.
...
Uslov za `if` mora proveriti da `value` ne uključuje `id` od `element`.
...
Vaš deo `if` treba da bude prazan.
