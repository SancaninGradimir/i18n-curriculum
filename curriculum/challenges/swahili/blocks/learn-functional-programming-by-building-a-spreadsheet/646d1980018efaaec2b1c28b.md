...
Koristi istu sintaksu kao svoj funkcionar ``addCharacters`` da popraviš svoj funkcionar ``elemValue``. Ne bi trebalo više da objavljuje ``inner``, već treba da vraća funkcionar na indirektan način.

# --hints--

Vaša funkcija za `elemValue` treba da koristi indirektan povratak.
...
Vaša funkcija sa `elemValue` treba da indirektno vrati streličnu funkciju sa parametrom `character`.
...
Tvoj interni pokazivač/funkcija bi trebalo da koristi indirektan povratak.
...
Tvoja unutrašnja funkcija sa strelicom treba da vrati rezultat poziva na `idToText()` za `character + num` kao argument.
