...
Tvoja funkcija ``addCharacters`` konačno vraća red slova. Želiš da vrati red brojeva ćelija. Poveži put ``.map()`` sa svojim pozivom ``charRange()``. Nemoj još da izvršiš funkciju povratnog poziva.

# --hints--

Trebalo bi da povežete `.map()` sa jednostavnim pozivom `charRange()`.
...
Ne smeš da pređeš ništa sa svog poziva `.map()`.
