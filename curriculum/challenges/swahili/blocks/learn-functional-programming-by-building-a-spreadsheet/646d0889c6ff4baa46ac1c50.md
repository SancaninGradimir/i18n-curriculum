...
U vašoj funkciji od `elemValue`, deklarisati funkciju nazvanu `inner` koja prima parametar `character`.

Zatim, ponovi svoju funkcionalnost za `inner`.

# --hints--

Trebalo bi da objaviš parametar `inner`.
...
Trebalo bi da koristiš `const` za deklarisanje svog parametra `inner`.
...
Tvoj deo za `inner` treba da bude strelična funkcija.
...
Tvoja funkcija sa `inner` treba da ima `character` kao jedini parametar.
...
Vaša funkcija za `inner` treba da bude prazna.
...
Trebalo bi da ponovo pokreneš svoju funkciju `inner`.
