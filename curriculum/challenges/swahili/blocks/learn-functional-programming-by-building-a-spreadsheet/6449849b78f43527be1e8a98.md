...
Potrebno je da znate kako da uporedite opsege ćelija u matematičkom zapisu. Opsezi ćelija mogu izgledati kao `A1:B12` ili `A3:A25`. Možete koristiti regulare izraze za poređenje ovih obrazaca.

Počnite deklaracijom razlike od `rangeRegex` i postavite njenu vrednost na regularni izraz koji odgovara `A` do `J` (tip redova u vašoj bazi podataka). Koristite grupu za hvatanje sa klasom karaktera da biste ovo postigli.

# --hints--

Trebalo bi da objaviš razliku `rangeRegex` nakon svoje funkcije `idToText`.
...
Treba ti da koristiš `const` za objavljivanje svoje razlike od `rangeRegex`.
...
Твоја варијација од `rangeRegex` треба да буде уобичајечки израз.
...
`rangeRegex` Trebalo bi da koristi grupisanje za hvatanje.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

`rangeRegex` Trebalo bi da koristi klasu karaktera unutar grupe za hvatanje.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

`rangeRegex` treba da koristi karakter klasu za podudarnost `A` do `J`.
