...
Možete proslediti referencu funkcije kao argument poziva za vraćanje. Referenca funkcije je ime funkcije bez standardnih zagrada. Na primer:
...
Metoda `.map()` ovde će pozvati funkciju `myFunc`, prosljeđujući iste argumente koje prima povratni poziv od `.map()`. Prvi argument je vrednost niza podataka u trenutnoj petlji, stoga će `newArray` biti `[value: 1, value: 2, value: 3]`.

Proverite referencu vaše funkcije `elemValue` kao povratni poziv za vaš metod `.map()`.

# --hints--

Trebalo bi da pozoveš/koristiš svoju funkciju za `elemValue`.
...
Morate proslediti referencu `elemValue` kao povratni poziv pomoću vaše metode `.map()`.
