...
Sada vaša funkcija `update()` može da evaluira matematičke izraze. Zapamtite da ste napisali uslov za `if` kako biste proverili da li je funkcija pozvana.

Unutar deklaracije `if`, postavi vrednost svojstva `value` od `element` na rezultat vaše funkcije `evalFormula()`. Ne prosleđuj nijedan argument.

# --hints--

Trebalo bi da ispraviš atribut `value` na `element` unutar svog dela `if`.
...
Trebalo bi postaviti vrednost svojstva `value` da bude rezultat tvog poziva funkcije `evalFormula()`.
...
Ne smete da prosledite bilo koji argument vašem pozivu `evalFormula()`.
