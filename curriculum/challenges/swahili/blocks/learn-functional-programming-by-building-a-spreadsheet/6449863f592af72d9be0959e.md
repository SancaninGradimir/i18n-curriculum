...
Nakon dobijanja `rangeRegex` i `:`, potrebno je pretražiti isti uzorak slova i brojeva kao ranije.

Kopiraj vaše dve postojeće grupe i zalepi ih posle vertikalne tačke.

# --hints--

Trebalo bi da dodate treću uhvatnu grupu u `rangeRegex`, nakon vertikalne tačke.
...
Treća uhvatljiva grupa treba da koristi klasu slova.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Treća grupa za hvatanje treba da prati karaktere od `A` do `J`.
...
Trebalo bi da dodaš četvrti sklop za hvatanje na `rangeRegex`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Četvrta grupa za hvatanje treba da bude bazirana na jednoj ili dve brojne.
