...
Prvi argument za vaš poziv od `evalFormula` bi trebalo da bude sadržaj ćelije (koji ste sačuvali u `value`). Međutim, sadržaj počinje sa karakterom od `=` kako bi se inicijalizovala funkcija, stoga je potrebno da prosledite manji deo niza karaktera od `value` počevši od indikatora `1`.

# --hints--

Trebalo bi da proslediš `value` kao prvi argument svom pozivu `evalFormula()`.
...
Treba ti da pozoveš putanju `.slice()` u argumentu `value`.
...
Treba da proslediš broj `1` kao argument za svoj poziv `.slice()`.
