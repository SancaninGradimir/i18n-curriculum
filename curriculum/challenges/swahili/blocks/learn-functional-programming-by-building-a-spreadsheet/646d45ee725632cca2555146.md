...
Sada tvoj `applyFunction` mora da vrati rezultate. Vratite rezultate poziva metode `.replace()` na `str2`. Takođe, vaš standardni izraz `functionCall` i prazan povratak.

# --hints--

Vaša funkcija za `applyFunction` treba da vrati rezultat poziva metode `.replace()` u `str2`.
...
Trebalo bi da prosledite `functionCall` kao prvi argument svom pozivu `.replace()`.
...
Trebalo bi da prosledite funkciju strelica prazna kao drugi argument vašem pozivu za `.replace()`.
