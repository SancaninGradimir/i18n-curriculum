...
Zato što `elemValue` vraća funkciju, vaša funkcija `addCharacters` na kraju vraća niz referenci na funkcije. Želite da način `.map()` izvrši unutrašnju funkciju vaše funkcije `elemValue`, što znači da morate pozvati `elemValue` umesto da ga referencirate. Prosledite `num` kao argument vašoj funkciji `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Trebaće ti da pozoveš `elemValue()` u tvom putu do `.map()`.
...
Moraš da proslediš `num` za tvoj poziv `elemValue()`.
