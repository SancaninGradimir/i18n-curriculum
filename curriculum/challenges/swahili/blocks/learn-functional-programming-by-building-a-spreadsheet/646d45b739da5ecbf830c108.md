...
Tvoja funkcija `apply` vraća funkciju radnog lista, ali se ne koristi direktno. Ažuriraj `apply` da pozove tu funkciju. Pregledaj rezultat pozivanja `toNumberList` sa `args` kao argumentom.

# --hints--

Vaša funkcija `apply` treba da pozove funkciju `spreadsheetFunctions[fn.toLowerCase()]`.
...
Moraš da proslediš poziv `toNumberList()` za svoj poziv `spreadsheetFunctions[fn.toLowerCase()]`.
...
Trebalo bi da prosledite `args` za poziv `toNumberList()`.
