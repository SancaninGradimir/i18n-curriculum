...
Sada tvoja funkcija `.map()` prima referencu na vraćenu funkciju `num => charRange(...).map(...)` iz zakazanih poziva `addCharacters`, iteriraće kroz komponente i prosleđivati svaku komponentu kao `n` toj funkciji.

Videćete da ne koristite svoj parametar za `match`. U JavaScript, uobičajena je praksa staviti prefiks podznak (underline) `_` za neupotrebljen parametar. Takođe možete ostaviti taj parametar prazan kao ovde: `(, char1)`, ali je često jasnije citirati parametar radi lakšeg čitanja u budućnosti.

Postavi prefiks linije ispod svog parametra `match`.

# --hints--

Trebalo bi da postavite prefiks linije ispod vašeg kriterijuma `match`.
