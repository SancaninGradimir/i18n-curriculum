...
Pozovi funkciju `toNumberList` koja prima parametar `args` i vraća rezultat razdvajanja `args` po zarezi bez direktnog prikazivanja. Zatim spoji putanju `map` sa tvojom putanjom `split` i prenesi `parseFloat` kao argument putanji `map`.

# --hints--

Treba da se deklariše parametar za `toNumberList`.
...
Trebalo bi da koristiš `const` za deklarisanje svog parametra `toNumberList`.
...
Tvoj uslov za `toNumberList` treba da bude strelična funkcija.
...
Tvoja funkcija za `toNumberList` mora imati parametar za `args`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

Tvoja funkcija za `toNumberList` treba da koristi ne-prikaznu restauraciju.
...
Vaša funkcija za `toNumberList` trebalo bi da vrati rezultate poziva metode `.split()` od `args`.
...
Trebalo bi da podeliš `args` po slovima od `","`.
...
Treba da povežete putanju `.map()` sa putanjom `.split()`.
...
Trebalo bi da prosledite referencu `parseFloat` kao povratni poziv za `.map()`.
