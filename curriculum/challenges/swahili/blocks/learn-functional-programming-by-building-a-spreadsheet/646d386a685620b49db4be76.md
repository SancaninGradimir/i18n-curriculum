...
Pozovi putanju `.map()` u vašem pozivu `rangeFromString()`, koristeći referencu `addCharacters` kao funkciju odgovora.

# --hints--

Trebalo bi da izvršite poziv metode `.map()` u vašem pozivu za `rangeFromString()`.
...
Trebalo bi da prosledite referencu `addCharacters` kao odgovor na vaš način `.map()`.
