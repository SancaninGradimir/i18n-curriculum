...
`addCharacters(char1)` takođe vraća funkciju, koja vraća drugu funkciju. Morate izvršiti još jedan poziv funkcije da biste dobili referencu na najdublju funkciju za povratni poziv `.map()`. JavaScript vam omogućava da odmah pozovete funkcije koje se vraćaju:

```js
myFunc(1)("hi");
```

Pozovite odmah funkciju koja se vraća iz vašeg poziva od `addCharacters(char1)`, i prosledite `char2` kao argument.

# --hints--

Trebalo bi da povežete funkcionalski poziv sa vašim pozivom `addCharacters(char1)`.
...
Trebalo bi da prosledite `char2` kao argument vašem povezanom funkcionaiskom pozivu.
