...
Pošto imate uobičajenu formulu za množenje ili deljenje, možete evaluirati taj matematički izraz.

Deklariši parametar `str2` i postavi mu vrednost kao rezultat poziva `infixEval`, koji ima/sadrži `str` i `regex` kao argumente.

# --hints--

Treba da objaviš parametar `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*(?:const|let|var)\s+str2/);
```

Trebalo bi da koristiš `const` za deklarisanje svog parametra `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2/);
```

Trebalo bi da postavite vrednost `str2` na rezultat vašeg poziva funkcije `infixEval`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(/);
```

Trebalo bi da prosledite `str` kao prvi argument svom pozivu `infixEval`.
...
Trebalo bi da prosledite `regex` kao drugi argument vašem pozivu `infixEval`.
