...
`addCharacters` vraća funkciju, pa ćete želeti da je koristite kao poziv. Proslijedite `char1` kao argument.

# --hints--

Trebalo bi da pozoveš svoju funkciju `addCharacters()` u svom putu `.map()`.
...
Trebalo bi da prosledite `char1` kao argument za vaš poziv `addCharacters()`.
