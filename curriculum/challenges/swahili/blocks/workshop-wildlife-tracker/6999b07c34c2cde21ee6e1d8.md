...
U ovom koraku, kreiraćete funkciju koja proverava da li objekat ima određeno svojstvo.
...
Sada kreirajte funkciju nazvanu `hasHabitat`. Funkcija treba da primi jedan parametar pod imenom `animal`.

Koristite metod `hasOwnProperty` da vratite `true` ako objekat `animal` ima svojstvo nazvano `"habitat"`, i `false` u suprotnom slučaju.
