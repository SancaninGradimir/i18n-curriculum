...
U ovom koraku, kreiraćete funkciju koja ažurira svojstvo objekta.

Evo primera ažuriranja svojstva unutar funkcije:
...
U ovom primeru, svojstvo `age` se ažurira na novu vrednost.

Sada kreirajte funkciju nazvanu `updateAge`. Funkcija treba da primi dva parametra: `animal` i `newAge`.

Unutar funkcije, ažurirajte svojstvo `age` objekta `animal` na `newAge`. Vratite ažurirani objekt `animal`.

Nakon kreiranja funkcije, koristite `console.log` da pozovete `updateAge(elephant, 12)`, kako biste videli ažurirani objekt `elephant` u konzoli.
