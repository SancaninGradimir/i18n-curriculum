---
id: 587d78af367417b2b2512b03
title: Napravi anketni formular
challengeType: 14
saveSubmissionToDB: true
forumTopicId: 301145
dashedName: build-a-survey-form
---

# --description--

**Cilj:** Napravi aplikaciju koja radi kao <a href="https://survey-form.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://survey-form.freecodecamp.rocks</a>. **Nemoj kopirati ovaj demo projekat**.

**Korisnicke price:**

1. Treba da imas naslov stranice u `h1` elementu sa `id` vrednoscu `title`
1. Treba da imas kratak opis u `p` elementu sa `id` vrednoscu `description`
1. Treba da imas `form` element sa `id` vrednoscu `survey-form`
1. Unutar forme treba da uneses ime u `input` polje sa `id` vrednoscu `name` i `type` vrednoscu `text`
1. Unutar forme treba da uneses email u `input` polje sa `id` vrednoscu `email`
1. Ako uneses email koji nije pravilno formatiran, treba da vidis HTML5 gresku validacije
1. Unutar forme treba da postoji numericko `input` polje sa `id` vrednoscu `number`
1. Polje za broj ne sme da prihvati nenumericki unos, bilo tako sto ga blokira ili prikaze HTML5 gresku validacije (zavisno od browsera)
1. Ako uneses broj van opsega definisanog atributima `min` i `max`, treba da vidis HTML5 gresku validacije
1. Za ime, email i broj treba da postoje odgovarajuci `label` elementi u formi sa sledecim identifikatorima: `id="name-label"`, `id="email-label"` i `id="number-label"`
1. Za ime, email i broj treba da vidis placeholder tekst koji daje opis ili smernice za svako polje
1. Unutar forme treba da postoji `select` element sa `id` vrednoscu `dropdown`, sa najmanje dve opcije
1. Unutar forme treba da mozes da izaberes opciju iz grupe od bar dva radio dugmeta grupisana istim `name` atributom
1. Unutar forme treba da mozes da izaberes vise opcija preko checkbox polja, gde svaki ima `value` atribut
1. Unutar forme treba da postoji `textarea` za dodatne komentare
1. Unutar forme treba da postoji dugme sa `id` vrednoscu `submit` za slanje svih unosa

Ispuni korisnicke price i prodji sve testove ispod da zavrsis projekat. Dodaj svoj licni stil. Uzivaj u kodiranju!

**Napomena:** Obavezno dodaj `<link rel="stylesheet" href="styles.css">` u svoj HTML da povezes stylesheet i primenis CSS.

# --hints--

Treba da imas `h1` element sa `id` vrednoscu `title`.

```js
const el = document.getElementById('title');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'H1');
```

Tvoj `#title` ne treba da bude prazan.

```js
const el = document.getElementById('title');
assert.isNotNull(el);
assert.isAbove(el.innerText.length, 0);
```

Treba da imas `p` element sa `id` vrednoscu `description`.

```js
const el = document.getElementById('description');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'P');
```

`#description` yako haipaswi kuwa tupu.

```js
const el = document.getElementById('description');
assert.isNotNull(el);
assert.isAbove(el.innerText.length, 0);
```

Unapaswa kuwa na kipengele cha `form` chenye `id` ya `survey-form`.

```js
const el = document.getElementById('survey-form');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'FORM');
```

Unapaswa kuwa na kipengele cha `input` chenye `id` ya `name`.

```js
const el = document.getElementById('name');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'INPUT');
```

`#name` yako inapaswa kuwa na `type` ya `text`.

```js
const el = document.getElementById('name');
assert.isNotNull(el);
assert.strictEqual(el.type, 'text');
```

`#name` yako inapaswa kuhitaji ingizo.

```js
const el = document.getElementById('name');
assert.isNotNull(el);
assert.isTrue(el.required);
```

`#name` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #name');
assert.isNotNull(el);
```

Unapaswa kuwa na kipengele cha `input` chenye `id` ya `email`.

```js
const el = document.getElementById('email');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'INPUT');
```

`#email` yako inapaswa kuwa na `type` ya `email`.

```js
const el = document.getElementById('email');
assert.isNotNull(el);
assert.strictEqual(el.type, 'email');
```

`#email` yako inapaswa kuhitaji ingizo.

```js
const el = document.getElementById('email');
assert.isNotNull(el);
assert.isTrue(el.required);
```

`#email` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #email');
assert.isNotNull(el);
```

Unapaswa kuwa na kipengele cha `input` chenye `id` ya `number`.

```js
const el = document.getElementById('number');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'INPUT');
```

`#number` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #number');
assert.isNotNull(el);
```

`#number` yako inapaswa kuwa na `type` ya `number`.

```js
const el = document.getElementById('number');
assert.isNotNull(el);
assert.strictEqual(el.type, 'number');
```

`#number` yako inapaswa kuwa na sifa ya `min` yenye thamani ya nambari.

```js
const el = document.getElementById('number');
assert.isNotNull(el);
assert.isNotEmpty(el.min);
assert.isTrue(isFinite(el.min));
```

`#number` yako inapaswa kuwa na sifa ya `max` yenye thamani ya nambari.

```js
const el = document.getElementById('number');
assert.isNotNull(el);
assert.isNotEmpty(el.max);
assert.isTrue(isFinite(el.max));
```

Unapaswa kuwa na kipengele cha `label` chenye `id` ya `name-label`.

```js
const el = document.getElementById('name-label');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'LABEL');
```

Unapaswa kuwa na kipengele cha `label` chenye `id` ya `email-label`.

```js
const el = document.getElementById('email-label');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'LABEL');
```

Unapaswa kuwa na kipengele cha `label` chenye `id` ya `number-label`.

```js
const el = document.getElementById('number-label');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'LABEL');
```

`#name-label` yako inapaswa kuwa na maandishi yanayoelezea ingizo.

```js
const el = document.getElementById('name-label');
assert.isNotNull(el);
assert.isAbove(el.innerText.length, 0);
```

`#email-label` yako inapaswa kuwa na maandishi yanayoelezea ingizo.

```js
const el = document.getElementById('email-label');
assert.isNotNull(el);
assert.isAbove(el.innerText.length, 0);
```

`#number-label` yako inapaswa kuwa na maandishi yanayoelezea ingizo.

```js
const el = document.getElementById('number-label');
assert.isNotNull(el);
assert.isAbove(el.innerText.length, 0);
```

`#name-label` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #name-label');
assert.isNotNull(el);
```

`#email-label` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #email-label');
assert.isNotNull(el);
```

`#number-label` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #number-label');
assert.isNotNull(el);
```

`#name` yako inapaswa kuwa na sifa ya `placeholder` yenye thamani.

```js
const el = document.getElementById('name');
assert.isNotNull(el);
assert.isNotNull(el.placeholder);
assert.isAbove(el.placeholder.length, 0);
```

`#email` yako inapaswa kuwa na sifa ya `placeholder` yenye thamani.

```js
const el = document.getElementById('email');
assert.isNotNull(el);
assert.isNotNull(el.placeholder);
assert.isAbove(el.placeholder.length, 0);
```

`#number` yako inapaswa kuwa na sifa ya `placeholder` yenye thamani.

```js
const el = document.getElementById('number');
assert.isNotNull(el);
assert.isNotNull(el.placeholder);
assert.isAbove(el.placeholder.length, 0);
```

Unapaswa kuwa na sehemu ya ingizo ya `select` yenye `id` ya `dropdown`.

```js
const el = document.getElementById('dropdown');
assert.isNotNull(el);
assert.strictEqual(el.tagName, 'SELECT');
```

`#dropdown` yako inapaswa kuwa na angalau vipengele viwili vya `option` vinavyoweza kuchaguliwa (visivyozimwa).

```js
const els = document.querySelectorAll('#dropdown option:not([disabled])');
assert.isAtLeast(els.length, 2);
```

`#dropdown` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #dropdown');
assert.isNotNull(el);
```

Unapaswa kuwa na angalau vitufe viwili vya radio vya `input` vyenye `type` ya `radio`.

```js
const els = document.querySelectorAll('input[type="radio"]');
assert.isAtLeast(els.length, 2);
```

Unapaswa kuwa na angalau vitufe viwili vya radio ambavyo ni mzao wa `#survey-form`.

```js
const els = document.querySelectorAll('#survey-form input[type="radio"]');
assert.isAtLeast(els.length, 2);
```

Vitufe vyote vya radio vinapaswa kuwa na sifa ya `value` yenye thamani.

```js
const els1 = document.querySelectorAll('input[type="radio"]');
const els2 = document.querySelectorAll(
  'input[type="radio"][value=""], input[type="radio"]:not([value])'
);
assert.isAbove(els1.length, 0);
assert.lengthOf(els2, 0);
```

Vitufe vyote vya radio vinapaswa kuwa na sifa ya `name` yenye thamani.

```js
const els1 = document.querySelectorAll('input[type="radio"]');
const els2 = document.querySelectorAll(
  'input[type="radio"][name=""], input[type="radio"]:not([name])'
);
assert.isAbove(els1.length, 0);
assert.lengthOf(els2, 0);
```

Kila kundi la vitufe vya radio linapaswa kuwa na angalau vitufe viwili vya radio.

```js
const radioButtons = document.querySelectorAll('input[type="radio"]');
const groups = {};

if (radioButtons) {
  radioButtons.forEach(el => {
    if (!groups[el.name]) groups[el.name] = [];
    groups[el.name].push(el);
  });
}

const groupKeys = Object.keys(groups);

groupKeys.forEach(key => {
  if (groups[key].length < 2) assert(false);
});

assert.isAbove(groupKeys.length, 0);
```

Unapaswa kuwa na angalau visanduku viwili vya kuchagua vya `input` vyenye `type` ya `checkbox` ambavyo ni mzao wa `#survey-form`.

```js
const els = document.querySelectorAll('#survey-form input[type="checkbox"]');
assert.isAtLeast(els.length, 2);
```

Visanduku vyote vya kuchagua ndani ya `#survey-form` vinapaswa kuwa na sifa ya `value` yenye thamani.

```js
const els1 = document.querySelectorAll('#survey-form input[type="checkbox"]');
const els2 = document.querySelectorAll(
  '#survey-form input[type="checkbox"][value=""], #survey-form input[type="checkbox"]:not([value])'
);
assert.isAbove(els1.length, 0);
assert.lengthOf(els2, 0);
```

Unapaswa kuwa na angalau kipengele kimoja cha `textarea` ambacho ni mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form textarea');
assert.isNotNull(el);
```

Unapaswa kuwa na kipengele cha `input` au `button` chenye `id` ya `submit`.

```js
const el = document.getElementById('submit');
assert.isNotNull(el);
assert.isTrue(el.tagName === 'INPUT' || el.tagName === 'BUTTON');
```

`#submit` yako inapaswa kuwa na `type` ya `submit`.

```js
const el = document.getElementById('submit');
assert.isNotNull(el);
assert.strictEqual(el.type, 'submit');
```

`#submit` yako inapaswa kuwa mzao wa `#survey-form`.

```js
const el = document.querySelector('#survey-form #submit');
assert.isNotNull(el);
```

# --seed--

## --seed-contents--

```html

```

```css

```

# --solutions--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <link rel="stylesheet" href="styles.css" />
    <title>Survey Form</title>
  </head>
  <body>
    <h1>Survey Form</h1>
    <p>The card below was built as a sample survey form for freeCodeCamp.</p>
    <main id="main">
      <h1 id="title">Join the Togepi Fan Club!</h1>
      <p id="description">
        Enter your information here to receive updates about club activities,
        our monthly newsletter, and other email communications.
      </p>
      <form id="survey-form" action="#">
        <label for="name" id="name-label"
          ><p>Name:</p>
          <input type="text" id="name" placeholder="e.g. John Smith" required />
        </label>
        <label for="email" id="email-label"
          ><p>Email:</p>
          <input
            type="email"
            id="email"
            placeholder="e.g. john.smith@email.com"
            required
          />
        </label>
        <label for="age" id="number-label"
          ><p>Age<em>(optional)</em>:</p>
          <input
            type="number"
            id="number"
            placeholder="e.g. 19"
            min="18"
            max="99"
          />
        </label>
        <label for="interest" id="interest-label"
          ><p>What are you most interested in?</p>
          <select id="dropdown">
            <option selected disabled hidden></option>
            <option id="battles">Battling</option>
            <option id="breeding">Breeding</option>
            <option id="catching">Completing my Pokedex</option>
            <option id="exploring">Exploring new regions</option>
          </select>
        </label>
        <p>Who is your favourite Pokemon?</p>
        <label for="togepi">
          <input
            id="togepi"
            type="radio"
            name="favorite"
            value="togepi"
          />Togepi!
        </label>
        <label for="pikachu">
          <input
            id="pikachu"
            type="radio"
            name="favorite"
            value="pikachu"
          />Pikachu
        </label>
        <label for="other">
          <input id="other" type="radio" name="favorite" value="other" />Other
        </label>
        <p>Which communications do you want to receive?</p>
        <label for="newsletter">
          <input
            id="newsletter"
            type="checkbox"
            name="communications"
            value="newsletter"
          />Newsletter
        </label>
        <label for="events">
          <input
            id="events"
            type="checkbox"
            name="communications"
            value="events"
          />Event updates
        </label>
        <label for="updates">
          <input
            id="updates"
            type="checkbox"
            name="communications"
            value="updates"
          />Club updates
        </label>
        <p>Any other information you would like to share?</p>
        <textarea id="additional-information" rows="4" cols="50">
You can provide comments, suggestions, or feedback here.</textarea
        >
        <p>
          <em
            >Please note: This form is a proof of concept. Submitting the form
            will not actually transmit your data.</em
          >
        </p>
        <button type="Submit" id="submit">Submit</button>
      </form>
    </main>
  </body>
  <footer>
    <a href="../">Return to Project List</a> |
    <a href="https://www.nhcarrigan.com">Return to HomePage</a>
  </footer>
</html>
```

```css
main {
  text-align: center;
  background-color: #92869c;
  background-blend-mode: lighten;
  max-width: 500px;
  margin: 20px auto;
  border-radius: 50px;
  box-shadow: 10px 10px rgba(0, 0, 0, 0.5);
  color: black;
}
body {
  text-align: center;
  background: #3a3240;
  color: white;
}
input,
textarea,
select,
button {
  background: #3a3240;
  color: white;
}
a {
  color: white;
}
```
