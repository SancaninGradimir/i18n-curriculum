...
Sledeći deo koji će prikazati je biografski opis o autoru. Ovo možete učiniti pomoću `bio` koji ste prethodno razbili.

Dodaj element paragrafa koji ima `class` `"bio"`, zatim stavi `bio` unutar elementa paragrafa.
...
Vaš element od `p` treba da ima klasu objekata `"bio"`

```js
assert.include(document.querySelector('p')?.className, 'bio');
```

Treba da staviš `bio` unutar svog elementa `p`.
