...
Sledeće, nema velikog razdvajanja između imena autora i slike, zajedno sa drugim opisima na kartici. Separator će pružiti jasan redosled za pregled kartica autora.

Dodaj element za `div` na kratak opis autora i nagradu `class` `"purple-divider"`.
...
Treba da kreirate element za `div` pre vašeg elementa `p`.

```js
await __helpers.retryingTest(() => document.querySelector('p')?.previousElementSibling?.tagName === 'DIV', "'div' element not found");
```

Vaš element od `div` treba da ima `class` postavljeno na `"purple-divider"`.
