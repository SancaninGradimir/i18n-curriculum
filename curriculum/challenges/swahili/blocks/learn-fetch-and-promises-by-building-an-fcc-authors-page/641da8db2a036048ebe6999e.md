...
U funkciji `fetchMoreAuthors`, postavite parametre `startingIndex` na `endingIndex` kao `+= 8` svaki.
...
Trebalo bi da postavite parametar `startingIndex` na `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Trebalo bi da postavite parametar `endingIndex` na `+=8`.
