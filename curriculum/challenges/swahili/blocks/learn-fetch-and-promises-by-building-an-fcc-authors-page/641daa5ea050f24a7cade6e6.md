...
Ako se ovo stanje ispuni, onemogući dugme postavljanjem njegovog atributa `disabled` u `true`. Takođe, postavi atribut dugmeta `textContent` na `"No more data to load"`.
...
Trebalo bi da postavite svojstvo `disabled` za `loadMoreBtn` na `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Treba da postavite `textContent` iz `loadMoreBtn` na `"No more data to load"`.
