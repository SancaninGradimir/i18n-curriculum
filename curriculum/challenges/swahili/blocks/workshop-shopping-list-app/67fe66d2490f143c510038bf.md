...
Zatim, u elementu za čekboks ``input``, dodajte atribut ``checked`` i postavite ga na ``isChecked``. Iako nije strogo neophodno postavljati atribut ``checked``, dobra praksa je osigurati da svaki čekboks odražava trenutno stanje niza ``selectedItems``.
