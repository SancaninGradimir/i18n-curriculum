...
Sada ćete raditi na nekoj logici za prekiravanje bilo kojih odabranih stavki sa liste.

Pre svega, pošto je sve unutar ovog poziva `filteredItems.map()` implicitno vraćeno, obuhvatite sav kod unutar njega uglastim zagradama i eksplicitno vratite element `li`.

Na primer, ovako biste to uradili sa jednostavnom komponentom koja renderuje listu imena:
