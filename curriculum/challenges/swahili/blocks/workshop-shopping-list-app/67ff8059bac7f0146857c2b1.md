...
Ako koristite aplikaciju sada, videćete da se funkcija `toggleItem()` ponovo kreira svaki put kada označite ili odoznačite stavku, ili ukucate u traku za pretragu.

Da biste poboljšali performanse, možete koristiti kukač (hook) `useCallback()` da memoizujete ili keširate funkciju `toggleItem()`. Tada će React ponovo kreirati funkciju samo kada se promene njene zavisnosti.

Prvo, dekonstruišite kukač (hook) `useCallback()` iz React-a na vrhu vašeg fajla.
