...
Dok još uvek ništa ne filtrirate, i kada `filteredItems` odgovara nizu `items`, bilo bi lepo videti listu stavki na stranici.

Prvo, dodajte element `ul` ispod elementa `p` u `form`. Unutar neuređenog spiska (unordered list), koristite metodu `map()` da iterirate preko niza `filteredItems`, i prosledite joj anonimnu funkciju. Anonimna funkcija bi trebalo da primi `item` kao argument i vrati element `li` za svaku stavku. Svaki `li` bi trebalo da prikaže tekst `item`.
