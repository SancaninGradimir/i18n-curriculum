...
Hook ``useCallback()`` prima dva argumenta: funkciju i niz zavisnosti. Funkcija će biti ponovo kreirana samo ako se promeni jedna od zavisnosti.

Evo osnovne sintakse za ``useCallback()``:
...
Postavite ``toggleItem`` jednakom ``useCallback()``, i prosledite mu anonimnu funkciju koja prima ``item`` kao argument. Premestite postojeću logiku vaše funkcije ``toggleItem()`` unutar anonimne funkcije, gde se poziva ``setSelectedItems``. Na kraju, dodajte ``setSelectedItems`` kao zavisnost u niz zavisnosti.

Nakon toga, završili ste optimizaciju! Vaša aplikacija bi trebala i dalje da radi isto, ali ćete videti svoje konzolne logove samo kada se promeni stanje ``query`` ili kada se ponovo kreira funkcija ``toggleItem()``.
