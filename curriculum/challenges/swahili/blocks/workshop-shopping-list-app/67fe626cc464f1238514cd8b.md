...
Pre navode ``return`` unutar poziva ``filteredItems.map()``, kreirajte varijablu nazvanu ``isChecked`` i koristite metod ``includes()`` da proverite da li je trenutni ``item`` u nizu ``selectedItems``.

Zatim, dodajte ``style`` svojstvo elementu ``li``, i koristite ternarni operator da postavite svojstvo ``textDecoration`` na ``line-through`` ako je ``isChecked`` tačno, ili ``none`` ako je netačno. Zapamtite da morate obaviti ternarni operator u vitičnim zagradama jer se radi o JavaScript izrazu.

Na primer, evo kako možete koristiti ternarni operator za uslovno postavljanje boje paragrafa u jednostavnoj komponenti:
