...
Zatim, koristite metod `includes()` da proverite da li malonakrasna verzija `item` uključuje malonakrasnu verziju `query` kao podstring.

Na primer, evo kako da koristite metode `toLowerCase()` i `includes()` za proveru da li string `freeCodeCamp` uključuje `code`:
...
Povežite metod `includes()` sa malonakrasnom verzijom `item` i proverite da li uključuje malonakrasnu `query`. Ne zaboravite da pretvorite `query` u mala slova.
