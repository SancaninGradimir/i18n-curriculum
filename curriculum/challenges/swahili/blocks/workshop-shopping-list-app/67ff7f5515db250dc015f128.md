...
Sada kada možete videti kada se definicija funkcije `toggleItem()` prvi put postavlja ili menja, takođe biste trebali da logujete i kada je funkcija ista tokom renderovanja.

Dodajte izjavu `else` u izjavu `if` koju ste upravo napisali. Unutar bloka `else`, logujte niz (string) `Current toggleItem function` u konzolu.
