...
Kreiraj varijablu pod nazivom `filteredItems` i dodeli joj rezultat filtriranja niza `items` metodom `filter()`. Proširi anonimnu funkciju na metodu `filter()`, koja prima `item` kao argument. Unutar anonimne funkcije, samo vrati `item` za sada.
