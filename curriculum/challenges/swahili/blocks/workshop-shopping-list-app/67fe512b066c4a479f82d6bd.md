...
Unutar `li` elementa, iznad teksta `item`, dodajte `input` element sa atributom `type` podešenim na `checkbox`. Takođe, postavite njegov atribut `onChange` na anonimnu funkciju koja poziva `toggleItem()` sa `item` kao argumentom.

Na kraju, obuhvatite tekst `input` i tekst `item` u `label` element za bolju pristupačnost.
