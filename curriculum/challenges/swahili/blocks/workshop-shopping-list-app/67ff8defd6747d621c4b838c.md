...
Zatim, napišite izjavu `if` koja proverava da li `prevToggleItem` nije strogo jednako `toggleItem`. Ako nisu jednaki, logujte niz znakova `New toggleItem function` na konzolu. Zatim, postavite `prevToggleItem` jednako `toggleItem`.
