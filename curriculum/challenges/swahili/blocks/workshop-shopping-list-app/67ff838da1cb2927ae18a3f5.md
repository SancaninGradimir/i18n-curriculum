...
`useMemo()` prima dva argumenta: funkciju koja vraća vrednost koju želite da memorizujete, i niz zavisnosti. Memorizovana vrednost će se ponovo izračunati samo kada se promeni jedna ili više njenih zavisnosti.

Evo osnovne sintakse za `useMemo()`:
...
Postavite `filteredItems` jednakom `useMemo()`, i prosledite mu anonimnu funkciju sa vitičnim zagradama. Unutar vitičnih zagrada, koristite izjavu `return` da eksplicitno vratite svoju postojeću logiku filtriranja. Takođe, dodajte `query` kao jedinu zavisnost u nizu zavisnosti. Ovo osigurava da se operacija filtriranja ponovo pokreće samo kada se promeni `query`.
