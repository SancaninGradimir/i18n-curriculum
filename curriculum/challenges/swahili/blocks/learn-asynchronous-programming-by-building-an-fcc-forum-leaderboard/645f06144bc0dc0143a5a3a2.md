...
Svaki svaki objav na forumu će sadržavati spisak avatara korisnika koji predstavljaju sve korisnike koji učestvuju u diskusiji o toj temi.

Počnite kreiranjem strelične funkcije nazvane `avatars`, sa dva parametra koja se nazivaju `posters` i `users`.

# --hints--

Trebalo bi da imaš prazan strelbeni atribut koji se zove `avatars`.

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

Trebalo bi da imaš parametre nazvane `posters` i `users` u funkciji pod nazivom `avatars`.
