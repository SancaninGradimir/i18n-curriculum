...
Nakon atributa elementa `href`, postavite atribut elementa `class` da bude stabilno nazvan `linkClass`.

Nakon atributa elementa `class`, postavi atribut elementa `target` na `"_blank"`.

Na kraju, umestite čvrsto `linkText` između etiketa jangkorne oznake kako biste prikazali tekst u linku.

# --hints--

Trebalo bi da ima atribut elementa `class` nakon atributa elementa `href`.
...
Morate postaviti vrednost atributa elemenata `class` da bude konstanta nazvana `linkClass`.
...
Trebalo bi da ima atribut elementa `target` nakon atributa elementa `class`.
...
Trebalo bi da postaviš vrednost atributa elementa `target` na `"_blank"`.
...
Trebalo bi da koristiš `linkText` između oznaka (tagova) sidra.
