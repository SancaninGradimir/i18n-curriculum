...
Preostali deo stvari od `allCategories` je kompletan za tebe.

U narednim koracima, kreiraćeš funkciju za dobijanje imena grupe iz objekta `allCategories`.

Počnite kreiranjem strelnog funkcije koja se zove `forumCategory`, i `id` kao ime parametra.

# --hints--

Treba vam da kreirate praznu funkciju pod nazivom `forumCategory`.

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

Trebalo bi da imaš parametar nazvan `id` u funkciji `forumCategory`.
