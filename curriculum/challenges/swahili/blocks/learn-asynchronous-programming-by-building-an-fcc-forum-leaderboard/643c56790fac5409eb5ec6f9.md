...
Idi na svom funkcijoneru `showLatestPosts()`, koristi rešenje dizajna da dobiješ parametre za `topic_list` i `users` iz objekta `data`.

# --hints--

Trebalo bi da koristiš `const` za otključavanje `topic_list` i `users` iz objekta `data`.```js
