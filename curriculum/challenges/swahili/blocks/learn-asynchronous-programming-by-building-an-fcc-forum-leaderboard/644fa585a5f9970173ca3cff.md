...
Napravite konstantu nazvanu `linkClass` i postavite joj vrednost kao izraz šablona. Unutar tog izraza šablona, dodajte vrednost `category ${selectedCategory.className}`.

Ova imena klasa objekata će se koristiti za postavljanje stilova za anker element.

# --hints--

Trebalo bi da imaš nešto što se zove `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

Trebalo bi da postavite sintaksu šablona za vaše promene `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

Trebalo bi da ima vrednost `category ${selectedCategory.className}` unutar teksta šablona.
