...
Unutar izraza `if`, slomi `className` na `category` iz objekta `allCategories[id]`.

# --hints--

Trebaće ti da skupljaš `className` sa `category` iz objekta `allCategories[id]`. Predmeti mogu biti u bilo kom redosledu.
