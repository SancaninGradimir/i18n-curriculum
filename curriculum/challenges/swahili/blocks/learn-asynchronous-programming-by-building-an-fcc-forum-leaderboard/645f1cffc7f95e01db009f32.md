...
Sljedeći korak je prolazak kroz niz podataka `posters` ili dobijanje svih njihovih slika profila.

Počnite dodavanjem glavnog termina `return` praćenog sa `posters.map()`. Za funkciju povratnog poziva (callback), dodajte parametar nazvan `poster`.

# --hints--

Trebalo bi da vratiš `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

Vaša funkcija za povratni poziv mora imati parametar nazvan `poster`.
