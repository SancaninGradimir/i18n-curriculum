...
Unutar svoje funkcije `forumCategory`, kreiraj novi parametar `let` nazvan `selectedCategory` i postavi mu vrednost na prazno. Ovo će se koristiti za skladištenje imena grupe i imena klase objekata za svaku grupu.

# --hints--

Trebalo bi da koristite `let` za deklarisanje parametra pod nazivom `selectedCategory`.

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

Trebalo bi da postavite vrednost praznog objekta za parametar `selectedCategory`.
