...
Za otvarajući anker tag `a`, postavi atribut elementa `target` na `"_blank"`. Zatim, postavi atribut elementa `href` na `${forumTopicUrl}${slug}/${id}`.

Nakon tih promena, tvoj projekat liste lidera za platformu freeCodeCamp je sada završen!

# --hints--

Vaš otvarajući sidreni tag mora imati atribut elementa `target`, postavljen na `"_blank"`.
...
Vaš tag za otvaranje sidra mora imati atribut elementa `href` postavljen na `${forumTopicUrl}${slug}/${id}`.
