...
Sada je vreme da se prikažu podaci na stranici.

Počnite kreiranjem strelne funkcije nazvane `showLatestPosts`, koja prima jedan parametar od `data`.

# --hints--

Trebalo bi da imaš funkciju strele koja se zove `showLatestPosts`.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

Vaša funkcija `showLatestPosts` mora imati parametar sa `data`.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts` mora biti definisano.

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` Trebalo bi da bude funkcija.
