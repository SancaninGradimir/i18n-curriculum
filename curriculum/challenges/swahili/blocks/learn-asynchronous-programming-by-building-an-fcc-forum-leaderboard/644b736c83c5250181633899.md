...
Kreiraj izjavu za `if` ili proveri da li nešto od `allCategories` poseduje svojstvo `id`. Zapamti, možeš koristiti metodu za `hasOwnProperty()` za ovo.

# --hints--

Trebalo bi da imaš deklaraciju `if` koja proverava da li nešto od tipa `allCategories` poseduje atribut `id`
