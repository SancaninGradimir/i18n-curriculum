...
Kreiraj konstantu nazvanu `linkText` i postavi njenu vrednost na `selectedCategory.category`. Ovo će prikazati ime kategorije u polju sidra (anchor).

# --hints--

Trebalo bi da imaš konstantu nazvanu `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Treba da postaviš vrednost `selectedCategory.category` za svoje promene `linkText`.
