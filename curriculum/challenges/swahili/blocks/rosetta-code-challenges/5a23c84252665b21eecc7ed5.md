...
Knightova šetnja: Imaš šahovsku tablu od `width` \* `height` koja je prazna, osim jednog Knjazova (Knighta) na određenom kvadratiću. Knight mora da napravi niz validnih poteza koji će ga naterati da poseti svaki kvadratić na šahovskoj tabli samo jednom. Zapamti da nije nužno da je šetnja "zatvorena"; to jest, nije bitno da završi na udaljenosti od jednog poteza od mesta gde je počeo.

# --instructions--

Andika kitendakazi kinachopokea `width` na `height` kama vigezo na kurudisha idadi ya nafasi za mwanzo ambapo inawezekana kufanikisha zoezi lililotajwa hapo juu.

# --hints--

`knightTour` Trebalo bi da bude funkcija.

```js
assert(typeof knightTour == 'function');
```

`knightTour(6, 6)` inapaswa kurudisha nambari.

```js
assert(typeof knightTour(6, 6) == 'number');
```

`knightTour(6, 6)` inapaswa kurudisha `36`.

```js
assert.equal(knightTour(6, 6), 36);
```

`knightTour(5, 6)` treba da vrati `30`.

```js
assert.equal(knightTour(5, 6), 30);
```

`knightTour(4, 6)` trebalo bi da vrati `12`.

```js
assert.equal(knightTour(4, 6), 12);
```

`knightTour(7, 3)` trebalo bi da vrati `10`.

```js
assert.equal(knightTour(7, 3), 10);
```

`knightTour(8, 6)` inapaswa kurudisha `48`.
