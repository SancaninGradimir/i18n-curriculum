...
title: Changamoto ya jozi ya karibu zaidi
challengeType: 1
forumTopicId: 302232
...
Napravite funkciju za pronalaženje para dve najbliže tačke među skupom datoih tačaka u dve dimenzije.

Suluhisho rahisi ni algoriti ya $O(n^2)$ (ambayo tunaweza kuitaja kama *algoriti ya nguvu ghafi*); pseudo-msimbo (kutumia vielezo) unaweza kuwa rahisi kama ifuatavyo:

<pre><strong>bruteForceClosestPair</strong> ya P(1), P(2), ... P(N)
...
Algoriti bora zaidi inategemea mbinu ya kugawanya na kushinda kwa kurudia, ambayo ni $O(n\log n)$; pseudo-msimbo unaweza kuwa:

<pre><strong>closestPair</strong> ya (xP, yP)
  ambapo xP ni P(1) .. P(N) zilizopangwa kwa kielezo cha x, na
  yP ni P(1) .. P(N) zilizopangwa kwa kielezo cha y (mpangilio wa kuongezeka)
<strong>if</strong> N ≤ 3 <strong>then</strong>
  <strong>return</strong> pointi zilizo karibu zaidi za xP kwa kutumia algoriti ya nguvu ghafi
<strong>else</strong>
  xL ← pointi za xP kutoka 1 hadi ⌈N/2⌉
  xR ← pointi za xP kutoka ⌈N/2⌉+1 hadi N
  xm ← xP(⌈N/2⌉)<sub>x</sub>
  yL ← { p ∈ yP : p<sub>x</sub> ≤ xm }
  yR ← { p ∈ yP : p<sub>x</sub> > xm }
  (dL, pairL) ← closestPair ya (xL, yL)
  (dR, pairR) ← closestPair ya (xR, yR)
  (dmin, pairMin) ← (dR, pairR)
  <strong>if</strong> dL &#x3C; dR <strong>then</strong>
    (dmin, pairMin) ← (dL, pairL)
  <strong>endif</strong>
  yS ← { p ∈ yP : |xm - p<sub>x</sub>| &#x3C; dmin }
  nS ← idadi ya pointi katika yS
...
Kwa ingizo, tarajia hoja kuwa safu ya vitu vya `Point` vyenye wanachama `x` na `y` waliowekwa kuwa nambari. Rudisha kitu chenye jozi ya funguo:thamani za `distance` na `pair` (jozi ya pointi mbili zilizo karibu zaidi).

Kwa mfano `getClosestPair` na safu ya ingizo `points`:
...
**Kumbuka:** Panga safu ya `pair` kwa thamani zao za `x` kwa mpangilio wa kuongezeka.
...
`getClosestPair` Trebalo bi da bude funkcija.

```js
assert(typeof getClosestPair === 'function');
```

`getClosestPair(points1).distance` inapaswa kuwa `0.0894096443343775`.

```js
assert.equal(getClosestPair1_().distance, answer1.distance);
```

`getClosestPair(points1).pair` trebalo bi da bude `[ { x: 7.46489, y: 4.6268 }, { x: 7.46911, y: 4.71611 } ]`.
...
`getClosestPair(points2).distance` treba da bude `65.06919393998976`.

```js
assert.equal(getClosestPair2_().distance, answer2.distance);
```

`getClosestPair(points2).pair` treba da bude `[ { x: 37134, y: 1963 }, { x: 37181, y: 2008 } ]`.
...
`getClosestPair(points3).distance` trebalo bi da bude `6754.625082119658`.

```js
assert.equal(getClosestPair3_().distance, answer3.distance);
```

`getClosestPair(points3).pair` treba biti `[ { x: 46817, y: 64975 }, { x: 48953, y: 58567 } ]`.
