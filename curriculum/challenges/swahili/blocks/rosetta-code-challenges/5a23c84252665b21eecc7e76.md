...
Moj algoritam(i) za izračunavanje Gama funkcije (samo u polju realnih brojeva).

Kitendakazi cha Gamma kinaweza kufafanuliwa kama:
...
`gamma` inapaswa kuwa kitendakazi.

```js
assert(typeof gamma == 'function');
```

`gamma(.1)` inapaswa kurudisha nambari.

```js
assert(typeof gamma(0.1) == 'number');
```

`gamma(.1)` inapaswa kurudisha `9.513507698668736`.

```js
assert.equal(round(gamma(0.1)), round(9.513507698668736));
```

`gamma(.2)` inapaswa kurudisha `4.590843711998803`.

```js
assert.equal(round(gamma(0.2)), round(4.590843711998803));
```

`gamma(.3)` inapaswa kurudisha `2.9915689876875904`.

```js
assert.equal(round(gamma(0.3)), round(2.9915689876875904));
```

`gamma(.4)` treba da vrati `2.218159543757687`.

```js
assert.equal(round(gamma(0.4)), round(2.218159543757687));
```

`gamma(.5)` inapaswa kurudisha `1.7724538509055159`.
