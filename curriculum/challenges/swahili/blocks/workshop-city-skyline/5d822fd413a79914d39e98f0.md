...
U prethodnom modulu ste naučili da su gradijenti u CSS-u način za prelazak između boja duž udaljenosti elemenata. Oni se primenjuju na svojstvo `background` i sintaksa izgleda ovako:
...
U primeru, `color1` je čvrst na vrhu, `color2` je čvrst na dnu, a između njih se ravnomerno prebacuje od jednog ka drugom. U `.bb1a`, dodajte svojstvo `background` ispod svojstva `background-color`. Postavite ga kao gradijent tipa `linear-gradient` koji koristi `--building-color1` kao prvu boju i `--window-color1` kao drugu.
