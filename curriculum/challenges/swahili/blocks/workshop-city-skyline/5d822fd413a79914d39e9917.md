...
Možete dodati više gradijenata na element razdvajanjem ih zarezom (`,`) ovako:
...
Dodajte `repeating-linear-gradient` u `.fb1c` ispod onoga što je tu; koristite svoj `--building-color4` od `0%` do `10%` i `--window-color4` od `10%` i `90%`. Ovo će popuniti pozadinu gradijenta koji ste poslednji dodali.
