...
Dajte delove vaše zgrade `width` i `height` svojstvima sa ovim vrednostima: `70%` i `10%` na `.bb1a`, `80%` i `10%` na `.bb1b`, `90%` i `10%` na `.bb1c`, a `100%` i `70%` na `.bb1d`. Zapamtite da su ovi procenati relativni u odnosu na roditeljski element i napomenite da će visine iznositi 100% - vertikalno popunjavajući kontejner.
