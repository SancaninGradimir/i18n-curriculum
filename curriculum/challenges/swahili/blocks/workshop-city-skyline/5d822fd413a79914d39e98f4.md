...
Gradijenti mogu koristiti koliko god boja kakve želite, kao što je ovo:
...
Dodajte `linear-gradient` u `.bb1d` sa `orange` kao prvu boju, `--building-color1` kao drugu i `--window-color1` kao treću. Zapamtite da koristite gradijent na svojstvu `background`.
