...
Trebalo bi da optimizujete svoj kod. Premestite svojstva i vrednosti `position` i `top` iz `.foreground-buildings` u `.background-buildings`. Zatim odaberite i `.background-buildings` i `.foreground-buildings` tamo, efektivno primenjujući te stilove na oba elementa. Možete koristiti zarez (`,`) za razdvajanje selektora kao što je ovo: `selector1, selector2`.
