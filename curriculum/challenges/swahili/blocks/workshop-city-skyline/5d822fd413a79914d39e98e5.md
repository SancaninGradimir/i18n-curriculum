...
Želite da kontejner `.foreground-buildings` stoji direktno iznad elementa `.background-buildings`. Dajte mu `width` i `height` od `100%`, postavite `position` na `absolute`, i `top` na `0`. Ovo će ga učiniti iste veličine kao telo (body) i pomeriti mu početak u gornji levi ugao.
