...
Gradijentni prelazi često postepeno menjaju od jedne boje na drugu. Kada je potreban nagliji prelaz, tranzicija se može napraviti sa oštrim stajanjem (hard stop) kao što je ovo:
...
Dodajte `linear-gradient` u `.bb2b` koji koristi `--building-color2` iz `0%` do `6%` i `--window-color2` iz `6%` do `9%`.
