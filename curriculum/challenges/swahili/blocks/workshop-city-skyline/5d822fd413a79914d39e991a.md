...
Nastavi sa sledećim ugradnjom! Ubaci dva elementa iz `div` unutar `.fb2` i dodeli im klase objekata `fb2a` i `fb2b`, u tom redosledu.

# --hints--

Trebalo bi da dodaš dva elementa `div` unutar `.fb2`.

```js
assert.lengthOf(document.querySelectorAll("div.fb2 > div"), 2);
```

Unapaswa kumpa `div` mpya ya kwanza darasa la vitu la `fb2a`.
...
Unapaswa kumpa `div` mpya ya pili darasa la vitu la `fb2b`.
