...
Proizori su složeni jedan na drugi na levoj strani sekcije, iza ljubičaste zgrade. Dodajte novu klasu ispod `.building-wrap` nazvanu `window-wrap`. Napravite `.window-wrap` fleksboks kontejner (flexbox container), i koristite svojstva `align-items` i `justify-content` da centrirate njegove elemente dece vertikalno i ravnomerno ih rasporedite u njihovom roditelju, respektivno.
