...
Premesti poziciju `.fb4` u odnosu na ono gde je sada dodavanjem `position` od `relative` i `left` od `10%`. Uradi isto za `.fb5`, ali koristi `right` umesto `left`. Ovo će pokriti preostali prazan prostor između zgrada.
