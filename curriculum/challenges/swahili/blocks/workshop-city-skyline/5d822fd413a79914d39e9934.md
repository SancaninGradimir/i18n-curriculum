...
Konačno! Stigli ste do poslednje zgrade! Dodajte ponavljajući smjer za `90deg`. Koristite boju zgrade od `0%` do `10%` i `transparent` od `10%` do `30%`.

# --hints--

Treba da dodate `repeating-linear-gradient` na `.fb6` u svojstvo `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Trebalo bi mu dati smer ka `repeating-linear-gradient`, na `90deg`.
