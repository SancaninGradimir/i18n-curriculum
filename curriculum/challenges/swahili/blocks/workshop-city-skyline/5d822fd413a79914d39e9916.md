...
Ne brinite zbog prostora na dnu, sve će kasnije biti pomereno dole kada dodate visinu elementu na vrhu zgrade.

Dodajte `repeating-linear-gradient` do `.fb1c` sa uglom `90deg`, vaš `--building-color4` od `0%` do `10%` i `transparent` od `10%` do `15%`.
