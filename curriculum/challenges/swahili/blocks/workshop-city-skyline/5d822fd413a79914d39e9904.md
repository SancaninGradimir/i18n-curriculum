...
Do sada, svi gradijenti koje ste kreirali idu odozgo do dole, to je podrazumevan pravac. Možete specificirati drugi pravac dodavanjem ga pre vaših boja na ovaj način:
...
Popunite `.bb3` sa `repeating-linear-gradient`. Koristite `90deg` za pravac, vaše `building-color3` za prve dve boje, i `window-color3` na `15%` za treću. Kada ne specificirate rastojanje za boju, koristiće vrednosti koje imaju smisla. U ovom slučaju, prve dve boje će podrazumevano biti `0%` i `7.5%` jer počinje na `0%`, a `7.5%` je pola od `15%`.
