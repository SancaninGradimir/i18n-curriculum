...
title: Izgradite aplikaciju za kviz sa flaskartama
challengeType: 25
...
U ovom laboratorijumu, kreiraćete aplikaciju koja prikazuje i skladišti podatke o flas-karticama koje se kasnije mogu povući.

**Cilj:** Ispunite priče korisnika ispod i učinite da svi testovi prođu kako biste završili laboratorijum.

**Priče korisnika:**

1) Trebalo bi da imate HTML element sa ID-om `flashcard`.
2) Trebalo bi da imate interfejs nazvan `FlashCard`.
3) Interfejs `FlashCard` treba da sadrži sledeća svojstva `questionText` i `questionAnswer`, oba tipa `string`.
4) Trebalo bi da imate kolekciju elemenata `FlashCard` nazvanu `currentCards`.
5) Kada se klikne na element `#flashcard`, kartica treba da ima klasu `flipped`.
6) Trebalo bi da imate element sa ID-om `delete-btn`.
7) Kada se klikne na element `#delete-btn`, trebalo bi da ukloni trenutnu karticu i prikaže podatke prethodne kartice.
8) Trebalo bi da kreirate formular za unos sa ID-om `entry-form` kako biste mogli dodati više flas-kartica u kolekciju `currentCards` prilikom slanja (submit).
9) Dva elementa unutar forme treba da imaju ID-ove `front-text` i `back-text` respektivno.
10) Trebalo bi da kreirate i pozovete `InvalidUserInputError` kada je tekst pitanja ili odgovor na pitanje prazan u formularu za unos.
