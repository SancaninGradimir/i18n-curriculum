...
title: Napravite izložbu proizvoda
challengeType: 25
...
Za ovaj laboratorijski rad, obezbedili su vam sve HTML i CSS fajlove. Završićete prikaz proizvoda koristeći TypeScript.

**Cilj:** Ispunite korisničke priče ispod i prođite sve testove kako biste završili laboratorijski rad.

**Korisničke priče:**

1. Treba da kreirate interfejs pod imenom `Item` sa:
   - Svojstvo `type` koje može biti samo `"book"`, `"electronics"` ili `"clothing"`.
   - Svojstvo `id` tipa `string`.
   - Svojstvo `price` tipa `number`.

1. Treba da kreirate interfejs pod imenom `Book` koji nasleđuje `Item`. Njegovo svojstvo `type` treba da bude postavljeno na literalnu vrednost `"book"`. Takođe bi trebalo da ima svojstva `title` i `author` tipa `string`.

1. Treba da kreirate interfejs pod imenom `Electronics` koji nasleđuje `Item`. Njegovo svojstvo `type` treba da bude postavljeno na literalnu vrednost `"electronics"`. Takođe bi trebalo da ima svojstva `item` i `model` tipa `string`, i opcionalno svojstvo `warranty` tipa `number`.

1. Treba da kreirate interfejs pod imenom `Clothing` koji nasleđuje `Item`. Njegovo svojstvo `type` treba da bude postavljeno na literalnu vrednost `"clothing"`. Takođe bi trebalo da ima svojstva `item` i `brand` tipa `string`, i opcionalno svojstvo `size` koje može biti samo `"S"`, `"M"` ili `"L"`.

1. Treba da kreirate unioni tip pod imenom `Product` koji kombinuje interfejse `Book`, `Electronics` i `Clothing`.

1. Treba da kreirate generičku klasu pod imenom `Collection` sa tipskim parametrom `T`. Konstruktor bi trebalo da prihvati niz stavki tipa `T` i skladišti ih u svojstvo pod nazivom `items`.

1. Klasa `Collection` treba da ima metod `getAll()` koji vraća sve stavke iz kolekcije.

1. Klasa `Collection` treba da ima metod `filter()` koji prima povratnu funkciju (callback) i vraća samo elemente iz niza `items` koji zadovoljavaju zadati uslov. Povratna funkcija bi trebalo da primi element tipa `T` i vrati boolean vrednost.

1. Treba da kreirate funkciju pod imenom `renderProduct` sa parametrom tipa `Product`. Trebalo bi da vrati HTML string koji predstavlja proizvod sa:
   - Elementom sa klasom `item` i ID vrednošću proizvoda `id`.
   - Elementom sa klasom `price` i tekstom proizvoda `price`.
   - Elementom koji sadrži dodatne informacije o proizvodu.

1. Funkcija `renderProduct` bi trebalo da koristi sužavanje tipa (type narrowing) za renderovanje različitog HTML-a zavisno od tipa proizvoda:

   - Za knjige, renderovani HTML treba da ima format `Book: [title] by [author]`, gde bi `[title]` i `[author]` trebalo da budu zamenjeni naslovom i autorom, respektivno.
   - Za elektroniku, renderovani HTML treba da ima format `Electronics: [item] - [model]`. Ako proizvod ima garanciju, dodajte ` - Warranty: [warranty] year(s)` na format. Zamenite `[item]`, `[model]` i `[warranty]` stavkom, modelom i garancijom, respektivno.
   - Za odeću, renderovani HTML treba da ima format `Clothing: [item] by [brand]`. Ako proizvod ima veličinu, dodajte ` - Size [size]` na format. Zamenite `[item]`, `[brand]` i `[size]` stavkom, markom i veličinom, respektivno.
   - Ako se naiđe na neispravan tip proizvoda, baciti grešku sa formatom: `Unknown product type: [product]`, gde bi `[product]` trebalo da bude zamenjen neispravnim objektom proizvoda. Koristite `JSON.stringify` za konverziju neispravnog proizvoda u string.

1. Treba da kreirate instancu `Collection<Product>` koja uključuje najmanje jednu stavku za svaki tip proizvoda i dodelite instancu promenljivoj pod nazivom `products`.

1. Treba da kreirate funkciju `showProducts` koja prikazuje proizvode u elementu `#output`. Bi trebalo da prima opcionalni filter koji predstavlja tip proizvoda; kada se ne navodi, prikaže sve proizvode; kada je dostupan, prikazuje samo proizvode tog tipa.

1. Treba da renderujete proizvode konvertovanjem svakog proizvoda u HTML string koristeći funkciju `renderProduct` i ubacivanjem kombinovanog rezultata u element `#output`.

1. Klikom na dugmad `#all`, `#books`, `#electronics` i `#clothing` bi trebalo da se prikaže odgovarajući set proizvoda u elementu `#output`.

1. Pri učitavanju stranice, svi proizvodi bi trebalo da budu prikazani podrazumevano.
