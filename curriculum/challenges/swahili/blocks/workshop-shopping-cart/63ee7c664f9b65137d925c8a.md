...
Vaša klasa `ShoppingCart` treba da ima mogućnost dodavanja stavki. Kreirajte praznu metodu `addItem`, koja prima dva parametra: `id` i `products`. Kreiranje metode bi moglo izgledati ovako:
...
Prvi parametar, `id`, je `id` proizvoda koji je korisnik dodao u korpu. Drugi parametar, `products`, je niz instanci `Dessert`. Korišćenjem parametra umesto direktnog referenciranja vašeg postojećeg niza `products`, ova metoda će biti fleksibilnija ako želite u budućnosti dodati dodatne liste proizvoda.
