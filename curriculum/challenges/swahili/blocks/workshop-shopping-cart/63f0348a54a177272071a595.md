...
Sada postavi vrednost za `textContent` promena `showHideCartSpan`, rezultat matematičkog izraza ternarnog uslovnog kalkulatora koji proverava da li `isCartShowing` je tačno. Ako je tačno, postavi `textContent` na `"Hide"`, inače postavi na `"Show"`.

# --hints--

Treba da koristiš distribucijsku operaciju na svojstvu `textContent` promena `showHideCartSpan`.

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*/)
```

Trebalo bi da koristite sintaksu trostrukog uslovnog operatora za proveru da li je `isCartShowing` tačno.

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*/)
```

Trebalo bi da postaviš izmenu `textContent` u `showHideCartSpan` na `"Hide"` ako je `isCartShowing` tačno.

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*('|"|`)Hide\1\s*:\s*/)
```

Unapaswa kuweka `textContent` ya mabadiliko `showHideCartSpan` kuwa `"Show"` ikiwa `isCartShowing` si kweli.
