...
Sada dodelite `textContent` od varijable `showHideCartSpan` rezultat ternarnog izraza koji proverava da li je `isCartShowing` tačno. Ako jeste, postavite `textContent` na `"Hide"`, inače ga postavite na `"Show"`.
