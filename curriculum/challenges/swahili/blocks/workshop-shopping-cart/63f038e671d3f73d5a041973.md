...
Prva stvar koju treba da uradite je proveriti da li je niz `items` prazan. Ako jeste, prikažite korisniku `alert` sa tekstom `Your shopping cart is already empty`, a zatim se vratite iz funkcije.

Zapamtite da je `0` lažno (falsy) vrednost, pa možete koristiti operator `!` za proveru da li je niz prazan.

Nakon prikazivanja upozorenja, vratite se iz funkcije kako biste zaustavili izvršavanje.
