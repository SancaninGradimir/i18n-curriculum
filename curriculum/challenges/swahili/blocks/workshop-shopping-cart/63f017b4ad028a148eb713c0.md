...
Za vaš falsy izraz, moraćete da dodate novi HTML u svoj `productsContainer`. Počnite brisanjem `undefined`, zatim koristite operator dodeljivanja sabiranja i sintaksu template literala da dodate `div` sa `class` podešenim na `product` i `id` podešenim na `dessert${id}` u svojstvo `innerHTML` objekta `productsContainer`.
