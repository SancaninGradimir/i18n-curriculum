...
U vašem povratnom pozivu slušaoca događaja, pozovite metodu `.addItem()` vašeg objekta `cart`, i prosledite `event.target.id`. Zapamtite da će `id` ovde biti string, pa ga morate konvertovati u broj.

Prosledite svoj niz `products` kao drugi argument.
