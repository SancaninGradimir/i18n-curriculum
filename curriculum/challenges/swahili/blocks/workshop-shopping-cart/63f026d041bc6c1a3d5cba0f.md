...
Morate da iterirate kroz dugmad u vašoj varijabli `addToCartBtns`. Međutim, `.getElementsByClassName()` vraća Kolekciju, koja nema metod `forEach`.

Koristite spread operater na varijabli `addToCartBtns` da je konvertujete u niz. Zatim, koristite metod `forEach` za iteraciju kroz niz. Još ne prosleđujte funkciju povratnog poziva (callback function).
