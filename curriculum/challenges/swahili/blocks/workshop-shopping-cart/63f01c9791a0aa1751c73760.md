...
Još nedostaje funkcionalnost koju vaša `ShoppingCart` klasa zahteva, ali prvo morate biti u stanju da testirate kod koji ste trenutno napisali. Biće vam potrebno da <dfn>kreirate instancu</dfn> novog objekta `ShoppingCart` i dodelite ga promenljivoj. Evo primera kreiranja instance klase `Computer` iz ranijih primera:

```js
const myComputer = new Computer();
```

Deklarišite promenljivu `cart`, i dodelite joj novi objekat `ShoppingCart`. Obratite pažnju na korišćenje ključne reči `new` prilikom kreiranja instance objekta.
