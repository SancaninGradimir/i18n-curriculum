...
Bado kuna vipengele zaidi ambavyo darasa lako la `ShoppingCart` linahitaji, lakini kwanza unahitaji kuweza kujaribu msimbo uliouandika hadi sasa. Utahitaji <dfn>kutengeneza mfano mpya</dfn> wa kitu cha `ShoppingCart` na kuuweka katika kigezo. Hapa kuna mfano wa kutengeneza mfano wa darasa la `Computer` kutoka mifano ya awali:

```js
const myComputer = new Computer();
```

Deklarišite parametar `cart`, i postavite vrednost novog primera/instancu `ShoppingCart`. Proverite upotrebu glavne ključne reči `new` prilikom kreiranja tog primera.
...
Trebalo bi da koristiš ključnu reč `new` za kreiranje novog primerka objekta `ShoppingCart`.

```js
assert.match(code, /new\s+ShoppingCart\s*\(\s*\)/);
```

Trebalo bi da postaviš novu vrednost `ShoppingCart` u parametar `cart`.
