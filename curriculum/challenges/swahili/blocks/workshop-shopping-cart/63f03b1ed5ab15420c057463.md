...
Vaš poslednji korak je da učinite vaš dugme za brisanje funkcionalnim. Dodajte `click` slušalicu događaja na `clearCartBtn`. Za povratni poziv, možete direktno proslediti `cart.clearCart`.

Međutim, to neće raditi, jer će kontekst `this` biti element `clearCartBtn`. Morate povezati metodu `clearCart` sa objektom `cart`.

Ovo možete učiniti prosleđivanjem `cart.clearCart.bind(cart)` kao povratnog poziva.

I time je radionica za korpu za kupovinu završena!
