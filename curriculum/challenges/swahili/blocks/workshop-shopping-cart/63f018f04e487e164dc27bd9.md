...
U vašem prvom `p` elementu, dodajte element `span`. Dajte `span` klasu `product-count` i atribut `id` sa vrednošću `product-count-for-id${id}`. Zatim, nakon spana, dajte vašem elementu `p` tekst varijable `name`.
