...
Izradićete aplikaciju za korpu za kupovinu. HTML i CSS su već obezbeđeni, ali ćete morati da napravite JavaScript kako biste učinili stranicu interaktivnom.

Da biste počeli, moraćete da dohvatite neke od svojih elemenata iz DOM-a. Počnite korišćenjem `document.getElementById()` da dođete do elemenata`#cart-container`, `#products-container` i `#dessert-card-container`. Sačuvajte ih u varijable nazvane `cartContainer`, `productsContainer` i `dessertCards`, respektivno.

Pošto se oni neće menjati, zapamtite da koristite `const` za njihovo deklarisanje.
