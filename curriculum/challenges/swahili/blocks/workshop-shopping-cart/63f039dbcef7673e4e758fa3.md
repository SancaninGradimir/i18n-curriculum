...
Pretraživači imaju ugrađenu funkciju ``confirm()`` koja prikazuje prozor za potvrdu korisniku. ``confirm()`` prima niz znakova, koji je poruka prikazana korisniku. Vraća ``true`` ako korisnik potvrdi, i ``false`` ako korisnik otkaže.

Deklarišite varijablu ``isCartCleared`` i dodelite joj vrednost pozivanjem ``confirm()`` sa nizom znakova ``"Are you sure you want to clear all items from your shopping cart?"``.
