...
Konačno, ažurirajte svojstvo `display` objekta `style` promenljive `cartContainer` na drugi ternarni izraz koji proverava da li je `isCartShowing` tačno. Ako jeste, postavite svojstvo `display` na `"block"`, inače ga postavite na `"none"`.

Sada bi trebalo da vidite svoju korpu i dodate artikle u nju.
