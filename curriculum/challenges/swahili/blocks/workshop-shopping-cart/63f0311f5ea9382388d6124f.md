...
Problem sa `.toFixed()` koji vraća string je što želite da budete u mogućnosti da izvodite proračune sa poreskom stopom. Da biste ovo popravili, možete proslediti poziv `.toFixed()` (uključujući proračun) funkciji `parseFloat()`. Ovo će konvertovati fiksni string nazad u broj, čuvajući postojeće decimale.

Prosledite svoj poziv `.toFixed()` funkciji `parseFloat()`.
