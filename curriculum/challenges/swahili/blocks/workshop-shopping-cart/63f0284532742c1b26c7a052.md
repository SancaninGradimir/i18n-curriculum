...
Dodajte svoju funkciju povratnog poziva na metodu `forEach`. Trebalo bi da primi parametar `btn`. Zatim, u povratnom pozivu, dodajte slušalicu događaja na `btn`. Slušalica događaja treba da prati događaj `click`, i trebalo bi da primi drugi povratni poziv sa parametrom `event`. Drugi povratni poziv treba da bude prazan.
