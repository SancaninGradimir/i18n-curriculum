...
Morate pronaći proizvod koji korisnik dodaje u korpu. Zapamtite da nizovi imaju metod `.find()`. U vašoj funkciji `addItem`, deklarišite varijablu `product` i dodelite joj vrednost pozivanjem metode `.find()` na nizu `products`.

Da bi se izvršio callback za `.find()`, prođite funkciju koja prima jedan parametar `item`, i vraća da li je svojstvo `id` od `item` striktno jednako parametru `id` prosleđenom u `addItem`.
