...
Sada kada imate `Dessert` klasu, možete je koristiti za kreiranje proizvoda. Svakvi poziv `new Dessert(...)` vraća novu instancu sa istom strukturom, što je mnogo kraće za pisanje od punog objektnog literala.

Dodajte jednu vrednost u vaš niz `products` pozivom `new Dessert()` sa ovim argumentima, po redu: broj `1`, string `"Vanilla Cupcakes (6 Pack)"`, broj `12.99` i string `"Cupcake"`.
