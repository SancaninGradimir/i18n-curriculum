...
Naučili ste u prethodnoj predaji da ključna reč `this` se odnosi na trenutnu instancu koja se konstruiše, kao što ste videli u konstruktoru `Dessert`. Ovde ćete primeniti istu ideju na klasu `ShoppingCart`.
...
U vašem konstruktoru, koristite ključnu reč `this` da postavite svojstvo `items` na prazan niz. Takođe, postavite svojstvo `total` na `0`, a svojstvo `taxRate` na `8.25`.
