...
Sada kada imate spisak proizvoda, možete koristiti JavaScript da ih umetnete u HTML. Ovim pristupom, ako odlučite dodati više proizvoda, HTML će automatski odražavati to.

Počnite pozivanjem metode `.forEach` vašeg `products` niza. Koristite strelicasti sintaks za kreiranje prazne funkcije povratnog poziva (callback).
