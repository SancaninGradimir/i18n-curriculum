...
Takođe morate ažurirati ukupnu cenu korpe kada korisnik doda artikl. Kreirajte metod `calculateTotal` u klasi `ShoppingCart`.

U tom metodu, deklarišite varijablu `subTotal` i koristite metod `reduce` na nizu `items` da izračunate zbir svojstva `price` svakog artikla u nizu. Koristite `total` i `item` kao parametre za vaš callback.

Zapamtite da postavite početnu vrednost u metod `reduce`.
