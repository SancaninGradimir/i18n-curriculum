...
Morate dobiti sva dugmad `Add to cart` koja ste ranije dodali u DOM. Deklariš varijablu `addToCartBtns`, i dodeli joj vrednost pozivanjem metode `getElementsByClassName()` na objektu `document`, prosleđujući niz (string) `"add-to-cart-btn"`.
