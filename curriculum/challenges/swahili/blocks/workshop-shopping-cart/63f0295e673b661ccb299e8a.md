...
Potrebno vam je način da pristupite ukupnom broju artikala u korpi. Najbolji način za to je dodavanje nove metode vašoj klasi `ShoppingCart`, umesto direktnog pristupa nizu `items`.

Dodajte metodu `getCounts` klasi `ShoppingCart`. Trebalo bi da vrati broj artikala u nizu `items`.
