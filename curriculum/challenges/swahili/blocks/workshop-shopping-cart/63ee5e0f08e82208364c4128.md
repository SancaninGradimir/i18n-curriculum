...
Na kraju, nakon vaših `p` elemenata, kreirajte `button` element. Dajte mu `id`, podešen na vrednost varijable `id`, i `class` od `btn add-to-cart-btn`, i koristite `"Add to cart"` za tekst.
