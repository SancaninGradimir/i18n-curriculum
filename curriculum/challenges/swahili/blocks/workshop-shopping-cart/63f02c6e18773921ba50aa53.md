...
Zbog načina na koji računari skladište i rade sa brojevima, proračuni koji uključuju decimalne brojeve mogu rezultirati nekim čudnim ponašanjem. Na primer, `0.1 + 0.2` nije jednako `0.3`. To je zato što računari skladište decimalne brojeve kao binarni frakcije, a neke binarne frakcije se ne mogu tačno predstaviti kao decimalne frakcije.

Želimo da očistimo brojčani rezultat vašeg proračuna. Umotajte svoj proračun u zagrade (nemojte uključivati izjavu `return`!) i pozovite metod `.toFixed()` na njemu. Prođite broj `2` kao argument metodi `.toFixed()`. Ovo će zaokružiti broj na dve decimale i vratiti niz (string).
