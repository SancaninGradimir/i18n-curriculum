...
Nakon vašeg `h2` elementa, kreirajte dva `p` elementa. Prvom dodelite `class` sa `dessert-price`, i postavite tekst na dolar znak `"$"` praćen vrednošću varijable `price`. Drugom dodelite `class` sa `product-category`, a tekst je `"Category: "` praćen vrednošću varijable `category`.
