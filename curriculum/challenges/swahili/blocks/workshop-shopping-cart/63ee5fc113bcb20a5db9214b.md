...
Klasa ima specijalnu metodu `constructor`, koja se poziva kada se kreira nova instanca klase. Metoda `constructor` je odlično mesto za inicijalizovanje svojstava klase. Evo primera klase sa metodom `constructor`:
...
Dodajte praznu metodu `constructor` klasi `ShoppingCart`.
