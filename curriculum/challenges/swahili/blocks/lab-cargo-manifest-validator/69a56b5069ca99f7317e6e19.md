...
title: Izgradite validator Cargo manifesta
challengeType: 26
...
U ovom laboratorijumu, koristićete JavaScript za normalizaciju i validaciju manifesta tereta. Manifest tereta je dokument koji obično navodi robu koja se transportuje (na primer, brodom ili vozom) i uključuje detalje o toj robi.

Svaki manifest tereta će biti predstavljen kao objekat sa sledećim svojstvima:

- `containerId`: pozitivan ceo broj koji identifikuje povezani kontejner za teret.
- `destination`: neprzizan niz znakova (nakon uklanjanja razmaka) koji označava odredište tereta.
- `weight`: pozitivan broj koji predstavlja težinu tereta.
- `unit`: niz znakova koji opisuje jedinice za svojstvo težine tereta (ili `"kg"` za kilograme ili `"lb"` za funte).
- `hazmat`: logička vrednost koja ukazuje da li je potrebno rukovanje opasnim materijalom.

Primer objekta manifesta tereta:
...
1. Trebalo bi da implementirate funkciju pod nazivom `normalizeUnits` sa parametrom `manifest`.
    - Funkcija ne sme da mutira originalni objekat manifesta i uvek mora vratiti novi objekat gde je `weight` normalizovan na kilograme, a `unit` postavljen na `"kg"`.
    - Ako se težina objekta manifesta izražava u funtama (`unit: "lb"`), funkcija bi trebalo da konvertuje `weight` u kilograme koristeći približnu konverziju, 1 lb = 0.45 kg, i odgovarajuće ažurira `unit`.
    - Ako je težina već izražena u kilogramima (`unit: "kg"`), `weight` i `unit` bi trebalo da ostanu nepromenjeni.
2. Trebalo bi da implementirate funkciju pod nazivom `validateManifest` sa parametrom `manifest`.
    - Funkcija ne sme da mutira originalni objekat manifesta i uvek mora vratiti novi objekat.
    - Ako je ulazni manifest validan (bez nedostajućih ili nevažećih svojstava), funkcija bi trebalo da vrati prazan objekat.
    - Ako ulazni manifest nije validan, funkcija bi trebalo da vrati objekat koji sadrži unose za svako nedostajuće ili nevažeće svojstvo. Nedostajuća svojstva treba da imaju vrednost `"Missing"` a nevažeća svojstva treba da imaju vrednost `"Invalid"`.

Primer vrednosti povratne funkcije kada je ulazni objekat nedostajuće svojstvo `destination` i ima nevažeće svojstvo `weight`:
...
3. Trebalo bi da implementirate funkciju pod nazivom `processManifest` sa parametrom `manifest`. Funkcija bi trebalo da loguje:
    - Ako je objekat manifesta validan, `Validation success: ${containerId}` i zatim težinu manifesta u kilogramima kao takvu, `Total weight: ${weight} kg`. Koristite `normalizeUnits()` za ovu konverziju.
    - Ako objekat manifesta nije validan, `Validation error: ${containerId}` i zatim objekat koji se vraća pozivom `validateManifest()` sa objektom manifesta.
**Napomena:** oba ova slučaja bi trebalo da imaju dva `console.log()` poziva.
