...
Ispravite vaš povratni poziv za `.map()` ili koristite funkciju `Number()`. Koristite `dropdown.value` za taj poziv funkcije.

Otvorite panel konzole da vidite da je `inputValues` sada red podataka sa brojevima.

# --hints--

Trebalo bi da pozoveš funkciju `Number()` u svom povratnom pozivu `.map()`.
...
Trebalo bi da prosledite `dropdown.value` u vašem pozivu `Number()`.
