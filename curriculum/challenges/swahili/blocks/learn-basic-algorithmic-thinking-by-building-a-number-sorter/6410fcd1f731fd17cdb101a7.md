...
Unutar vaše strukturirane petlje unutar `for`, dodajte poziv `console.log()` da proverite vrednosti `array`, `array[j]`, i `array[minIndex]` u svakoj petlji. Možete kliknuti na dugme `Sort` da vidite kako vaš algoritam prolazi kroz red podataka.

Kisha andika usemi wa `if` unaokagua kama thamani ya `array[j]` ni ndogo kuliko thamani ya `array[minIndex]`. Ikiwa ni kweli, weka `minIndex` kuwa `j`.

# --hints--

Treba ti da imaš poziv `console.log()` unutar svog definisanog ciklusa u `for`.
...
Trebalo bi da proslediš `array` kao prvi argument za `console.log()`.
...
Trebalo bi da prosledite `array[j]` kao drugi argument za `console.log()`.
...
Treba da proslediš `array[minIndex]` kao treći argument za `console.log()`.
...
Trebalo bi da imaš izraz `if` nakon tvog poziva `console.log()`.
...
Trebalo bi da postoji uslov u vašoj deklaraciji za `if`, koji proverava da li `array[j]` je manji od `array[minIndex]`.
...
Vaš `if` treba da postavi `minIndex` na `j`.
