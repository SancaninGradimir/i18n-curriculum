...
Kada je vaš status `if` tačan, morate promeniti dva elementa, stvarajući veliki „balončić“ element ka kraju redova podataka.

Da biste to uradili, deklarišite parametar `temp` i postavite mu vrednost na `array[j]`. Zatim postavite `array[j]` na vrednost `array[j + 1]`. Na kraju, postavite `array[j + 1]` na vrednost `temp`.

# --hints--

U vašoj deklaraciji za `if`, treba da deklariš parametar `temp`.
...
Treba da postavite `temp` na vrednost `array[j]`.
...
Trebalo bi da postaviš vrednost `array[j]` na `array[j + 1]`.
...
Treba da postaviš vrednost `temp` na `array[j + 1]`.
