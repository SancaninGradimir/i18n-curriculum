...
Deklariši varijablu `currValue` i postavi njenu vrednost na onu koja je `array[i]`. Zatim, deklariši varijablu `j` i postavi vrednost `i - 1`. Tvoja varijabla `j` treba da bude ponovo dodeljiva (re-assignable).

# --hints--

Treba da deklariš varijablu `currValue` i `const`.
...
Trebalo bi da postaviš vrednost `currValue` na onu koja je `array[i]`.
...
Trebalo bi da deklarišete varijablu `j` u `let`.
...
Treba da postaviš vrednost `j` na vrednost `i - 1`.
