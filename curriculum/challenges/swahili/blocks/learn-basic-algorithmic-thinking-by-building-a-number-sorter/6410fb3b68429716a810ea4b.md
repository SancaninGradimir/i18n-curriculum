...
Algoritam Selection Sort-a zavisi od praćenja elementa najmanje vrednosti u nizu podataka. Deklarišite varijablu `minIndex` i postavite joj vrednost `i` – ovo osigurava da, ako je vaša trenutna vrednost najmanja, ona će biti zamenjena samom sobom i neće se pomeriti. Biće vam potrebna mogućnost postavljanja nove vrednosti za `minIndex` dok iterirate kroz niz podataka.

Zatim, napiši još jedan ciklus za `for`, koristeći `j` kao ciklus. Ovaj ciklus mora početi na sledećem primeru `i` i ponoviti kroz preostali deo redova podataka.

# --hints--

Trebalo bi da koristite `let` za objavu `minIndex`.
...
Treba da staviš `minIndex` na mesto `i`.
...
Trebalo bi da imaš kružicu od `for` ugrađenu unutra.
...
Vaš ciklus od `for` treba da inicijalizuje `j` za `i + 1`.
...
Tvoj ciklus od `for` treba da prođe kroz preostali deo niza podataka.
...
Tvoj ciklus od `for` treba da doda `j`.
