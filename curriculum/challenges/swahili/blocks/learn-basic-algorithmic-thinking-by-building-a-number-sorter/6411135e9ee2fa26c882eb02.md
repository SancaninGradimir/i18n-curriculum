...
Svaka iteracija tvog `while` loop-a pronalazi element veći od tvoje trenutne vrednosti. Moraš pomeriti taj element udesno kako bi napravio prostor za tvoju trenutnu vrednost.

Uradite to postavljanjem vrednosti koja se nalazi u polju `j` u sledećem polju.

# --hints--

Pre smanjenja `j`, postavite vrednost koja se nalazi `j` na model `j + 1`.
