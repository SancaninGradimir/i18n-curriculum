...
Radi potrebe za ispravkom grešaka, dodajte poziv `console.log()` u vašu unutrašnju petlju. Prosledite argumente `array`, `array[j]` i `array[j+1]`.

# --hints--

Trebalo bi da koristite poziv `console.log()` u vašoj interno petlji.
...
```js
assert.match(code, /const\s+bubbleSort\s*=\s*(\(\s*array\s*\)|array)\s*=>\s*{\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*array\.length\s*;\s*i\s*\+\+\s*\)\s*{\s*for\s*\(\s*let\s+j\s*=\s*0\s*;\s*j\s*<\s*array\.length\s*-\s*1\s*;\s*j\s*\+\+\s*\)\s*{\s*console\.log\s*\(\s*array\s*,\s*array\s*\[\s*j\s*\]\s*,/);
```

Trebalo bi da prosledite `array[j+1]` kao treći argument za `console.log()`.
