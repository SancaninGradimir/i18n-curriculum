...
Da biste postigli rezultat "bubble up", morate proveriti da li je trenutni element veći od sledećeg elementa. To možete učiniti tako što ćete dobiti pristup `array` u `j` i `j+1`.

Tvorite stanje `if` koje proverava da li je trenutni element veći od sledećeg elementa.

# --hints--

Ne smete da uklonite vašu izjavu za `console.log`.
...
Morate koristiti deklaraciju `if` u svom ugrađenom ciklusu.
...
Vaš izjava za `if` treba da proveri da li `array[j]` je veće od `array[j+1]`.
...
Vaša deklaracija za `if` treba da bude prazna.
