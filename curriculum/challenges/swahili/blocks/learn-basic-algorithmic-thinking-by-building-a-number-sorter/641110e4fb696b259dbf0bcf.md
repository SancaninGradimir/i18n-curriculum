...
Za ovaj algoritam, potrebno je koristiti kolo `while`. Ovo kolo zahteva dva uslova:

- Prvo, ne bi trebalo da nastavi dalje od početka niza podataka (dostignutog preko `j`).
- Drugo, petlja ne bi trebalo da nastavi nakon pronalaženja vrednosti manjeg od trenutne vrednosti.

Da biste spriječili beskonačnu petlju, smanjite `j` unutar vaše petlje.
...
Tvoja petlja za `while` treba da ima prvi uslov koji proverava da li je vrednost od `j` veća ili jednaka `0`.
...
Tvoj ciklus od `while` bi trebalo da koristi operator AND.
...
Tvoja petlja za `while` treba da ima uslov koji proverava da li vrednost od `array[j]` je veća od `currValue`.
...
Tvoj opseg `while` bi trebalo da smanji `j` unutar kruga.
