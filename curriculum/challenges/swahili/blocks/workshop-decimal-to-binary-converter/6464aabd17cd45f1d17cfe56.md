...
Za objekat u sredini steka, dodajte svojstvo `msg` postavljeno na sledeći niz:

```md
"decimalToBinary(2) returns '1' + 0 (2 % 2) and gives that value to the stack below. Then it pops off the stack."
```

Takođe, dodajte svojstvo `showMsgDelay` postavljeno na `10000` i svojstvo `removeElDelay` postavljeno na `15000`.
