...
Sada je vaša funkcija `decimalToBinary` kompletirana. Slobodno eksperimentišite sa njom.

Ali postoje načini za poboljšanje. Na primer, nije neophodno pratiti ulaze i kvocijente. Možemo da sve pojednostavimo kako bi funkcija bila efikasnija.

Prvo, uklonite sve u telu funkcije `decimalToBinary`. Zatim, koristite `let` da kreirate varijablu pod nazivom `binary` i dodelite joj prazan string.
