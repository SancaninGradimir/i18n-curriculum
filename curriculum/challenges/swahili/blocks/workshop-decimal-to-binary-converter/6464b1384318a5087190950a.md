...
Sada kada ste ciljali ispravan element, možete ažurirati njegov tekst nakon kašnjenja koje ste ranije specifikovali.

Postavite svojstvo ``textContent`` ciljanog elementa jednako svojstvu ``msg`` trenutnog objekta korišćenjem lančeg povezivanja svojstava direktno na vašu metodu ``.getElementById()``.
