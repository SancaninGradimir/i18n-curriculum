...
Za poslednji objekat animacije, dodajte svojstvo `msg` postavljeno na sledeći niz karaktera:

```md
"decimalToBinary(5) returns '10' + 1 (5 % 2). Then it pops off the stack."
```

Takođe, dodajte svojstvo `showMsgDelay` postavljeno na `15000` i svojstvo `removeElDelay` postavljeno na `20000`.
