...
Prikašićete animaciju kada korisnici pokušaju da konvertuju decimalni broj `5` u binarni, pa ćete morati da dodate proveru za to unutar vaše funkcije `checkUserInput()`.

Koristite izjavu `if` da proverite da li je atribut `value` od `numberInput` jednak broju `5`. Zapamtite da koristite funkciju `parseInt()` da konvertujete niz u broj pre nego što ga uporedite sa `5`. Ostavite izjavu `if` praznu za sada.
