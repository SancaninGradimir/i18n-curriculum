...
Dobar način za testiranje da li sve radi je logovanje atributa `value` od `numberInput` u konzolu. Kao podsetnik, možete pristupiti atributu `value` elementa korišćenjem tačke ili zagradne notacije.

Unutar funkcije `checkUserInput`, koristite `console.log()` da logujete `value` od `numberInput` u konzolu.
