...
U ovoj radionici ćete napraviti dekadijalni i binarni konverter i vežbati oba brojnih sistema. Takođe ćete koristiti rekurziju za izvršavanje konverzija.

Sav HTML i CSS za ovaj projekat obezbeđen je za vas.

Kada budete spremni da počnete, koristite metod `.getElementById()` da dobijete element `input` sa id-om `"number-input"`, i sačuvajte ga u varijablu nazvanu `numberInput`. Koristite isti metod da dobijete element `button` sa id-om `"convert-btn"` i sačuvate ga u varijablu nazvanu `convertBtn`, te element `output` sa id-om `"result"` i sačuvajte ga u varijablu nazvanu `result`.


**NAPOMENA**: Ovaj projekat će konvertovati samo pozitivne brojeve u binarni sistem.
