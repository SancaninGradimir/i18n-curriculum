...
Na kraju, koristite string interpolaciju da postavite tekst paragrafa na `decimalToBinary(${currVal})`, gde je `currVal` svojstvo trenutnog objekta.`inputVal`. Nakon toga, testujte svoj kod unosom broja `5` u polje za broj i klikom na dugme `Convert`.
