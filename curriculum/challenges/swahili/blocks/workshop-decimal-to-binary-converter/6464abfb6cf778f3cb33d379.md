...
Za sledeću fazu animacije, ažuriraćete pasuse sa tekstom `msg`. Pošto već imate kašnjenja za svaki korak animacije, možete dodati svoj kod u isti `.forEach()` petlju.

Dodajte još jednu funkciju `setTimeout()`. Prosledite praznu callback funkciju kao prvi argument, i prosledite svojstvo `showMsgDelay` trenutnog objekta kao drugi argument.
