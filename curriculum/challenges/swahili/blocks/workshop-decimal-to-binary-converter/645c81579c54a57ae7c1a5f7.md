...
Zapamtite da, svaki put kada petlja radi, `input` je količnik prethodne vrednosti `input` podeljene sa `2`, zaokružen ka dole. Na kraju, `input` je manji od `1`, i petlja prestaje da radi.

Ovo možete uraditi u jednom koraku.

Unutar vaše `while` petlje, postavite `input` jednak količniku `input` podeljenom sa `2`. Takođe, zapamtite da koristite `Math.floor()` za zaokruživanje količnika ka dole.
