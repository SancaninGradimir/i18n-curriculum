...
Sada bi sve trebalo da radi kako je očekivano. I pošto znate da će `input` u ovom trenutku biti ili brojevi `0` ili `1`, možete kombinovati svoje dve osnovne slučaja i jednostavno vratiti `input` kao string.

Za pouzdan način konverzije vrednosti u string, čak i za lažne vrednosti kao što su `null` i `undefined`, možete koristiti funkciju `String()`. Na primer:
...
Kombinuje svoje `if` i `else if` izjave u jednu `if` izjavu koja proverava da li je `input` jednako `0` ili `1`. Ako jeste, koristite funkciju `String()` za konverziju `input` u string i vratite ga.
