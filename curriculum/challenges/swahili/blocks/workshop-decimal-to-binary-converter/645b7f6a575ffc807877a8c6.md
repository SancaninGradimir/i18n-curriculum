...
Vaša funkcija `decimalToBinary` dobro radi, ali postoji problem – zbog uslova u vašoj petlji `while`, ona radi samo za brojeve veće od `0`. Ako pokušate da konvertujete `0` u binarni sistem, ništa neće biti dodato na stranicu.

Da biste ovo popravili, dodajte izjavu `if` da proverite da li je `input` jednako `0`. Ostavite telo izjave `if` prazno za sada.
