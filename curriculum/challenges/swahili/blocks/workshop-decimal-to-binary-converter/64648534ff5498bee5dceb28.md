...
Sledeće, kreiraćete objekat koji će predstavljati prvi kadar vaše animacije. Vaš objekat bi trebalo da ima dva svojstva ili ključa: `inputVal` i `addElDelay`.

`inputVal` će predstavljati vrednost unosa svaki put kada vaša rekurzivna funkcija radi. A `addElDelay` će biti kašnjenje između dodavanja DOM elemenata na stranicu.

Dodajte objekat u `animationData` sa svojstvom `inputVal` postavljeno na `5`, i svojstvom `addElDelay` postavljeno na `1000`.
