...
Na kraju, morate da obradite slučajeve gde `input` je `0`. Umesto da ažurirate DOM i vratite se rano kao što ste ranije, možete ažurirati string `binary` i pustiti ostatak koda u funkciji da radi.

Kreirajte izjavu `if` koja proverava da li je `input` jednako `0`. Ako jeste, postavite `binary` jednak stringu `"0"`.
