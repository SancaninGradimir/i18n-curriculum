...
Sada kada ste upoznati sa binarnim brojevima, vreme je da završite izgradnju funkcije koja će vam obaviti konverziju. Prvo ćete početi sa jednostavnijim rešenjem, a zatim ga preurediti u rekurentno rešenje.

Prvo, morate kreirati nizove za skladištenje ulaza i rezultata deljenja koje ćete obaviti u sledećim koracima. Ovo će olakšati praćenje kako radi konverzija iz decimalnog u binarni sistem.

Uklonite deklaraciju ``return`` iz vaše funkcije ``decimalToBinary``. Zatim, deklarišite varijable nazvane ``inputs``, ``quotients`` i ``remainders``, i dodelite prazan niz svakoj od njih.
