...
Sada kada je vaša funkcija `showAnimation()` postavljena, vreme je za testiranje.

Dodajte tri `console.log()` naredbe u funkciju `showAnimation()` da biste logovali tekst `"free"`, `"Code"` i `"Camp"` na konzolu. Trebalo bi da vidite taj tekst u konzoli kada unesete `5` u polje za broj i kliknete na dugme `Convert`.
