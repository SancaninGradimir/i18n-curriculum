...
U vašoj funkciji `checkUserInput`, uklonite izjavu `console.log()`. Zatim, pozovite funkciju `decimalToBinary` i prosledite `value` od `numberInput` kao argument. Takođe, uverite se da koristite funkciju `parseInt()` za konverziju unosa u broj.
