...
Pre nego što počnete da pišete kod za animaciju, pogledajte funkciju koju ćete koristiti za dodavanje i uklanjanje elemenata iz DOM-a: `setTimeout`.

Funkcija `setTimeout` prima dva argumenta: funkciju povratne pozive (callback) i broj koji predstavlja vreme u milisekundama čekanja pre izvršavanja funkcije povratnog poziva.

Na primer, ako želite da logujete `"Hello, world!"` u konzolu nakon `3` sekundi, napisali biste:
...
Koristite funkciju `setTimeout` da dodate odlaganje od jedne sekunde pre nego što se tekst `"Code"` loguje u konzolu. Zatim vidite šta se dešava nakon što unesete `5` u polje za unos broja i kliknete na dugme `Convert`.
