...
Pošto imate vremenski raspored za svaki animacioni kadar sačuvan u `addElDelay`, možete koristiti tu vrednost sa `setTimeout()` da postavite odlaganje za dodavanje elemenata u DOM.

Unutar tela funkcije povratne pozive (callback) metode `.forEach()`, dodajte funkciju `setTimeout()`. Prosledite praznu funkciju povratne pozive kao prvi argument, i `obj.addElDelay` kao drugi argument.
