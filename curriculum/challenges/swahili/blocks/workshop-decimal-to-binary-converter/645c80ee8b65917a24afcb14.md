...
Kreiraj petlju `while` koja radi dok je `input` veće od `0`. Unutar petlje, dodeli `0` na `input` za sada.

Note: Pazite da ne pokrenete funkciju `decimalToBinary` pre nego što postavite `input` jednako `0` unutar petlje. Inače, možete izazvati beskonačnu petlju.
