...
Sada ako unesete broj `6` i kliknete na dugme `Convert`, videćete sledeći izlaz:
...
Napomena da je niz `remainders` binarna reprezentacija broja `6`, ali u obrnutom redosledu.

Koristite metod `.reverse()` da obrnete redosled niza `remainders`, i `.join()` sa praznim stringom kao separatorom da spojite elemente u binarni broj (string). Zatim, postavite `result.innerText` jednak binarnom broju (string).
