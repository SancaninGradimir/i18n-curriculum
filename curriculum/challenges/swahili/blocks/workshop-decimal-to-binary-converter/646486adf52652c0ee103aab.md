...
Zapamtite da je pila poziva (call stack) struktura podataka tipa LIFO (Last In, First Out). To znači da se, kako funkcije pozivaju, dodaju na vrh ili kraj steka, a kada funkcije vraćaju kontrolu, uklanjaju se sa vrha steka.

Tretirajte svoj niz `animationData` kao stek i dodajte mu novi objekat. Vaš novi objekat treba da ima svojstva `inputVal`, a `addElDelay` postavljeno na `2`, i `1500`, respektivno. Zapamtite da dodate ovaj objekat na vrh steka, ili rečeno, na kraj niza `animationData`.
