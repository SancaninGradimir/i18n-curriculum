...
Potreban vam je još jedan interfejs nazvan `Deck`. Ovaj interfejs bi trebalo da ima svojstvo `cards`, koje je niz prethodnog interfejsa.

# --hints--

You should declare an interface named `Deck`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.interfaces.Deck);
```

Your `Deck` interface should have a property `cards` of type `Card[]`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Deck.hasTypeProps([{ name: "cards", type: "Card[]" }]));
```
