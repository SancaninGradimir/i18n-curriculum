...
Uključite preostale argumente `chosenCard.name_short` i `chosenCard.img`. Zatim izvan funkcije `renderCard`, resetujte `multipleCard` elementovo `innerHTML` na prazan string.

# --hints--

You should pass `chosenCard.name_short` as an argument to `renderCard`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /renderCard\s*\([\s\S]*?chosenCard\.name_short/);
```

You should pass `chosenCard.img` as the last argument to `renderCard`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /renderCard\s*\([\s\S]*?chosenCard\.img/);
```

You should reset `this.elements.multipleCard.innerHTML` to an empty string.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /this\.elements\.multipleCard\.innerHTML\s*=\s*["']{2}/);
```
