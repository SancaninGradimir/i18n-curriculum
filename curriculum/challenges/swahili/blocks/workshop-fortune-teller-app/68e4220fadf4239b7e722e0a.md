...
Pošto aplikacija za sreću ima samo tri trenutka: Prošlost, Sadašnjost i Budućnost, deklariši enum pod imenom `DrawingType` sa tri svojstva: `Past`, `Present`, i `Future`. Svako svojstvo treba da ima string vrednost koja odgovara njegovom imenu.

# --hints--

You should have `enum` named `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Your enum should have a `Past` property with a string value of `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Your enum should have a `Present` property with a string value of `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Your enum should have a `Future` property with a string value of `future`.

```js
assert.match(__helpers.removeJSComments(code), /Future\s*=\s*['"]future['"]/)
```
