...
Unutar konstruktora, dodajte pozive na `fetchCardsData()` i `initializeEventListeners()` koristeći ključnu reč `this`, tako da se izvrše kada se klasa inicijalizuje.

Metoda `fetchCardsData` će se baviti učitavanjem podataka o tarotu sa API-ja, a `initializeEventListeners` će postaviti rukovače za klik na dugme. Oba metoda ćete kreirati u narednim koracima.

# --hints--

You should call `fetchCardsData()` method inside the `constructor`.

```js
const explorer = await __helpers.Explorer(code);
const constructor = explorer.classes.Game.classConstructor.toString();
assert.include(constructor, 'this.fetchCardsData()');
```

You should call `initializeEventListeners()` method inside the constructor.

```js
const explorer = await __helpers.Explorer(code);
const constructor = explorer.classes.Game.classConstructor.toString();
assert.include(constructor, 'this.initializeEventListeners()');
```
