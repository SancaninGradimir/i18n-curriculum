...
Sada, unutar *template* literala funkcije `renderCard`, kreiraj element `div`. Unutar `div`, dodaj element `h2` koji ispisuje `drawingType`, i element `figure` sa klasom `card_container ${isReversed ? "reversed-card" : ""}` i atributom `data-id="${shortName}"`.

# --hints--

You should have a `div` element in the template literal of the `renderCard` function.

```js
const tmpl = __helpers.removeJSComments(code).match(/renderCard[\s\S]*?=>\s*(?:\{\s*return\s*)?`([\s\S]*?)`/)?.[1] ?? '';
assert.match(tmpl, /<div>/);
```

You should have an `h2` element that interpolates `drawingType` inside the `div` element.

```js
const tmpl = __helpers.removeJSComments(code).match(/renderCard[\s\S]*?=>\s*(?:\{\s*return\s*)?`([\s\S]*?)`/)?.[1] ?? '';
assert.match(tmpl, /<h2>\s*\$\{drawingType\}\s*<\/h2>/);
```

You should have a `figure` element inside the `div` element.

```js
const tmpl = __helpers.removeJSComments(code).match(/renderCard[\s\S]*?=>\s*(?:\{\s*return\s*)?`([\s\S]*?)`/)?.[1] ?? '';
assert.match(tmpl, /<figure[\s\S]*?>/);
```

Your `figure` element should have the class `card_container` and use a ternary to conditionally add `reversed-card`.

```js
const tmpl = __helpers.removeJSComments(code).match(/renderCard[\s\S]*?=>\s*(?:\{\s*return\s*)?`([\s\S]*?)`/)?.[1] ?? '';
assert.match(tmpl, /<figure[^>]*class\s*=\s*"card_container\s+\$\{isReversed\s*\?\s*"reversed-card"\s*:\s*""\s*\}"/);
```

Your `figure` element should have a `data-id` attribute set to `${shortName}`.

```js
const tmpl = __helpers.removeJSComments(code).match(/renderCard[\s\S]*?=>\s*(?:\{\s*return\s*)?`([\s\S]*?)`/)?.[1] ?? '';
assert.match(tmpl, /<figure[^>]*data-id\s*=\s*"\$\{shortName\}"/);
```
