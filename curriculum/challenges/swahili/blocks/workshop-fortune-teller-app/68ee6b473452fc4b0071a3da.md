...
Sada uradite isto za `multipleCardsBtn` i `newReadingBtn` tako što ćete priložiti slušalicu za događaj klika na `multipleCardsBtn` koja poziva `multipleCardSelected`, i drugu na `newReadingBtn` koja poziva `newReading`.

Dve metode ćete kreirati u narednim koracima.

# --hints--

You should attach a `click` event to the `multipleCardsBtn` element.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Inside the `multipleCardsBtn` callback event, you should call the `multipleCardSelected` method.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

You should attach a `click` event to the `newReadingBtn` element.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Inside the `newReadingBtn` callback event, you should call the `newReading` method.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?newReading\s*\(/);
```
