...
Sada, dodajte `div` sa klasom `img-loader`. Nakon toga, kreirajte element `img` i date mu uslovni URL za sliku koristeći `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--

You should have a `div` element inside your `figure` element.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Your `div` should have a class `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

You should have an `img` element inside your `figure` element.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

The `img` element should use `CDN_URL` and `img` for its `src` when `img` is provided, and fall back to `LOCAL_DEFAULT_IMG` when it is not.

```js
assert.match(
  __helpers.removeJSComments(code),
  /src\s*=\s*"\$\{img\s*\?\s*`\$\{CDN_URL\}\/\$\{img\}`\s*:\s*LOCAL_DEFAULT_IMG\s*\}"/
);
```

Your `img` element should have the classes `card-img` and `hidden`.

```js
assert.match(__helpers.removeJSComments(code), /<img[\s\S]*?class\s*=\s*"card-img\s+hidden"/);
```
