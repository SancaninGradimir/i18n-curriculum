...
Na ovom mestu, trebalo biste da vidite sreću koju svaka kartica prikazuje nakon klika na karticu. Sve što vam je ostalo je da učinite dugme za novo čitanje funkcionalnim.

Unutar metode ``newReading``, pozovite ``showElements()`` i ``hideElements()``.
