...
Sledeće, treba da kreirate funkciju za skrivanje elemenata iz DOM-a. Deklarišite funkciju `hideElements` koja prima `...elements` kao parametar tipa `HTMLElement[]`.

Ova funkcija, sa CSS klasom `hidden`, biće korišćena širom aplikacije za menjanje vidljivosti elemenata korisničkog interfejsa (UI).

# --hints--

You should have declared a function named `hideElements`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.allFunctions.hideElements);
```

Your `hideElements` function should have a parameter named `...elements` with the type `HTMLElement[]`.

```js
const explorer = await __helpers.Explorer(code);
const { hideElements } = explorer.allFunctions;
assert.isTrue(hideElements.parameters[0].matches("...elements: HTMLElement[]"));
```
