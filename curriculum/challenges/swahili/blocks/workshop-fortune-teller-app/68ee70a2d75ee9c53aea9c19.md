...
Unutar `map`, vrati funkciju `renderCard` prosleđivanjem `type`, `isReversed`, `name_short` i `img` kao argumente, i dodaj metod `join` na `map` sa praznim stringom.

# --hints--

You should return the `renderCard` function.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

You should pass `type` as the first argument in the `renderCard` function.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

You should pass `isReversed` as the second argument in the `renderCard` function.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

You should pass `card.name_short` as an argument in the `renderCard` function.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

You should pass `card.img` as an argument in the `renderCard` function.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

You should attach `join("")` on the `map` method.

```js
assert.match(__helpers.removeJSComments(code), /\.join\s*\(\s*["']{2}\s*\)/);
```
