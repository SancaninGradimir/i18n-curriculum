...
Unutar funkcije `getElement`, koristite `querySelector` sa generičkim tipovima i sačuvajte rezultat u varijablu. Dodajte izjavu `if` da proverite da li element ne postoji, i ako ne, bacite grešku sa porukom `Element not found: ${error message here}`. Ako element postoji, vratite ga.

# --hints--

You should declare a variable named `el` inside the `getElement` function.

```js
const explorer = await __helpers.Explorer(code);
const { getElement } = explorer.allFunctions;
assert.exists(getElement.variables.el);
```

Your `el` variable should be assigned the value `document.querySelector<T>(selector)`.

```js
const explorer = await __helpers.Explorer(code);
const { getElement } = explorer.allFunctions;
assert.isTrue(getElement.variables.el.value.matches("document.querySelector<T>(selector)"));
```

You should have if statement that check if `el` is not found.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

You should throw a new Error with the message `Element not found: ${selector}` inside your `if` statement.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

You should return the `el` variable inside the getElement function.

```js
const explorer = await __helpers.Explorer(code);
const { getElement } = explorer.allFunctions;
assert.isTrue(getElement.hasReturn('el'));
```
