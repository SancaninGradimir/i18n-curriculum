...
U nekim slučajevima, karte moraju biti obrnute, jer nisu uvek na ispravnoj poziciji.

Declare a variable called `isReversed` inside the `singleCardSelected` method. Use the `Math.random()` method to generate a random boolean value. To ensure the value stays within bounds, compare it with `0.5`.
...
assert.match(method, /const\s+isReversed\s*=/);
```
...
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /isReversed\s*=\s*Math\.random\s*\(\s*\)\s*<\s*0\.5/);
```
