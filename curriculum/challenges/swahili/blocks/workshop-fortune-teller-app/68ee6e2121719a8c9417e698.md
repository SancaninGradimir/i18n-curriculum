...
Odmah nakon što očistite `multipleCard`, unutar metode `singleCardSelected`, pozovite funkciju `showElements` i prosledite `singleCard` kao prvi argument a `fortuneContainer` kao drugi argument.

# --hints--

You should call the `showElements` function.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

You should pass `elements.singleCard` as the first argument.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

You should pass `elements.fortuneContainer` as the second argument.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\([\s\S]*?this\.elements\.fortuneContainer/);
```
