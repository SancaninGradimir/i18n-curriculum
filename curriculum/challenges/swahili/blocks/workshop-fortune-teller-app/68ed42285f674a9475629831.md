...
Unutar klase, dodajte privatno svojstvo `elements` kao objekat. Unutar njega, dodajte svojstva `singleCardBtn`, `singleCard` i `multipleCardsBtn`, svako tipizovano kao `HTMLElement`.

# --hints--

You should declare a `private` property named `elements` inside the `Game` class.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.classProps.elements);
assert.isTrue(explorer.classes.Game.classProps.elements.isPrivate());
```

Your `elements` private property should have a `singleCardBtn` property typed as `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.singleCardBtn.hasAnnotation('HTMLElement'));
```

Your `elements` private property should have a `multipleCardsBtn` property typed as `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.multipleCardsBtn.hasAnnotation('HTMLElement'));
```

Your `elements` private property should have a `singleCard` property typed as `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.singleCard.hasAnnotation('HTMLElement'));
```
