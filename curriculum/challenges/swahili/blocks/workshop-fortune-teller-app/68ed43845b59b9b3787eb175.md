...
Dodajte `constructor` unutar klase. U konstruktoru, morate koristiti elemente koje ste ranije kreirali da biste izabrali odgovarajuće DOM elemente koristeći funkciju `getElement`.

Za sada, obradite `singleCardBtn` i `singleCard`. Za `singleCardBtn`, prosledite `#btn-single-card`, a za `singleCard`, prosledite `.single_card`.

# --hints--

The `Game` class should have a `constructor`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.classConstructor);
```

Your constructor should assign `this.elements`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.constructorProps.elements);
```

`singleCardBtn` should be set using `getElement` with `'#btn-single-card'`.

```js
const explorer = await __helpers.Explorer(code);
const constructor = explorer.classes.Game.classConstructor.toString();
assert.match(constructor, /singleCardBtn:\s*getElement\(['"]#btn-single-card['"]\)/);
```

`singleCard` should be set using `getElement` with `'.single_card'`.

```js
const explorer = await __helpers.Explorer(code);
const constructor = explorer.classes.Game.classConstructor.toString();
assert.match(constructor, /singleCard:\s*getElement\(['"].single_card['"]\)/);
```
