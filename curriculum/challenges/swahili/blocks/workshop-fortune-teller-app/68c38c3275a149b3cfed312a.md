...
U ovoj radionici, nastavićete da vežbate rad sa TypeScript-om kreiranjem aplikacije za predviđanje sudbine.

Počnite deklarisanjem interfejsa pod nazivom `Card`. Unutar interfejsa `Card`, dodajte sledeća svojstva: `name`, `name_short` i `value`. Svojstva `name` i `name_short` treba da imaju tipove postavljene na `string`, a `value` treba da ima unioni tip postavljen na `string | number`.

# --hints--

You should declare an interface named `Card`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.interfaces.Card);
```

Your `Card` interface should have a `name` property of type `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "name", type: "string" }]));
```

Your `Card` interface should have a `name_short` property of type `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "name_short", type: "string" }]));
```

Your `Card` interface should have a `value` property with a union type of `string` and `number`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "value", type: "string | number" }]));
```
