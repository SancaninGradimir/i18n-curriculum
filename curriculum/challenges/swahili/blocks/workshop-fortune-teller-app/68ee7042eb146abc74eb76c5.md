...
Ponovo, moraš da učiniš karticu nasumičnom i obrnuto. Deklariši varijablu pod imenom `isReversed` i `card`. Za prvu, pozovi metodu `Math.random` i uporedi je sa `< 0.5`. Za drugu, pozovi `getRandomItem` i prosledi `this.cards`.

# --hints--

You should declare a variable named `isReversed`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?\bconst\s+isReversed\b/);
```

Your `isReversed` variable should use `Math.random()` to randomly decide if the card is reversed.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?const\s+isReversed\s*=\s*Math\.random\s*\(\s*\)\s*<\s*0\.5/);
```

You should declare a variable named `card`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?\bconst\s+card\b/);
```

Your `card` variable should be assigned by calling the `getRandomItem` method with `this.cards` as an argument.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)\s*=>\s*\{[\s\S]*?const\s+card\s*=\s*getRandomItem\s*\(\s*this\.cards\s*\)/);
```
