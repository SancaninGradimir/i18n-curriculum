...
Pošto projekat sada radi sa HTML DOM elementima, potreban vam je način da prikažete sreću na DOM.

Deklarišite funkciju pod imenom `renderCard` koja prima četiri argumenta: `drawingType`, `isReversed`, `shortName` i `img`.

Parametri `drawingType`, `shortName` i `img` moraju biti tipa `string`, dok `isReversed` mora biti tipa `boolean`. Funkcija bi trebalo da ima tip povratne vrednosti `string`.

# --hints--

You should have a function named `renderCard`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.allFunctions.renderCard);
```

Your `renderCard` function should have four parameters.

```js
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.exists(renderCard.parameters[3]);
```

Your first parameter should be `drawingType` of type `string`.

```js
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.isTrue(renderCard.parameters[0].matches("drawingType: string"));
```

Your second parameter should be `isReversed` of type `boolean`.

```js
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.isTrue(renderCard.parameters[1].matches("isReversed: boolean"));
```

Your third parameter should be `shortName` of type `string`.

```js
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.isTrue(renderCard.parameters[2].matches("shortName: string"));
```

Your fourth parameter should be `img` of type `string`.

```js
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.isTrue(renderCard.parameters[3].matches("img: string"));
```

Your `renderCard` function should have a return type of `string`.

```js
const explorer = await __helpers.Explorer(code);
const { renderCard } = explorer.allFunctions;
assert.isTrue(renderCard.hasReturnAnnotation("string"));
```

Your `renderCard` function should return a template literal.

```js
assert.match(
  __helpers.removeJSComments(code),
  /renderCard\s*=\s*\([^)]*\)\s*:\s*string\s*=>\s*(?:\{\s*return\s*)?`/
);
```
