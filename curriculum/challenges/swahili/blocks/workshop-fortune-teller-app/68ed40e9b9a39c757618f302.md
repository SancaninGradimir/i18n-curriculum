...
Sada vam je potrebna glavna klasa koja će pomoći da se korisnički interfejs (UI) dinamički prikaže u DOM-u.

Kreirajte klasu pod nazivom `Game`. Unutar nje, dodajte svojstvo nazvano `cards` tipa `Card[]`, i za sada ga inicijalizujte praznim nizom.

# --hints--

You should have class named `Game`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game);
```

You should have a `cards` property inside the `Game` class.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.classProps.cards);
```

Your `cards` property should have a type of `Card[]`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.classes.Game.classProps.cards.hasAnnotation("Card[]"));
```

Your cards property should be set to an empty `array`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.classes.Game.classProps.cards.value.matches("[]"));
```
