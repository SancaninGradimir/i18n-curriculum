...
Sada pozovi funkciju `showElements` i prosledi `multipleCard`, `fortuneContainer` i `text` kao svojstva.

# --hints--

You should call the `showElements` function inside the `multipleCardSelected` method.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/
);
```

You should pass `multipleCard` as the first argument to the `showElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

You should pass `fortuneContainer` as an argument to the `showElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\([^)]*this\.elements\.fortuneContainer/);
```

You should pass `text` as an argument to the `showElements` function.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\([^)]*this\.elements\.text/);
```
