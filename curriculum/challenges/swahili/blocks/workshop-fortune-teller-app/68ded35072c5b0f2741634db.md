...
Potreban vam je još jedna funkcija koja prikazuje elemente u DOM-u. Slično poslednjoj funkciji, kreirajte funkciju pod nazivom `showElements`. Ovog puta koristite `classList.remove("hidden")`.

# --hints--

You should have a function named `showElements`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.allFunctions.showElements);
```

Your `showElements` function should have a parameter named `...elements` with the type `HTMLElement[]`.

```js
const explorer = await __helpers.Explorer(code);
const { showElements } = explorer.allFunctions;
assert.isTrue(showElements.parameters[0].matches("...elements: HTMLElement[]"));
```

Your `showElements` function you should have a `forEach` method attached to the `elements` parameter.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Your `forEach` method should take a parameter named `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

You should remove the `hidden` class using the `classList.remove` method inside the `forEach` method.

```js
assert.match(__helpers.removeJSComments(code), /el\s*\.\s*classList\s*\.\s*remove\s*\(\s*['"]hidden['"]\s*\)/);
```
