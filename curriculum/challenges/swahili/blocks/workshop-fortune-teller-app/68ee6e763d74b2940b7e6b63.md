...
Aplikacija za vidovanta podržava prikaz više karata, pa vam je potreban metod koji može pomoći u prikazu više karata. Deklarišite metod nazvan `multipleCardSelected`.

Unutar metode, pozovite funkciju `hideElements` i prosledite svojstva `singleCard`, `singleCardBtn`, `multipleCardsBtn`, i `headerTitle`.vNe zaboravite da ih morate pristupiti iz `this.elements`.

# --hints--

You should declare a `multipleCardSelected` method.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.multipleCardSelected;
assert.isFalse(method.isEmpty());
```

You should call the `hideElements` function inside the `multipleCardSelected` method.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.multipleCardSelected.toString();
assert.match(method, /hideElements\s*\(/);
```

You should pass `singleCard` as the first argument to the `hideElements` function.
...
You should pass `singleCardBtn` as an argument to the `hideElements` function.
...
You should pass `multipleCardsBtn` as an argument to the `hideElements` function.
...
You should pass `headerTitle` as an argument to the `hideElements` function.
