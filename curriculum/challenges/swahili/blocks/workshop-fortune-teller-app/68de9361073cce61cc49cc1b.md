...
Na prethodnoj predaji ste naučili da je inferencija tipova sposobnost TypeScript-a za automatsno određivanje i dodeljivanje tipova promenljivama, povratnim vrednostima funkcija i izrazima na osnovu njihovog korišćenja i konteksta.

U TypeScript-u, generici čine funkcije, klase i tipove fleksibilnijim. Generike omogućavaju kreiranje varijabli tipova, eliminišući potrebu da se tipovi eksplicitno definišu svaki put. Ovo omogućava istoj funkciji da radi sigurno sa različitim tipovima.

Evo jednostavnog primera generika:
...
Funkcije mogu koristiti dve ili više generika. `T` se koristi po konvenciji, a dodatne varijable tipova kao što su `S` mogu se dodati po potrebi.
...
Kreirajte funkciju `getElement` koja prima `selector` tipa `string` kao parametar. Takođe bi trebalo da primi generički tip `T` koji proširuje (extends) `HTMLElement`, i da ima tip povratne vrednosti `T`.
...
assert.exists(explorer.allFunctions.getElement);
```
...
```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.allFunctions.getElement.parameters[0].matches("selector: string"));
```
...
assert.isTrue(getElement.typeParameters[0].matches("T extends HTMLElement"));
```
...
const { getElement } = explorer.allFunctions;
assert.isTrue(
  getElement.hasReturnAnnotation('T')
);
```
