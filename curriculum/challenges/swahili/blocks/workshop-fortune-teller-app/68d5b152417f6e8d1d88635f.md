...
Postoji više svojstava za dodavanje u interfejs `Card`. Počnite postavljanjem `value_int` na tip `number`. Sva ostala svojstva bi trebalo da budu tipa: `string`: `suit`, `type`, `img`, `meaning_up`, `meaning_rev`, i `desc`.

# --hints--

You should have a property `value_int` with a type of `number`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "value_int", type: "number" }]));
```

You should have a property `suit` with a type of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "suit", type: "string" }]));
```

You should have a property `type` with a type of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "type", type: "string" }]));
```

You should have a property `img` with a type of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "img", type: "string" }]));
```

You should have a property `meaning_up` with a type of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "meaning_up", type: "string" }]));
```

You should have a property `meaning_rev` with a type of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "meaning_rev", type: "string" }]));
```

You should have a property `desc` with a type of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.Card.hasTypeProps([{ name: "desc", type: "string" }]));
```
