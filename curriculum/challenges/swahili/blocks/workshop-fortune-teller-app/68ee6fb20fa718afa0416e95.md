...
Kao što je urađeno za prethodni `multipleCard`, ponovo dohvati element `multipleCard` i postavi mu `innerHTML` iteriranjem kroz `DrawingType`. Koristi `Object.values()` i pozovi na njemu metodu `map`, prosleđujući `type` kao argument.

# --hints--

You should assign `multipleCard.innerHTML` by calling `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

You should pass `DrawingType` to the `Object.values` method.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

You should call the `map` method on `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Your `map` method should have a parameter named `type`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(\s*\(\s*type\s*\)/);
```
