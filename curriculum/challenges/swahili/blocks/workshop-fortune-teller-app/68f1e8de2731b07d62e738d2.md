...
Deklariš drugu varijablu pod imenom `foundCard`, i dodeli je rezultatu pozivanja metode `find()` na `cards`. Prosledi `card` kao parametar povratnog poziva (callback), i uporedi `card.name_short` sa `cardId` kako bi pronašao odgovarajuću karticu koristeći strogo jednakost.
