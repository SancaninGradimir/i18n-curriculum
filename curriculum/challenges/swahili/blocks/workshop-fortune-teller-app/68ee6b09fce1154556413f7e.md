...
Unutar metode `initializeEventListeners`, povežite slušalicu događaja `click` na `singleCardBtn`. U unutar rukovaoca događaja, pozovite metodu `singleCardSelected`. Zapamtite da uzmete `singleCardBtn` iz `elements`.

Ovu metodu ćete kreirati u narednim koracima.

# --hints--

You should attach a `click` event to the `singleCardBtn` element. Don't forget you need the `this` keyword here.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Inside the event callback, you should call the `singleCardSelected` method.

```js
assert.match(__helpers.removeJSComments(code), /singleCardSelected\s*\(/);
```
