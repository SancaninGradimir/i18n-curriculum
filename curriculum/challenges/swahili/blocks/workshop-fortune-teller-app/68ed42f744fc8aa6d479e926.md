...
Sada dodajte preostala svojstva na privatni `elements` objekat. To su: `multipleCard`, `title`, `newReadingBtn`, `fortuneContainer`, `fortuneDescription`, `headerTitle`, `subTitle`, `cardTitle`, `description`, i `text`. Uverite se da svako svojstvo koristi tip `HTMLElement`.

# --hints--

You should have the `multipleCard` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.multipleCard.hasAnnotation('HTMLElement'));
```

You should have the `title` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.title.hasAnnotation('HTMLElement'));
```

You should have the `newReadingBtn` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.newReadingBtn.hasAnnotation('HTMLElement'));
```

You should have the `fortuneContainer` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.fortuneContainer.hasAnnotation('HTMLElement'));
```

You should have the `fortuneDescription` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.fortuneDescription.hasAnnotation('HTMLElement'));
```

You should have the `headerTitle` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.headerTitle.hasAnnotation('HTMLElement'));
```

You should have the `subTitle` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.subTitle.hasAnnotation('HTMLElement'));
```

You should have the `cardTitle` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.cardTitle.hasAnnotation('HTMLElement'));
```

You should have the `description` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.description.hasAnnotation('HTMLElement'));
```

You should have the `text` property inside the `elements` property assigned the type `HTMLElement`.

```js
const explorer = await __helpers.Explorer(code);
const elementsType = explorer.classes.Game.classProps.elements.annotation.typeProps;
assert.isTrue(elementsType.text.hasAnnotation('HTMLElement'));
```
