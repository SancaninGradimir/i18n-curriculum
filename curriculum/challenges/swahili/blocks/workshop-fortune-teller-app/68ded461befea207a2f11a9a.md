...
Do sada imate tri funkcije: `getElement`, `hideElements` i `showElements`.
Sada vam je potrebna funkcija koja vam pomaže da dobijete nasumično predviđanje.

Deklarišite funkciju pod imenom `getRandomItem` koja prima niz od `items` kao argument. Koristite generike za ovu funkciju.

Unutar funkcije, koristite `Math.random()` i `Math.floor()` da izaberete nasumično predviđanje iz niza `items`.
Ne zaboravite da vratite izabrano predviđanje.

# --hints--

You should declare a function named `getRandomItem`.
...
Your `getRandomItem` function should use the generic type parameter `T`.
...
Your `getRandomItem` function should have a parameter named `items` with the type `T[]`.
...
Your `getRandomItem` function should have a return type of `T`.
...
You should declare a variable named `index` inside the `getRandomItem` function.
...
The `index` variable should use `Math.floor()` and `Math.random()` with `items.length` to produce a random index.
...
Your `getRandomItem` function should return `items[index]`.
