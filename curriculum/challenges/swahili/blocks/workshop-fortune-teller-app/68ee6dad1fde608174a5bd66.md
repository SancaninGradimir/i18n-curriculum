...
Unutar metode `singleCardSelected`, dohvati element `singleCard` i ažuriraj njegovu `innerHTML` pozivanjem funkcije `renderCard`. Prosledi `"Click the card and reveal the fortune"` kao prvi argument, a `isReversed` kao drugi argument.

# --hints--

You should update `this.elements.singleCard.innerHTML` by calling the `renderCard` function.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /this\.elements\.singleCard\.innerHTML\s*=\s*renderCard\s*\(/);
```

Your `renderCard` call should have `"Click the card and reveal the fortune"` as the first argument.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /renderCard\s*\(\s*["']Click the card and reveal the fortune["']/);
```

You should pass `isReversed` as the second argument to `renderCard`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.singleCardSelected.toString();
assert.match(method, /renderCard\s*\([\s\S]*?["']Click the card and reveal the fortune["']\s*,\s*isReversed/);
```
