...
Dodajte još jedan slušalicu događaja na globalni objekat `document` koji poziva funkciju `showFortune`. Unutar povratne funkcije, uključite parametar `e` tipa `Event`, i prosledite `e` kao argument metodi `showFortune`.

`showFortune` će rukovati događajima klika na karticu, kao što je čitanje ID-a kliknute kartice iz DOM-a i prikazivanje odgovarajućih detalja sreće kartice. Kreiraćete ga u narednim koracima.
