...
Dostavljena je podrazumevana slika za karticu i URL za izvršavanje API zahteva; potrebne su vam samo neke varijable da ih sačuvate.

Kreirajte varijablu `CDN_URL` i postavite je na `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Takođe kreirajte varijablu `defaultImg` sa vrednošću `new URL("default.svg", import.meta.url)`, i varijablu `LOCAL_DEFAULT_IMG` sa vrednošću `defaultImg`.

# --hints--

You should create a variable named `CDN_URL`.

```js
const explorer = await __helpers.Explorer(code);
assert.isFalse(explorer.variables.CDN_URL.isEmpty());
```

Your `CDN_URL` variable should be set to `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.variables.CDN_URL.matches('const CDN_URL = "https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"'));
```

You should create a variable named `defaultImg`.

```js
const explorer = await __helpers.Explorer(code);
assert.isFalse(explorer.variables.defaultImg.isEmpty());
```

Your `defaultImg` variable should be set to `new URL("default.svg", import.meta.url)`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.variables.defaultImg.matches('const defaultImg = new URL("default.svg", import.meta.url)'));
```

You should create a variable named `LOCAL_DEFAULT_IMG`.

```js
const explorer = await __helpers.Explorer(code);
assert.isFalse(explorer.variables.LOCAL_DEFAULT_IMG.isEmpty());
```

Your `LOCAL_DEFAULT_IMG` variable should be set to `defaultImg`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.variables.LOCAL_DEFAULT_IMG.matches('const LOCAL_DEFAULT_IMG = defaultImg'));
```
