...
Dodajte handler `onerror` na element slike kako biste obradili neuspešna učitavanja. Prilikom prvog neuspeha, označite ga koristeći `dataset.failed` i zamenite izvor lokalnom podrazumevanom slikom. Ako slika ponovo ne uspe, uklonite skriveni klas i sakrijte placeholder za učitavanje koristeći `previousElementSibling`.

# --hints--

Your `img` element should have an `onerror` handler.
...
Your `onerror` handler should contain an `if` statement that checks if `dataset.failed` is falsy.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onerror\s*=\s*"[^"]*if\s*\(\s*!this\.dataset\.failed\s*\)/
);
```

`dataset.failed` should be set to `'1'` inside your `if` block. Don't forget the `this` keyword here.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onerror\s*=\s*"[^"]*this\.dataset\.failed\s*=\s*'1'/
);
```

Your `if` block should update `this.src` to the local default image. Don't forget the `this` keyword here.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onerror\s*=\s*"[^"]*this\.src\s*=/
);
```

You should have an `else` block should that removes the `hidden` class. Don't forget the `this` keyword here.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onerror\s*=\s*"[^"]*else\s*\{[^"]*classList\.remove\(\s*'hidden'\s*\)/
);
```

Your `else` block should set `previousElementSibling.style.display` to `'none'`. Don't forget the `this` keyword here.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onerror\s*=\s*"[^"]*this\.previousElementSibling\.style\.display\s*=\s*'none'/
);
```
