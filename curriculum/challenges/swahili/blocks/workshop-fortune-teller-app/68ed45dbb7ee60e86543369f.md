...
Nakon dohvaćanja podataka, sačuvajte odgovor u varijablu i parsirajte ga u JSON. Datim parsiranim podacima tip `Deck`, i zatim ga dodelite svojstvu `cards`. Unutar bloka `catch`, uverite se da logujete grešku sa ovom porukom: `"Error fetching cards:", error`.

# --hints--

You should declare a variable named `data` inside `fetchCardsData`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.fetchCardsData.toString();
assert.match(method, /const\s+data\s*[=:]/);
```

Your `data` variable should have the type `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Your `data` variable should be assigned by parsing the response as JSON.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.fetchCardsData.toString();
assert.match(method, /data[^=]*=\s*await\s+response\.json\s*\(\s*\)/);
```

You should store `data.cards` in `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Your `catch` block should use `console.error()` to log the error with the message `"Error fetching cards:", error`.

```js
assert.match(__helpers.removeJSComments(code), /catch\s*\(\s*error\s*\)\s*\{[\s\S]*?console\.error\s*\(\s*["']Error fetching cards:['"],\s*error/);
```
