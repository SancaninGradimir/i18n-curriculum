...
Unutar metode `fetchCardsData`, deklarujte varijablu pod imenom `response` i dodelite joj vrednost pozivanjem funkcije `fetch` sa sledećom URL adresom: `` `${CDN_URL}/card_data.json` ``.

Dodajte ključnu reč `await` pre poziva `fetch`, i obavijte celu logiku unutar bloka `try...catch`.

# --hints--

You should declare a `response` variable inside the `fetchCardsData` method.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.fetchCardsData.toString();
console.log("Method: ", method)
assert.match(method, /const\s+response\s*=/);
```

Your `response` variable should be assigned by calling `await fetch` with the URL `` `${CDN_URL}/card_data.json` ``.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.fetchCardsData.toString();
assert.match(method, /const\s+response\s*=\s*await\s+fetch\s*\(\s*`\$\{CDN_URL\}\/card_data\.json`\s*\)\s*;?/);
```

Your `fetch` call should be wrapped inside a `try...catch` block, with `error` as the `catch` parameter.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.fetchCardsData.toString();
assert.match(method, /try\s*\{/);
