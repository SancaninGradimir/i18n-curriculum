...
Trebalo bi da obavesti igrača o oružju koje je kupio. Između dve linije koje ste napisali, koristite `let` da objavite novi parametar nazvan `newWeapon`. Ovaj parametar treba uporediti sa `weapons`.

# --hints--

Vaša funkcija za `buyWeapon` treba da ima parametar nazvan `newWeapon`.

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

Trebalo bi da koristiš `let` za objavu `newWeapon`.

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` treba biti inicijalizovan sa vrednošću `weapons`. Ne zaboravi tačku.

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

`newWeapon` treba da se objavi pre izmene `text`.
