...
Korišćenjem liste birača (`selector1, selector2`) dajte njegove komponente za `#controls` i `#stats` `border` od `1px solid #0a0a23`, boju teksta za `#0a0a23`, i `5px` od `padding`.

# --hints--

Trebalo bi da imaš listu birača za `#controls, #stats`.
...
Vaš selektor za `#controls, #stats` mora da ima `border` od `1px solid #0a0a23`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

Selektor za tvoj `#controls, #stats` mora imati `color` od `#0a0a23`.
...
Vaš selektor za `#controls, #stats` mora imati `5px` unutrašnjeg prostora.
