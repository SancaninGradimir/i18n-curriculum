...
Primeni na vaš elementu za `#monsterStats` `border` i `padding`, isto kao i kod vašeg elementa za `#stats`. Podesi mu boju na `#ffffff` i dodaj pozadinsku temu od `#c70d0d`.
...
Vaš selektor za `#monsterStats` mora imati `border` od/za `1px solid #0a0a23`.
...
Tvoj selektor za `#monsterStats` treba da ima `color` od `#ffffff`.
...
Vaš selektor za `#monsterStats` mora imati `background-color` od `#c70d0d`.
