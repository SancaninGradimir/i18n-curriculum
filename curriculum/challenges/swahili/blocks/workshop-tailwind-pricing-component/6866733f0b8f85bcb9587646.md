...
Premium kartica za cene treba da bude ona koja se najviše ističe na stranici. Takođe, to je jedina kartica koja će imati nalepnicu `Most popular`. Da bi se ova kartica i njena nalepnica istakle, možete koristiti Tailwindove utilitetske klase za relativno pozicioniranje i skaliranje.

Evo kako biste učinili element većim skaliranjem u Tailwind CSS-u:
...
Pozicioniranje ćete rešiti u narednim koracima. Trenutno, dodelite klasu `scale-105` omotaču (wrapper) `div` kako biste ga učinili malo većim od ostalih kartica za cene.
