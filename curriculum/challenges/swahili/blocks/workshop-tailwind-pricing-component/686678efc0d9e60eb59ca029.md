...
Badge se može bolje stilizovati. Za to, koristićete neke Tailwind gradijent klase. Evo kako možete koristiti gradijent stilove u Tailwind-u:
...
Dodajte klase `bg-gradient-to-r`, `from-fuchsia-500` i `to-indigo-500` na badge `div`. Takođe, dodajte `rounded-full`, `px-3`, `py-1`, `text-xs`, `font-bold` i `text-white`.
