...
Iz redovnog CSS-a, naučili ste da dodeljivanje `display` vrednosti od `grid` elementu ništa ne radi dok ne navedete redove i kolone mreže (grid). To tako funkcioniše i u Tailwind-u.

U Tailwind-u, da dodelite red mreže (grid row) i kolonu mreže (grid column), koristili biste njihove odgovarajuće utilitetske klase na ovaj način:
...
Dodelite omotnici (wrapperu) `div` utilitetske klase `grid-cols-1` i `md:grid-cols-3`.

Klasa `grid-cols-1` aranžira tri elementa u jednoj koloni, vertikalno ih slažući (stacking). Na srednjim i većim ekranima, `md:grid-cols-3` prebacuje raspored na tri kolone, prikazujući elemente jedan pored drugog.
