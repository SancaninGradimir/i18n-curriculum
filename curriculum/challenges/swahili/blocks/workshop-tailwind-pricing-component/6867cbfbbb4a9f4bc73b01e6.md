...
Markeri stavki liste treba da budu malo razmaknuti. Stoga, dodajte svakom `span` u stavkama liste klase `text-green-700` i `mr-2`.

Takođe, dodajte atribut `aria-hidden` svakom elementu `span`.
