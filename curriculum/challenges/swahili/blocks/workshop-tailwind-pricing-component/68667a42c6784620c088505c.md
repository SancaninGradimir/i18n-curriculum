...
Osim CTA-a `Go Premium`, ostatak sadržaja je u `div`. To čini CSS grid odličnom opcijom za organizovanje svega.

Dodajte klase `grid`, `grid-rows-[auto_auto_auto_1fr]` i `gap-y-2` na `div` koji obuhvata elemente `h2`, `p` i `ul`.
