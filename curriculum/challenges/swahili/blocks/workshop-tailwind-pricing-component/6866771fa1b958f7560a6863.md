...
To to move tekst nalepnice, ali ne tačno gde želite. Magija je u utilitarnim klasama `right-[num]`, `left-[num]`, `top-[num]` i `bottom-[num]`.

Evo kako da ih koristite:
...
Takođe možete ukazati na negativne vrednosti dodavanjem prefiksa sa crticom:
...
Dodajte klase `-top-3` i `right-3` na nalepnicu `Most popular` `div`.
