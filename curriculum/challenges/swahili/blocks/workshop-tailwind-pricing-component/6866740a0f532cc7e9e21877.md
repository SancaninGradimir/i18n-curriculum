...
Sada pređite na oznaku (badge) `Most popular`. Morate je postaviti u gornji desni ugao kartice, pa možete koristiti neke od utilitarnih klasa za pozicioniranje Tailwind-a da biste to uradili.

Evo kako biste koristili CSS pozicioniranje u Tailwindu:
...
Dodelite klasu `relative` na omot (wrapper) `div`, a `absolute` na `div` sa tekstom `Most popular`.
