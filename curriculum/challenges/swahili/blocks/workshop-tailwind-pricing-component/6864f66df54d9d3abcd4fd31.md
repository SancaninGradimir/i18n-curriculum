...
Kwa `span` ndani ya kipengele cha `p` chenye maandishi `/month`, mpe darasa la vitu `text-base`, `font-medium`, na `text-gray-500`. Hii itaitofautisha na bei yenyewe kwa kufanya maandishi kuwa mepesi zaidi.

# --hints--

Tvoj element za `span` treba da ima klasu objekata `text-base`.
...
Vaš element za `span` treba da ima klasu objekata `font-medium`.
...
Element vašeg `span` treba da ima klasu objekata `text-gray-500`.
