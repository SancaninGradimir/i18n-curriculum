...
Sada bi trebalo da radite na postavljanju stavki unutar kartice za plan.

Da biste to uradili, podesite kontejner da koristi grid prikaz i navedite prilagođenu veličinu reda koristeći uglaste zagrade u vašoj Tailwind klasi.

Evo kako možete postaviti prilagođeni grid red koristeći frakcionalne jedinice:
...
Dajte spoljašnjem `div` za plan utilitetske klase `grid` i `grid-rows-[1fr_auto]`.
