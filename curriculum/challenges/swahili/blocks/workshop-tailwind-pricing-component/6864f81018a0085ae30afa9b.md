...
Znak za proveri u svakom `span` na početku stavke liste je dekorativan. Dodajte `aria-hidden` ovim `span` elementima da ih sakrijete od čitača ekrana.

Takođe, dodelite klase `text-green-700` i `mr-2` svakom od `span` elemenata.
