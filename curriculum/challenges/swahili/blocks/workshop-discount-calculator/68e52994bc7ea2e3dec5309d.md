...
Za treću strategiju popusta, kreirajte klasu pod nazivom `PremiumUserDiscount` koja nasleđuje od `DiscountStrategy`.

Za razliku od prethodnih strategija, ova ne zahteva metod `__init__` jer ne čuva nikakvu konfiguraciju. Primeniće fiksni popust od 20% svim premium korisnicima.

Trenutno, samo kreirajte praznu klasu koja nasleđuje od `DiscountStrategy` koristeći `pass`.
