...
Implementirajte metod `is_applicable` za vašu klasu `FixedAmountDiscount`. Popust fiksnog iznosa bi trebalo da se primeni samo ako je ekonomski opravdan.

Vratite `True` ako je `product.price * 0.9` veće od `self.amount`, inače vratite `False`. Ovo osigurava da je popust manji od 90% originalne cene, tako da nije nerealno.
