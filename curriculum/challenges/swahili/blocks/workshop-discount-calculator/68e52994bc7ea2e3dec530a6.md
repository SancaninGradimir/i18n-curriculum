...
Vreme je da se testira kompletan sistem popusta! Dodajte izjavu `if` koja proverava da li je `__name__ == '__main__'`, i premestite `product = Product('Wireless Mouse', 50.0)` unutra.

Nakon toga, unutar bloka `if`, kreirajte varijablu `user_tier` i postavite je na string `Premium`. Zatim kreirajte varijablu `strategies` postavljenu na listu strategija koja sadrži `PercentageDiscount(10)`, `FixedAmountDiscount(5)` i `PremiumUserDiscount()`.
