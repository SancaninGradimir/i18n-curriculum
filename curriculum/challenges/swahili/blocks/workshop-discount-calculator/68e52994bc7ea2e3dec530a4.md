...
Sada kreiraj petlju `for` koja iterira preko `self.strategies`. Za svaki `strategy`, proveri da li je primenljivo koristeći metodu `is_applicable`. Ako jeste, izračunaj sniženu cenu koristeći `apply_discount` i sapremi je u varijablu nazvanu `discounted`. Zatim, dodaj sniženu cenu na listu `prices`.
