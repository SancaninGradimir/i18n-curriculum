...
Sada kreiraj metod nazvan `calculate_best_price` u vašoj klasi `DiscountEngine`. Ovaj metod bi trebalo da primi `self`, `product`, i `user_tier` kao parametre, i vrati `float`.

Za sada, samo koristi `pass` u telu metoda. Implementiraćeš logiku u sledećim koracima.
