...
Sada je vreme da kreirate konkretnu implementaciju vaše apstraktne klase. Kreirajte klasu pod imenom `PercentageDiscount` koja nasleđuje od `DiscountStrategy`.

Dajte joj metod `__init__` koji prima `self` i `percent` kao parametre. Dajte tipsku indikaciju (type hint) od `int` za `percent` i postavite povratni tip metoda na `None`. Unutar metode `__init__`, sačuvajte `percent` kao atribut instance.
