...
Implementiraj metodu `is_applicable` za `PremiumUserDiscount`. Ova metoda bi trebalo da proveri da li je korisnik premium član.

Vrati `True` ako je `user_tier.lower()` jednako `'premium'`, inače vrati `False`. Korišćenje `.lower()` osigurava da je provera neosetljiva na veličinu slova.
