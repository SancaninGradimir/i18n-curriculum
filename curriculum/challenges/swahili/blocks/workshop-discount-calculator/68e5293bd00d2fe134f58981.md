---
id: 68e5293bd00d2fe134f58981
title: Korak 8
challengeType: 20
dashedName: step-8
---

# --description--

Kreiraj klasu pod nazivom `DiscountStrategy` koja nasleđuje od `ABC`. Ovo će biti apstraktna bazna klasa koja definiše interfejs za sve strategije popusta.

Trenutno, samo koristite `pass` u telu klase da biste kreirali praznu klasu.

# --hints--

You should create a class named `DiscountStrategy`.

```js
({
  test: () => runPython(`assert _Node(_code).has_class("DiscountStrategy")`)
})
```

Your `DiscountStrategy` class should inherit from `ABC`.

```js
({
  test: () => runPython(`
  assert _Node(_code).find_class("DiscountStrategy").inherits_from("ABC")
  `)
})
```

# --seed--

## --seed-contents--

```py
from abc import ABC, abstractmethod

class Product:
    def __init__(self, name: str, price: float) -> None:
        self.name = name
        self.price = price

    def __str__(self) -> str:
        return f'{self.name} - ${self.price}'
--fcc-editable-region--

--fcc-editable-region--
product = Product('Wireless Mouse', 50.0)
print(product)
```
