...
title: Napravite stranicu za muzičku korpu za kupovinu
challengeType: 25
...
1. Trebalo bi da imate element sa `id` nazvan `shopping-cart-container`.
2. Vaš `#shopping-cart-container` element treba da koristi odgovarajuću utilitarnu klasu za postavljanje elementa u Flexbox raspored.
3. Vaš `#shopping-cart-container` element treba da postavi smer flex stavki na kolonu za manje uređaje i red za veće uređaje. Zapamtite da Tailwind CSS koristi pristup "mobile first" (prvo mobilni) i da ćete koristiti prefiks prekida `lg:` za ciljanje većih uređaja.
4. Unutar vašeg `#shopping-cart-container` elementa, trebalo bi da imate element sa `id` nazvan `products-container`.
5. Vaš `#products-container` element treba da ima najmanje dva elementa-deteta, svaki sa klasom nazvanom `card`.
6. Svaki `.card` element treba da ima sledeće elemente ugnježđene unutra:
   - Element `h2` sa tekstom koji predstavlja naziv proizvoda, i utilitarna klasa koja postavlja veličinu fonta koristeći samo unapred definisane klase veličine kao što su `text-sm`, `text-md`, `text-lg`, `text-xl`, `text-2xl`, itd.
   - Element sa klasom nazvanom `quantity` i tekstom za broj artikala u korpi za taj proizvod.
   - Element sa klasom nazvanom `price` i tekstom za cenu.
   - `button` sa klasom nazvanom `remove-button` i tekstom `Remove`. Vaš `button` treba da ima utilitarne klase za unapred definisanu crvenu pozadinsku boju po izboru i različitu unapred definisanu crvenu pozadinsku boju za hover stanje. Primeri unapred definisanih crvenih pozadinskih boja uključuju `bg-red-500`, `bg-red-600`, itd. Primjeri unapred definisanih crvenih pozadina pri prelizanju mišem (hover) uključuju `hover:bg-red-500`, `hover:bg-red-600`, itd.
7. Unutar vašeg `#shopping-cart-container` elementa, trebalo bi da imate element sa `id` nazvan `order-summary-container`.
8. Vaš `#order-summary-container` element treba da ima sledeće stilove:
   - Utilitarna klasa po izboru za unapred definisane zaoblj
