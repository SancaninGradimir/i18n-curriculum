...
Unutar trećeg stavke liste, kreirajte element `button` sa atributom `aria-expanded` postavljenim na `false`. Takođe, postavite tekst za `button` na `Apps`.

Atribut `aria-expanded` će komunicirati korisnicima čitača ekrana da dugme pokreće proširiv podmeni.
