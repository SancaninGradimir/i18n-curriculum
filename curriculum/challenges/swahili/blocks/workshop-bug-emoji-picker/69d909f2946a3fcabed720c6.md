...
Sada kada je provera `if` postavljena, koristite `e.target.value` da pronađete odgovarajući bug u `bugMap` i pozovete njegovu metodu `render`. Nakon toga, trebalo bi da možete izabrati bugove i odmah videti rezultat.

Time je vaš radioniceljnik za Bug Emoji Picker završen!
