...
Sve do sada, ništa se ne menja kada odaberete drugačiji bug.

Dodajte `change` slušalicu događaja na `selectElement`. U povratnoj funkciji (callback), koristite `isSelect` da proverite da li je `e.target` [code] `HTMLSelectElement`. Još ne morate ništa raditi unutar bloka `if`.
