...
Dohvati element `select` sa ID-om `species` iz DOM-a i sačuvaj ga u konstantu pod nazivom `selectElement`. Koristi operater asertacije ne-nula (non-null assertion operator) `!` da bi potvrdio da element nije `null`.
