...
U klasi `Spider`, kreiraj konstruktor koji prima jedan parametar `emojiElement` tipa `HTMLParagraphElement`.

Unutar konstruktora, pozovi funkciju `super` sa parametrom `emojiElement` da bi se pozvao konstruktor roditeljske klase `Bug`. Zatim, postavi svojstvo `emoji` na string vrednost `"🕷️"`.
