...
Sada ćete početi da se pozivate na potrebne DOM elemente.

Dohvatite paragraf element sa ID-om `bug-emoji` i sačuvajte ga u konstantu pod nazivom `bugEmojiElement`. Koristite operater non-null asertacije `!` da potvrdite da element nije `null`.
