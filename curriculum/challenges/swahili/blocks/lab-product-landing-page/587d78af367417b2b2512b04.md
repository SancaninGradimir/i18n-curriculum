...
title: Izgradite odredišnu stranicu proizvoda
challengeType: 25
...
1. Vaša landing stranica proizvoda treba da ima element ``header`` sa odgovarajućim ``id="header"``.
2. Trebalo bi da imate sliku unutar elementa ``header`` sa odgovarajućim ``id="header-img"`` (Logo bi ovde bio dobra slika).
3. Unutar elementa ``#header``, trebalo bi da imate element ``nav`` sa odgovarajućim ``id="nav-bar"``.
4. Trebalo bi da imate najmanje tri klikabilna elementa unutar elementa ``nav``, svaki sa klasom ``nav-link``.
5. Kada kliknete na dugme ``.nav-link`` u elementu ``nav``, trebalo bi da vas prebaci na odgovarajuću sekciju landing stranice.
6. Trebalo bi da imate ugrađeno video snimanje proizvoda sa ``id="video"``.
7. Vaša landing stranica treba da ima element ``form`` sa odgovarajućim ``id="form"``.
8. Unutar obrasca, trebalo bi da postoji polje ``input`` sa ``id="email"`` gde možete uneti e-mail adresu.
9. Polje za unos ``#email`` treba da ima tekst naslovnice kako bi korisnicima bilo jasno za šta je polje.
10. Polje za unos ``#email`` treba da koristi validaciju HTML5 da potvrdi da je uneti tekst e-mail adresa.
11. Unutar obrasca, trebalo bi da postoji dugme za slanje ``input`` sa odgovarajućim ``id="submit"``.
12. Kada kliknete na element ``#submit``, e-mail treba da se pošalje na statičnu stranicu (koristite ovu mock URL: ``https://www.freecodecamp.org/email-submit``).
13. Navigaciona traka (navbar) uvek mora biti na vrhu vidokruga.
14. Vaša landing stranica proizvoda treba da ima najmanji jedan media upit (media query).
15. Vaša landing stranica proizvoda bi trebalo da koristi CSS flexbox barem jednom.
