---
id: 697f448d81f541e6af994687
title: Vežba 3
challengeType: 19
dashedName: task-3
lang: zh-CN
---
<!-- (Audio) Wang Hua: 九 (jiǔ) -->

# --description--

Wang Hua izgovara broj na kineskom.

# --instructions--

Slušaj zvuk odgovora na pitanje koje se nalazi ispod.

# --questions--

## --text--

Šta broj čuješ u glasu?

## --answers--

`九 (jiǔ)`

[No Swahili text provided.]

`六 (liù)`

### --feedback--

Wang Hua piše broj `六 (liù)`.

[No Swahili text provided.]

`十 (shí)`

### --feedback--

Wang Hua neće navesti broj `十 (shí)`.

[No Swahili text provided.]

`零 (líng)`

### --feedback--

Wang Hua potpisuje broj `零 (líng)`.

## --video-solution--

1

# --explanation--

Wang Hua kaže `九 (jiǔ)`, što odgovara broju 9.

# --scene--

```json
{
  "setup": {
    "background": "company3-reception.png",
    "characters": [
      {
        "character": "Wang Hua",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "ZH_A1_numbers_and_personal_information_numbers_0_10.mp3",
      "startTime": 1,
      "startTimestamp": 18.87,
      "finishTimestamp": 19.74
    }
  },
  "commands": [
    {
      "character": "Wang Hua",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Wang Hua",
      "startTime": 1,
      "finishTime": 1.97,
      "dialogue": {
        "text": "九",
        "align": "center"
      }
    },
    {
      "character": "Wang Hua",
      "opacity": 0,
      "startTime": 2.47
    }
  ]
}
```
