...
Sada dodajte `div` koji ima `className` od `card-stats`. Unutar njega, dodajte `div` koji ima `className` od `stat-col`. Unutar njega, dodajte tri elementa `div`, svaki sa `className` od `stat-row`.

# --hints--

Unapaswa kuongeza `div` yenye `className` ya `card-stats` baada ya `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Trebalo bi da dodate `div` sa `className` unutar `stat-col` u `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Treba da dodate tri elementa za `div`, svaka sa `className` iz `stat-row` unutar `.stat-col`.

```js
assert.lengthOf(document.querySelectorAll('.stat-col .stat-row'), 3);
```
