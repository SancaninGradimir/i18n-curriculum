...
Sada dodaj `div` sa `className` od `card-stats`. Unutra, dodaj `div` sa `className` od `stat-col`. Unutar toga, dodaj tri `div` elementa, od kojih svaki ima `className` od `stat-row`.

# --hints--

You should add a `div` with the `className` of `card-stats` after `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

You should add a `div` with the `className` of `stat-col` inside `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

You should add three `div` elements each with the `className` of `stat-row` inside `.stat-col`.
