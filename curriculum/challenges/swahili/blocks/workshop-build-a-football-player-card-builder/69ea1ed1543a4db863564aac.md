...
Promenite vrednost ključa ``position`` u izvedeni ``Position``.

# --hints--

Your `PlayerData` interface `position` field should use the `Position` type instead of `string`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.interfaces.PlayerData.hasTypeProps({ name: 'position', type: 'Position' }));
```
