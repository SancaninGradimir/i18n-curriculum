...
Sasa utaanza kujenga fomu. Ndani ya `div` yenye `className` ya `form-panel`, ongeza `div` kufunika sehemu ya "Player Info". Ndani ya `div` mpya, ongeza `p` yenye `className` ya `form-section-title` na maandishi `Player Info`.

Nastavićete da dodajete sekcije ovog obrasca u narednim koracima.

# --hints--

`div.form-panel` treba da ima `div` za pokrivanje sekcije Player Info.
...
`div` unutar `div.form-panel` treba da ima `p` sa `className` od `form-section-title` i tekstom `Player Info`.
