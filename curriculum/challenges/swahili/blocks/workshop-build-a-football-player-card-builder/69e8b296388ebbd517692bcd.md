...
Sada ćete početi da gradite obrazac. Unutar `div` sa `className` od `form-panel`, dodajte `div` da obuhvati sekciju "Player Info". Unutar tog novog `div`, dodajte `p` sa `className` od `form-section-title` i tekstom `Player Info`.

Nastavićete da dodajete polja u ovu sekciju u narednim koracima.

# --hints--

Your `div.form-panel` should contain a `div` to wrap the Player Info section.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.form-panel > div'));
```

The `div` inside `div.form-panel` should contain a `p` with the `className` of `form-section-title` and the text `Player Info`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const el = container.querySelector('.form-panel > div p.form-section-title');
assert.exists(el);
assert.strictEqual(el.textContent.trim(), 'Player Info');
```
