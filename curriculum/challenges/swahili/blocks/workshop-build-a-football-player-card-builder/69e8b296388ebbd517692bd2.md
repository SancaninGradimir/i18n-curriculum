...
Sehemu ya Taarifa za Mchezaji sasa imekamilika. Baada ya kufunga `div` ya sehemu ya Taarifa za Mchezaji na bado ndani ya `form-panel`, ongeza `div` mpya. Ndani yake, ongeza kipengele cha `p` chenye `className` ya `form-section-title` na maandishi ya `Player Stats`. Kisha ongeza `div` yenye `className` ya `stats-grid`. Acha gridi ya takwimu kuwa tupu kwa sasa.

# --hints--

`div.form-panel` mora imati `div` drugi deo u sekciji Statistika igrača.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const sections = container.querySelectorAll('.form-panel > div');
assert.isAtLeast(sections.length, 2);
```

Sehemu ya Takwimu za Mchezaji inapaswa kuwa na `p` yenye `className` ya `form-section-title` na maandishi ya `Player Stats`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const titles = container.querySelectorAll('.form-panel p.form-section-title');
const statsTitle = Array.from(titles).find(el => el.textContent.trim() === 'Player Stats');
assert.exists(statsTitle);
```

Sehemu ya Takwimu za Mchezaji inapaswa kuwa na `div` yenye `className` ya `stats-grid`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.form-panel div.stats-grid'));
```
