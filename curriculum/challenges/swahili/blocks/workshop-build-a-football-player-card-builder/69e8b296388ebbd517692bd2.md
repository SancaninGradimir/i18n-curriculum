...
Sekcija informacija o igraču je sada kompletirana. Nakon zatvaranja `div` sekcije informacija o igraču i i dalje unutar `form-panel`, dodajte novi `div`. Unutra, dodajte element `p` sa `className` od `form-section-title` i tekstom `Player Stats`. Zatim dodajte `div` sa `className` od `stats-grid`. Ostavite mrežu statistika praznom za sada.

# --hints--

Your `div.form-panel` should contain a second `div` for the Player Stats section.
...
The Player Stats section should contain a `p` with the `className` of `form-section-title` and the text `Player Stats`.
...
The Player Stats section should contain a `div` with the `className` of `stats-grid`.
