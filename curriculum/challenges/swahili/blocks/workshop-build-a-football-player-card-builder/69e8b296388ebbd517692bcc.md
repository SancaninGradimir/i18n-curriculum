...
Sada obuhvatite `PlayerCard` komponentu u `main` element od `className` od `main`.

Unutar `main` elementa, dodajte `div` od `className` od `layout`. Unutar `layout`, dodajte dva odvojena `div` elementa od `className` od `form-panel` i `preview-panel` respektivno.

# --hints--

Your `FootballPlayerCard` should render a `main` element with the `className` of `main` inside `div.page`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('div.page > main.main'));
```

Your `main.main` should contain a `div` with the `className` of `layout`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('main.main > div.layout'));
```

Your `layout` should contain a `div` with the `className` of `form-panel`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.layout > div.form-panel'));
```

Your `layout` should contain a `div` with the `className` of `preview-panel`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.layout > div.preview-panel'));
```
