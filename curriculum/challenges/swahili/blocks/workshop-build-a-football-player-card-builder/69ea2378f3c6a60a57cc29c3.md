...
Sada iteriraj kroz niz `POSITIONS` koristeći streličnu funkciju koja prima svaku poziciju kao string `pos` i vraća element `option`. Opcija bi trebalo da ima `key` od `pos` i vrednost takođe `pos`. Takođe, prikažite `pos` kao tekst svake generisane `option`.

Slobodno koristite bilo šta kao parametar za vašu map funkciju; ne mora biti `pos`.

# --hints--

Your position `select` should contain `option` elements mapped from the `POSITIONS` array.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const options = container.querySelectorAll('select#position option');
assert.isAtLeast(options.length, 11);
```

Each option's `value` should match its displayed text and be a valid position.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const values = Array.from(container.querySelectorAll('select#position option')).map(option => option.value);
assert.include(values, 'GK');
assert.include(values, 'ST');
assert.include(values, 'CF');
```
