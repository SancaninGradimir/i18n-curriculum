...
Dodajte atribut `value` sa vrednošću `player.sho` na SHO `input`. Zatim postavite njegov atribut `onChange` na streličnu funkciju koja uzima `e` kao parametar i poziva `setPlayer` sa raspršenim vrednostima (spread) od `player` i `sho` postavljene na `Number(e.target.value)`.

# --hints--

Your SHO `input` should have a `value` equal to the current player's `sho` stat.
...
Your SHO `input` should have an `onChange` handler that calls `setPlayer` with a spread of `player` and `sho` set to `Number(e.target.value)`.
