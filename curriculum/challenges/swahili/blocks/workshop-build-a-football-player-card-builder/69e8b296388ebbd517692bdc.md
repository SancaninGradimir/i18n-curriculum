...
Ongeza sifa ya `value` ya `player.sho` kwa SHO `input`. Kisha weka sifa yake ya `onChange` kuwa kitendakazi cha mshale kinachopokea `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `sho` kuwekwa kuwa `Number(e.target.value)`.

# --hints--

SHO `input` yako inapaswa kuwa na `value` sawa na takwimu za `sho` za mchezaji wa sasa.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(Number(container.querySelector('input#sho')?.value), 98);
```

SHO `input` yako inapaswa kuwa na mshughulikiaji wa `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na `sho` kuwekwa kuwa `Number(e.target.value)`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*sho:\s*Number\s*\(\s*e\.target\.value\s*\),?\s*\}\)\s*\}/);
```
