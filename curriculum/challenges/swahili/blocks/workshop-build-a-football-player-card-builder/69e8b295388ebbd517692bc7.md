...
Zameni hardkodirano ime igrača ``PELE`` unutar ``card-name`` sa ``player.name``.

# --hints--

You should replace the hardcoded `PELE` with `player.name` inside `.card-name`.

```js
assert.match(__helpers.removeJSComments(code), /className="card-name">\{player\.name\}/);
```
