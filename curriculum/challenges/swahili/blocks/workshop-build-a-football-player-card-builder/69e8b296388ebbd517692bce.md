...
Sada dodaj `div` sa `className` od `form-group`. Unutar njega, dodaj `label` sa `className` od `label` i atribut `htmlFor` od `name`. Ispod toga dodaj `input` sa `id` postavljenim na `name`, `className` od `input`, i atributom `type` od `text`.
