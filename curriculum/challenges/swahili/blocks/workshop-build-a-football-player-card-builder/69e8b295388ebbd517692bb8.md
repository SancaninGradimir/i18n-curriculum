...
Ispod elementa `div` sa `className` od `card-tier-badge`, dodajte `div` sa `className` od `card-club` i tekst `Santos FC`.

# --hints--

You should add a `div` with the `className` of `card-club` below `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

The `.card-club` element should have the text `Santos FC`.

```js
assert.equal(document.querySelector('.card-club')?.textContent.trim(), 'Santos FC');
```
