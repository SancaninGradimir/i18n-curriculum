...
Unutar `card-stats` diva, nakon prvog `stat-col`, morate dodati još dvoje potomaka.

Prvo, samozatvarajući `div` sa `className` od `stat-divider` da prikaže tanku vertikalnu liniju između dve statističke kolone, i drugi `div` sa `className` od `stat-col` koji sadrži još tri `div` elementa, svaki sa `className` od `stat-row`.

# --hints--

You should add a self-closing `div` with the `className` of `stat-divider` after the first `.stat-col`.

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

You should add a second `div` with the `className` of `stat-col` after `.stat-divider`.

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

The second `.stat-col` should contain three `div` elements each with the `className` of `stat-row`.

```js
const cols = document.querySelectorAll('.card-stats > .stat-col');
assert.lengthOf(cols[1]?.querySelectorAll('.stat-row') ?? [], 3);
```
