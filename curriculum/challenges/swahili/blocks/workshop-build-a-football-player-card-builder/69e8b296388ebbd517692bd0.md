...
Nakon `form-row`, dodajte `div` sa `className` od `form-group`. Unutar njega, dodajte element `label` sa `className` od `label`, `htmlFor` od `club`, i tekst `Club`. Zatim dodajte element za unos sa `id` od `club`, `className` od `input`, i `type` od `text`.
