...
Unutar `stats-grid`, dodajte dva `div` elementa, svaki sa `className` od `form-group`.

Za prvi, dodajte `label` sa `className` od `label`, `htmlFor` od `pac`, i tekst `PAC`, zatim unos sa `id` od `pac`, `className` od `input`, i `type` od `number`.

A za drugi, dodajte `label` sa `htmlFor` od `sho` i tekst `SHO`, zatim unos sa `id` od `sho` i `type` od `number`.
