...
Unutar elementa `preview-panel`, dodajte element `p` od `className` od `preview-label` i tekst `Live Preview`, još jedan element `p` od `className` od `preview-hint` i tekst `Updates as you type`, i `div` sa `className` podešenim na šablonski literališ `preview-box tier-${getPlayerTier(player.overallRating)}`.

Unutar poslednjeg `div`, premestite komponentu `PlayerCard` i njeno svojstvo (prop).
