...
Sada ćete započeti proces menjanja polja za unos u padajući meni.

Badilisha ingizo kuwa kipengele cha `select` chenye kitambulisho cha `position`, `className` ya `input`, na `value` ya `player.position`.

# --hints--

Unapaswa kubadilisha `input` ya nafasi kuwa kipengele cha `select` chenye `id` ya `position` na `className` ya `input`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('select#position.input'));
```

`select` vaš prostor bi trebalo da ima `value` od `player.position`.

```js
assert.match(code, /value={player\.position}/);
```
