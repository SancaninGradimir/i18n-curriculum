...
Sada ćete započeti proces menjanja polja za poziciju u select.

Promenite input u element ``select`` sa id-om ``position``, ``className`` od ``input``, i ``value`` od ``player.position``.

# --hints--

You should change the position `input` to a `select` element with an `id` of `position` and `className` of `input`.
...
Your position `select` should have a `value` of `player.position`.
