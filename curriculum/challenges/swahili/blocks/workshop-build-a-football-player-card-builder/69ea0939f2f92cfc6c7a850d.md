...
Zatim, dodajte `useEffect` hook koji se izvršava svaki put kada stanje `player` promeni. Unutra, koristite strelicastu funkciju i dodajte `try..catch` sa `error` kao parametar za blok `catch`.

# --hints--

You should call `useEffect` inside `FootballPlayerCard`.

```js
const abuseEffect = __helpers.spyOn(React, "useEffect");
...
assert.isAtLeast(abuseEffect.calls.length, 1);
```

Your `useEffect` should have `[player]` as its dependency array.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

Your `useEffect` callback should contain a `try...catch` block with `error` as the parameter of the `catch` block.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\(\(\)\s=>\s*\{\s*try\s*\{\s*\}\s*catch\s*\(error\)\s*\{\s*/);
```
