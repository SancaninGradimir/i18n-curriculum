...
Unutar `card` div-a, dodajte `div` sa `className` od `card-header`. Ovaj deo će sadržati ocenu igrača, poziciju, znak nivoa i ime kluba.

# --hints--

You should add a `div` with the `className` of `card-header` inside the `div` with `className` of `.card`.

```js
assert.exists(document.querySelector('.card > .card-header'));
```
