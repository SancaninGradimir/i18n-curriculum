...
Sada ažuriraj ``FootballPlayerCard`` da prosledi ``defaultPlayer`` u ``PlayerCard`` umesto inline objekta.

# --hints--

You should have `defaultPlayer` as the value of the `player` prop passed to `PlayerCard`.

```js
assert.match(__helpers.removeJSComments(code), /player\s*=\s*\{\s*defaultPlayer\s*\}/);
```
