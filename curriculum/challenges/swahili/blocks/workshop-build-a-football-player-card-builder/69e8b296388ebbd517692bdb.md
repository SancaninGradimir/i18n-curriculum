...
Dodajte `value` atribut od `player.pac` na PAC `input`. Zatim dodajte `onChange` atribut postavljen na strelicastu funkciju koja prima `e` kao svoj parametar i poziva `setPlayer` sa raspršivanjem `player` i `pac` postavljenim na `Number(e.target.value)`.

# --hints--

Your PAC `input` should have a `value` equal to the player's `pac` stat.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(Number(container.querySelector('input#pac')?.value), 97);
```

Your PAC `input` should have an `onChange` handler that calls `setPlayer` with a spread of `player` and `pac` set to `Number(e.target.value)`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*pac:\s*Number\s*\(\s*e\.target\.value\s*\),?\s*\}\)\s*\}/);
```
