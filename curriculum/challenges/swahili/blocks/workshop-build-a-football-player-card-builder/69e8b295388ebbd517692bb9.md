...
Zaglavlje kartice je kompletirano. Sada morate dodati sekciju sa fotografijom igrača.

Nakon zatvaranja `div` u `card-header`, i i dalje unutar `div` sa `className` od `card`, dodajte `div` sa `className` od `card-image-wrap`. Ostavite ga praznim za sada. Sliku ćete dodati u sledećem koraku.

# --hints--

You should add a `div` with the `className` of `card-image-wrap`.

```js
assert.exists(document.querySelector('.card-header + .card-image-wrap'));
```
