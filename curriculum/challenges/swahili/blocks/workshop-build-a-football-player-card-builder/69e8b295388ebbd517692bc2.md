...
U JSX-u i TSX-u, prosleđujete JavaScript izraze kao vrednosti atributa koristeći vitičaste zagrade. Template literali koriste *backtick*-ove i `${}` za interpolaciju. Primer:

```tsx
<div className={`prefix-${someVariable}`}>
```

Omot (wrapper) kartice `className` je trenutno hardkodovan kao `card-wrapper tier-gold`. Zamenite `gold` dinamičkom vrednošću korišćenjem template literala koji poziva `getPlayerTier` sa `player.overallRating`. Ovo će referencirati ispravnu klasu nivoa (tier) iz CSS-a, na osnovu ukupne ocene igrača.

# --hints--

You should use a template literal with `getPlayerTier(player.overallRating)` for the `className` of `.card-wrapper`.
