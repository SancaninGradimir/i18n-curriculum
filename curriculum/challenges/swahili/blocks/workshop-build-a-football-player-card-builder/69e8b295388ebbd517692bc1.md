...
Sada kada je `PlayerData` interfejs definisan, ažurirajte komponentu `PlayerCard` da prihvati prop `player` tipizovan kao `PlayerData` kako bi odgovarao interfejsu `PlayerData`.

Zapamtite da možete dekonstruisati propove na ovaj način:
...
You should update `PlayerCard` to accept a `player` prop typed as `PlayerData`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.allFunctions.PlayerCard.parameters[0].matches("{ player }: { player: PlayerData }"));
```
