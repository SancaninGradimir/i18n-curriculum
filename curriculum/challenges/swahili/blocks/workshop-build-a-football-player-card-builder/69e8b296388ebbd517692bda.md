...
Dodajte atribut `value` sa vrednošću `player.imageUrl` na Image URL `input`. Zatim postavite atribut `onChange` na streličnu funkciju koja uzima `e` kao parametar i poziva `setPlayer` sa raspršenim vrednostima (spread) za `player` i `imageUrl` postavljene na `e.target.value`.

# --hints--

Your Image URL `input` should have a `value` equal to the player's image URL.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(container.querySelector('input#imageUrl')?.value, 'https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg');
```

Your Image URL `input` should have an `onChange` handler that calls `setPlayer` with a spread of `player` and `imageUrl` set to `e.target.value`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*imageUrl:\s*e\.target\.value,?\s*\}\)\s*\}/);
```
