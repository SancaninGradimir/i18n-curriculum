...
Dodajte dva elementa `span` sa `className` i `stat-value` respektivno u svaki `stat-row`.

# --hints--

Each `.stat-row` in the second `.stat-col` should contain a `span` with class `stat-value` and a `span` with class `stat-label`.

```js
const cols = document.querySelectorAll('.card-stats > .stat-col');
cols[1]?.querySelectorAll('.stat-row').forEach(row => {
  assert.exists(row.querySelector('.stat-value'));
  assert.exists(row.querySelector('.stat-label'));
});
```
