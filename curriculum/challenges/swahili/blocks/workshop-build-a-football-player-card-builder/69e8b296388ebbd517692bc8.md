...
Konačne hardkodovane vrednosti su šest statistika igrača. Zamenite prvu od tri skupa hardkodovanih brojeva odgovarajućim svojstvom `player`:
...
You should replace the value `97` for `PAC` with `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

You should replace the value `98` for `SHO` with `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

You should replace the value `83` for `PAS` with `player.pas`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pas\}/);
```
