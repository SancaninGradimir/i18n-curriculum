...
Zatim, kreiraj tip `Position` sa vrednošću `typeof POSITIONS[number]`. Ovo razpakuje niz `POSITIONS` u uniju njegovih pojedinačnih string literala, efektivno ga čineći tako da izgleda kao `"GK" | "CB" | "LB" | ...` i tako dalje.

# --hints--

You should create a type alias called `Position`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.types.Position);
```

Your `Position` type should be set to `typeof POSITIONS[number]`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.types.Position.matches('type Position = typeof POSITIONS[number]'));
```
