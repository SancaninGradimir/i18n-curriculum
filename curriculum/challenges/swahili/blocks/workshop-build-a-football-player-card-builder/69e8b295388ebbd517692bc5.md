...
Zameni hardkodirano ime kluba ``Santos FC`` u ``card-club`` sa ``player.club``.

# --hints--

You should replace the hardcoded `Santos FC` with `player.club` in `.card-club`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.club\}/);
