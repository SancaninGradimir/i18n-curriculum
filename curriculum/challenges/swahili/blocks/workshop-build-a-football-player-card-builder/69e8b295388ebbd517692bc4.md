...
Statusna oznaka i dalje prikazuje hardkodirani tekst `GOLD`. Morate ga zameniti dinamičkim izrazom kako bi se prikazao ispravan nivo.

Pozovite `getPlayerTier()` sa `player.overallRating` da biste dobili niz nivoa, a zatim pozovite `.toUpperCase()` na rezultat kako biste ga prikazali u velikim slovima. Ne zaboravite da ceo izraz obuhvatite vitičnim zagradama.

# --hints--

You should use `getPlayerTier(player.overallRating).toUpperCase()` inside `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

The `.card-tier-badge` element should display `ELITE` for a player with a rating of 98.

```js
assert.equal(document.querySelector('.card-tier-badge')?.textContent.trim(), 'ELITE');
```
