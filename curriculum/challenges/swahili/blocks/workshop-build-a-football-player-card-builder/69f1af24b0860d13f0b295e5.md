...
Sada zamenite preostale hardkodirane brojeve sa sledećim:
...
You should replace the hardcoded `99` for `DRI` value with `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

You should replace the hardcoded `41` for `DEF` value with `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

You should replace the hardcoded `99` for `PHY` value with `player.phy`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.phy\}/);
```
