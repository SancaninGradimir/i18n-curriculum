...
Dodaj još jedan `div` sa `className` od `form-group`. Unutra, dodaj element `label` sa `className` od `label`, element `htmlFor` od `imageUrl`, i tekst `Image URL`. Zatim dodaj element `input` sa `id` od `imageUrl`, elementom `className` od `input`, i `type` od `text`.
