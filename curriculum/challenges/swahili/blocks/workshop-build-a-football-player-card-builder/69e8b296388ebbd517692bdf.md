...
Dodajte `value` i `onChange` atribute na oba ulaza DEF i PHY.

Za DEF, postavite `value` na `player.def`, a `onChange` na strelicastu funkciju koja poziva `setPlayer` sa raspršivanjem (spread) `player` i `def` postavljeno na `Number(e.target.value)`.

Za PHY, postavite `value` na `player.phy`, a `onChange` na strelicastu funkciju koja poziva `setPlayer` sa raspršivanjem (spread) `player` i `phy` postavljeno na `Number(e.target.value)`.

Ovim korakom, svi ulazi su sada povezani na stanje (state), a uživo predgled će se ažurirati za svako polje!

# --hints--

Your DEF `input` should have a `value` equal to the player's `def` stat.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(Number(container.querySelector('input#def')?.value), 41);
```

Your DEF `input` should have an `onChange` handler that calls `setPlayer` with the spread of `player` and `def` set to `Number(e.target.value)`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*def:\s*Number\s*\(\s*e\.target\.value\s*\),?\s*\}\)\s*\}/);
```

Your PHY `input` should have a `value` equal to the player's `phy` stat.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(Number(container.querySelector('input#phy')?.value), 75);
```

Your PHY `input` should have an `onChange` handler that calls `setPlayer` with the spread of `player` and `phy` set to `Number(e.target.value)`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*phy:\s*Number\s*\(\s*e\.target\.value\s*\),?\s*\}\)\s*\}/);
```
