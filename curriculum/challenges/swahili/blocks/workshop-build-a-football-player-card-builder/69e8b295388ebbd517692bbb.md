...
Nakon elementa `card-image-wrap`, dodajte `div` sa `className` od `card-name-strip`. Unutar njega, dodajte `span` sa `className` od `card-name` i tekstom `PELE`.

# --hints--

You should add a `div` with the `className` of `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

You should add a `span` with the `className` of `card-name` inside `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

The `.card-name` span should have the text `PELE`.

```js
assert.equal(document.querySelector('.card-name')?.textContent.trim(), 'PELE');
```
