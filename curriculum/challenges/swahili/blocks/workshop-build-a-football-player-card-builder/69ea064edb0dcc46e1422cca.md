...
Sada ćete dodati logiku za čuvanje u `localStorage` koristeći `useEffect`. 

Pristup ili čuvanje podataka u `localStorage` smatra se sporednim efektom (side effect) u React-u, pa biste trebali koristiti `useEffect` za ovu logiku.

Počnite dekonstruisanjem `useEffect` iz React-a, baš kao što je to urađeno sa `useState`.

# --hints--

You should destructure `useEffect` from `React`.

```js
assert.match(__helpers.removeJSComments(code), /const\s*\{[^}]*useEffect[^}]*\}\s*=\s*React/);
```
