...
Zatim, kreiraj funkciju `loadPlayer` sa `PlayerData` kao tipom povratne vrednosti. Unutar nje, dodaj sintaksu `try...catch` sa `error` kao parametrom bloka `catch`. Ostavi sve ostalo prazno za sada.

# --hints--

You should create a function called `loadPlayer`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.allFunctions.loadPlayer);
```

Your `loadPlayer` function should have a return type of `PlayerData`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.allFunctions.loadPlayer.hasReturnAnnotation('PlayerData'));
```

Your `loadPlayer` function should contain a `try...catch` block.

```js
assert.match(__helpers.removeJSComments(code), /try\s*\{\s*\}\s*catch\s*\(error\)\s*\{\s*/);
```
