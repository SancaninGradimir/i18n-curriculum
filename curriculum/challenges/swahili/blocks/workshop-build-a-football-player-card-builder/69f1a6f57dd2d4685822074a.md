...
Dodajte dva elementa `span` sa `className` i `stat-value` respektivno u svaki `div.stat-row`.

# --hints--

Each `.stat-row` should contain a `span` with `className` of `stat-value` and a `span` with `className` of `stat-label`.
