...
U bloku `catch`, logujte string `Failed to load player data, using defaults:` praćen parametrima `error` na konzolu.

Zatim izvan `catch`, ali i dalje unutar funkcije `loadPlayer`, vratite `defaultPlayer`.

Sve ovo bi učitalo podatke igrača, popunilo ih u UI i učitalo objekat `defaultPlayer` ako proces ne uspe.

# --hints--

Your `catch` block should log `'Failed to load player data, using defaults:', error` to the console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

Your `loadPlayer` function should return `defaultPlayer` as the fallback value.

```js
assert.match(__helpers.removeJSComments(code), /return\s+defaultPlayer\b/);
```
