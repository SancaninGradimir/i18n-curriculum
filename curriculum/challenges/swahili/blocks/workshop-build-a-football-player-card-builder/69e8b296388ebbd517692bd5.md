...
Nakon DRI `form-group`, dodajte još dve komponente od `div`, svaka sa `className` od `form-group` unutar `stats-grid`, jedno za DEF i drugo za PHY.

Za deo DEF, dodaj `label` koji ima `htmlFor` iz `def` u tekst od `DEF`, zatim unos sa `id` iz `def` i `type` iz `number`.

Na kwa ile ya PHY, ongeza lebo yenye `htmlFor` ya `phy` na maandishi ya `PHY`, kisha ingizo lenye `id` ya `phy` na `type` ya `number`.

# --hints--

Treba da dodate još jedan set od dve komponente za `div`, svaka sa `className` od `form-group` unutar `div` od `className` od `stats-grid`.
...
Kipengele chako cha tano cha `div` chenye `className` ya `form-group` kinapaswa kuwa na `label` yenye sifa ya kipengele `htmlFor` ya `def`, `className` ya `label`, na maandishi ya `DEF`.
...
Tvoj peti element od `div` sa `className` od `form-group` treba da ima `input` sa `className` od `input`, `id` od `def` i `type` od `number`.
...
Kipengele chako cha sita cha `div` chenye `className` ya `form-group` kinapaswa kuwa na `label` yenye sifa ya kipengele `htmlFor` ya `phy`, `className` ya `label`, na maandishi ya `PHY`.
...
Vaš šesti element od `div` koji ima `className` od `form-group` bi trebalo da ima `input` sa `className` od `input`, `id` od `phy` i `type` od `number`.
