...
Nakon DRI `form-group`, dodajte još dva `div` elementa, svaki sa `className` od `form-group` unutar `stats-grid`, jedno za DEF, a drugo za PHY.

Za DEF, dodajte `label` sa `htmlFor` od `def` i tekstom `DEF`, zatim unos (input) sa `id` od `def` i `type` od `number`.

A za PHY, dodajte labelu sa `htmlFor` od `phy` i tekstom `PHY`, zatim unos (input) sa `id` od `phy` i `type` od `number`.
