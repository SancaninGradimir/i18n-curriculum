...
Ongeza sifa ya kipengele ya `value` ya `player.pas` kwa PAS `input`. Kisha ongeza sifa ya kipengele ya `onChange` iliyowekwa kuwa kitendakazi cha mshale kinachochukua `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `pas` iliyowekwa kuwa `Number(e.target.value)`.

# --hints--

PAS `input` tvoj treba da ima `value` jednako kao statistika za `pas` igrača.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(Number(container.querySelector('input#pas')?.value), 83);
```

PAS `input` tvoj mora imati handler za `onChange` koji poziva `setPlayer` za širenje `player` i `pas`, što je postavljeno na `Number(e.target.value)`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*pas:\s*Number\s*\(\s*e\.target\.value\s*\),?\s*\}\)\s*\}/);
```
