...
Dodajte atribut `value` sa vrednošću `player.pas` na PAS `input`. Zatim dodajte atribut `onChange` postavljen na strelicastu funkciju koja uzima `e` kao svoj parametar i poziva `setPlayer` sa raspršenim vrednostima (spread) od `player` i `pas` postavljene na `Number(e.target.value)`.

# --hints--

Your PAS `input` should have a `value` equal to the player's `pas` stat.
...
Your PAS `input` should have an `onChange` handler that calls `setPlayer` with a spread of `player` and `pas` set to `Number(e.target.value)`.
