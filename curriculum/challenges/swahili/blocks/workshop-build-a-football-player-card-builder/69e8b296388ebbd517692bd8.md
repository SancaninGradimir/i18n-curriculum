...
Dodaj svojstvo atributa `value` za `player.overallRating` ukupno za `input`. Nakon toga, dodaj svojstvo atributa `onChange` postavljeno kao strelica funkcija koja prima `e` kao parametar i poziva `setPlayer` za raspršivanje `player` i postavljeno na `overallRating` da bude `Number(e.target.value)`.

Pošto je `overallRating` broj, koristi `Number()` za modifikovanje vrednosti niza znakova iz unosa.

# --hints--

`input` Vaš ukupan treba da ima `value` jednak opštem nivou trenutnog igrača.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(Number(container.querySelector('input#overallRating')?.value), 98);
```

`input` yako ya jumla inapaswa kuwa na sifa ya kipengele ya `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

`onChange` od vašeg ukupnog `input` mora da izvrši poziv za `setPlayer` radi širenja `player` i `overallRating`, postavljen na `Number(e.target.value)`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*overallRating:\s*Number\s*\(\s*e\.target\.value\s*\),?\s*\}\)\s*\}/);
```
