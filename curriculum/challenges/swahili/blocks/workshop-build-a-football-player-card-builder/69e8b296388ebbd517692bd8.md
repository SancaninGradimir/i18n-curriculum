...
Dodajte atribut `value` sa vrednošću `player.overallRating` na ukupan `input`. Nakon toga, dodajte atribut `onChange` postavljen na strelicastu funkciju koja uzima `e` kao svoj parametar i poziva `setPlayer` sa raspršivanjem (spread) od `player` i `overallRating` postavljeno na `Number(e.target.value)`.

Pošto je `overallRating` broj, koristite `Number()` da konvertujete string vrednost iz unosa.

# --hints--

Your Overall `input` should have a `value` equal to the current player overall rating.
...
Your Overall `input` should have an `onChange` attribute.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

The `onChange` of your Overall `input` should call `setPlayer` with a spread of `player` and `overallRating` set to `Number(e.target.value)`.
