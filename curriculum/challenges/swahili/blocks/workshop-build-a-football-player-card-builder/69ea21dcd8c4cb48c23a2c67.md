...
Dodajte atribut ``onChange`` postavljen na streličnu funkciju koja uzima ``e`` kao svoj parametar i poziva ``setPlayer`` sa raspršivanjem (spread) ``player`` i ``position`` postavljenim na ``e.target.value``, kastovano kao ``Position``.

# --hints--

Your position `select` should have an `onChange` handler that calls `setPlayer` with a spread of `player` and updates `position`.

```js
assert.match(__helpers.removeJSComments(code), /onChange.*setPlayer.*\.\.\.\s*player[\s\S]*position/);
```
