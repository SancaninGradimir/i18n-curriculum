...
Fotografija igrača se učitava sa hardkodiranog URL-a. Zamenite vrednost atributa `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` sa `player.imageUrl`.

Takođe zamenite hardkodirani tekst `alt` dinamičkom vrednošću koristeći šablon literala sa `player.name` i tekstom `headshot`.

Napomena da kada koristite JavaScript izraz kao vrednost atributa u TSX-u, koristite vitičaste zagrade `{}` umesto navodnika. Na primer `src={player.imageUrl}`, a ne `src="{player.imageUrl}"`.

# --hints--

You should replace the hardcoded image URL with `player.imageUrl` as the `src` attribute.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

You should replace the hardcoded `alt` text with a template literal using `player.name` and the text `headshot`.

```js
assert.match(__helpers.removeJSComments(code), /alt=\{`\$\{player\.name\} headshot`\}/);
```
