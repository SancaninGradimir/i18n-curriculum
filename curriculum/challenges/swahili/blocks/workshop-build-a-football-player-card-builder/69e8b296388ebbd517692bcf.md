...
Nastavite dodavanjem `div` od `className` od `form-row`. Unutar njega, dodajte dva `div` elementa sa `className` od `form-group` za svaki.

Prvi `div` element treba da ima `label` od `className` od `label`, atribut `htmlFor` od `position` sa tekstom `Position`, i `input` sa `id` od `position`, `className` od `input` i `type` od `text`.

# --hints--

Your Player Info section should contain a `div` with the `className` of `form-row`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.form-panel div.form-row'));
```

Your `div.form-row` should contain two `div` elements each with the `className` of `form-group`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const groups = container.querySelectorAll('.form-row > div.form-group');
assert.lengthOf(groups, 2);
```

Your first `form-group` in the row should have a `label` with the `className` of `label` and an `htmlFor` of `position`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const label = container.querySelector('.form-row .form-group label.label');
assert.exists(label);
assert.equal(label.htmlFor, 'position');
```

Your position `label` should have the text `Position`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const label = container.querySelector('.form-row .form-group label.label');
assert.include(label?.textContent.trim(), 'Position');
```

Your first `form-group` in the row should have an `input` with the `className` of `input`, an `id` of `position` and `type` of `text`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('input#position.input[type="text"]'));
```
