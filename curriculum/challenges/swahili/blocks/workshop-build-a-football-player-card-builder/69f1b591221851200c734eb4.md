...
Prelazak na drugi `div` element, dodajte `label` od `className` od `label`, jedan `htmlFor` od `overallRating` tekst od `Overall`, i `input` sa `id` podešen na `overallRating`, `className` od od `input` i tip od `number`.

# --hints--

Your second `form-group` in the row should have a `label` with the `className` of `label` and an `htmlFor` of `overallRating`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const labels = container.querySelectorAll('.form-row .form-group label.label');
const label = labels[1];
assert.exists(label);
assert.equal(label.htmlFor, 'overallRating');
```

Your overall rating `label` should have the text `Overall`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const labels = container.querySelectorAll('.form-row .form-group label.label');
assert.include(labels[1]?.textContent.trim(), 'Overall');
```

Your second `form-group` in the row should have an `input` with an `id` of `overallRating` and a `type` of `number`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('input#overallRating.input[type="number"]'))
```
