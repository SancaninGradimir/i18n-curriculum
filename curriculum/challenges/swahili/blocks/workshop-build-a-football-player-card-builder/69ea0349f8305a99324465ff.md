...
U `try` bloku, kreirajte varijablu `saved`. Za vrednost, koristite metod `getItem()` da dohvatite `STORAGE_KEY`. Pravde ispod te varijable, kreirajte izjavu `if` sa uslovom `saved`. Ostavite `if` blok prazan za sada.

# --hints--

Your `loadPlayer` function should call `localStorage.getItem` with `STORAGE_KEY` inside the `try` block.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

The result of `localStorage.getItem` should be stored in a variable called `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

The `try` block should have an `if` statement with the condition `saved`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*saved\s*\)/);
```
