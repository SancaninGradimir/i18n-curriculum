...
Problem je sada u tome što se sve što dodate za karticu igrača gubi kada se pregled osvježi ili ponovo učitate stranicu.

Čuvanje svih ulaznih podataka u `localStorage` zaustit će ovo. Da biste počeli s tim, kreirajte konstantu `STORAGE_KEY` sa stringom `football_player_card` kao vrednošću.

# --hints--

You should declare a `STORAGE_KEY` variable.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.variables.STORAGE_KEY);
```

Your `STORAGE_KEY` should have the string `football_player_card` as its value.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.variables.STORAGE_KEY.value.matches("'football_player_card'"));
```
