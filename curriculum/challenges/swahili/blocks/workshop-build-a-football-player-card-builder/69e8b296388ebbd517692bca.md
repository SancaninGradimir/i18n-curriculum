...
`FootballPlayerCard` kwa sasa inaonyesha kadi yenye `defaultPlayer` iliyowekwa. Ili kufanya programu iwe ya mwingiliano, unahitaji kusimamia data ya mchezaji katika hali ya React ili masasisho yaweze kusababisha kuonyeshwa upya.

Ndani ya sehemu ya `FootballPlayerCard`, tengeneza kigezo cha hali kinachoitwa `player` pamoja na mabadilishaji `setPlayer` ambacho kitashikilia data ya mchezaji wa sasa. Anzisha hali hii kwa `defaultPlayer`, na bainisha kuwa hali hii inatumia aina ya `PlayerData` kama generic ili TypeScript ijue muundo wa kitu cha mchezaji unapaswa kuwa.

Baada ya hapo, sasisha kile kinachorudiwa ili kupitisha kigezo cha hali cha `player` kwa `PlayerCard`.

# --hints--

Unapaswa kutumia ufumbuzi wa muundo wa safu ya data kuweka kigezo cha hali cha `player` na mabadilishaji `setPlayer`.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

`player` na `setPlayer` zako zinapaswa kutumia hook ya `useState`.
...
`useState` yako inapaswa kuanzishwa na `defaultPlayer` na kuwa na `PlayerData` kama aina yake ya generic.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Unapaswa kuwa na `player` kama thamani ya kigezo cha `player` kinachopitishwa kwa `PlayerCard`.
