...
`FootballPlayerCard` trenutno renderuje karticu sa fiksiranim `defaultPlayer`. Da bi aplikacija bila interaktivna, morate upravljati podacima igrača u React stanju (state) kako bi ažuriranja mogla pokrenuti ponovno renderovanje.

Unutar komponente `FootballPlayerCard`, kreirajte varijablu stanja (state) nazvanu `player` sa seterom `setPlayer` koja će držati trenutne podatke igrača. Inicijalizujte ovo stanje sa `defaultPlayer`, i navedite da stanje koristi tip `PlayerData` kao generički (generic) kako bi TypeScript znao kakav oblik treba imati objekat igrača.

Nakon toga, ažurirajte `return` da prosledite varijablu stanja `player` kao prop komponenti `PlayerCard`.

# --hints--

You should use array destructuring to set a `player` state variable and a `setPlayer` setter.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

Your `player` and `setPlayer` should use the `useState` hook.
...
Your `useState` should be initialized with `defaultPlayer` and have `PlayerData` as its generic type.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

You should have `player` as the value of the `player` prop passed to `PlayerCard`.
