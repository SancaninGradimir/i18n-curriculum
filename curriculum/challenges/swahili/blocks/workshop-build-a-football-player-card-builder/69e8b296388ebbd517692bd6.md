...
Forma je sada kompletirana. Da biste omogućili da se karta pregleda ažurira dok kucate, morate povezati ulaze na stanje `player`. 

Počevši od imena Name `input`, dodajte mu atribut `value` sa vrednošću `player.name`. Zatim dodajte atribut `onChange` postavljen na strelicastu funkciju koja uzima `e` kao svoj parametar i poziva `setPlayer` sa spread-om od `player` i `name` podešenim na `e.target.value`. Ne zaboravite da stavite kako spread, tako i podešavanje vrednosti `name` u objekat.

# --hints--

Your name input `value` should be bound to the player state `name` field.
...
assert.exists(input);
assert.strictEqual(input.value, 'PELE');
```

Your name input should have an `onChange` handler.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

The `onChange` handler should call `setPlayer` with `setPlayer` having an object that spreads `player` and sets `name` to `e.target.value`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*name:\s*e\.target\.value\s*\}\)\s*\}/);
```
