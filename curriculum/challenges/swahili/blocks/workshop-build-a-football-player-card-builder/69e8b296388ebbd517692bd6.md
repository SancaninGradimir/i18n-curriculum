...
Fomu sasa imekamilika. Ili kadi ya onyesho ibadilike unapoandika, unahitaji kuunganisha v-ingizo na hali ya `player`.

Kuanzia na Jina `input`, ongeza sifa ya `value` ya `player.name` kwake. Kisha ongeza sifa ya `onChange` iliyowekwa kwa kitendakazi cha mshale kinachochukua `e` kama kigezo chake na kuita `setPlayer` kwa kueneza `player` na `name` kuwekwa kuwa `e.target.value`. Usisahau kuweka kueneza na kuweka thamani ya `name` ndani ya kitu.

# --hints--

Ingizo lako la jina `value` linapaswa kuunganishwa na uwanja wa hali ya mchezaji `name`.
...
Ingizo lako la jina linapaswa kuwa na mshughulikiaji wa `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Mshughulikiaji wa `onChange` unapaswa kuita `setPlayer` na `setPlayer` ikiwa na kitu kinachoeneza `player` na kuweka `name` kuwa `e.target.value`.
