...
Zapamtite iz JSX-a da vam vitičaste zagrade omogućavaju ugradnju bilo kog JavaScript izraza direktno unutar JSX-a. Na ovaj način prikazujete dinamičke podatke iz propova. Isto radite i u TSX-u.

Zaglavlje kartice i dalje prikazuje hardkodovane vrednosti. Zamenite tekst `98` unutar `card-rating` sa `player.overallRating`, i zamenite `ST` unutar `card-position` sa `player.position`.

# --hints--

You should replace the hardcoded `98` with `player.overallRating` in `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

You should replace the hardcoded `ST` with `player.position` in `.card-position`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.position\}/);
```
