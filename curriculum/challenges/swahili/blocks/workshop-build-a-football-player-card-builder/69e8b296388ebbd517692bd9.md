...
Dodajte atribut `value` od `player.club` na Klub `input`. Nakon toga, dodajte atribut `onChange` postavljen na streličnu funkciju koja uzima `e` kao svoj parametar i poziva `setPlayer` sa raspršivanjem od `player` i `club` podešenog na `e.target.value`.

# --hints--

Your Club `input` should have a `value` equal to the player's club.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.equal(container.querySelector('input#club')?.value, 'Santos FC');
```

Your Club `input` should have an `onChange` handler that calls `setPlayer` with a spread of `player` and `club` set to `e.target.value`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{\(e\)\s*=>\s*setPlayer\s*\(\{\s*\.\.\.player\s*,\s*club:\s*e\.target\.value\s*\}\)\s*\}/);
```
