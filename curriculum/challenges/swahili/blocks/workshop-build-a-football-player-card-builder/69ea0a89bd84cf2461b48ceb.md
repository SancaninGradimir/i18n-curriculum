...
Unutar bloka `try`, sačuvajte trenutne podatke igrača u `localStorage` pozivom funkcije `localStorage.setItem` sa ključem za skladištenje i stringifikovanim objektom igrača.

I u bloku `catch`, logujte string `Failed to save player data:` praćen parametrom `error` na konzolu.

# --hints--

Your `useEffect` try block should call `localStorage.setItem` with `STORAGE_KEY` and the stringified `player` object.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Your `useEffect` catch block should log `'Failed to save player data:'` and the `error` to the console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to save player data:['"`],\s*error\)/);
```
