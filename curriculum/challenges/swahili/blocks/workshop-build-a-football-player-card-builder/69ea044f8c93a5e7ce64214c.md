...
U bloku izjave `if`, vratite objekat koji raspršuje oba `defaultPlayer` i `JSON.parse(saved)` odvojeno. Uverite se da su oba odvojena zarezom, jer su to dva odvojena objekta koji se spajaju u jedan.

# --hints--

Your `if (saved)` block should return an object that spreads both `defaultPlayer` and the parsed saved data.

```js
assert.match(__helpers.removeJSComments(code), /return\s*\{\s*\.\.\.\s*defaultPlayer\s*,\s*\.\.\.\s*JSON\.parse\s*\(\s*saved\s*\)/);
```
