...
Dodajte još dva komponenta `div`, svaki sa `className` od `form-group` unutar `stats-grid`, jedno za PAS i drugo za DRI.

Kwa ile ya PAS, ongeza `label` yenye `htmlFor` ya `pas` na maandishi ya `PAS`, kisha ingizo lenye `id` ya `pas` na `type` ya `number`.

I za DRI, dodaj `label` koji ima `htmlFor` od `dri` i tekst od `DRI`, zatim unos sa `id` od `dri` i `type` od `number`.

# --hints--

Trebalo bi da dodate još dva elementa `div`, svaki sa `className` od `form-group` unutar `div` od `className` od `stats-grid`.
...
Kipengele chako cha tatu cha `div` chenye `className` ya `form-group` kinapaswa kuwa na `label` yenye sifa ya kipengele `htmlFor` ya `pas`, `className` ya `label`, na maandishi ya `PAS`.
...
Treći element vašeg `div` koji ima `className` od `form-group` mora da sadrži `input` sa `className` od `input`, `id` od `pas` i `type` od `number`.
...
Četvrti vaš element od `div`, koji ima `className` od `form-group`, trebalo bi imati `label` sa svojstvom elementa `htmlFor` od `dri`, `className` od `label` i tekst za `DRI`.
...
Kipengele chako cha nne cha `div` chenye `className` ya `form-group` kinapaswa kuwa na `input` yenye `className` ya `input`, `id` ya `dri` na `type` ya `number`.
