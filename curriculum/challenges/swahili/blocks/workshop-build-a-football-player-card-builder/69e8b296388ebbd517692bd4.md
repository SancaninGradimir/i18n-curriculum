...
Dodajte još dva `div` elementa, svaki sa `className` od `form-group` unutar `stats-grid`, jedan za PAS i drugi za DRI.

Za PAS, dodajte `label` sa `htmlFor` od `pas` i tekstom `PAS`, zatim unos sa `id` od `pas` i `type` od `number`.

I za DRI, dodajte `label` sa `htmlFor` od `dri` i tekstom `DRI`, zatim unos sa `id` od `dri` i `type` od `number`.
