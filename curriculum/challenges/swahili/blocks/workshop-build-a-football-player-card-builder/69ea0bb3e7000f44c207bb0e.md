...
Da bi funkcija localStorage konačno radila, promenite inicijalizaciju stanja sa `defaultPlayer` na `loadPlayer`. Ovo osigurava da stanje počinje sa svim dostupnim sačuvanim podacima.

Sada testirajte funkciju lokalnog skladišta (localstorage). Sve što unesete trebalo bi da ostane nakon osvežavanja pregledača ili ponovnog učitavanja stranice.

Takođe možete promeniti link slike `src` na `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` da biste videli kako se slika menja u realnom vremenu. Samo se pobrinite da promenite ime igrača na Maradonu.

Time je vaš projekat za kreiranje kartica fudbalskih igrača završen!

# --hints--

Your `useState` should be initialized with `loadPlayer` instead of `defaultPlayer`.

```js
assert.match(__helpers.removeJSComments(code), /useState\s*<\s*PlayerData\s*>\s*\(\s*loadPlayer\b/);
```
