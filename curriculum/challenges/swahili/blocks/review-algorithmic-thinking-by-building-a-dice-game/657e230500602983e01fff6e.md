...
# --description--
Za poslednji deo igre, biće vam potrebno da kreirate algoritam koji proverava prisustvo straight-a. Mali straight se dešava kada četiri od kockica imaju uzastopne vrednosti u bilo kom redosledu (Primer: kod bacanja `41423`, imamo `1234`) što rezultira bodom od `30` poena. Veliki straight se dešava kada sve pet kockica imaju uzastopne vrednosti u bilo kom redosledu (Primer: kod bacanja `35124`, imamo `12345`) što rezultira bodom od `40` poena.

Napišite funkciju `checkForStraights` koja prima niz brojeva. Ako korisnik dobije veliki niz (straight), ažurirajte petu radio dugme sa bodom `40`. Ako korisnik dobije mali niz, ažurirajte četvrto radio dugme sa bodom `30`. U svakom slučaju, ona uvek treba da ažurira poslednje radio dugme kako bi prikazala bod 0, uz odgovarajuće atribute.

Pozovite svoju funkciju ``checkForStraights`` kada se klikne na ``rollDiceBtn`` kako biste završili svoju igru kockama!
# --hints--
Vaša varijabla ``checkForStraights`` bi trebalo da bude funkcija.

```js
assert.isFunction(checkForStraights);
```

Ako se razvije mali pravac, vaša funkcija `checkForStraights` treba da omogući četvrti radio dugme, postavi vrednost na `30`, ažurira prikazani tekst na `, score = 30` i ostavi peto radio dugme onemogućeno.
...
Ako se razvije duga prava linija, vaša funkcija `checkForStraights` bi trebalo da omogući četvrti dugme, postavi vrednost na `30`, i ažurira prikazani tekst na `, score = 30`. Pored toga, funkcija bi trebalo da omogući peto radio dugme, postavi vrednost na `40`, i ažurira prikazani tekst na `, score = 40`.
...
Ako se ne baci prava sekvenca, vaša funkcija ``checkForStraights`` ne bi trebalo da omogući četvrto ili peto radio dugme.
...
Ako se ne izvlači straight, vaša funkcija `checkForStraights` bi trebalo da omogući konačno radio dugme i postavi mu vrednost na `0`, te ažurira prikazani tekst na `, score = 0`.
...
Treba da pozovete funkciju ``checkForStraights`` kada se klikne na ``rollDiceBtn``.
