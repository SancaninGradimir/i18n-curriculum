...
# --description--
Napokon, trebalo bi da uredite svoj kod malo refaktoringom! Možda ćete primetiti da vaš kod omogućava poslednje radio dugme ("None of the Above") više puta unutar funkcija `getHighestDuplicates`, `detectFullHouse` i `checkForStraights`. Refaktorišite svoj kod tako da umesto omogućavanja poslednjeg radio dugmeta na kraju svake funkcije, to se radi samo jednom unutar `rollDiceBtn`.
# --hints--
Vaša funkcija ``getHighestDuplicates`` ne bi trebalo da omogući konačno radijusno dugme, niti da postavi konačni tekst ``span``, bez obzira na to da li su generisani duplikati.
...
Vaša funkcija ``detectFullHouse`` ne bi trebalo da omogući poslednje radio dugme, niti da postavi tekst za ``span``, bez obzira na to da li je izvučen full house.
...
Vaša funkcija ``checkForStraights`` ne bi trebalo da omogući konačno radio dugme niti da postavi tekst za konačni ``span``, bez obzira na to da li je bacena prava (straight).
...
Trebalo bi da omogućite konačno radio dugme, postavite vrednost na `0`, i ažurirate prikazani tekst na `, score = 0` kada se klikne na vaš `rollDiceBtn`.
