...
Ako nije dostavljen argument `gradientDirection` funkciji `linear-gradient`, ona po podrazumevanju raspoređuje boje od vrha do dna ili duž linije od 180 stepeni.

Očistite svoj kod još malo uklanjanjem argumenta `gradientDirection` iz obe funkcije `linear-gradient`.
