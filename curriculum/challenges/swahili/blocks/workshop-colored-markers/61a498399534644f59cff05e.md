...
Kao što vidite, funkcija `linear-gradient` proizvela je glatki crveno-zeleni gradijent. Iako funkcija `linear-gradient` zahteva najmanje dva argumenta boje da bi radila, može prihvatiti mnogo argumenata boja.

Koristite funkciju `rgb` da dodate čistu plavu boju kao treći argument boje funkciji `linear-gradient`.
