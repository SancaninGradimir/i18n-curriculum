...
Senke za vaše zelene i plave markere će imati istu poziciju, zamućenje (blur) i raspršivanje (spread). Jedina razlika će biti boje.

U CSS pravilima ``.green`` i ``.blue``, ažurirajte vrednosti za svojstva ``box-shadow`` tako da je ``offsetX`` ``0``, ``offsetY`` je ``0``, ``blurRadius`` je ``20px``, a ``spreadRadius`` je ``0``. Ostavite boje kao ``green`` i ``blue`` za sada.
