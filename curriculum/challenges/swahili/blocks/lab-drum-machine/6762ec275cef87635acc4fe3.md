...
title: Izgraditi tamburinsku mašinu
challengeType: 25
...
1. Trebalo bi da imate element `div` sa `id` od `drum-machine` koji sadrži sve ostale elemente.

2. Unutar elementa `#drum-machine` trebalo bi da imate još jedan `div` sa `id` od `pad-bank`.

3. Unutar elementa `#drum-machine` trebalo bi da imate element `p` sa `id` od `display`.

4. Unutar vašeg elementa `#pad-bank` trebalo bi da imate devet klikabilnih elemenata za bubnjeve, svaki sa klasom `drum-pad`, jedinstvenim `id` koji opisuje audio klip na koji će se postaviti aktiviranje elementa za bubnjeve, i unutrašnjim tekstom koji odgovara jednom od sledećih tastera na tastaturi: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`. Elementi za bubnjeve MORAJU biti u ovom redosledu.

5. Svaki `.drum-pad` treba da ima element `audio` koji ima klasu `clip`, atribut `src` koji ukazuje na audio klip, i `id` koji odgovara unutrašnjem tekstu njegovog roditeljskog elementa `.drum-pad` (npr. `id="Q"`, `id="W"`, `id="E"` itd.).

6. Kada kliknete na element `.drum-pad`, audio klip sadržan u njegovom elementu za audio treba da se aktivira.

7. Kada pritisnete taster za aktiviranje povezan sa svakim `.drum-pad`, audio klip sadržan u njegovom elementu za audio treba da se aktivira (npr. pritiskanje tastera `Q` trebalo bi da aktivira element za bubnjeve koji sadrži string `Q`, pritiskanje tastera `W` trebalo bi da aktivira element za bubnjeve koji sadrži string `W`, itd.).

8. Kada se aktivira `.drum-pad`, trebalo bi da prikažete string koji opisuje povezani audio klip kao unutrašnji tekst elementa `#display` (svaki string mora biti jedinstven).

Neki audio uzorci koje možete koristiti za svoju bubnjevu mašinu mogu se pronaći na `https://cdn.freecodecamp.org/curriculum/drum/<fileName>`, gde je `<fileName>` kao što sledi:
