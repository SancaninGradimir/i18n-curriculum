...
title: Izgradite konverter valuta
challengeType: 25
...
1. Vaš `CurrencyConverter` komponenta treba da prikaže element `input` za unos iznosa koji se konvertuje iz.
2. Vaš `input` element treba da prima brojeve.
3. Vaš `CurrencyConverter` komponenta treba da prikaže dva `select` elementa za odabir valute iz koje se konvertuje **iz** i u koju se konvertuje.**
4. Vaš `select` element treba da uključuje opcije za **najmanje** `USD`, `EUR`, `GBP` i `JPY`. Možete koristiti bilo koji kurs razmene, pod uslovom da ne postoji jedinstveno mapiranje između valuta. Evo nekoliko primera dobrih i loših mapiranja:
...
5. Vaš `CurrencyConverter` komponenta treba da memorizuje izračunavanje konvertovanih iznosa za valutu **iz** tako da promena u opciji **do** `select` neće ponovo izračunati konvertovane iznose.
6. Vaš `CurrencyConverter` komponenta treba da prikaže element koji prikazuje konvertovani iznos u formatu `XX.XX CCC`, gde je `XX.XX` konvertovani iznos, a `CCC` je kod valute.
7. Konvertovani iznos treba da bude zaokružen na dve decimale.
