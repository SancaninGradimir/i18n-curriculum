...
Skrolujte dole do elementa `footer`, postavite sav tekst da ima vrednost od `14px` za veličinu fonta.

# --hints--

Trebalo bi da imaš birač za `footer`.
...
Tvoj selektor za `footer` treba da bude ispod vaših komentara.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Trebalo bi da postaviš atribut `font-size` na `14px`.
...
Vaš deo za `footer` mora imati `font-size` od `14px`.
