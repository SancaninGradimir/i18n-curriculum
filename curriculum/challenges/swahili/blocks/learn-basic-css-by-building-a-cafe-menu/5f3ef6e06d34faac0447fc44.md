...
Neka ``Est. 2020`` bude niz znakova za petlju kako bi se kreirao selektor klase objekata ``established`` i mu dodeljena svojstvo ``font-style`` sa vrednošću ``italic``.

# --hints--

Trebalo bi da imaš birač za `.established`.
...
Trebalo bi da postaviš atribut `font-style` na `italic`.
...
Tvoj selektor za `.established` treba da postavi svojstvo `font-style` na `italic`.
