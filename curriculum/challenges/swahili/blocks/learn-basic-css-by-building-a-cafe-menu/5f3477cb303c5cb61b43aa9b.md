...
Tekst je ponovo centriran, pa veza sa fajlom CSS funkcioniše. Dodajte drugi stil u fajl koji menja svojstvo od `background-color` na `brown` za element `body`.
...
Trebalo bi da postaviš atribut `background-color` na `brown`.
...
Vaš element za `body` trebao bi imati temu od `brown`.
