...
Slika koju ste dodali nije horizontalno centrirana kao zaglavlje sekcije `Coffee` koje je iznad nje. Elementi `img` su "kao" elementi jedne linije.

Da bi slika funkcionisala kao elementi na nivou bloka, poput naslova sekcija, kreiraj selektor tipa `img`. Postavi atribut `display` na `block`, i koristi atribute `margin-left` i `margin-right` da ga centriraju ravnomerno.

# --hints--

Trebalo bi da koristiš alat `img`.
...
Vaš atribut `img` mora imati `margin-left` i `margin-right` od `auto`.
