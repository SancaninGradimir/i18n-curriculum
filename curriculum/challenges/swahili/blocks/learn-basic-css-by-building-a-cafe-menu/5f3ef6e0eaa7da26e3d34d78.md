...
Možete dodati malo prostora između vašeg menija i sadržaja, kao i po stranama, koristeći različita svojstva za `padding`.

Klasa od `menu` `padding-left` na `padding-right` sa istom vrednošću kao `20px`.

# --hints--

Trebalo bi da postaviš vrednost `padding-left` na `20px`.
...
Trebalo bi da postaviš atribut `padding-right` na `20px`.
...
Vaš element za `.menu` mora da ima `padding-left` iz `20px`.
...
Kipengele chako cha `.menu` kinapaswa kuwa na `padding-right` ya `20px`.
