...
Koristeći vašu novu klasu `flavor` kao selektor, postavite vrednost atributa `text-align` na `left`.

# --hints--

Trebalo bi da imate birač za razred `flavor`.
...
Trebalo bi da postaviš atribut `text-align` na `left`.
...
Vaš selektor klase za `flavor` treba da postavi svojstvo `text-align` na `left`.
