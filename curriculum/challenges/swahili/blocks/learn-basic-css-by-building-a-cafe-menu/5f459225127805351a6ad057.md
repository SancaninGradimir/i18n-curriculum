...
Promeni pozadinsku boju elementa `hr` u `brown` ili da odgovara boji kafe zrna.

# --hints--

Trebalo bi da postavite vrednost atributa `background-color` na `brown`.

```js
const hasBackgroundColor = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['background-color'] === 'brown');
```

Vaša karakteristika od `hr` treba da ima svojstvo `background-color` za `brown`.
