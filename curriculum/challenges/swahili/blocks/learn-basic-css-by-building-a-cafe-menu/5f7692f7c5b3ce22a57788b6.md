...
Da biste dovršili šablon, dodajte odgovarajuća imena klasa za elemente `flavor` i `price` za sve preostale elemente `p`.

# --hints--

Trebalo bi da ima pet elemenata `.flavor`.

```js
assert.lengthOf(document.querySelectorAll('.flavor'), 5);
```

Trebalo bi da ima pet elemenata `.price`.

```js
assert.lengthOf(document.querySelectorAll('.price'), 5);
```

Vaši elementi za `.flavor` moraju biti vaši elementi za `p` sa tekstom `French Vanilla`, `Caramel Macchiato`, `Pumpkin Spice`, `Hazelnut`, i `Mocha`.
...
Vaši elementi za `.price` treba da budu vaši elementi za `p` sa tekstovima od `3.00`, `3.75`, `3.50`, `4.00` i `4.50`.
