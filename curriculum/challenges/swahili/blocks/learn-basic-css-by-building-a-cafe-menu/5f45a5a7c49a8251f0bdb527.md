...
S obzirom na isti selektor stila iz prethodnog koraka, povećajte veličinu fonta za artikle i cene koristeći vrednost `18px`.

# --hints--

Trebalo bi da postaviš atribut `font-size` na `18px`.
...
Trebalo bi da koristiš čitač `.item p` koji je dostupan.
...
Vaši `p` koji su postavljeni unutar stvari od `.item` moraju imati `font-size` od `18px`.
