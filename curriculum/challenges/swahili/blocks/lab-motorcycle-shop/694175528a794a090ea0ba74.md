...
title: Izgradite motorističku radnju
challengeType: 25
...
U ovoj laboratoriji, dobio/dobila si sve HTML i CSS. Koristićeš TypeScript da završiš zadatke za stranicu motorističke radnje.

**Cilj:** Popunite korisničke priče navedene ispod i prođite sve testove da biste završili laboratoriju.

**Priče korisnika:**

1. Unapaswa kuunda `type` inayoitwa `Category` ambayo ina thamani zinazowezekana zifuatazo: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. Unapaswa kuunda `interface` inayoitwa `Motorcycle` ambayo ina sifa zifuatazo: `id` ya aina `string`, `name` ya aina `string`, `manufacturer` ya aina `string`, `category` ya aina `Category`, `price` ya aina `number`, `image_url` ya aina `string`, sifa ya `created_at` ya aina `Date`, `description` ya aina `string`, na mwaka wa aina `number`.
3. Unapaswa kuunda kitendakazi kinachoitwa `fetchMotorcycles` kinachorejesha `Promise` kinachotatua kuwa safu ya vitu vya `Motorcycle`.
4. Unapaswa kupakia data ya `Motorcycle` kutoka kiishio cha `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
5. Unapaswa kuunda kitendakazi kinachoitwa `renderMotorcycleCard` kinachochukua kitu kimoja cha `Motorcycle` kama kigezo na kurejesha mfuatano wa herufi unaoonyesha HTML iliyopangwa kwa kadi ya pikipiki.
   1. Kitendakazi cha `renderMotorCycleCard` kinapaswa kuchukua kigezo kinachoitwa `motorcycle` cha aina `Motorcycle`.
   2. Kitendakazi cha `renderMotorCycleCard` kinapaswa kurejesha aina ya `string`.
   3. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele cha `img` chenye picha halali na darasa la `motorcycle-card-image-container`.
   4. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-year-badge` kinachoonyesha mwaka kwa maandishi ya kawaida.
   5. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-title` kinachoonyesha jina la pikipiki kwa maandishi ya kawaida.
   6. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-manufacturer` kinachoonyesha jina la mtengenezaji wa pikipiki kwa maandishi ya kawaida.
   7. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-category` kinachoonyesha aina ya pikipiki kwa maandishi ya kawaida.
   8. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-description` kinachoonyesha maelezo ya pikipiki kwa maandishi ya kawaida.
   9. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-price` kinachoonyesha bei ya pikipiki kwa maandishi ya kawaida.
   10. Kila sehemu ya kadi ya pikipiki inapaswa kuwa na kipengele chenye darasa la `motorcycle-card-engine` kinachoonyesha nguvu ya farasi ya kila injini.
6. Unapaswa kuwa na darasa linaloitwa `MotorcycleGalleryApp`.
   1. Darasa hilo linapaswa kuwa na safu ya `private` inayoitwa `allMotorcycles` ambayo imeandikwa kwa aina ya safu ya vitu vya `Motorcycle`.
   2. Darasa hilo linapaswa kuwa na kitendakazi kinachoitwa `renderMotorcycles` kinachoonyesha sehemu za kadi za pikipiki zinazolingana na thamani ya `allMotorcycles` ndani ya kipengele chenye kitambulisho cha `motorcycle-grid`.
7. Idadi ya vipengele vya `Motorcycle` vinavyoonyeshwa inapaswa kuorodheshwa ndani ya kipengele chenye kitambulisho cha `results-number`.
8. Kichujio kinapaswa kusasishwa kwenye tukio la `input`.

# --hints--

Trebalo bi da imaš `type` pod nazivom `Category`.
...
Tvoja Aina za `Category` mora biti postavljena na kombinaciju tačnih vrednosti.
...
Trebalo bi da imaš `interface` pod nazivom `Motorcycle`.
...
Trebalo bi da imaš sve kvalifikacije navedene u interfejsu `Motorcycle`.
...
Trebalo bi da imate funkciju nazvanu `fetchMotorcycles`.

```js
assert.isFunction(fetchMotorcycles);
```

Trebalo bi da koristiš krajnik `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
...
Trebalo bi da vratiš obećanje za 25 stavki od `Motorcycle` iz `fetchMotorcycles`.
...
Trebalo bi da imate funkciju nazvanu `renderMotorcycleCard`.

```js
assert.isFunction(renderMotorcycleCard);
```

Funkcija za `renderMotorcycle` treba da primi argument definisan tipom `Motorcycle`.
...
Funkcija za `renderMotorcycleCard` treba da vrati eksplicitni tip `string`.
...
Funkcija `renderMotorcycleCard` treba da restaurira `string`.
...
Trebalo bi da imaš klasu nazvanu `MotorcycleGalleryApp`.
...
`MotorcycleGalleryApp` mora imati red nazvan `allMotorcycles`.
...
Atribut za `allMotorcycles` treba da bude `private`.
...
Svojstvo od `allMotorcycles` treba biti napisano tipa `Motorcycle[]`.
...
`MotorcycleGalleryApp` treba da ima funkciju nazvanu `renderMotorcycles`.
...
Funkcija `renderMotorcycles` treba da prikaže 25 elemenata sa klasom `motorcycle-card-image-container`.
...
Funkcija za `renderMotorcycles` treba da prikaže 25 elemenata sa klasom `motorcycle-card-year-badge`.
...
Komponenta za `renderMotorcycles` treba da prikaže 25 elemenata sa klasom `motorcycle-card-title`.
...
Komponenta za `renderMotorcycles` treba prikazivati 25 elemenata sa klasom `motorcycle-card-manufacturer`.
...
Funkcija za `renderMotorcycles` treba da prikaže 25 elemenata koji imaju klasu `motorcycle-card-category`.
...
Funkcija za `renderMotorcycles` mora da prikaže 25 elemenata sa klasom `motorcycle-card-description`.
...
Funkcija `renderMotorcycles` treba da prikaže 25 elemenata sa klasom `motorcycle-card-price`.
...
Komponenta za `renderMotorcycles` treba da prikaže 25 elemenata sa klasom `motorcycle-card-engine`.
