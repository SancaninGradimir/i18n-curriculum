...
Za ovu laboratoriju, obezbeđeno su svi HTML i CSS kodovi. Koristićete TypeScript da dovršite funkcionalnosti stranice za prodaju motocikala.

**Cilj:** Ispunite priče korisnika ispod i osigurajte da se svi testovi prođu kako biste završili laboratoriju. 

**Priče korisnika:**

1. Trebalo bi da kreirate `type` nazvan `Category` koji se sastoji od sledećih mogućih vrednosti: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. Trebalo bi da kreirate `interface` nazvan `Motorcycle` koji ima sledeća svojstva: `id` tipa `string`, `name` tipa `string`, `manufacturer` tipa `string`, `category` tipa `Category`, `price` tipa `number`, `image_url` tipa `string`, svojstvo `created_at` tipa `Date`, `description` tipa `string`, i godinu tipa `number`.
3. Trebalo bi da kreirate funkciju nazvanu `fetchMotorcycles` koja vraća `Promise` koji rešava niz objekata `Motorcycle`.
4. Treba da učitate podatke `Motorcycle` sa `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` endpoint-a. 
5. Trebalo bi da kreirate funkciju nazvanu `renderMotorcycleCard` koja prima jedan `Motorcycle` objekat kao parametar i vraća string koji sadrži HTML, a taj HTML sadrži formatirani HTML niz za motociklističku karticu. 
   1. Funkcija `renderMotorCycleCard` treba da primi parametar nazvan `motorcycle` tipa `Motorcycle`. 
   2. Funkcija `renderMotorCycleCard` treba da vrati tip `string`. 
   3. Svaka komponenta motociklističke kartice treba da ima element sa `img` koji ima validnu sliku i klasu `motorcycle-card-image-container`.
   4. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-year-badge` koji navodi godinu u čistom tekstu.
   5. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-title` koji navodi ime motocikla u čistom tekstu.
   6. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-manufacturer` koji navodi proizvođača motocikala u čistom tekstu.
   7. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-category` koji navodi kategoriju motocikla u čistom tekstu. 
   8. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-description` koji navodi opis motocikala u čistom tekstu. 
   9. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-price` koji navodi cenu motocikla u čistom tekstu. 
   10. Svaka komponenta motociklističke kartice treba da ima element sa klasom `motorcycle-card-engine` koji navodi konjske snage svakog motora.
6. Trebalo bi da imate klasu nazvanu `MotorcycleGalleryApp`. 
   1. Klasa treba da ima niz `private`, nazvan `allMotorcycles`, koji je statički tipizovan na tip niza od objekata `Motorcycle`. 
   2. Ta klasa treba da ima funkciju nazvanu `renderMotorcycles` koja renderuje ekvivalentne komponente motociklističke kartice varijable `allMotorcycles` unutar elementa sa id-om `motorcycle-grid`. 
7. Broj elemenata `Motorcycle` koji se prikazuju treba da bude naveden unutar elementa sa id-om `results-number`.
8. Filter bi trebalo ažurirati na `input` događaju.
