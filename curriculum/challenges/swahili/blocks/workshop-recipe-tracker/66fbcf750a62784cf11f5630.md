...
Trebalo bi sada da ubacite tri objekta u niz `recipes`. Za to, možete koristiti metod `push()`.

Koristite metod `push()` da ubacite sve objekte recepata u niz `recipes`. Pazite da ubacite `recipe1`, `recipe2` i `recipe3` u tom redosledu.

Takođe obrišite varijable `recipe1Name`, `recipe2CookingTime` i `recipe3Ingredients`, kao i izjave `console.log` koje te varijable loguju.
