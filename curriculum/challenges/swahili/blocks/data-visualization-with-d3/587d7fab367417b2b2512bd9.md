...
title: Dodavanje oznaka za tačke na scatter grafikonu
challengeType: 6
forumTopicId: 301477
...
Možete dodati tekst kako biste kreirali oznake za tačke na scatter grafikonu.

Cilj je prikazivanje vrednosti sa zadžonom za prvi deo (`x`) i drugi deo (`y`) svakog elementa u `dataset`-u.

Čvorovi `text` zahtevaju atribute `x` i `y` kako bi se postavili u SVG. U ovom izazovu, vrednost `y` (koja određuje visinu) može koristiti istu vrednost koju `circle` koristi za svoj atribut `cy`. Vrednost `x` može biti nešto veća od vrednosti `cx` kruga, kako bi oznaka bila vidljiva. Ovo će pomeriti oznaku u desni deo postavljene tačke.

# --instructions--

Označite svaku tačku na scatter grafikonu koristeći elemente `text`. Tekst oznake bi trebalo da bude dve vrednosti razdvojene zarezom i razmakom. Na primer, oznaka prve tačke je `34, 78`. Postavite atribut `x` tako da ima vrednost jedinica za `5` više od vrednosti koju ste koristili za atribut `cx` na `circle`-u. Postavite atribut `y` na isti način kao što se koristi za vrednost `cy` na `circle`-u.

# --hints--

Vaš kod bi trebalo da ima 10 elemenata `text`.

```js
assert.lengthOf(document.querySelectorAll('text'), 10);
```

Prva oznaka bi trebalo da ima tekst `34, 78`, vrednost `x` od `39`, i vrednost `y` od `422`.
...
Druga oznaka bi trebalo da ima tekst `109, 280`, vrednost `x` od `114`, i vrednost `y` od `220`.
...
Treća oznaka bi trebalo da ima tekst `310, 120`, vrednost `x` od `315`, i vrednost `y` od `380`.
...
Četvrta oznaka bi trebalo da ima tekst `79, 411`, vrednost `x` od `84`, i vrednost `y` od `89`.
...
Peta oznaka bi trebalo da ima tekst `420, 220`, vrednost `x` od `425`, i vrednost `y` od `280`.
...
Šesta oznaka bi trebalo da ima tekst `233, 145`, vrednost `x` od `238`, i vrednost `y` od `355`.
...
Sedma oznaka bi trebalo da ima tekst `333, 96`, vrednost `x` od `338`, i vrednost `y` od `404`.
...
Osma oznaka bi trebalo da ima tekst `222, 333`, vrednost `x` od `227`, i vrednost `y` od `167`.
...
Deveta oznaka bi trebalo da ima tekst `78, 320`, vrednost `x` od `83`, i vrednost `y` od `180`.
...
Deseta oznaka bi trebalo da ima tekst `21, 123`, vrednost `x` od `26`, i vrednost `y` od `377`.
