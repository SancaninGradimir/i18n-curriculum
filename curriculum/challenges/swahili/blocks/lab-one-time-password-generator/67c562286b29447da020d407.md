...
title: Napravite generator jednokratnih lozinki
challengeType: 25
...
U ovom laboratorijumu, generisaćete 6-cifreni OTP (One-Time Password) i prikazati ga korisniku. OTP će isteći nakon 5 sekundi, a novi OTP će biti generisan kada korisnik klikne na dugme `"Generate New OTP"`.

**Cilj:** Ispunite priče korisnika ispod i osigurajte da svi testovi prođu kako biste završili laboratorijum.

**Priče korisnika:**

1. Trebalo bi da koristite kukač (hook) `useEffect` za upravljanje tajmerom broja unazad.
2. Vaša komponenta `OTPGenerator` treba da vrati element `div` sa imenom klase `container`.
3. Element `div`, koji ima klasu `container`, treba da sadrži sledeće elemente:

    - Element `h1` sa ID-om `otp-title` i tekstom `"OTP Generator"`.
    - Element `h2` sa ID-om `otp-display`, koji prikazuje poruku `"Click 'Generate OTP' to get a code"` ili pokazuje generisani OTP ako je dostupan.
    - Element `p` sa ID-om `otp-timer` i atributom `aria-live` postavljenim na validnu vrednost koja:
        - Počinje prazna.
        - Prikazuje `"Expires in: X seconds"` nakon klika na dugme, gde `X` predstavlja preostalo vreme pre isteka OTP-a.
        - Prikazuje poruku `"OTP expired. Click the button to generate a new OTP."` kada tajmer broja unazad dostigne `0`.
    - Element `button` sa ID-om `generate-otp-button` i oznakom `"Generate OTP"`. Kada se klikne, treba da generiše novi OTP i pokrene tajmer broja unazad od 5 sekundi.
    - Dugme `"Generate OTP"` mora biti onemogućeno dok je tajmer aktivan.

4. Trebalo bi da osigurate da se tajmer broja unazad automatski zaustavi kada dostigne `0` sekunde kako bi se sprečile nepotrebne ažuriranja.
