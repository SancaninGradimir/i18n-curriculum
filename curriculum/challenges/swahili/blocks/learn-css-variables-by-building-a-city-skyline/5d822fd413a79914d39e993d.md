...
Dodajte selektor `:root` na vaš medijski upit (media query). Zatim ponovo definišite sve četiri izmene od `--building-color` kako biste koristili vrednost `#000` tamo.

# --hints--

Trebalo bi da dodaš selektor `:root` u medijski upitnik (media query).
...
Treba da dodaš `--building-color1` sa vrednošću `#000`.
...
Treba da dodate `--building-color2` sa vrednošću `#000`.
...
Trebalo bi da dodate `--building-color3` sa vrednošću `#000`.
...
Treba da dodaš `--building-color4` sa vrednošću `#000`.
