...
Sledeće, uklonite `width` i `height` iz `.bb2a`, i zamenite `border-left` sa `border-right` koristeći `5vw` umesto `1vw`. Element će sada imati nulu veličine, a ivice će se sastajati po sredini.

# --hints--

Treba da uklonite `width` iz `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

Treba da uklonite `height` iz `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

Trebalo bi da promeniš `border-left` koristeći `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid rgb(153, 153, 153)");
```

Trebalo bi da zamenite `border-right` sa `5vw`.
