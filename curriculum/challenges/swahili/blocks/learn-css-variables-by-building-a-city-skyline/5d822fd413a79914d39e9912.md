...
Ka `.fb1b` `width` od `60%` i `height` od `10%`, i `.fb1c` `width` od `100%` i `height` od `80%`.

# --hints--

Trebalo bi mu dati `.fb1b` `width` od `60%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.width, "60%");
```

Trebalo bi mu dati `.fb1b` `height` od `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.height, "10%");
```

Trebalo bi mu dati `.fb1c` `width` od `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1c")?.width, "100%");
```

Trebalo bi mu dati `.fb1c` `height` od `80%`.
