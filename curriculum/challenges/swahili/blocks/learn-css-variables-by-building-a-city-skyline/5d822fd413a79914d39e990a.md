...
Premestite imovinsko i vrednosti za `display`, `flex-direction`, na `align-items` od `.bb1` ka novoj klasi `building-wrap`.

# --hints--

Trebalo bi da uklonite `display` iz `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.display);
```

Trebalo bi da prebacite `display` sa vrednošću `flex` na `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.display, "flex");
```

Treba da uklonite `flex-direction` iz `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.flexDirection);
```

Treba da prebaciš `flex-direction` sa vrednošću od `column` na `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.flexDirection, "column");
```

Trebalo bi da uklonite `align-items` iz `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.alignItems);
```

Trebalo bi da prebacite `align-items` sa vrednošću `center` na `.building-wrap`.
