...
Nastavi sa sledećim ugradnjom! Umetni dva elementa `div` unutar `.fb2` i dodeli im klase objekata `fb2a` i `fb2b`, u tom redosledu.

# --hints--

Treba da dodate dva elementa od `div` unutar `.fb2`.

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

Trebaš mu dati novi prvi predmet u razredu stvari `div` `fb2a`.
...
Trebaš mu dati novi deo za predmet `div` u razredu stvari `fb2b`.
