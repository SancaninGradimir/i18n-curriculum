...
Sledeće, promenite `#999` dve od `.bb2a` u `transparent`. Ovo će učiniti da leva i desna granica ne budu vidljive.

# --hints--

Trebalo bi da promeniš `border-left` ili koristi `transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

Trebalo bi da promeniš `border-right` ili koristi `transparent`.
