...
Uklonite mali na `margin` sa `border-top` iz `.bb2a` ili ga zamenite trouglnikom na vrhu zgrade.

# --hints--

Treba da uklonite `margin` iz `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.margin);
```

Treba da uklonite `border-top` iz `.bb2a`.
