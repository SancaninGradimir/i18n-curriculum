...
Nećete vam trebati karakteristike za `height` ili `background-color` u `.bb1a`, `.bb1b` ili `.bb1c` više, pa nastavite i uklonite ih.

# --hints--

Treba da uklonite `background-color` iz `.bb1a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

Trebalo bi da uklonite atribut `height` iz `.bb1b`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle('.bb1b')?.height);
```

Trebaš ukloniti atribut `background-color` iz `.bb1b`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle('.bb1b')?.backgroundColor);
```

Treba da uklonite atribut od `height` iz `.bb1c`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle('.bb1c')?.height);
```

Trebalo bi da uklonite atribut `background-color` iz `.bb1c`.
