...
Povećaj ponavljajući gradijent boje na ovoj zgradi; učini ga istim kao onaj koji je nedavno dodan, osim što ne povećavaš smer za `90deg` i koristi svoju boju prozora umesto dve boje od `transparent`.

# --hints--

Morate mu dati `.fb6` `repeating-linear-gradient` drugog u svojstvu `background`.
...
Treba ti da mu/javiš `repeating-linear-gradient`, od druge primarne boje `--building-color3`, od `0%` do `10%`.
...
Trebalo bi da mu/jajš (ili: trebaće ti) `repeating-linear-gradient` od drugog boja, od `--window-color3` do `10%` do `30%`.
