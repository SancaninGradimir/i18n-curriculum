...
Možete dodati kombinaciju više boja na element razdvajanjem zarezima (`,`) kao što je ovo:
...
Dodaj `repeating-linear-gradient` na `.fb1c` ispod postojećeg; koristi svoj `--building-color4` od `0%` do `10%` i `--window-color4` od `10%` i `90%`. Ovo će popuniti pozadinu mešavine boja koju ste poslednje dodali.

# --hints--

Ne mogu da promenim prvi `repeating-linear-gradient`.
...
Trebalo bi da dodate `repeating-linear-gradient` sa prvom bojom `--building-color4` od `0%` do `10%`.
...
Trebalo bi da koristite boju od `--window-color4` do `10%` do `90%`.
