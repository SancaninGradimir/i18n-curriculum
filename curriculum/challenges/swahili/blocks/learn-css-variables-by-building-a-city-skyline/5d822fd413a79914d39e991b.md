...
Dajte `.fb2a` `width` od `100%` na `.fb2b` `width` od `100%` na `height` od `75%`.

# --hints--

Trebalo bi mu dati `.fb2a` `width` od `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

Trebalo bi mu dati `.fb2b` `width` od `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

Trebalo bi mu dati `.fb2b` `height` od `75%`.
