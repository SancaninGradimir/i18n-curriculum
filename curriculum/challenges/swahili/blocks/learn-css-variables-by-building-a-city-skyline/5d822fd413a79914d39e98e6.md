...
Rasporedi šest elemenata od `div` unutar `.foreground-buildings` i dodeli im klase od `fb1` do `fb6` u tom redosledu. "fb" znači "prednja zgrada". Ovo će biti šest dodatnih prednjih zgrada.

# --hints--

Trebalo bi da kreirate novi `div` sa klasom `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Trebalo bi da kreirate novi `div` sa klasom `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Treba da kreiraš novi `div` sa klasom `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Treba da kreiraš `div` nov sa klasom `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Treba da kreiraš novi `div` sa klasom `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Treba da kreiraš novi `div` sa klasom `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

Treba da umetnete ove nove komponente za `div` unutar elementa `.foreground-buildings`.
