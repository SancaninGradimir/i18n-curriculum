...
Promena nijanse boje često se menja polako od jedne boje do druge. Kada su potrebni nagli prelazi, promena se može napraviti naglim zaustavljanjem, kao što je ovde:
...
Dodaj `linear-gradient` za `.bb2b` koja koristi `--building-color2` od `0%` do `6%` i `--window-color2` od `6%` do `9%`.

# --hints--

Trebalo bi da mu/jajmo atribut `.bb2b` sa vrednošću `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

Treba da koristiš `linear-gradient` na `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

Trebalo bi da koristite `--building-color2` od `0%` do `6%`.
...
Trebalo bi da koristite `--window-color2` od `6%` do `9%`.
...
`.bb2b` bi trebalo da ima izmene za `linear-gradient` od `--building-color2` sati `0%` do `6%`, i `--window-color2` sati `6%` do `9%`.
