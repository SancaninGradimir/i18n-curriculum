...
Dodajte još jedan `repeating-linear-gradient` ispod onoga što ste postavili. Navudi smer za `90deg`, koristite boju zgrade od `0%` do `12%` i boju prozora od `12%` do `44%`. Ovo će stvoriti mnogo pravougaono prozora.

# --hints--

Treba da mu/j joj da `.fb5` `repeating-linear-gradient` od uloge `background`.
...
Treba da mu daš `repeating-linear-gradient` u smeru `90deg`.
...
Moraš mu dostaviti `repeating-linear-gradient`, drugi deo prve boje od `--building-color2`, od `0%` do `12%`.
...
Trebalo bi da mu/jajilo (ili: trebaš dati) `repeating-linear-gradient` od drugog boje, koji je iz `--window-color2` do `12%` do `44%`.
