...
Dodajte novu klasu za `bb1-window` za elemente `.bb1a`, `.bb1b`, i `.bb1c`. Ovo će im obraditi šemu boja.

# --hints--

Nije moguće ukloniti klasu `bb1a`.

```js
assert.exists(document.querySelector('div.bb1a'));
```

Treba da dodaš klasu `bb1-window` za element/komponentu `.bb1a`.

```js
assert.exists(document.querySelector('div.bb1a.bb1-window'));
```

Ne možete ukloniti klasu `bb1b`.

```js
assert.exists(document.querySelector('div.bb1b'));
```

Trebalo bi da dodaš klasu `bb1-window` na element `.bb1b`.

```js
assert.exists(document.querySelector('div.bb1b.bb1-window'));
```

Ne smete da uklonite klasu za `bb1c`.

```js
assert.exists(document.querySelector('div.bb1c'));
```

Trebalo bi dodati klasu `bb1-window` za element `.bb1c`.

```js
assert.exists(document.querySelector('div.bb1c.bb1-window'));
```

Ne smete promeniti element `.bb1d`.
