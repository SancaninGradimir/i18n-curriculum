...
Nastavi na sledeću zgradu! To je zelena koja je ispred. Promeni boju svoje zgrade od `repeating-linear-gradient` od `0%` do `5%`, i `transparent` od `5%` do `10%`.

# --hints--

Trebalo bi mu dati osobinu `.fb5` od `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Trebalo bi mu dati osobinu `background` od `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Trebalo bi mu dodeliti `repeating-linear-gradient` prvu boju od `--building-color2`, od `0%` do `5%`.
...
Trebalo bi mu dati boju drugog `repeating-linear-gradient` od `transparent` od `5%` do `10%`.
