...
Kreiraj klasu objekata za `bb4-window` `width` od `18%`, `height` od `90%`, i postavi svoju varijablu za `--window-color4` na `background-color`.

# --hints--

Treba mu dati `.bb4-window` `width` od `18%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

Trebalo bi mu dati `.bb4-window` `height` od `90%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

Trebalo bi mu dati `.bb4-window` `background-color` od `--window-color4`.
