...
Elementi za `.fb4-window` `width` od `30%`, `height` od `10%`, i `border-radius` od `50%`. Ovo će stvoriti okrugljene prozore za ovu zgradu.

# --hints--

Trebalo bi mu dati `.fb4-window` `width` od `30%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

Trebalo bi mu dati `.fb4-window` `height` od `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

Trebalo bi mu dati `.fb4-window` `border-radius` od `50%`.
