...
Konačno! Stigli ste do poslednje zgrade! Dodajte ponavljajući smer za `90deg`. Koristite boju zgrade od `0%` do `10%` i `transparent` od `10%` do `30%`.

# --hints--

Treba dodati `repeating-linear-gradient` za `.fb6` u atributu `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Trebalo bi mu dati smer ka `repeating-linear-gradient`, a zatim `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Unapaswa kumpa `repeating-linear-gradient` rangi ya kwanza ya `--building-color3` kutoka `0%` hadi `10%`.
...
Trebalo bi mu dati boju `repeating-linear-gradient` od `transparent` do `10%` do `30%`.
