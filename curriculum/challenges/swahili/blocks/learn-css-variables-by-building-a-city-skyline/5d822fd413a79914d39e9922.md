...
Uklonite element `.fb3a` `width` iz `80%` na `height` iz `15%`. Zatim uklonite element `.fb3b` `width` iz `100%` na `height` iz `35%`.

# --hints--

Treba da mu daš `.fb3a` `width` od `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

Trebalo bi da mu/joj daš `.fb3a` `height` od `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

Trebalo bi mu dati `.fb3b` `width` od `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

Trebalo bi mu dati `.fb3b` `height` od `35%`.
