...
Ukloni atribut sa vrednosti `background-color` iz `.fb3`, i postavi ga u `.fb3a` i `.fb3b`.

# --hints--

Treba da uklonite `background-color` iz `.fb3`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

Trebalo bi mu dati atribut `.fb3a` sa vrednošću `background-color`, koja je `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Trebalo bi mu dati atribut `.fb3b` sa vrednošću `background-color`, koja je `--building-color1`.
