...
Premesti svojstvo i vrednost od `background-color` iz `.fb1` u `.fb1b`.

# --hints--

Treba da uklonite `background-color` iz `.fb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb1")?.backgroundColor);
```

Treba da dodaš `background-color` od `--building-color4` u `.fb1b`.
