...
Ne brinite za prostor ispod, sve će biti smješteno niže kasnije, kada povećate visinu elementa koji se nalazi na vrhu zgrade.

Povećaj `repeating-linear-gradient` za `.fb1c` za ugao od `90deg`, `--building-color4` od `0%` do `10%` i `transparent` od `10%` do `15%`.

# --hints--

Trebalo bi mu dati `.fb1c` `background` koji ima `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.background, "repeating-linear-gradient");
```

Trebalo bi da koristiš smernik `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Treba ti da koristiš prvu boju od `--building-color4` do `0%` do `10%`.
...
Trebalo bi da koristiš drugu boju od `transparent` do `15%`, koja je u `10%`.
