...
Želite da dodate isti bojni atribut u sledeće dve sekcije. Umesto toga, kreirajte novi selektor objekta klase koji se zove `bb1-window` i premestite svojstva i vrednosti `height` i `background` iz `.bb1a` u novi selektor objekta klase.

# --hints--

Treba da kreiraš novi selektor za klasu objekata nazvanog `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Trebalo bi da prebaciš atribut i vrednost `height` sa `.bb1a` na `.bb1-window`.
...
Treba da prebacite atribut i vrednost za `background` iz `.bb1a` u `.bb1-window`.
...
Ne smeš da premeštate atribut iz `background-color` na `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

Ne treba da premesiš atribut iz `width` na `.bb1a`.
