...
Unutar `.btn-container`, kreiraj tri dugmeta za svaki tip priče. Daj svakom dugmetu klasu `btn` i jedan `id` od `scary-btn`, `funny-btn`, i `adventure-btn`. Tekst za svako dugme treba da bude `Scary Story`, `Funny Story`, i `Adventure Story` respektivno.
