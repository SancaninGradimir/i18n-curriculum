...
Modifikujte slušalicu događaja `scaryStoryBtn` da pozove funkciju `displayStory` sa argumentom `"scary"`. Takođe, dodajte slušalicu događaja `click` na `funnyStoryBtn`, i `adventureStoryBtn` dugmeta koja pozivaju funkciju `displayStory` sa argumentima `"funny"` i `"adventure"` respektivno.

Time ste uspešno završili radionicu.
