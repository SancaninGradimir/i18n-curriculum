...
Nakon provere da li je inbox prazan, odštampajte poruku `\nYour Emails:`. Nakon toga, iterirajte kroz sve e-poruke u inboxu koristeći petlju `for` sa enumeracijom. Ovo je sintaksa za enumeraciju sa početnim brojem: `enumerate(iterable, start=start_number)`.

Koristite enumeraciju da numerišete e-poruke počevši od `1`. Koristite `i` (indeks) i `email` (objekat e-poruke) kao varijable iteracije.

Unutar petlje, odštampajte f-string sa indeksom iteracije praćenim `.`, zatim razmak i string reprezentacijom objekta e-poruke u ovom formatu:

```md
i. string_representation
```

Gde je `i` indeks, a `string_representation` reprezentacija objekta e-poruke.
