...
Vaš inbox takođe treba način za brisanje e-pošte.

Ključna reč Python-a ``del`` može se koristiti za brisanje stavki iz liste po njihovom indeksu.

Dodajte metod nazvan ``delete_email`` u vašu klasu ``Inbox``. Kao i ``read_email``, trebalo bi da:

- Prihvati parametar ``index``, proveri da li je inbox prazan i odštamši poruku ``Inbox is empty.\n`` ako jeste. Koristi ``return`` za izlazak iz metode.
