...
Sada, proveri metodu `mark_as_read` koju si kreirao u klasi `Email`. Koristi metodu na postojećem objektu `Email` `email_obj` da promeniš status. Nakon označavanja e-pošte kao pročitane, odštampaj atribut `read` objekta `email_obj` kako bi potvrdio promenu.
