...
Nakon dobijanja stvarnog indeksa, kreirajte `if` izjavu da proverite da li je `actual_index` manje od 0 ili veće/jednako veće od dužine liste `self.emails`. Ako jeste, štampajte poruku `Invalid email number.\n` i koristite `return` da izađete iz metode.
