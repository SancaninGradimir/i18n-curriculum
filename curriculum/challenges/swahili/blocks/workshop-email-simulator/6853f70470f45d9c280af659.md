...
Sada prikažite vremenski pečat prilikom prikazivanja celog e-pošte.

Ispod naslova, štampajte primljeni vremenski pečat koristeći `strftime` kako biste ga formatirali kao `'%Y-%m-%d %H:%M'`. Koristite sledeći format:

```py
Received: date
```

Gde je `date` formatiran kao `'%Y-%m-%d %H:%M'`.
