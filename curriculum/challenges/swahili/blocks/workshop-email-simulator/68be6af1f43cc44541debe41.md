...
Trebalo bi da obradite slučaj u kojem korisnik pokušava da izbriše e-mail koristeći nevažeći indeks, baš kao što ste uradili u metodi `read_email`.

Unutar metode `delete_email`:

- Pretvorite parametar `index`, koji je baziran na 1, u indeks baziran na 0 oduzimanjem 1 i skladištenjem ga u varijablu nazvanu `actual_index`.
- Kreirajte `if` izjavu da proverite da li je `actual_index` manje od 0 ili veće/jednako dužini liste `self.emails`. Ako jeste, ispišite poruku `Invalid email number.\n` i koristite `return` za izlazak iz metode.
