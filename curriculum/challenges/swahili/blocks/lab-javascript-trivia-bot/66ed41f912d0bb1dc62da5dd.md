...
title: Izgradite JavaScript Trivia Bota
challengeType: 26
...
1. Trebalo bi da logujete `"Hello! I'm your coding fun fact guide!"` u konzolu kao pozdravnu poruku korisniku.
1. Trebalo bi da kreirate tri varijable: `botName`, `botLocation` i `favoriteLanguage`, koje čuvaju ime bota, odakle potiče i njegov omiljeni programski jezik, respektivno.
1. Trebalo bi da logujete `"My name is (botName) and I live on (botLocation)."` u konzolu.
1. Trebalo bi da logujete `"My favorite programming language is (favoriteLanguage)."` u konzolu.
1. Trebalo bi da koristite `let` da kreirate varijablu `codingFact` i dodelite joj string koji je zanimljiva činjenica o omiljenom programskom jeziku vašeg bota, uključujući korišćenje varijable `favoriteLanguage`.
1. Trebalo bi da logujete `codingFact` u konzolu.
1. Trebalo bi da ponovo dodelite varijabli `codingFact` novu činjenicu o omiljenom jeziku bota koristeći ponovo varijablu `favoriteLanguage`.
1. Trebalo bi da ponovo logujete `codingFact` u konzolu.
1. Trebalo bi da ponovo dodelite varijabli `codingFact` još jednu novu činjenicu o omiljenom jeziku bota koristeći varijablu `favoriteLanguage`.
1. Trebalo bi da logujete `codingFact` u konzolu treći put.
1. Trebalo bi da logujete `"It was fun sharing these facts with you. Goodbye! - (botName) from (botLocation)."` u konzolu kao oproštajnu izjavu od bota.
