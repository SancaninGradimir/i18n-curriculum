...
Kontrola misije vas je obavestila da lista astronauta kvalifikovanih za EVA takođe treba da bude sortirana po prioritetu opadajuće. Postoji nekoliko načina za sortiranje niza - možda je najjednostavniji `bubble sort`.

`Bubble sort` radi ponavljanjem prolazak kroz listu, poređenjem susednih elemenata i zamjenjivanjem ih ako nisu u pravom redosledu. Nakon svakog prolaska, element koji bi trebalo da dođe prvi na osnovu vaših kriterijuma sortiranja približava se (ili „klupa“ ka) svojoj ispravnoj poziciji u nizu. Evo kako možete sortirati `crew` po prioritetu opadajuće koristeći `bubble sort`:
...
Kreirajte novu pomoćnu funkciju pod nazivom `sortByPriorityDescending` koja prima parametar `crew`. Ova funkcija bi trebalo direktno da sortiruje ulazni niz `crew` po prioritetu opadajuće, koristi dve ugnježđene `for` petlje i ne bi trebalo ništa da vraća.
