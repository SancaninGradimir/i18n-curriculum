...
Dodali ste poziv `console.log()` u svoj skript da biste validirali vaš niz `EVAReadySquad`, a sada više nije potreban. Da biste sprečili da se terminal zaprlja u budućim koracima, uklonite trenutnu petlju `for` koja loguje `EVAReadySquad` astronauta na konzolu.
