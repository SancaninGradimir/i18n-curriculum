...
Prvo, kreiraj prazan niz nazvan `eligible`. Zatim prođi kroz svakog astronoma u ulaznom `crew` nizu i, ako su kvalifikovani za EVA (što znači da je njihovo svojstvo `isEVAEligible` jednako `true`), dodaj ih u `eligible` niz. Nakon petlje, vrati `eligible` niz.
