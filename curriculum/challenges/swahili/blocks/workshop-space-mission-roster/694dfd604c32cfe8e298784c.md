...
Kontrola misije je tražila da zameniš pozicije dva astronauta u tvom `squad` masivu, konkretno članove na indeksima `2` i `5`. Koristi svoju funkciju `swapCrewMembers()` da izvrši zamenu i sačuvaj rezultat u novu varijablu pod nazivom `updatedSquad`.
