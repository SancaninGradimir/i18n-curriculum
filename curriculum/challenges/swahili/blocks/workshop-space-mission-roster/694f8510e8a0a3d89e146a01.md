...
Da biste kreirali blokove veličine `n` iz `array` bez menjanja istog, možete kombinovati petlju `for` sa metodom `slice()` na sledeći način:
...
- kreira prazan niz pod nazivom `result`

- prolazi kroz originalni `array` koracima veličine `n`

- kreira blokove iz `array` veličine `n` i dodaje ih u `result` koristeći `slice()`

Da biste dovršili funkciju `chunkCrew`, morate:

- kreirati niz pod nazivom `chunks`

- prolaziti kroz `crew` koracima veličine `size`, kreirajući i dodajući blokove iz `crew` u `chunks` koristeći `slice()`
