...
Možda ste primetili da je `EVAChunks` u suštini niz sa nizovima unutra ili dvodimenzionalni (2D) niz. Ugnježdena `for` petlja se može koristiti za logovanje podataka iz 2D niza:
...
U gornjem primeru, `rootArray` je glavni niz, `rootArray[i]` je podniz unutar glavnog niza, a `rootArray[i][j]` je jedan objekat unutar tog podniza, i `.property` pristupa vrednosti u tom objektu.

Koristite ugnježdenu `for` petlju za iteraciju kroz `EVAChunks` i logovanje `name` svakog astronauta u svakom delu (chunk). Obavezno:

- koristite `EVAChunks` kao vaš glavni niz
- logujte `Chunk ${i+1}:` između delova (chunkova)
