...
Pozovite `getEVAReadyCrew()` sa vašim spiskom/rosterom `updatedSquad` i sačuvajte rezultat u novu varijablu nazvanu `EVAReadySquad`. Zatim, koristite petlju `for` da snimite (logujete) `name` svakog astronoma u nizu `EVAReadySquad`.
