...
Gurnite stavku `astronaut` unutar reda podataka `crew` i napišite `Added ${astronaut.name} as ${astronaut.role}` u konzolu. Možete koristiti placeholder karaktere ili kombinovati niz karaktera za poruku.

# --hints--

Trebalo bi da umetnete `astronaut` unutar redova podataka za `crew`.
...
Unapaswa kuandika `name` wa mwanadamu wa anga kwa muundo ufuatao, `Added {astronaut.name} as {astronaut.role}`.
