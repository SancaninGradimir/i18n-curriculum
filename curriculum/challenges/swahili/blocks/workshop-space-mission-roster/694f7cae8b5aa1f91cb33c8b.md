...
Kao i pre, trebalo bi da validirate svoj unos. Konkretno, ako je `size` manje od `1`, trebalo bi da logujete `"Chunk size must be >= 1"` na konzolu, a zatim pozovete `return` da izađete iz funkcije.
