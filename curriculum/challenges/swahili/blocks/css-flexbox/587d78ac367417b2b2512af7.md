...
title: Koristi svojstvo `justify-content` u polju za unos tvita
challengeType: 0
...
# --description--
Poslednji izazov je pokazao primer funkcije `justify-content`. Za unos tvita, ova funkcija se može koristiti za organizovanje stvari u sekciji `.profile-name`.

# --instructions--
Dodajte atribut CSS `justify-content` na polje `.profile-name` zaglavlja i postavite vrednost na jednu od opcija iz poslednjeg izazova.

# --hints--
`.follow-btn` Trebalo bi da se prikaže na stranici. Proverite da ste onemogućili sve ekstenzije, kao što su blokatori oglasa.
...
Element za `.profile-name` mora imati atribut `justify-content` sa jednim od ovih vrednosti: `center`, `flex-start`, `flex-end`, `space-between`, `space-around`, ili `space-evenly`.
