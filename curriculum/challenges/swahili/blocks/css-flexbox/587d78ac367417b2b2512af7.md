...
title: Koristite svojstvo justify-content u Twitter embedu
challengeType: 0
...
Prethovna vežba je prikazala primer svojstva `justify-content`. Za Twitter embed, ovo svojstvo se može koristiti za poravnavanje elemenata u elementu `.profile-name`.

# --instructions--

Dodajte CSS svojstvo `justify-content` elementu `.profile-name` u zaglavlju i postavite vrednost na jednu od opcija iz prethodne vežbe.

# --hints--

Vaš `.follow-btn` treba da bude vidljiv na stranici. Uverite se da ste onemogućili bilo kakve dodatke, kao što su reklamski blokovi.
...
Element `.profile-name` treba da ima svojstvo `justify-content` sa jednom od ovih vrednosti: `center`, `flex-start`, `flex-end`, `space-between`, `space-around`, ili `space-evenly`.
