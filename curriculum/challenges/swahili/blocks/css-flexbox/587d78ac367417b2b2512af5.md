...
title: Koristi svojstvo `flex-direction` da kreira red u polju za unos tvita
challengeType: 0
...
# --description--
Unos tviita `header` i `footer` koristio je svojstvo `flex-direction` prvobitno za vrednost reda (row). Isto tako, stavke unutar polja `.profile-name` će funkcionisati dobro ako su organizovane kao kolona (column).

# --instructions--
Dodaj atribut za CSS `flex-direction` na elementu `.profile-name` zaglavlja i postavi vrednost na `column`.

# --hints--
`.follow-btn` Vaše bi trebalo da se prikaže na stranici. Uverite se da ste onemogućili sve dodatke, kao što su blokatori oglasa.
...
Element `.profile-name` mora imati svojstvo `flex-direction` postavljeno na `column`.
