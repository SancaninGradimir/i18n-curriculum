...
title: Koristi svojstvo `flex-direction` za kreiranje redova u polju za unos tvita
challengeType: 0
...
# --description--
`header` i `footer` u primeru unosa tvita, imaju elemente deteta koji se mogu organizovati u redove koristeći svojstvo `flex-direction`. Ovo opisuje CSS organizovanje elemenata deteta vertikalno i ravnomerno.

# --instructions--
Povećaj svojstvo CSS `flex-direction` za `header` i `footer`, sve i postavi vrednost na `row`.

# --hints--
`.follow-btn` To treba da se prikaže na stranici. Proverite da ste onemogućili sve ekstenzije, kao što su blokatori reklama.
...
`header` treba imati svojstvo `flex-direction` postavljeno na `row`.

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

`footer` trebalo bi imati svojstvo `flex-direction` postavljeno na `row`.
