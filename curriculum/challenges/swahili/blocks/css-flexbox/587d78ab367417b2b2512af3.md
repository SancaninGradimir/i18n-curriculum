...
title: Koristite svojstvo flex-direction za kreiranje redova u embedu tvita
challengeType: 0
...
`header` i `footer` elementi u embedu tvita imaju potomke koje možete rasporediti u redove koristeći svojstvo `flex-direction`. Ovo objašnjava kako CSS da poravna potomke horizontalno (u redu).

# --instructions--

Dodajte CSS svojstvo `flex-direction` za sve elemente `header` i `footer`, postavljajući vrednost na `row`.

# --hints--

Vaš `.follow-btn` treba da bude vidljiv na stranici. Uverite se da niste onemogućili nikakve dodatke, kao što su reklame blokatori.
...
`header` treba da ima svojstvo `flex-direction` postavljeno na `row`.

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

`footer` treba da ima svojstvo `flex-direction` postavljeno na `row`.
