...
title: Koristi svojstvo `align-items` u ulazu za tweet
challengeType: 0
...
# --description--
Poslednji izazov je uveo `align-items` i pružanje primera. Ova funkcija se može koristiti za nekoliko elemenata unosa tvita radi strukturiranja promenljivih delova unutar njega.

# --instructions--
Dodaj atribut CSS `align-items` za element `.follow-btn` u zaglavlje. Postavi vrednost na `center`.

# --hints--
`.follow-btn` Mora da se prikaže na stranici. Proverite da ste onemogućili sve proširenja, kao što su blokatori oglasa.
...
Polje za `.follow-btn` treba da ima svojstvo `align-items` ako ima vrednost `center`.
