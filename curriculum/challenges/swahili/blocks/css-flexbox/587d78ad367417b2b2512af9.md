...
title: Koristite svojstvo align-items u embedu tvita
challengeType: 0
...
Prethovna vežba je predstavila svojstvo `align-items` i dala primer. Ovo svojstvo se može primeniti na nekoliko elemenata u embedu tvita kako bi se poravnati dinamički delovi unutar njega.

# --instructions--

Dodajte CSS svojstvo `align-items` elementu sa klasom `.follow-btn` u zaglavlju. Postavite vrednost na `center`.

# --hints--

Vaš `.follow-btn` element treba da bude vidljiv na stranici. Uverite se da niste onemogućili nikakve proširenje, kao što su reklame blokatori.
...
Element sa klasom `.follow-btn` treba da ima svojstvo `align-items` sa vrednošću `center`.
