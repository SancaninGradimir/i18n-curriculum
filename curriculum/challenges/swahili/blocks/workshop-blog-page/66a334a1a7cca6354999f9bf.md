...
Za prvi blog post, koristićeš `<dfn>article</dfn>` element.

Element ``article`` predstavlja samostalni sadržaj na veb stranici.
...
Razlog zašto se ovde koristi ``h3`` je što je održavanje pravilne strukturalne hijerarhije za naslovne elemente važno. Pošto je podnaslov posta element tipa ``h2``, sledeći nivo u hijerarhiji bio bi ``h3``.
