...
title: Napravite stilizovanu listu zadataka
challengeType: 25
...
U ovom laboratorijumu, veštaći ćete različite stilove koji se mogu primeniti na linkove kada su pređeni mišem (hovered), fokusirani, kliknuti i posetašnji.
...
2. Unutar neuređene liste, trebalo bi da imate četiri stavke liste.
3. Unutar svake stavke liste, treba da ima:

   - Element `input` sa tipom `checkbox` i `id` podešen na jedinstvenu vrednost.
   - Element `label` sa atributom `for`, podešen na `id` odgovarajućeg elementa `input`.
   - Neuređena lista sa klasom `sub-item`.
   - Stavka liste sa ankor elementom unutra. Ankor treba da ima klasu `sub-item-link`, validnu vrednost `href` i vrednost `target`, koja čini da se link otvara u novoj karti (tab).

4. Vaši `a` elementi ne bi trebalo da imaju nikakve tekstualne dekoracije.
5. Trebalo bi da postavite boju teksta za neposetašene linkove na boju po izboru.
6. Kada su vaši linkovi posetašeni, boja bi trebalo da se promeni u drugu boju po izboru.
7. Kada je mišem pređeno preko vaših linkova (hovered), boja bi trebalo da se promeni u drugu boju po izboru.
8. Kada su vaši linkovi fokusirani, oko linka bi trebalo da postoji obojena ivica (outline).
9. Kada se vašim linkovima klikne, boja bi trebalo da se promeni u drugu boju po izboru.
