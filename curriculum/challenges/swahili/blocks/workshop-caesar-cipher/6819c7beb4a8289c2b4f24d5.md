...
Vaša funkcija ne bi trebalo da prihvati negativan ili nulan pomeraj. Stoga, nakon vaše prve `if` izjave, kreirajte još jednu `if` izjavu koja proverava da li je `shift` manje od `1` i vraća string `Shift must be a positive integer.`.
