...
Promeni nivo steka elementa `.back-mountain` ili bude direktno iza elementa `.left-mountain`.
...
Trebalo bi da postaviš vrednost za `z-index` na `1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.zIndex, '1');
```

Ne smete da menjate `z-index` elementa `.left-mountain`.
