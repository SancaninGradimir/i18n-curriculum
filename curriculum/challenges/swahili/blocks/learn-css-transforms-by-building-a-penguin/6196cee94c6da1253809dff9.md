...
Ciljaj element sa `.back-mountain`, i postavi `width` na `height` da bude `300px`. Zatim, postavi `background` da bude pravac linije koja počinje sa `rgb(203, 241, 228)` i završava se sa `rgb(47, 170, 255)`.

# --hints--

Trebalo bi da koristiš alat `.back-mountain`.

```js
assert.match(code, /\.back-mountain\s*\{/);
```

Trebalo bi mu dati `.back-mountain` `width` od `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

Treba da mu daš `.back-mountain` `height` od `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

Trebalo bi mu dati `.back-mountain` `background` od `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
