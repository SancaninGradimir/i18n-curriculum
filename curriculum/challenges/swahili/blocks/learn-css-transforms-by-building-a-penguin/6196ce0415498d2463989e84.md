...
Da biste stvorili efekat planinskog lanca, dodajte drugu planinu, kreiranjem `div` novu odmah nakon `.left-mountain`, i postavite `div` novu `class` od `back-mountain`.

# --hints--

Treba da dodaš novi `div` unutar `body`.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 4);
```

Trebalo bi mu dati `div` `class` od `back-mountain`.

```js
assert.include(document.querySelector('div:not(.left-mountain, .ground, .penguin)')?.className, 'back-mountain');
```

Treba da staviš `.back-mountain` nakon `.left-mountain`.
