...
Omogući funkciju za `.sun` `width` na `height` od `200px`, na `background-color` od `yellow`.

# --hints--

Trebalo bi da koristiš čitač `.sun`.

```js
assert.match(code, /\.sun\s*\{/);
```

Trebalo bi mu dati `.sun` `width` od `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

Trebalo bi mu dati `.sun` `height` od `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

Trebalo bi mu dati `.sun` `background-color` od `yellow`.
