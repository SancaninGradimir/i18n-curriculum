...
Da bi noge majmuna više izgledale kao noge majmuna, savijte levi nogu pomoću `80deg`, i desnu nogu pomoću `-80deg`.

# --hints--

Trebalo bi mu dati `.foot.left` `transform` od `rotate(80deg)`.
...
Trebalo bi mu dati `.foot.right` `transform` od `rotate(-80deg)`.
