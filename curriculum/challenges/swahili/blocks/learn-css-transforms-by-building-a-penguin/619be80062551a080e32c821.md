...
Zaobljest vrh, dajući donjem uglu radijus od `100%`, ostavljajući gornje uglove kao `0%`.

# --hints--

Trebalo bi da koristiš atribut `border-radius` za obuhvatanje vrha.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Trebalo bi mu dati `.penguin-body::before` `border-radius` od `0% 0% 100% 100%`.
