...
Iznad komponente `.ground`, dodaj `div` koji ima `class` od `penguin`. Ovaj `div` će imati Flappy Penguin.
...
Trebalo bi mu dati `div` `class` od `penguin`.

```js
assert.include(document.querySelector('body > div:not(.ground)')?.className, 'penguin');
```

Trebalo bi da povežete kabl `div.penguin` sa kablom `div.ground`.
