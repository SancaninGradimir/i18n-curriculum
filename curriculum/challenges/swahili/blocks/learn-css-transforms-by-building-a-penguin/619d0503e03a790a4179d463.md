...
Postavi element `.chin` na način da je `25%` od gore, i `5%` od leve strane njegovog roditelja. Zatim, ukloni gornje uglove kruga `70%`, i donje uglove kruga `100%`.

# --hints--

Trebalo bi da `.chin` dobije atribut od `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Trebalo bi da muješ `.chin` svojstvo od `top` sa vrednošću `--fcc-expected--`, ali si dobio/dobila `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Trebalo bi mu dati pohvalu za `.chin` `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Unapaswa kumpa `.chin` sifa ya `left` yenye thamani ya `--fcc-expected--`, lakini umeipata `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Trebalo bi mu dati atribut `.chin` sa vrednošću `border-radius`, koja je `70% 70% 100% 100%`.
