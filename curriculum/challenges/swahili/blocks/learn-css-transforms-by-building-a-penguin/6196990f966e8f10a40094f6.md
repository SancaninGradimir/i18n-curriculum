...
Postavi karakteristiku za `.penguin`, na lokaciju `width` na `height` da bude `300px`.

# --hints--

Trebalo bi da koristiš alat `.penguin`.

```js
assert.match(code, /\.penguin\s*\{/);
```

Trebalo bi mu dati `.penguin` `width` od `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.width, '300px');
```

Trebalo bi mu dati `.penguin` `height` od `300px`.
