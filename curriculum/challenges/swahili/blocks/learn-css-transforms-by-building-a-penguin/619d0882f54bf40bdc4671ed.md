...
Gde je potrebno, promenite vrednosti svojstava za `--penguin-face` vaše.

# --hints--

Treba da mu/jaj `.face` `background-color` od `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Trebalo bi mu dati `.chin` `background-color` od `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Ne smiješ koristiti `var(--penguin-face)` u izbirniku `.penguin-body`.
