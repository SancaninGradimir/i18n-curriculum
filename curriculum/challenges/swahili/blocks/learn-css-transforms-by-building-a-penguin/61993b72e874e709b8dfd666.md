...
Izaberi element `.penguin-head`, i daj mu `width` pola od njegovog roditelja `45%`, i `height` od `45%`. Zatim, postavi `background` kao smer kružne linije od `gray` do `rgb(239, 240, 228)` u `45deg`.

# --hints--

Trebalo bi da koristiš alat `.penguin-head`.

```js
assert.match(code, /\.penguin-head\s*\{/);
```

Trebalo bi mu dati `.penguin-head` `width` od `50%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

Trebalo bi mu dati `.penguin-head` `height` od `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

Trebalo bi da mu/judi daju `.penguin-head` `background` od `linear-gradient(45deg, gray, rgb(239, 240, 228))`.
