...
Promeni nivo steka elementa `.penguin` ili pojavi se pre elementa `.ground`, na kipa `position` od `relative`.

# --hints--

Trebalo bi da koristiš atribut `z-index` za promenu nivoa steka.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex);
```

Trebalo bi mu dati atribut `.penguin` `z-index` od `4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex, '4');
```

Treba da mu daš osobinu `.penguin` od `position`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.position);
```

Trebalo bi mu dati `.penguin` `position` od `relative`.
