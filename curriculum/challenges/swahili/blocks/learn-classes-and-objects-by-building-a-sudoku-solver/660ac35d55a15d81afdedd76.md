...
Unutar petlje `for`, deklarisati parametar `row_str` i postaviti vrednost ponavljajuće liste koja se ponavlja u `row` i konvertovati svaki element `i` u `row` u string. Koristiti funkciju `str()` za to.

# --hints--

Trebalo bi da objaviš parametar `row_str`.
...
Treba da postavite vrednost ponavljajuće liste koja se ponavlja na `row` za parametar `row_str`.
...
Ponovljiva lista sa dodeljenom vrednošću parametra `row_str` treba da koristi `i` kao ponavljajući parametar.
...
Ponovni spisak sa vrednostnim parametrom `row_str` bi trebalo da pozove `str()` za svaki element `i` u `row`.
