...
Badilisha `row_str` comprehension ili izlazi samo niz slova kada element nije nula, i zvezdane karaktere inače.

# --hints--

Lista komprehenzije koja je dobila vrednost u `row_str` bi trebalo da pozove `str()` za svaki element `i` u `row` ako je `i` tačno, i bi trebalo da se evaluira na `'*'` inače.
