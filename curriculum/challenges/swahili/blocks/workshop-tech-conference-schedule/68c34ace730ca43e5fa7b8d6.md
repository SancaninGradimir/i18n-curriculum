...
Počnite dodavanjem još jednog `tr` elementa. Unutar tog `tr` elementa, dodajte `th` element sa atributom `scope` podešenim na `"row"` i tekstualnim sadržajem `10:00 AM`. 
