...
Zatim, dodajte peti red u tabelu. Počnite sa dodavanjem još jednog `tr` elementa. Unutar tog `tr` elementa, dodajte `th` element sa atributom `scope` postavljenim na `"row"` i tekstualnim sadržajem `12:30 PM`. Zatim ispod tog `th` elementa, dodajte `td` element sa tekstualnim sadržajem `Lunch Break`. Vaš `td` element takođe bi trebalo da ima atribut `colspan` postavljen na `3`, kako bi se protezao preko svih tri trake.
