...
Unutar vašeg elementa `tr`, dodajte element `th` sa tekstom `9:00 AM`. Zatim ispod tog elementa `th`, dodajte tri elementa `td` sa tekstom `Keynote: Tech Future`, `Intro to Web Dev` i `UX for All`.
