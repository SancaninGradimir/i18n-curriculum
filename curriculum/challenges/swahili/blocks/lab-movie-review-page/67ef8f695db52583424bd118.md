...
title: Dizajniraj stranicu za recenziju filmova
challengeType: 25
...
2. Unutar elementa `main`, trebalo bi da imate element `h1` za naslov filma.
3. Ispod elementa `h1`, trebalo bi da imate element `img` koji prikazuje omotnicu filma. Vaš element `img` treba da ima opisni tekst `alt` koji opisuje sliku. Slobodni ste da koristite sledeću sliku ako želite: `https://cdn.freecodecamp.org/curriculum/labs/rise-beyond-2.png`.
4. Trebalo bi da imate element `p` koji sadrži kratak opis filma.
5. Trebalo bi da imate još jedan element `p` za prikaz ocene filma. Unutar njega, trebalo bi da imate ove stavke u navedenom redosledu:
   - Element `strong` sa tekstom `Movie Rating:`.
   - Element `span` sa atributom `aria-hidden` postavljenim na `true`, koji sadrži vizuelnu reprezentaciju ocene korišćenjem zvezdica `⭐⭐⭐⭐⭐⭐⭐⭐⭐☆`.
   - Numerička vrednost, koja predstavlja ocenu, u zagradi (npr. `9.2/10`) nakon span elementa.
6. Trebalo bi da imate element `h2` sa tekstom `Cast Members`.
7. Trebalo bi da imate element `ul`.
8. Unutar elementa `ul`, trebalo bi da imate više elemenata `li`, svaki od kojih sadrži element `strong` za ime glumca, praćeno odgovarajućim imenom lika prethodno tekstom `as`. (npr., `James Holloway as Ethan Carter`).
