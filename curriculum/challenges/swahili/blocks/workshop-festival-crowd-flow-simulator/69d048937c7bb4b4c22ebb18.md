...
Sada je vreme da se obraduju prisutni kroz kapiju. Unutar petlje `while`:

- Dekrementirajte `currentTickQueue` za 1 da biste pokazali da je prošao jedan prisutni. Koristite dekrementator (`--`).
- Inkrementirajte `processed` za 1 da biste pratili koliko je prisutnih obrađeno. Koristite inkrementator (`++`).
