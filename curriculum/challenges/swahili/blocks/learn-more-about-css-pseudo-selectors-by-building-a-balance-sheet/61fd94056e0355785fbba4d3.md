...
U drugom `tr`, dodaj element sa tekstom `th` koji sadrži `Expenses Annual anticipated expenses, such as payroll.`. Obuhvati taj tekst, osim `Expenses `, unutar elementa `span` sa atributom `class` postavljenim na `description`.

Dodaj tri elementa od `td` ispod, i dodijeli im sljedeće tekstove u ovom redoslijedu: `$200`, `$300`, i `$400`. Postavi svojstvo trećeg elementa od `td` na vrijednost `current` za atribut `class`.
...
Vaše polje za `th` trebalo bi da ima tekst od `Expenses Annual anticipated expenses, such as payroll.`.
...
Treba da obuhvatite tekst od `Annual anticipated expenses, such as payroll.` unutar elementa `span`.
