...
title: Napravite obrazac za pritužbe kupaca
challengeType: 25
...
Za ovaj laboratorijski zadatak, obezbedili su vam sav HTML i CSS. Koristićete JavaScript za validaciju obrasca za pritužbe.

**Cilj:** Ispunite korisničke priče ispod i učinite da svi testovi prođu kako biste završili laboratorijski zadatak. 

**Korisničke priče:**

1. Kada se obrazac pošalje, trebalo bi da osigurate da:
    - `#full-name` nije prazan.
    - `#email` je validni format e-mail adrese.
    - `#order-no` je sekvenca od deset brojeva koja počinje sa `2024`.
    - `#product-code` prati obrazac `XX##-X###-XX#`, gde `X` predstavlja ili mala slova ili velika slova, a `#` predstavlja broj.
    - `#quantity` je pozitivan ceo broj.
    - barem jedna polje za označavanje (checkbox) iz `#complaints-group` je označena.
    - `#complaint-description` sadrži najmanje dvadeset karaktera ako je polje za označavanje `Other` označeno.
    - odabran je radio dugme (radio button) iz `#solutions-group`.
    - `#solution-description` sadrži najmanje dvadeset karaktera ako je radio dugme `Other` odabrano.
1. Trebalo bi da imate funkciju pod nazivom `validateForm` koja vraća objekat koji sadrži sledeće ključeve: `full-name`, `email`, `order-no`, `product-code`, `quantity`, `complaints-group`, `complaint-description`, `solutions-group` i `solution-description`. Vrednost svakog ključa treba da bude `true` ako je odgovarajuće polje obrasca ispravno popunjeno, i `false` u suprotnom. Polja za uslovne opise treba da budu `true` kada nisu obavezna, a inače validirana na osnovu toga da li sadrže najmanje dvadeset karaktera.
1. Trebalo bi da imate funkciju pod nazivom `isValid` koja uzima objekat koji vraća `validateForm` kao argument i vraća `true` ako su sva polja obrasca ispravno popunjena, i `false` u suprotnom.
1. Ako se pokrene događaj promene (change event) na polju obrasca i ima validnu vrednost, trebalo bi da postavite boju ivice na `green`. U slučaju grupa za označavanje i radio dugmad, trebalo bi da postavite boju ivice roditeljskog elementa `fieldset`.
1. Ako se pokrene događaj promene (change event) na polju obrasca i ima nevažeću vrednost, trebalo bi da postavite boju ivice na `red`. U slučaju grupa za označavanje i radio dugmad, trebalo bi da postavite boju ivice roditeljskog elementa `fieldset`.
1. Kada pokušate da pošaljete obrazac, trebalo bi da pozovete `isValid` radi validacije obrasca.
1. Kada pokušate da pošaljete obrazac, ako ima bilo koje nevažeće polje, svako nevažeće polje treba da bude istaknuto postavljanjem boje ivice svakog nevažećeg unosa (input), tekstualnog područja (textarea) ili skupa polja (fieldset) (u slučaju grupa za označavanje i radio dugmad) na `red`.
