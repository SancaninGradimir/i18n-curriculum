...
title: Izgradite interfejs igrača
challengeType: 27
...
1. Trebalo bi da definišete apstraktnu klasu pod nazivom `Player` koja nasleđuje od klase `abc.ABC`.
1. Klasa `Player` treba da ima metod `__init__` koji postavlja:

   - Atribut `moves` na prazan spisak.
   - Atribut `position` na `(0, 0)`.
   - Atribut `path` na spisak koji sadrži početnu poziciju.

1. Klasa `Player` treba da ima metod pod nazivom `make_move` koji:

   - Koristi `random.choice` da dobije nasumičan potez iz atributa `moves` (definisanog u konkretnoj klasi).
   - Dodaje vrednosti iz odabranog poteza trenutnoj poziciji i ažurira atribut `position`.
   - Dodaje novi `position` tupel atributu `path`.
   - Vraća novi `position`.

1. Klasa `Player` treba da ima apstraktni metod pod nazivom `level_up` koji mora biti implementiran u konkretnim klasama.
1. Trebalo bi da definišete `Pawn` klasu koja nasleđuje od klase `Player`.
1. Klasa `Pawn` treba da koristi `super()` za pozivanje metoda `__init__` roditeljske klase, a zatim postavi atribut `moves` na spisak tupela koji predstavljaju koordinate `x, y`.
1. Svaki koordinatni tupel treba da predstavlja pomeranje od `1` jedinica u sledećim pravcima: gore, dole, levo, desno.
1. Klasa `Pawn` treba da implementira konkretan metod `level_up` dodavanjem više poteza atributu `moves`. Dodati potezi bi trebalo da predstavljaju četiri dijagonalna pomeranja (na primer, `1` jedinica dole plus `1` jedinica levo).

**Napomena:** Moduli standardne biblioteke treba uvesti bez korišćenja aliasa. Testovi vezani za klasu `Player` će propasti dok klasa `Pawn` ne postane instancirana.
