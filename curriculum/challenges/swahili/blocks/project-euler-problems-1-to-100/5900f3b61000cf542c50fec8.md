...
title: 'Vežba 73: Brojanje elemenata u nizu'
challengeType: 1
forumTopicId: 302186
...
Razmotrite deo, $\frac{n}{d}$, gde su `n` i `d` pozitivni ceo broj. Ako je `n` &lt; `d` sa najvećim zajedničkim parametrom, ${HCF}(n, d) = 1$, zvaničito se naziva pojednostavljeni tačan deo.

Ako navedemo skup tačnih pojednostavljenih delova za `d` ≤ 8 u rastućem redosledu veličine, dobijamo:
...
Izgleda da postoji deo `3` između $\frac{1}{3}$ i $\frac{1}{2}$.

Ni sehemu ngapi ziko kati ya $\frac{1}{3}$ na $\frac{1}{2}$ katika seti iliyopangwa ya sehemu sahihi iliyorahisishwa kwa `d` ≤ `limit`?

# --hints--

`countingFractionsInARange(8)` Trebalo bi da vrati broj.

```js
assert(typeof countingFractionsInARange(8) === 'number');
```

`countingFractionsInARange(8)` treba da vrati `3`.

```js
assert.strictEqual(countingFractionsInARange(8), 3);
```

`countingFractionsInARange(1000)` trebalo bi da vrati `50695`.

```js
assert.strictEqual(countingFractionsInARange(1000), 50695);
```

`countingFractionsInARange(6000)` inapaswa kurudisha `1823861`.

```js
assert.strictEqual(countingFractionsInARange(6000), 1823861);
```

`countingFractionsInARange(12000)` treba da vrati `7295372`.
