...
title: "'Zoezi la 36: Palindromu za misingi miwili'"
challengeType: 1
forumTopicId: 302020
...
Nambari ya desimali, 585 = 1001001001<sub>2</sub> (binary), ni palindromu katika misingi yote miwili.

Pronađi zbir svih brojeva, ispod `n`, gde su 1000 ≤ `n` ≤ 1000000, koji su palindromi u bazi 10 i bazi 2.

(Molimo vas zapamtite da palindromski broj, u bilo kojoj bazi, ne sme imati vođuju nule.)

# --hints--

`doubleBasePalindromes(1000)` inapaswa kurudisha nambari.

```js
assert(typeof doubleBasePalindromes(1000) === 'number');
```

`doubleBasePalindromes(1000)` inapaswa kurudisha 1772.

```js
assert(doubleBasePalindromes(1000) == 1772);
```

`doubleBasePalindromes(50000)` trebalo bi da vrati 105795.

```js
assert(doubleBasePalindromes(50000) == 105795);
```

`doubleBasePalindromes(500000)` inapaswa kurudisha 286602.

```js
assert(doubleBasePalindromes(500000) == 286602);
```

`doubleBasePalindromes(1000000)` treba da vrati 872187.
